...
| **.github/workflows/snapshot-builder.yml** | Runs `snapshot_builder.py` on the 25th of each month (09:00 UTC) and on manual dispatch with optional `--date` override, then rebuilds the Time-lapse series and commits the resulting `builder_history/` changes back to master. Feeds the Time-lapse panel in the Builder tab. |
