...
| **scripts/fetch_canada.py** | Automated data ingestion for Canada. Reads Statistics Canada's Web Data Service (WDS cube **20-10-0024** "New motor vehicle registrations"): one `getCubeMetadata` call resolves the dimension members by name (Geography=Canada, Vehicle type, the Fuel type leaves), then a batched `getDataFromCubePidCoordAndLatestNPeriods` call pulls the latest quarters per fuel. Maps the fuel leaves (battery electric→BEV, plug-in hybrid→PHEV, hybrid→HEV, gasoline→PETROL, diesel→DIESEL, other/fuel-cell→OTHERS), stores each quarter under its middle month (Q1→02 … Q4→11). Produces two variants: `Whole` (Vehicle type=Passenger cars) → `data/Canada.csv`, and `Non-Passenger` (Vehicle type=Trucks = StatCan's all-non-cars catch-all: minivans/SUVs/pickups/vans/lorries/buses — deliberately not called HDV) → `data/Canada_Non-Passenger.csv`. Full logic in the module docstring and [docs/architecture/17-source-canada.md](docs/architecture/17-source-canada.md). |
