...
| **scripts/fetch_usa.py** | Automated data ingestion for the USA. Auto-discovers the latest ANL "Total Sales for Website" PDF on the ESIA reference page (or takes a direct `--pdf-url`), parses the full monthly Light-Duty table (BEV/PHEV/HEV/Total LDV; ICE = Total − BEV − PHEV − HEV, OTHERS = 0), and upserts the most recent month into `data/USA.csv`. ANL revises the last ~3 months (and occasionally older months) between releases, so historical CSV rows may differ from a newer PDF — per the project rule only the latest month is written. Self-throttles via the CSV's latest period. |
| **.github/workflows/fetch-usa.yml** | Runs `fetch_usa.py` daily from the 10th of each month (08:00 UTC) and on manual dispatch — most runs are no-ops until ANL publishes the previous month. If `data/USA.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=USA`. |
