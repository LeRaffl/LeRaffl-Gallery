...
| **scripts/fetch_canada.py** | Automated data ingestion for Canada. Reads Statistics Canada's Web Data Service (WDS cube **20-10-0024** "New motor vehicle registrations"): one `getCubeMetadata` call resolves the dimension members by name (Geography=Canada, Vehicle type=Passenger cars, the Fuel type leaves), then a batched `getDataFromCubePidCoordAndLatestNPeriods` call pulls the latest quarters per fuel. Maps the fuel leaves (battery electric→BEV, plug-in hybrid→PHEV, hybrid→HEV, gasoline→PETROL, diesel→DIESEL, other/fuel-cell→OTHERS), stores each quarter under its middle month (Q1→02 … Q4→11), and upserts `data/Canada.csv`. Full logic documented in the module docstring and [docs/architecture/17-source-canada.md](docs/architecture/17-source-canada.md). |
| **.github/workflows/fetch-canada.yml** | Runs `fetch_canada.py` daily on the 1st–15th (06:40 UTC) and on manual dispatch. The script always re-fetches the latest N quarters (StatCan revises recent ones); the commit step is change-gated, so steady-state runs are a no-op. If `data/Canada.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=Canada`. |
