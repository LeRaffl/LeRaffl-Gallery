...
| **scripts/build_builder_gif.py** | Renders each group's series into `builder_history/series/<group>.gif` (Pillow, no matplotlib). One file per group, **overwritten in place** — each run is the same animation one frame longer, so vintages are not kept. ~107 KB per group thanks to inter-frame differencing. |
