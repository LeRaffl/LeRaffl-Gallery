...
| **.github/workflows/fetch-canada.yml** | Runs `fetch_canada.py` daily on the 8th–20th of March/June/September/December (06:40 UTC) — the cube is quarterly and StatCan releases it ~2.5 months after quarter-end — and on manual dispatch. The script always re-fetches the latest N quarters (StatCan revises recent ones); the commit step is change-gated, so steady-state runs are a no-op. Detects which per-variant CSVs changed and triggers `render-country.yml` once per touched variant (`country=Canada`). |
