...
| **scripts/fetch_turkey.py** | Automated data ingestion for Türkiye. Takes a TÜİK "Motorlu Kara Taşıtları" press-bulletin PDF (the Veri Portalı is a React SPA so the maintainer passes the bulletin Sayı via `--press-id` / workflow_dispatch; auto-discovery TBD), pulls the authoritative monthly + YTD totals from the narrative, OCRs the fuel-breakdown table image (poppler + imagemagick + tesseract-tur), and upserts the row into `data/Türkiye.csv` (Elektrik→BEV, Hibrit→HEV, Benzin→PETROL, Dizel→DIESEL, LPG→OTHERS). Three validation layers: narrative-vs-OCR Toplam, per-fuel sum check with single-error auto-repair via Pay % cross-check, and previous-year-same-month column cross-check against the existing CSV row. Self-throttles via the CSV's latest period. |
| **.github/workflows/fetch-turkey.yml** | Runs `fetch_turkey.py` daily from the 15th of each month (08:00 UTC) and on manual dispatch with required `press_id` (or `pdf_url`) input. The daily cron self-throttles to a no-op when no press-id is supplied; the maintainer dispatches once per month when TÜİK publishes the new bulletin (date footnoted in the previous one as "Bu konu ile ilgili bir sonraki haber bülteninin yayımlanma tarihi …"). If `data/Türkiye.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=Türkiye`. |
