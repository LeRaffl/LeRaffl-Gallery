...
| **scripts/snapshot_builder.py** | Snapshots the Builder-tab aggregated BEV/ICE/PHEV curves (world + 13 regional/weight-based groups) to `builder_history/<date>.csv` plus a metadata entry in `builder_history/index.json`. Mirrors the in-page Builder math byte-for-byte (including the JS `Number('') === 0` quirk and the v1=0 anchor recovery), so a snapshot reproduces what the page plots that day. |
| **.github/workflows/snapshot-builder.yml** | Runs `snapshot_builder.py` on the 25th of each month (09:00 UTC) and on manual dispatch with optional `--date` override. Commits the resulting `builder_history/` changes back to master. Reserved for a future time-lapse visualisation of how the global bottom-up curve evolves. |
...
| **.github/workflows/fetch-uruguay.yml** | Runs `fetch_uruguay.py` daily from the 1st of each month (08:00 UTC) and on manual dispatch — most runs are no-ops until ACAU edits the previous month into the Compilado workbook (typically the first week of the following month). If `data/Uruguay.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=Uruguay`. |
| **scripts/fetch_turkey.py** | Automated data ingestion for Türkiye. Discovers the current TÜİK "Motorlu Kara Taşıtları" bulletin via `scripts/tuik_discover.py` (no `--press-id` needed), pulls the authoritative monthly + YTD totals from the bulletin narrative, and OCRs the fuel-breakdown table — which TÜİK publishes as a raster image — from the images embedded inline in the bulletin HTML (imagemagick + tesseract-tur), at 4/6/8× upscale until a reading validates. Upserts into `data/Türkiye.csv` (Elektrik→BEV, Hibrit→HEV, Benzin→PETROL, Dizel→DIESEL, LPG→OTHERS). A bulletin carries several fuel tables — the month's registrations and the registered stock — so every candidate is collected and only the one whose Toplam matches the narrative total is used. Three validation layers: narrative-vs-OCR Toplam, per-fuel sum check with single-error auto-repair via Pay % cross-check, and previous-year-same-month column cross-check against the existing CSV row. Self-throttles via the CSV's latest period. No PDF is downloaded on this path. |
| **.github/workflows/fetch-turkey.yml** | Runs `fetch_turkey.py` daily from the 15th of each month (08:30 UTC) and on manual dispatch. No input is required — the bulletin id is discovered automatically, and the daily cron self-throttles to a no-op until TÜİK publishes the new bulletin. The `year`/`month` inputs are for backfills, `press_id`/`pdf_url` for overriding a bad read, `force` to re-process an existing period, and `recon` to dump a portal reconnaissance pass when discovery starts missing. If `data/Türkiye.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=Türkiye`. |
| **scripts/tuik_discover.py** | Finds the TÜİK bulletin for a given month, so `fetch_turkey.py` needs no hand-supplied id. The Veri Portalı is a React SPA backed by a JSON API at `/api/tr/press/<id>`, whose records carry `title` and `period` — together an exact identification — plus `content`, the bulletin HTML. There is no listing endpoint (`/api/tr/press`, `/list`, `/presses`, `/search`, `/categories`, `/themes` all 404), so it walks ids outward from the newest one recorded in the CSV's `source` column and matches on both fields. The ids are **not** chronological (58041=Mart, 58042=Nisan, 58043=Haziran, 58044=Mayıs, 58051=Ocak 2026) but do form one contiguous block, so the usual case costs a single request and the anchor re-derives itself each month. `--debug` runs a reconnaissance pass (SPA chunk-graph crawl + endpoint probe) for when the portal changes shape. |
...
| **.github/workflows/fetch-canada.yml** | Runs `fetch_canada.py` daily on the 1st–15th (06:40 UTC) and on manual dispatch. The script always re-fetches the latest N quarters (StatCan revises recent ones); the commit step is change-gated, so steady-state runs are a no-op. Detects which per-variant CSVs changed and triggers `render-country.yml` once per touched variant (`country=Canada`). |
...
| **.github/workflows/fetch-usa.yml** | Runs `fetch_usa.py` daily from the 10th of each month (10:00 UTC — offset from the 08:00 slot used by other fetchers) and on manual dispatch. If `data/USA.csv` changes, the workflow commits it and triggers `render-country.yml` with `country=USA`. |
...
**Why isn't country X on the gallery?** Some countries were investigated and deliberately left off because their freely-available data is incomplete (e.g. the dominant EV brand doesn't report), paywalled, login-walled, or because the market doesn't fit the new-registration / organic-transition model (e.g. Ethiopia's 100 %-EV import *mandate* over an ~85 %-used-import market). The rationale per country (Argentina, Mexico, Colombia, Ethiopia, …) is documented in [docs/architecture/14-data-source-gaps.md](docs/architecture/14-data-source-gaps.md).
