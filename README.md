...
| **scripts/snapshot_builder.py** | Snapshots the Builder-tab aggregated BEV/ICE/PHEV curves (world + 13 regional/weight-based groups) to `builder_history/<date>.csv` plus a metadata entry in `builder_history/index.json`. Mirrors the in-page Builder math (including the v1=0 anchor recovery), so a snapshot reproduces what the page plots that day. Snapshots dated 2026-06-25…2026-09-09 sit one year off — each entry in `index.json` carries a `basis` field; see [#219](https://github.com/LeRaffl/LeRaffl-Gallery/issues/219). Regression tests in `scripts/test_snapshot_builder.py`. |
