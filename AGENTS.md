...
| `sources/`, `series/`, `schedule*.html/.ics`, `manifest.json`, `params.csv`, `weights.csv`, `images/`, `posts/`, `assets/theme.css`, `builder_history/`, `backtest/`, `market/` | **Generated** — do not hand-edit (see below). |
...
- **Add a country** → follow `docs/architecture/08-deploy-ops.md` §8.3 (write `data/<Country>.csv`, add to `SD_COUNTRIES` in `index.html`, add `assets/flags/<slug>.png`, map the flag emoji in `R/post_text.R`, PR, then render) and give it a source doc or a `country_source_stubs.yaml` entry. Its Time-lapse backtest history is backfilled automatically on the next `snapshot-builder.yml` run (or dispatch it with `backtest_only`); a fetched-but-never-rendered series goes into `DATA_ONLY_SERIES` in `R/build_backtest.R`.
