...
| How uncertain a fitted curve / crossing year is | `bands/<slug>.json`; what CI, PI and TI mean and how to quote them: `docs/architecture/44-uncertainty-bands.md` |
...
- **Uncertainty bands** (`bands/<slug>.json`) answer three different
...
fitted curve as an official forecast (it's this project's own model); read the
ribbon on the PNGs as a confidence interval (it is not one; `bands/` is).
...
| `market/` | Generated top brands / models per electrified class (`<slug>_top.json`), written by record-level fetchers via `scripts/market_top.py`; shown on source pages via `market_breakdown:`. See `docs/architecture/03-data-objects.md` §3.16. |
| `classification/` | Derived-powertrain classification (Argentina): `<slug>_rules.csv` is **hand-edited** and the classifier's source of truth; `<slug>_models.csv` + `<slug>_top.json` are generated by the fetcher. See `classification/README.md`. |
| `docs/architecture/` | **Canonical architecture docs** (numbered). The spec. |
| `sources/`, `series/`, `schedule*.html/.ics`, `manifest.json`, `params.csv`, `weights.csv`, `images/`, `posts/`, `bands/`, `assets/theme.css`, `builder_history/`, `backtest/`, `market/` | **Generated** — do not hand-edit (see below). |
...
1. `data/<Country>.csv` is the single source of truth. **Never hand-edit**
   `params.csv`, `weights.csv`, `manifest.json`, `images/`, `posts/`, `bands/`,
   `sources/`, `series/`, `schedule*`, `assets/theme.css`,
   `builder_history/`, `backtest/`, `classification/*_models.csv`,
   `classification/*_top.json`, `market/` — they are generated.
...
- **The model / plots / render** → `R/*.R`, then re-render the affected countries, then update the docs if behaviour changed. The PR's `preview-render.yml` run shows base vs PR charts before merging.
- **The uncertainty bands** → `R/bands.R` **and** `docs/architecture/44-uncertainty-bands.md` (the explanation end users and AIs rely on; re-check its §6 validation after any method change), then dispatch `backfill-bands.yml`. The file slug is `slug_country()` in `R/data.R`, mirrored by `bandSlug()` in `index.html` — change both or neither.
...
- **The palette or type stack** → edit `index.html`'s `:root` block only (and
  its dark twin `:root[data-theme="dark"]` — a new colour token needs a value
  in both), then re-run `scripts/build_theme.py`. Charts stay light in dark
  mode on purpose (`docs/architecture/02-components.md` §2.12). It is the single source of truth: the
...
- **Add a country** → follow **`docs/architecture/42-adding-a-country.md`** end to end (probe → scope → fetcher + tests → workflow → data → gallery wiring → **Builder groups in both mirrors** → docs → **backtest history on the PR branch** → checks → after-merge render), and make `scripts/check_country_integration.py` pass — CI runs it on every PR and fails on anything left out. A fetched-but-never-rendered series goes into `DATA_ONLY_SERIES` in `R/build_backtest.R`.
- **A derived-powertrain rule (Argentina)** → edit `classification/argentina_rules.csv` (ordered, first match wins; fill `reason`, `evidence` and a real example), run `scripts/test_fetch_argentina.py`, add a line to the decision log in `docs/architecture/39-source-argentina.md` §4.5, then dispatch the fetch with `force` so the whole history is re-classified. Never widen a brand-wide rule to fix one model.
- **Top brands / models for a country** → only for sources with brand + model per registration: count `(class, brand, model)` in the fetcher with the *same* class logic as its CSV, write `market/<slug>_top.json` via `scripts/market_top.py` (behind `guarded`, never in the render trigger), add it to the workflow's commit list and `market_breakdown:` to the source doc front-matter; extend `scripts/test_market_top.py`. Recipe: `03-data-objects.md` §3.16.
