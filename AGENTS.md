...
- **The Builder aggregation or its groups** → `scripts/snapshot_builder.py` **and** `index.html`'s `BUILDER_GROUPS` (they mirror each other), then re-run `scripts/build_builder_series.py` so the Time-lapse panel sees the change. `builder_history/<date>.csv` is the archive; `builder_history/series/<group>.json` is what the browser actually reads.
