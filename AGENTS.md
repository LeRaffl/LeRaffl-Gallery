...
1. `data/<Country>.csv` is the single source of truth. **Never hand-edit**
   ...
   `sources/`, `series/`, `schedule*`, `builder_history/` — they are
   generated.
...
- **The Builder aggregation or its groups** → `scripts/snapshot_builder.py` **and** `index.html`'s `BUILDER_GROUPS` (they mirror each other), then re-run `scripts/build_builder_series.py` **and** `scripts/build_builder_gif.py` so the Time-lapse panel and its downloadable animation see the change. `builder_history/<date>.csv` is the archive; `builder_history/series/<group>.json` is what the browser actually reads.
