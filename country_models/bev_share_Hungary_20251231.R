#### Hungary: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Hungary"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/hungary.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_hungary <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Hungary")
source <- data_hungary$Source[1]
...
#data_hungary$BEV <- data_hungary$`Electric/zero-emission`
#data_hungary$PHEV <- data_hungary$Hybrid
#data_hungary$TOTAL <- data_hungary$overall
#data_hungary$OTHERS <- data_hungary$`Other fuel`
#data_hungary$TOTAL - data_hungary$BEV - data_hungary$PHEV <- data_hungary$Fossil

data_hungary$`Electric/zero-emission` <- data_hungary$BEV
data_hungary$Hybrid <- data_hungary$PHEV
data_hungary$overall <- data_hungary$TOTAL
data_hungary$`Other fuel` <- data_hungary$OTHERS
data_hungary$Fossil <- data_hungary$TOTAL - data_hungary$BEV - data_hungary$PHEV


data_hungary$bev_share   <- data_hungary$`Electric/zero-emission`/data_hungary$overall
data_hungary$ice_share   <- data_hungary$Fossil/data_hungary$overall
data_hungary$hybrid_share<- data_hungary$Hybrid/data_hungary$overall
data_hungary$other_share <- data_hungary$`Other fuel`/data_hungary$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_hungary$pure_ICE    <- (data_hungary$Fossil)/data_hungary$overall

lastyearly <- ceiling(max(subset(data_hungary, data_hungary$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_hungary_monthly <- subset(data_hungary, data_hungary$time_interval=="monthly")
data_hungary_monthly <- subset(data_hungary_monthly, data_hungary_monthly$year>=min(data_hungary_monthly$year+1))
TTM_shares_hungary <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_hungary_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_hungary_monthly)), "%Y-%m"),
  Other = data_hungary_monthly$`Other TTM`,
  #`ICE` = data_hungary_monthly$`ICE TTM`,#-data_hungary_monthly$`HEV TTM`,
  Petrol = data_hungary_monthly$`Petrol TTM`,
  Diesel = data_hungary_monthly$`Diesel TTM`,
  #Ethanol = data_hungary_monthly$`Ethanol TTM`,
  #FlexFuel = data_hungary_monthly$`FlexFuel TTM`,
  HEV = data_hungary_monthly$`HEV TTM`,
  PHEV = data_hungary_monthly$`PHEV TTM`,
  BEV = data_hungary_monthly$`BEV TTM`
)

colnames(TTM_shares_hungary)[colnames(TTM_shares_hungary) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_hungary)[colnames(TTM_shares_hungary) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_hungary)[colnames(TTM_shares_hungary) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_hungary)[colnames(TTM_shares_hungary) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_hungary)[colnames(TTM_shares_hungary) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_hungary_long <- TTM_shares_hungary %>%
...
TTM_barplot_hungary <- ggplot(TTM_shares_hungary_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_hungary_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_hungary_long$month[substr(TTM_shares_hungary_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Hungary",
...
print(TTM_barplot_hungary)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_hungary$year)))
extrapol <- 2200
data_hungary <- subset(data_hungary, data_hungary$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_hungary$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_hungary$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_hungary$`Electric/zero-emission`))
for(i in 1:length(data_hungary$`Electric/zero-emission`)){
  data_hungary_looped <- data_hungary[1:i,]
  xg<-data_hungary_looped$year; yg<-as.double(data_hungary_looped$bev_share)
  ...
  xg<-data_hungary_looped$year; yg<-as.double(data_hungary_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  hungary <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_hungary_looped$overall; new_A$time_interval <- data_hungary_looped$time_interval
  
  hungary$Hybrid       <- pmax(hungary$Hybrid, 0)
  hungary$Hybrid_upper <- pmax(hungary$Hybrid_upper, 0)
  hungary$Hybrid_lower <- pmax(hungary$Hybrid_lower, 0)
  
  time_80_hungary <- max(subset(hungary, hungary$BEV<=0.8 & hungary$BEV >= 0.2)$x)
  time_50_hungary <- max(subset(hungary, hungary$BEV<=0.5 & hungary$BEV >= 0.2)$x)
  time_20_hungary <- max(subset(hungary, hungary$BEV<=0.2 & hungary$BEV >= 0.1)$x)
  time_20_to_80_hungary <- max(subset(hungary, hungary$BEV<=0.8 & hungary$BEV >= 0.2)$x)-min(subset(hungary, hungary$BEV<=0.8 & hungary$BEV >= 0.2)$x)
  time_80_to_20_hungary <- max(subset(hungary, hungary$ICE<=0.8 & hungary$ICE >= 0.2)$x)-min(subset(hungary, hungary$ICE<=0.8 & hungary$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_hungary; ice_time[i] <- time_80_to_20_hungary
}

timer_hungary <- data.frame("year"=data_hungary[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_hungary$BEV_time == -Inf), which(timer_hungary$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_hungary) > (last_inf_index + 12)) {
  timer_hungary_short <- timer_hungary[(last_inf_index + 1 + 12):nrow(timer_hungary), ]
} else {
  timer_hungary_short <- timer_hungary
}
...
plot_timer <- ggplot(timer_hungary_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_hungary_short$BEV_time[length(timer_hungary_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Hungary transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_hungary_short$BEV_time[length(timer_hungary_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_hungary <- ggplot(hungary, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Hungary - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_hungary), " years ", round(12*(time_20_to_80_hungary-floor(time_20_to_80_hungary)),0), " months"),
...
plot_hungary <- plot_hungary + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(hungary, hungary$x==2024+counter & hungary$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_hungary <- plot_hungary + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(hungary, hungary$x==2024+counter & hungary$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_hungary <- plot_hungary + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_hungary

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_hungary$overall)
BEV <- data.frame(BEV, "overall"=data_hungary$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_hungary$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_hungary <- ggplot(hungary, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Hungary - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_hungary), " years ",
      round(12 * (time_80_to_20_hungary - floor(time_80_to_20_hungary)), 0),
      " months"
    ),
...
plot_ICE_BEV_hungary <- plot_ICE_BEV_hungary +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(hungary, hungary$x == 2024 + counter - 1 & hungary$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_hungary <- plot_ICE_BEV_hungary +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(hungary, hungary$x == 2024 + counter & hungary$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_hungary)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("hungary_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_hungary,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_hungary",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("hungary_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_hungary,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_hungary",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("hungary_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_hungary",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("hungary_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_hungary,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_hungary",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_hungary$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_hungary, "Inf"),
  bev_50 = as_date_from_numeric(time_50_hungary, "Inf"),
  bev_80 = as_date_from_numeric(time_80_hungary, "Inf"),
  bev_20_80 = time_20_to_80_hungary,
  ice_80_20 = time_80_to_20_hungary,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_hungary$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_hungary)
month_label <- last_label_from_data(data_hungary, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_hungary$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_hungary)) nz(tail(data_hungary$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_hungary)) {
    nz(tail(data_hungary$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_hungary)) nz(tail(data_hungary$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_hungary)) {
    nz(tail(data_hungary$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_hungary),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_hungary)
upsert_weight(
