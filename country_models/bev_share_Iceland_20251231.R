#### Iceland: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Iceland"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/iceland.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_iceland <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Iceland")
source <- data_iceland$Source[1]
...
#data_iceland$BEV <- data_iceland$`Electric/zero-emission`
#data_iceland$PHEV <- data_iceland$Hybrid
#data_iceland$TOTAL <- data_iceland$overall
#data_iceland$OTHERS <- data_iceland$`Other fuel`
#data_iceland$TOTAL - data_iceland$BEV - data_iceland$PHEV <- data_iceland$Fossil

data_iceland$`Electric/zero-emission` <- data_iceland$BEV
data_iceland$Hybrid <- data_iceland$PHEV
data_iceland$overall <- data_iceland$TOTAL
data_iceland$`Other fuel` <- data_iceland$OTHERS
data_iceland$Fossil <- data_iceland$TOTAL - data_iceland$BEV - data_iceland$PHEV


data_iceland$bev_share   <- data_iceland$`Electric/zero-emission`/data_iceland$overall
data_iceland$ice_share   <- data_iceland$Fossil/data_iceland$overall
data_iceland$hybrid_share<- data_iceland$Hybrid/data_iceland$overall
data_iceland$other_share <- data_iceland$`Other fuel`/data_iceland$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_iceland$pure_ICE    <- (data_iceland$Fossil)/data_iceland$overall

lastyearly <- ceiling(max(subset(data_iceland, data_iceland$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_iceland_monthly <- subset(data_iceland, data_iceland$time_interval=="monthly")
data_iceland_monthly <- subset(data_iceland_monthly, data_iceland_monthly$year>=min(data_iceland_monthly$year+1))
TTM_shares_iceland <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_iceland_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_iceland_monthly)), "%Y-%m"),
  Other = data_iceland_monthly$`Other TTM`,
  #`ICE` = data_iceland_monthly$`ICE TTM`,#-data_iceland_monthly$`HEV TTM`,
  Petrol = data_iceland_monthly$`Petrol TTM`,
  Diesel = data_iceland_monthly$`Diesel TTM`,
  #Ethanol = data_iceland_monthly$`Ethanol TTM`,
  #FlexFuel = data_iceland_monthly$`FlexFuel TTM`,
  HEV = data_iceland_monthly$`HEV TTM`,
  PHEV = data_iceland_monthly$`PHEV TTM`,
  BEV = data_iceland_monthly$`BEV TTM`
)

colnames(TTM_shares_iceland)[colnames(TTM_shares_iceland) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_iceland)[colnames(TTM_shares_iceland) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_iceland)[colnames(TTM_shares_iceland) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_iceland)[colnames(TTM_shares_iceland) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_iceland)[colnames(TTM_shares_iceland) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_iceland_long <- TTM_shares_iceland %>%
...
TTM_barplot_iceland <- ggplot(TTM_shares_iceland_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_iceland_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_iceland_long$month[substr(TTM_shares_iceland_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Iceland",
...
print(TTM_barplot_iceland)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_iceland$year)))
extrapol <- 2200
data_iceland <- subset(data_iceland, data_iceland$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_iceland$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_iceland$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_iceland$`Electric/zero-emission`))
for(i in 1:length(data_iceland$`Electric/zero-emission`)){
  data_iceland_looped <- data_iceland[1:i,]
  xg<-data_iceland_looped$year; yg<-as.double(data_iceland_looped$bev_share)
  ...
  xg<-data_iceland_looped$year; yg<-as.double(data_iceland_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  iceland <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_iceland_looped$overall; new_A$time_interval <- data_iceland_looped$time_interval
  
  iceland$Hybrid       <- pmax(iceland$Hybrid, 0)
  iceland$Hybrid_upper <- pmax(iceland$Hybrid_upper, 0)
  iceland$Hybrid_lower <- pmax(iceland$Hybrid_lower, 0)
  
  time_80_iceland <- max(subset(iceland, iceland$BEV<=0.8 & iceland$BEV >= 0.2)$x)
  time_50_iceland <- max(subset(iceland, iceland$BEV<=0.5 & iceland$BEV >= 0.2)$x)
  time_20_iceland <- max(subset(iceland, iceland$BEV<=0.2 & iceland$BEV >= 0.1)$x)
  time_20_to_80_iceland <- max(subset(iceland, iceland$BEV<=0.8 & iceland$BEV >= 0.2)$x)-min(subset(iceland, iceland$BEV<=0.8 & iceland$BEV >= 0.2)$x)
  time_80_to_20_iceland <- max(subset(iceland, iceland$ICE<=0.8 & iceland$ICE >= 0.2)$x)-min(subset(iceland, iceland$ICE<=0.8 & iceland$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_iceland; ice_time[i] <- time_80_to_20_iceland
}

timer_iceland <- data.frame("year"=data_iceland[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_iceland$BEV_time == -Inf), which(timer_iceland$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_iceland) > (last_inf_index + 12)) {
  timer_iceland_short <- timer_iceland[(last_inf_index + 1 + 12):nrow(timer_iceland), ]
} else {
  timer_iceland_short <- timer_iceland
}
...
plot_timer <- ggplot(timer_iceland_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_iceland_short$BEV_time[length(timer_iceland_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Iceland transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_iceland_short$BEV_time[length(timer_iceland_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_iceland <- ggplot(iceland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Iceland - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_iceland), " years ", round(12*(time_20_to_80_iceland-floor(time_20_to_80_iceland)),0), " months"),
...
plot_iceland <- plot_iceland + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(iceland, iceland$x==2024+counter & iceland$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_iceland <- plot_iceland + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                              label=paste0("Jan ",2025+counter,": ",round(subset(iceland, iceland$x==2024+counter & iceland$Type=="New Registrations")$BEV*100, 1),"%"),
                                              size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_iceland <- plot_iceland + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                       width = unit(1*1920/1280
                                                                                    ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_iceland

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_iceland$overall)
BEV <- data.frame(BEV, "overall"=data_iceland$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_iceland$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_iceland <- ggplot(iceland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Iceland - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_iceland), " years ",
      round(12 * (time_80_to_20_iceland - floor(time_80_to_20_iceland)), 0),
      " months"
    ),
...
plot_ICE_BEV_iceland <- plot_ICE_BEV_iceland +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(iceland, iceland$x == 2024 + counter - 1 & iceland$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_iceland <- plot_ICE_BEV_iceland +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(iceland, iceland$x == 2024 + counter & iceland$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_iceland)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("iceland_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_iceland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_iceland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("iceland_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_iceland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_iceland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("iceland_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_iceland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("iceland_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_iceland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_iceland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_iceland$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_iceland, "Inf"),
  bev_50 = as_date_from_numeric(time_50_iceland, "Inf"),
  bev_80 = as_date_from_numeric(time_80_iceland, "Inf"),
  bev_20_80 = time_20_to_80_iceland,
  ice_80_20 = time_80_to_20_iceland,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_iceland$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_iceland)
month_label <- last_label_from_data(data_iceland, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_iceland$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_iceland)) nz(tail(data_iceland$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_iceland)) {
    nz(tail(data_iceland$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_iceland)) nz(tail(data_iceland$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_iceland)) {
    nz(tail(data_iceland$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_iceland),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_iceland)
upsert_weight(
