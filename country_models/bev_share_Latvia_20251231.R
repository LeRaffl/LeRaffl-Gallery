#### Latvia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Latvia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/latvia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_latvia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Latvia")
source <- data_latvia$Source[1]
...
#data_latvia$BEV <- data_latvia$`Electric/zero-emission`
#data_latvia$PHEV <- data_latvia$Hybrid
#data_latvia$TOTAL <- data_latvia$overall
#data_latvia$OTHERS <- data_latvia$`Other fuel`
#data_latvia$TOTAL - data_latvia$BEV - data_latvia$PHEV <- data_latvia$Fossil

data_latvia$`Electric/zero-emission` <- data_latvia$BEV
data_latvia$Hybrid <- data_latvia$PHEV
data_latvia$overall <- data_latvia$TOTAL
data_latvia$`Other fuel` <- data_latvia$OTHERS
data_latvia$Fossil <- data_latvia$TOTAL - data_latvia$BEV - data_latvia$PHEV


data_latvia$bev_share   <- data_latvia$`Electric/zero-emission`/data_latvia$overall
data_latvia$ice_share   <- data_latvia$Fossil/data_latvia$overall
data_latvia$hybrid_share<- data_latvia$Hybrid/data_latvia$overall
data_latvia$other_share <- data_latvia$`Other fuel`/data_latvia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_latvia$pure_ICE    <- (data_latvia$Fossil)/data_latvia$overall

lastyearly <- ceiling(max(subset(data_latvia, data_latvia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_latvia_monthly <- subset(data_latvia, data_latvia$time_interval=="monthly")
data_latvia_monthly <- subset(data_latvia_monthly, data_latvia_monthly$year>=min(data_latvia_monthly$year+1))
TTM_shares_latvia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_latvia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_latvia_monthly)), "%Y-%m"),
  Other = data_latvia_monthly$`Other TTM`,
  #`ICE` = data_latvia_monthly$`ICE TTM`,#-data_latvia_monthly$`HEV TTM`,
  Petrol = data_latvia_monthly$`Petrol TTM`,
  Diesel = data_latvia_monthly$`Diesel TTM`,
  #Ethanol = data_latvia_monthly$`Ethanol TTM`,
  #FlexFuel = data_latvia_monthly$`FlexFuel TTM`,
  HEV = data_latvia_monthly$`HEV TTM`,
  PHEV = data_latvia_monthly$`PHEV TTM`,
  BEV = data_latvia_monthly$`BEV TTM`
)

colnames(TTM_shares_latvia)[colnames(TTM_shares_latvia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_latvia)[colnames(TTM_shares_latvia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_latvia)[colnames(TTM_shares_latvia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_latvia)[colnames(TTM_shares_latvia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_latvia)[colnames(TTM_shares_latvia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_latvia_long <- TTM_shares_latvia %>%
...
TTM_barplot_latvia <- ggplot(TTM_shares_latvia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_latvia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_latvia_long$month[substr(TTM_shares_latvia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Latvia",
...
print(TTM_barplot_latvia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_latvia$year)))
extrapol <- 2200
data_latvia <- subset(data_latvia, data_latvia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_latvia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_latvia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_latvia$`Electric/zero-emission`))
for(i in 1:length(data_latvia$`Electric/zero-emission`)){
  data_latvia_looped <- data_latvia[1:i,]
  xg<-data_latvia_looped$year; yg<-as.double(data_latvia_looped$bev_share)
  ...
  xg<-data_latvia_looped$year; yg<-as.double(data_latvia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  latvia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_latvia_looped$overall; new_A$time_interval <- data_latvia_looped$time_interval
  
  latvia$Hybrid       <- pmax(latvia$Hybrid, 0)
  latvia$Hybrid_upper <- pmax(latvia$Hybrid_upper, 0)
  latvia$Hybrid_lower <- pmax(latvia$Hybrid_lower, 0)
  
  time_80_latvia <- max(subset(latvia, latvia$BEV<=0.8 & latvia$BEV >= 0.2)$x)
  time_50_latvia <- max(subset(latvia, latvia$BEV<=0.5 & latvia$BEV >= 0.2)$x)
  time_20_latvia <- max(subset(latvia, latvia$BEV<=0.2 & latvia$BEV >= 0.1)$x)
  time_20_to_80_latvia <- max(subset(latvia, latvia$BEV<=0.8 & latvia$BEV >= 0.2)$x)-min(subset(latvia, latvia$BEV<=0.8 & latvia$BEV >= 0.2)$x)
  time_80_to_20_latvia <- max(subset(latvia, latvia$ICE<=0.8 & latvia$ICE >= 0.2)$x)-min(subset(latvia, latvia$ICE<=0.8 & latvia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_latvia; ice_time[i] <- time_80_to_20_latvia
}

timer_latvia <- data.frame("year"=data_latvia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_latvia$BEV_time == -Inf), which(timer_latvia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_latvia) > (last_inf_index + 12)) {
  timer_latvia_short <- timer_latvia[(last_inf_index + 1 + 12):nrow(timer_latvia), ]
} else {
  timer_latvia_short <- timer_latvia
}
...
plot_timer <- ggplot(timer_latvia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_latvia_short$BEV_time[length(timer_latvia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Latvia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_latvia_short$BEV_time[length(timer_latvia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_latvia <- ggplot(latvia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Latvia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_latvia), " years ", round(12*(time_20_to_80_latvia-floor(time_20_to_80_latvia)),0), " months"),
...
plot_latvia <- plot_latvia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(latvia, latvia$x==2024+counter & latvia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_latvia <- plot_latvia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(latvia, latvia$x==2024+counter & latvia$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_latvia <- plot_latvia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_latvia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_latvia$overall)
BEV <- data.frame(BEV, "overall"=data_latvia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_latvia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_latvia <- ggplot(latvia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Latvia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_latvia), " years ",
      round(12 * (time_80_to_20_latvia - floor(time_80_to_20_latvia)), 0),
      " months"
    ),
...
plot_ICE_BEV_latvia <- plot_ICE_BEV_latvia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(latvia, latvia$x == 2024 + counter - 1 & latvia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_latvia <- plot_ICE_BEV_latvia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(latvia, latvia$x == 2024 + counter & latvia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_latvia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("latvia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_latvia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_latvia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("latvia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_latvia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_latvia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("latvia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_latvia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("latvia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_latvia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_latvia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_latvia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_latvia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_latvia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_latvia, "Inf"),
  bev_20_80 = time_20_to_80_latvia,
  ice_80_20 = time_80_to_20_latvia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_latvia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_latvia)
month_label <- last_label_from_data(data_latvia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_latvia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_latvia)) nz(tail(data_latvia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_latvia)) {
    nz(tail(data_latvia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_latvia)) nz(tail(data_latvia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_latvia)) {
    nz(tail(data_latvia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_latvia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_latvia)
upsert_weight(
