#### Brazil: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Brazil"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/brazil.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_brazil <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Brazil")
source <- data_brazil$Source[1]
...
#data_brazil$BEV <- data_brazil$`Electric/zero-emission`
#data_brazil$PHEV <- data_brazil$Hybrid
#data_brazil$TOTAL <- data_brazil$overall
#data_brazil$OTHERS <- data_brazil$`Other fuel`
#data_brazil$TOTAL - data_brazil$BEV - data_brazil$PHEV <- data_brazil$Fossil

data_brazil$`Electric/zero-emission` <- data_brazil$BEV
data_brazil$Hybrid <- data_brazil$PHEV
data_brazil$overall <- data_brazil$TOTAL
data_brazil$`Other fuel` <- data_brazil$OTHERS
data_brazil$Fossil <- data_brazil$TOTAL - data_brazil$BEV - data_brazil$PHEV


data_brazil$bev_share   <- data_brazil$`Electric/zero-emission`/data_brazil$overall
data_brazil$ice_share   <- data_brazil$Fossil/data_brazil$overall
data_brazil$hybrid_share<- data_brazil$Hybrid/data_brazil$overall
data_brazil$other_share <- data_brazil$`Other fuel`/data_brazil$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_brazil$pure_ICE    <- (data_brazil$Fossil)/data_brazil$overall

lastyearly <- ceiling(max(subset(data_brazil, data_brazil$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_brazil_monthly <- subset(data_brazil, data_brazil$time_interval=="monthly")
data_brazil_monthly <- subset(data_brazil_monthly, data_brazil_monthly$year>=min(data_brazil_monthly$year+1))
TTM_shares_brazil <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_brazil_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_brazil_monthly)), "%Y-%m"),
  Other = data_brazil_monthly$`Other TTM`,
  #`ICE` = data_brazil_monthly$`ICE TTM`,#-data_brazil_monthly$`HEV TTM`,
  Petrol = data_brazil_monthly$`Petrol TTM`,
  Diesel = data_brazil_monthly$`Diesel TTM`,
  #Ethanol = data_brazil_monthly$`Ethanol TTM`,
  FlexFuel = data_brazil_monthly$`FlexFuel TTM`,
  HEV = data_brazil_monthly$`HEV TTM`,
  PHEV = data_brazil_monthly$`PHEV TTM`,
  BEV = data_brazil_monthly$`BEV TTM`
)

colnames(TTM_shares_brazil)[colnames(TTM_shares_brazil) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_brazil)[colnames(TTM_shares_brazil) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_brazil)[colnames(TTM_shares_brazil) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_brazil)[colnames(TTM_shares_brazil) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_brazil)[colnames(TTM_shares_brazil) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_brazil_long <- TTM_shares_brazil %>%
...
TTM_barplot_brazil <- ggplot(TTM_shares_brazil_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_brazil_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_brazil_long$month[substr(TTM_shares_brazil_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Brazil",
...
print(TTM_barplot_brazil)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_brazil$year)))
extrapol <- 2200
data_brazil <- subset(data_brazil, data_brazil$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_brazil$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_brazil$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_brazil$`Electric/zero-emission`))
for(i in 1:length(data_brazil$`Electric/zero-emission`)){
  data_brazil_looped <- data_brazil[1:i,]
  xg<-data_brazil_looped$year; yg<-as.double(data_brazil_looped$bev_share)
  ...
  xg<-data_brazil_looped$year; yg<-as.double(data_brazil_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  brazil <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_brazil_looped$overall; new_A$time_interval <- data_brazil_looped$time_interval
  
  brazil$Hybrid       <- pmax(brazil$Hybrid, 0)
  brazil$Hybrid_upper <- pmax(brazil$Hybrid_upper, 0)
  brazil$Hybrid_lower <- pmax(brazil$Hybrid_lower, 0)
  
  time_80_brazil <- max(subset(brazil, brazil$BEV<=0.8 & brazil$BEV >= 0.2)$x)
  time_50_brazil <- max(subset(brazil, brazil$BEV<=0.5 & brazil$BEV >= 0.2)$x)
  time_20_brazil <- max(subset(brazil, brazil$BEV<=0.2 & brazil$BEV >= 0.1)$x)
  time_20_to_80_brazil <- max(subset(brazil, brazil$BEV<=0.8 & brazil$BEV >= 0.2)$x)-min(subset(brazil, brazil$BEV<=0.8 & brazil$BEV >= 0.2)$x)
  time_80_to_20_brazil <- max(subset(brazil, brazil$ICE<=0.8 & brazil$ICE >= 0.2)$x)-min(subset(brazil, brazil$ICE<=0.8 & brazil$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_brazil; ice_time[i] <- time_80_to_20_brazil
}

timer_brazil <- data.frame("year"=data_brazil[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_brazil$BEV_time == -Inf), which(timer_brazil$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_brazil) > (last_inf_index + 12)) {
  timer_brazil_short <- timer_brazil[(last_inf_index + 1 + 12):nrow(timer_brazil), ]
} else {
  timer_brazil_short <- timer_brazil
}
...
plot_timer <- ggplot(timer_brazil_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_brazil_short$BEV_time[length(timer_brazil_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Brazil transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_brazil_short$BEV_time[length(timer_brazil_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_brazil <- ggplot(brazil, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Brazil - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_brazil), " years ", round(12*(time_20_to_80_brazil-floor(time_20_to_80_brazil)),0), " months"),
...
plot_brazil <- plot_brazil + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(brazil, brazil$x==2024+counter & brazil$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_brazil <- plot_brazil + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(brazil, brazil$x==2024+counter & brazil$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_brazil <- plot_brazil + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_brazil

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_brazil$overall)
BEV <- data.frame(BEV, "overall"=data_brazil$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_brazil$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_brazil <- ggplot(brazil, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Brazil - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_brazil), " years ",
      round(12 * (time_80_to_20_brazil - floor(time_80_to_20_brazil)), 0),
      " months"
    ),
...
plot_ICE_BEV_brazil <- plot_ICE_BEV_brazil +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(brazil, brazil$x == 2024 + counter - 1 & brazil$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_brazil <- plot_ICE_BEV_brazil +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(brazil, brazil$x == 2024 + counter & brazil$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_brazil)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("brazil_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_brazil,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_brazil",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("brazil_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_brazil,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_brazil",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("brazil_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_brazil",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("brazil_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_brazil,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_brazil",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_brazil$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_brazil, "Inf"),
  bev_50 = as_date_from_numeric(time_50_brazil, "Inf"),
  bev_80 = as_date_from_numeric(time_80_brazil, "Inf"),
  bev_20_80 = time_20_to_80_brazil,
  ice_80_20 = time_80_to_20_brazil,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_brazil$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_brazil)
month_label <- last_label_from_data(data_brazil, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_brazil$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_brazil)) nz(tail(data_brazil$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_brazil)) {
    nz(tail(data_brazil$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_brazil)) nz(tail(data_brazil$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_brazil)) {
    nz(tail(data_brazil$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_brazil),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_brazil)
upsert_weight(
