#### China: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "China"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/china.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_china <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "China")
source <- data_china$Source[1]
...
#data_china$BEV <- data_china$`Electric/zero-emission`
#data_china$PHEV <- data_china$Hybrid
#data_china$TOTAL <- data_china$overall
#data_china$OTHERS <- data_china$`Other fuel`
#data_china$TOTAL - data_china$BEV - data_china$PHEV <- data_china$Fossil

data_china$`Electric/zero-emission` <- data_china$BEV
data_china$Hybrid <- data_china$PHEV + data_china$EREV
data_china$overall <- data_china$TOTAL
data_china$`Other fuel` <- data_china$OTHERS
data_china$Fossil <- data_china$TOTAL - data_china$BEV - data_china$PHEV - data_china$EREV


data_china$bev_share   <- data_china$`Electric/zero-emission`/data_china$overall
data_china$ice_share   <- data_china$Fossil/data_china$overall
data_china$hybrid_share<- Hybrid/data_china$overall
data_china$other_share <- data_china$`Other fuel`/data_china$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_china$pure_ICE    <- (data_china$Fossil)/data_china$overall

lastyearly <- ceiling(max(subset(data_china, data_china$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_china_monthly <- subset(data_china, data_china$time_interval=="monthly")
data_china_monthly <- subset(data_china_monthly, data_china_monthly$year>=min(data_china_monthly$year+1))
TTM_shares_china <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_china_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_china_monthly)), "%Y-%m"),
  Other = data_china_monthly$`Other TTM`,
  `ICE` = data_china_monthly$`ICE TTM`,#-data_china_monthly$`HEV TTM`,
  #Petrol = data_china_monthly$`Petrol TTM`,
  #Diesel = data_china_monthly$`Diesel TTM`,
  #Ethanol = data_china_monthly$`Ethanol TTM`,
  #FlexFuel = data_china_monthly$`FlexFuel TTM`,
  #HEV = data_china_monthly$`HEV TTM`,
  PHEV = data_china_monthly$`PHEV TTM`,
  EREV = data_china_monthly$`EREV TTM`,
  BEV = data_china_monthly$`BEV TTM`
)

colnames(TTM_shares_china)[colnames(TTM_shares_china) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_china)[colnames(TTM_shares_china) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_china)[colnames(TTM_shares_china) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_china)[colnames(TTM_shares_china) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_china)[colnames(TTM_shares_china) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_china_long <- TTM_shares_china %>%
...
TTM_barplot_china <- ggplot(TTM_shares_china_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_china_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_china_long$month[substr(TTM_shares_china_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in China",
...
print(TTM_barplot_china)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_china$year)))
extrapol <- 2200
data_china <- subset(data_china, data_china$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_china$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_china$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_china$`Electric/zero-emission`))
for(i in 1:length(data_china$`Electric/zero-emission`)){
  data_china_looped <- data_china[1:i,]
  xg<-data_china_looped$year; yg<-as.double(data_china_looped$bev_share)
  ...
  xg<-data_china_looped$year; yg<-as.double(data_china_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  china <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_china_looped$overall; new_A$time_interval <- data_china_looped$time_interval
  
  china$Hybrid       <- pmax(china$Hybrid, 0)
  china$Hybrid_upper <- pmax(china$Hybrid_upper, 0)
  china$Hybrid_lower <- pmax(china$Hybrid_lower, 0)
  
  time_80_china <- max(subset(china, china$BEV<=0.8 & china$BEV >= 0.2)$x)
  time_50_china <- max(subset(china, china$BEV<=0.5 & china$BEV >= 0.2)$x)
  time_20_china <- max(subset(china, china$BEV<=0.2 & china$BEV >= 0.1)$x)
  time_20_to_80_china <- max(subset(china, china$BEV<=0.8 & china$BEV >= 0.2)$x)-min(subset(china, china$BEV<=0.8 & china$BEV >= 0.2)$x)
  time_80_to_20_china <- max(subset(china, china$ICE<=0.8 & china$ICE >= 0.2)$x)-min(subset(china, china$ICE<=0.8 & china$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_china; ice_time[i] <- time_80_to_20_china
}

timer_china <- data.frame("year"=data_china[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_china$BEV_time == -Inf), which(timer_china$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_china) > (last_inf_index + 12)) {
  timer_china_short <- timer_china[(last_inf_index + 1 + 12):nrow(timer_china), ]
} else {
  timer_china_short <- timer_china
}
...
plot_timer <- ggplot(timer_china_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_china_short$BEV_time[length(timer_china_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for China transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_china_short$BEV_time[length(timer_china_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_china <- ggplot(china, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in China - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_china), " years ", round(12*(time_20_to_80_china-floor(time_20_to_80_china)),0), " months"),
...
plot_china <- plot_china + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(china, china$x==2024+counter & china$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_china <- plot_china + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(china, china$x==2024+counter & china$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_china <- plot_china + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_china

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_china$overall)
BEV <- data.frame(BEV, "overall"=data_china$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_china$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_china <- ggplot(china, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in China - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_china), " years ",
      round(12 * (time_80_to_20_china - floor(time_80_to_20_china)), 0),
      " months"
    ),
...
plot_ICE_BEV_china <- plot_ICE_BEV_china +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(china, china$x == 2024 + counter - 1 & china$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_china <- plot_ICE_BEV_china +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(china, china$x == 2024 + counter & china$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_china)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("china_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_china,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_china",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("china_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_china,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_china",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("china_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_china",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("china_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_china,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_china",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_china$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_china, "Inf"),
  bev_50 = as_date_from_numeric(time_50_china, "Inf"),
  bev_80 = as_date_from_numeric(time_80_china, "Inf"),
  bev_20_80 = time_20_to_80_china,
  ice_80_20 = time_80_to_20_china,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_china$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_china)
month_label <- last_label_from_data(data_china, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_china$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_china)) nz(tail(data_china$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_china)) {
    nz(tail(data_china$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_china)) nz(tail(data_china$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_china)) {
    nz(tail(data_china$`HEV TTM`, 1))
...
second_value_ttm <- if (is.finite(phev_ttm+erev_ttm)) phev_ttm+erev_ttm else if (is.finite(hybrid_ttm+erev_ttm)) hybrid_ttm+erev_ttm else 0
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_china),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_china)
upsert_weight(
