#### Australia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Australia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/australia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_australia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Australia")
source <- data_australia$Source[1]
...
#data_australia$BEV <- data_australia$`Electric/zero-emission`
#data_australia$PHEV <- data_australia$Hybrid
#data_australia$TOTAL <- data_australia$overall
#data_australia$OTHERS <- data_australia$`Other fuel`
#data_australia$TOTAL - data_australia$BEV - data_australia$PHEV <- data_australia$Fossil

data_australia$`Electric/zero-emission` <- data_australia$BEV
data_australia$Hybrid <- data_australia$PHEV
data_australia$overall <- data_australia$TOTAL
data_australia$`Other fuel` <- data_australia$OTHERS
data_australia$Fossil <- data_australia$TOTAL - data_australia$BEV - data_australia$PHEV


data_australia$bev_share   <- data_australia$`Electric/zero-emission`/data_australia$overall
data_australia$ice_share   <- data_australia$Fossil/data_australia$overall
data_australia$hybrid_share<- data_australia$Hybrid/data_australia$overall
data_australia$other_share <- data_australia$`Other fuel`/data_australia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_australia$pure_ICE    <- (data_australia$Fossil)/data_australia$overall

lastyearly <- ceiling(max(subset(data_australia, data_australia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_australia_monthly <- subset(data_australia, data_australia$time_interval=="monthly")
data_australia_monthly <- subset(data_australia_monthly, data_australia_monthly$year>=min(data_australia_monthly$year+1))
TTM_shares_australia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_australia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_australia_monthly)), "%Y-%m"),
  Other = data_australia_monthly$`Other TTM`,
  #`ICE` = data_australia_monthly$`ICE TTM`,#-data_australia_monthly$`HEV TTM`,
  Petrol = data_australia_monthly$`Petrol TTM`,
  Diesel = data_australia_monthly$`Diesel TTM`,
  #Ethanol = data_australia_monthly$`Ethanol TTM`,
  #FlexFuel = data_australia_monthly$`FlexFuel TTM`,
  HEV = data_australia_monthly$`HEV TTM`,
  PHEV = data_australia_monthly$`PHEV TTM`,
  BEV = data_australia_monthly$`BEV TTM`
)

colnames(TTM_shares_australia)[colnames(TTM_shares_australia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_australia)[colnames(TTM_shares_australia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_australia)[colnames(TTM_shares_australia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_australia)[colnames(TTM_shares_australia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_australia)[colnames(TTM_shares_australia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_australia_long <- TTM_shares_australia %>%
...
TTM_barplot_australia <- ggplot(TTM_shares_australia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_australia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_australia_long$month[substr(TTM_shares_australia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Australia",
...
print(TTM_barplot_australia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_australia$year)))
extrapol <- 2200
data_australia <- subset(data_australia, data_australia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_australia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_australia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_australia$`Electric/zero-emission`))
for(i in 1:length(data_australia$`Electric/zero-emission`)){
  data_australia_looped <- data_australia[1:i,]
  xg<-data_australia_looped$year; yg<-as.double(data_australia_looped$bev_share)
  ...
  xg<-data_australia_looped$year; yg<-as.double(data_australia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  australia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_australia_looped$overall; new_A$time_interval <- data_australia_looped$time_interval
  
  australia$Hybrid       <- pmax(australia$Hybrid, 0)
  australia$Hybrid_upper <- pmax(australia$Hybrid_upper, 0)
  australia$Hybrid_lower <- pmax(australia$Hybrid_lower, 0)
  
  time_80_australia <- max(subset(australia, australia$BEV<=0.8 & australia$BEV >= 0.2)$x)
  time_50_australia <- max(subset(australia, australia$BEV<=0.5 & australia$BEV >= 0.2)$x)
  time_20_australia <- max(subset(australia, australia$BEV<=0.2 & australia$BEV >= 0.1)$x)
  time_20_to_80_australia <- max(subset(australia, australia$BEV<=0.8 & australia$BEV >= 0.2)$x)-min(subset(australia, australia$BEV<=0.8 & australia$BEV >= 0.2)$x)
  time_80_to_20_australia <- max(subset(australia, australia$ICE<=0.8 & australia$ICE >= 0.2)$x)-min(subset(australia, australia$ICE<=0.8 & australia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_australia; ice_time[i] <- time_80_to_20_australia
}

timer_australia <- data.frame("year"=data_australia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_australia$BEV_time == -Inf), which(timer_australia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_australia) > (last_inf_index + 12)) {
  timer_australia_short <- timer_australia[(last_inf_index + 1 + 12):nrow(timer_australia), ]
} else {
  timer_australia_short <- timer_australia
}
...
plot_timer <- ggplot(timer_australia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_australia_short$BEV_time[length(timer_australia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Australia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_australia_short$BEV_time[length(timer_australia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_australia <- ggplot(australia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Australia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_australia), " years ", round(12*(time_20_to_80_australia-floor(time_20_to_80_australia)),0), " months"),
...
plot_australia <- plot_australia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(australia, australia$x==2024+counter & australia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_australia <- plot_australia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                                label=paste0("Jan ",2025+counter,": ",round(subset(australia, australia$x==2024+counter & australia$Type=="New Registrations")$BEV*100, 1),"%"),
                                                size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_australia <- plot_australia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                         width = unit(1*1920/1280
                                                                                      ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_australia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_australia$overall)
BEV <- data.frame(BEV, "overall"=data_australia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_australia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_australia <- ggplot(australia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Australia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_australia), " years ",
      round(12 * (time_80_to_20_australia - floor(time_80_to_20_australia)), 0),
      " months"
    ),
...
plot_ICE_BEV_australia <- plot_ICE_BEV_australia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(australia, australia$x == 2024 + counter - 1 & australia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_australia <- plot_ICE_BEV_australia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(australia, australia$x == 2024 + counter & australia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_australia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("australia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_australia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_australia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("australia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_australia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_australia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("australia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_australia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("australia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_australia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_australia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_australia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_australia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_australia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_australia, "Inf"),
  bev_20_80 = time_20_to_80_australia,
  ice_80_20 = time_80_to_20_australia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_australia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_australia)
month_label <- last_label_from_data(data_australia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_australia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_australia)) nz(tail(data_australia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_australia)) {
    nz(tail(data_australia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_australia)) nz(tail(data_australia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_australia)) {
    nz(tail(data_australia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_australia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_australia)
upsert_weight(
