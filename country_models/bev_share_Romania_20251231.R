#### Romania: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Romania"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/romania.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_romania <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Romania")
source <- data_romania$Source[1]
...
#data_romania$BEV <- data_romania$`Electric/zero-emission`
#data_romania$PHEV <- data_romania$Hybrid
#data_romania$TOTAL <- data_romania$overall
#data_romania$OTHERS <- data_romania$`Other fuel`
#data_romania$TOTAL - data_romania$BEV - data_romania$PHEV <- data_romania$Fossil

data_romania$`Electric/zero-emission` <- data_romania$BEV
data_romania$Hybrid <- data_romania$PHEV
data_romania$overall <- data_romania$TOTAL
data_romania$`Other fuel` <- data_romania$OTHERS
data_romania$Fossil <- data_romania$TOTAL - data_romania$BEV - data_romania$PHEV


data_romania$bev_share   <- data_romania$`Electric/zero-emission`/data_romania$overall
data_romania$ice_share   <- data_romania$Fossil/data_romania$overall
data_romania$hybrid_share<- data_romania$Hybrid/data_romania$overall
data_romania$other_share <- data_romania$`Other fuel`/data_romania$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_romania$pure_ICE    <- (data_romania$Fossil)/data_romania$overall

lastyearly <- ceiling(max(subset(data_romania, data_romania$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_romania_monthly <- subset(data_romania, data_romania$time_interval=="monthly")
data_romania_monthly <- subset(data_romania_monthly, data_romania_monthly$year>=min(data_romania_monthly$year+1))
TTM_shares_romania <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_romania_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_romania_monthly)), "%Y-%m"),
  Other = data_romania_monthly$`Other TTM`,
  #`ICE` = data_romania_monthly$`ICE TTM`,#-data_romania_monthly$`HEV TTM`,
  Petrol = data_romania_monthly$`Petrol TTM`,
  Diesel = data_romania_monthly$`Diesel TTM`,
  #Ethanol = data_romania_monthly$`Ethanol TTM`,
  #FlexFuel = data_romania_monthly$`FlexFuel TTM`,
  HEV = data_romania_monthly$`HEV TTM`,
  PHEV = data_romania_monthly$`PHEV TTM`,
  BEV = data_romania_monthly$`BEV TTM`
)

colnames(TTM_shares_romania)[colnames(TTM_shares_romania) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_romania)[colnames(TTM_shares_romania) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_romania)[colnames(TTM_shares_romania) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_romania)[colnames(TTM_shares_romania) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_romania)[colnames(TTM_shares_romania) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_romania_long <- TTM_shares_romania %>%
...
TTM_barplot_romania <- ggplot(TTM_shares_romania_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_romania_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_romania_long$month[substr(TTM_shares_romania_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Romania",
...
print(TTM_barplot_romania)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_romania$year)))
extrapol <- 2200
data_romania <- subset(data_romania, data_romania$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_romania$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_romania$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_romania$`Electric/zero-emission`))
for(i in 1:length(data_romania$`Electric/zero-emission`)){
  data_romania_looped <- data_romania[1:i,]
  xg<-data_romania_looped$year; yg<-as.double(data_romania_looped$bev_share)
  ...
  xg<-data_romania_looped$year; yg<-as.double(data_romania_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  romania <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_romania_looped$overall; new_A$time_interval <- data_romania_looped$time_interval
  
  romania$Hybrid       <- pmax(romania$Hybrid, 0)
  romania$Hybrid_upper <- pmax(romania$Hybrid_upper, 0)
  romania$Hybrid_lower <- pmax(romania$Hybrid_lower, 0)
  
  time_80_romania <- max(subset(romania, romania$BEV<=0.8 & romania$BEV >= 0.2)$x)
  time_50_romania <- max(subset(romania, romania$BEV<=0.5 & romania$BEV >= 0.2)$x)
  time_20_romania <- max(subset(romania, romania$BEV<=0.2 & romania$BEV >= 0.1)$x)
  time_20_to_80_romania <- max(subset(romania, romania$BEV<=0.8 & romania$BEV >= 0.2)$x)-min(subset(romania, romania$BEV<=0.8 & romania$BEV >= 0.2)$x)
  time_80_to_20_romania <- max(subset(romania, romania$ICE<=0.8 & romania$ICE >= 0.2)$x)-min(subset(romania, romania$ICE<=0.8 & romania$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_romania; ice_time[i] <- time_80_to_20_romania
}

timer_romania <- data.frame("year"=data_romania[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_romania$BEV_time == -Inf), which(timer_romania$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_romania) > (last_inf_index + 12)) {
  timer_romania_short <- timer_romania[(last_inf_index + 1 + 12):nrow(timer_romania), ]
} else {
  timer_romania_short <- timer_romania
}
...
plot_timer <- ggplot(timer_romania_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_romania_short$BEV_time[length(timer_romania_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Romania transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_romania_short$BEV_time[length(timer_romania_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_romania <- ggplot(romania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Romania - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_romania), " years ", round(12*(time_20_to_80_romania-floor(time_20_to_80_romania)),0), " months"),
...
plot_romania <- plot_romania + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(romania, romania$x==2024+counter & romania$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_romania <- plot_romania + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(romania, romania$x==2024+counter & romania$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_romania <- plot_romania + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_romania

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_romania$overall)
BEV <- data.frame(BEV, "overall"=data_romania$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_romania$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_romania <- ggplot(romania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Romania - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_romania), " years ",
      round(12 * (time_80_to_20_romania - floor(time_80_to_20_romania)), 0),
      " months"
    ),
...
plot_ICE_BEV_romania <- plot_ICE_BEV_romania +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(romania, romania$x == 2024 + counter - 1 & romania$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_romania <- plot_ICE_BEV_romania +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(romania, romania$x == 2024 + counter & romania$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_romania)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("romania_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_romania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_romania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("romania_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_romania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_romania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("romania_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_romania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("romania_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_romania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_romania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_romania$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_romania, "Inf"),
  bev_50 = as_date_from_numeric(time_50_romania, "Inf"),
  bev_80 = as_date_from_numeric(time_80_romania, "Inf"),
  bev_20_80 = time_20_to_80_romania,
  ice_80_20 = time_80_to_20_romania,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_romania$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_romania)
month_label <- last_label_from_data(data_romania, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_romania$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_romania)) nz(tail(data_romania$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_romania)) {
    nz(tail(data_romania$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_romania)) nz(tail(data_romania$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_romania)) {
    nz(tail(data_romania$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_romania),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_romania)
upsert_weight(
