#### Malta: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Malta"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/malta.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_malta <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Malta")
source <- data_malta$Source[1]
...
#data_malta$BEV <- data_malta$`Electric/zero-emission`
#data_malta$PHEV <- data_malta$Hybrid
#data_malta$TOTAL <- data_malta$overall
#data_malta$OTHERS <- data_malta$`Other fuel`
#data_malta$TOTAL - data_malta$BEV - data_malta$PHEV <- data_malta$Fossil

data_malta$`Electric/zero-emission` <- data_malta$BEV
data_malta$Hybrid <- data_malta$PHEV
data_malta$overall <- data_malta$TOTAL
data_malta$`Other fuel` <- data_malta$OTHERS
data_malta$Fossil <- data_malta$TOTAL - data_malta$BEV - data_malta$PHEV


data_malta$bev_share   <- data_malta$`Electric/zero-emission`/data_malta$overall
data_malta$ice_share   <- data_malta$Fossil/data_malta$overall
data_malta$hybrid_share<- data_malta$Hybrid/data_malta$overall
data_malta$other_share <- data_malta$`Other fuel`/data_malta$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_malta$pure_ICE    <- (data_malta$Fossil)/data_malta$overall

lastyearly <- ceiling(max(subset(data_malta, data_malta$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_malta_monthly <- subset(data_malta, data_malta$time_interval=="monthly")
data_malta_monthly <- subset(data_malta_monthly, data_malta_monthly$year>=min(data_malta_monthly$year+1))
TTM_shares_malta <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_malta_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_malta_monthly)), "%Y-%m"),
  Other = data_malta_monthly$`Other TTM`,
  #`ICE` = data_malta_monthly$`ICE TTM`,#-data_malta_monthly$`HEV TTM`,
  Petrol = data_malta_monthly$`Petrol TTM`,
  Diesel = data_malta_monthly$`Diesel TTM`,
  #Ethanol = data_malta_monthly$`Ethanol TTM`,
  #FlexFuel = data_malta_monthly$`FlexFuel TTM`,
  HEV = data_malta_monthly$`HEV TTM`,
  PHEV = data_malta_monthly$`PHEV TTM`,
  BEV = data_malta_monthly$`BEV TTM`
)

colnames(TTM_shares_malta)[colnames(TTM_shares_malta) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_malta)[colnames(TTM_shares_malta) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_malta)[colnames(TTM_shares_malta) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_malta)[colnames(TTM_shares_malta) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_malta)[colnames(TTM_shares_malta) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_malta_long <- TTM_shares_malta %>%
...
TTM_barplot_malta <- ggplot(TTM_shares_malta_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_malta_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_malta_long$month[substr(TTM_shares_malta_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Malta",
...
print(TTM_barplot_malta)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_malta$year)))
extrapol <- 2200
data_malta <- subset(data_malta, data_malta$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_malta$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_malta$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_malta$`Electric/zero-emission`))
for(i in 1:length(data_malta$`Electric/zero-emission`)){
  data_malta_looped <- data_malta[1:i,]
  xg<-data_malta_looped$year; yg<-as.double(data_malta_looped$bev_share)
  ...
  xg<-data_malta_looped$year; yg<-as.double(data_malta_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  malta <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_malta_looped$overall; new_A$time_interval <- data_malta_looped$time_interval
  
  malta$Hybrid       <- pmax(malta$Hybrid, 0)
  malta$Hybrid_upper <- pmax(malta$Hybrid_upper, 0)
  malta$Hybrid_lower <- pmax(malta$Hybrid_lower, 0)
  
  time_80_malta <- max(subset(malta, malta$BEV<=0.8 & malta$BEV >= 0.2)$x)
  time_50_malta <- max(subset(malta, malta$BEV<=0.5 & malta$BEV >= 0.2)$x)
  time_20_malta <- max(subset(malta, malta$BEV<=0.2 & malta$BEV >= 0.1)$x)
  time_20_to_80_malta <- max(subset(malta, malta$BEV<=0.8 & malta$BEV >= 0.2)$x)-min(subset(malta, malta$BEV<=0.8 & malta$BEV >= 0.2)$x)
  time_80_to_20_malta <- max(subset(malta, malta$ICE<=0.8 & malta$ICE >= 0.2)$x)-min(subset(malta, malta$ICE<=0.8 & malta$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_malta; ice_time[i] <- time_80_to_20_malta
}

timer_malta <- data.frame("year"=data_malta[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_malta$BEV_time == -Inf), which(timer_malta$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_malta) > (last_inf_index + 12)) {
  timer_malta_short <- timer_malta[(last_inf_index + 1 + 12):nrow(timer_malta), ]
} else {
  timer_malta_short <- timer_malta
}
...
plot_timer <- ggplot(timer_malta_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_malta_short$BEV_time[length(timer_malta_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Malta transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_malta_short$BEV_time[length(timer_malta_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_malta <- ggplot(malta, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Malta - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_malta), " years ", round(12*(time_20_to_80_malta-floor(time_20_to_80_malta)),0), " months"),
...
plot_malta <- plot_malta + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(malta, malta$x==2024+counter & malta$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_malta <- plot_malta + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                                label=paste0("Jan ",2025+counter,": ",round(subset(malta, malta$x==2024+counter & malta$Type=="New Registrations")$BEV*100, 1),"%"),
                                                size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_malta <- plot_malta + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                         width = unit(1*1920/1280
                                                                                      ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_malta

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_malta$overall)
BEV <- data.frame(BEV, "overall"=data_malta$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_malta$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_malta <- ggplot(malta, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Malta - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_malta), " years ",
      round(12 * (time_80_to_20_malta - floor(time_80_to_20_malta)), 0),
      " months"
    ),
...
plot_ICE_BEV_malta <- plot_ICE_BEV_malta +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(malta, malta$x == 2024 + counter - 1 & malta$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_malta <- plot_ICE_BEV_malta +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(malta, malta$x == 2024 + counter & malta$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_malta)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("malta_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_malta,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malta",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("malta_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_malta,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malta",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("malta_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malta",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("malta_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_malta,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malta",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_malta$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_malta, "Inf"),
  bev_50 = as_date_from_numeric(time_50_malta, "Inf"),
  bev_80 = as_date_from_numeric(time_80_malta, "Inf"),
  bev_20_80 = time_20_to_80_malta,
  ice_80_20 = time_80_to_20_malta,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_malta$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_malta)
month_label <- last_label_from_data(data_malta, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_malta$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_malta)) nz(tail(data_malta$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_malta)) {
    nz(tail(data_malta$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_malta)) nz(tail(data_malta$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_malta)) {
    nz(tail(data_malta$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_malta),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_malta)
upsert_weight(
