#### Poland: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Poland"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/poland.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_poland <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Poland")
source <- data_poland$Source[1]
...
#data_poland$BEV <- data_poland$`Electric/zero-emission`
#data_poland$PHEV <- data_poland$Hybrid
#data_poland$TOTAL <- data_poland$overall
#data_poland$OTHERS <- data_poland$`Other fuel`
#data_poland$TOTAL - data_poland$BEV - data_poland$PHEV <- data_poland$Fossil

data_poland$`Electric/zero-emission` <- data_poland$BEV
data_poland$Hybrid <- data_poland$PHEV
data_poland$overall <- data_poland$TOTAL
data_poland$`Other fuel` <- data_poland$OTHERS
data_poland$Fossil <- data_poland$TOTAL - data_poland$BEV - data_poland$PHEV


data_poland$bev_share   <- data_poland$`Electric/zero-emission`/data_poland$overall
data_poland$ice_share   <- data_poland$Fossil/data_poland$overall
data_poland$hybrid_share<- data_poland$Hybrid/data_poland$overall
data_poland$other_share <- data_poland$`Other fuel`/data_poland$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_poland$pure_ICE    <- (data_poland$Fossil)/data_poland$overall

lastyearly <- ceiling(max(subset(data_poland, data_poland$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_poland_monthly <- subset(data_poland, data_poland$time_interval=="monthly")
data_poland_monthly <- subset(data_poland_monthly, data_poland_monthly$year>=min(data_poland_monthly$year+1))
TTM_shares_poland <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_poland_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_poland_monthly)), "%Y-%m"),
  Other = data_poland_monthly$`Other TTM`,
  #`ICE` = data_poland_monthly$`ICE TTM`,#-data_poland_monthly$`HEV TTM`,
  Petrol = data_poland_monthly$`Petrol TTM`,
  Diesel = data_poland_monthly$`Diesel TTM`,
  #Ethanol = data_poland_monthly$`Ethanol TTM`,
  #FlexFuel = data_poland_monthly$`FlexFuel TTM`,
  HEV = data_poland_monthly$`HEV TTM`,
  PHEV = data_poland_monthly$`PHEV TTM`,
  BEV = data_poland_monthly$`BEV TTM`
)

colnames(TTM_shares_poland)[colnames(TTM_shares_poland) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_poland)[colnames(TTM_shares_poland) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_poland)[colnames(TTM_shares_poland) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_poland)[colnames(TTM_shares_poland) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_poland)[colnames(TTM_shares_poland) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_poland_long <- TTM_shares_poland %>%
...
TTM_barplot_poland <- ggplot(TTM_shares_poland_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_poland_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_poland_long$month[substr(TTM_shares_poland_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Poland",
...
print(TTM_barplot_poland)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_poland$year)))
extrapol <- 2200
data_poland <- subset(data_poland, data_poland$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_poland$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_poland$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_poland$`Electric/zero-emission`))
for(i in 1:length(data_poland$`Electric/zero-emission`)){
  data_poland_looped <- data_poland[1:i,]
  xg<-data_poland_looped$year; yg<-as.double(data_poland_looped$bev_share)
  ...
  xg<-data_poland_looped$year; yg<-as.double(data_poland_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  poland <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_poland_looped$overall; new_A$time_interval <- data_poland_looped$time_interval
  
  poland$Hybrid       <- pmax(poland$Hybrid, 0)
  poland$Hybrid_upper <- pmax(poland$Hybrid_upper, 0)
  poland$Hybrid_lower <- pmax(poland$Hybrid_lower, 0)
  
  time_80_poland <- max(subset(poland, poland$BEV<=0.8 & poland$BEV >= 0.2)$x)
  time_50_poland <- max(subset(poland, poland$BEV<=0.5 & poland$BEV >= 0.2)$x)
  time_20_poland <- max(subset(poland, poland$BEV<=0.2 & poland$BEV >= 0.1)$x)
  time_20_to_80_poland <- max(subset(poland, poland$BEV<=0.8 & poland$BEV >= 0.2)$x)-min(subset(poland, poland$BEV<=0.8 & poland$BEV >= 0.2)$x)
  time_80_to_20_poland <- max(subset(poland, poland$ICE<=0.8 & poland$ICE >= 0.2)$x)-min(subset(poland, poland$ICE<=0.8 & poland$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_poland; ice_time[i] <- time_80_to_20_poland
}

timer_poland <- data.frame("year"=data_poland[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_poland$BEV_time == -Inf), which(timer_poland$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_poland) > (last_inf_index + 12)) {
  timer_poland_short <- timer_poland[(last_inf_index + 1 + 12):nrow(timer_poland), ]
} else {
  timer_poland_short <- timer_poland
}
...
plot_timer <- ggplot(timer_poland_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_poland_short$BEV_time[length(timer_poland_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Poland transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_poland_short$BEV_time[length(timer_poland_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_poland <- ggplot(poland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Poland - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_poland), " years ", round(12*(time_20_to_80_poland-floor(time_20_to_80_poland)),0), " months"),
...
plot_poland <- plot_poland + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(poland, poland$x==2024+counter & poland$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_poland <- plot_poland + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                      label=paste0("Jan ",2025+counter,": ",round(subset(poland, poland$x==2024+counter & poland$Type=="New Registrations")$BEV*100, 1),"%"),
                                      size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_poland <- plot_poland + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                               width = unit(1*1920/1280
                                                                            ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_poland

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_poland$overall)
BEV <- data.frame(BEV, "overall"=data_poland$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_poland$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_poland <- ggplot(poland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Poland - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_poland), " years ",
      round(12 * (time_80_to_20_poland - floor(time_80_to_20_poland)), 0),
      " months"
    ),
...
plot_ICE_BEV_poland <- plot_ICE_BEV_poland +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(poland, poland$x == 2024 + counter - 1 & poland$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_poland <- plot_ICE_BEV_poland +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(poland, poland$x == 2024 + counter & poland$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_poland)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("poland_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_poland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_poland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("poland_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_poland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_poland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("poland_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_poland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("poland_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_poland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_poland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_poland$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_poland, "Inf"),
  bev_50 = as_date_from_numeric(time_50_poland, "Inf"),
  bev_80 = as_date_from_numeric(time_80_poland, "Inf"),
  bev_20_80 = time_20_to_80_poland,
  ice_80_20 = time_80_to_20_poland,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_poland$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_poland)
month_label <- last_label_from_data(data_poland, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_poland$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_poland)) nz(tail(data_poland$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_poland)) {
    nz(tail(data_poland$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_poland)) nz(tail(data_poland$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_poland)) {
    nz(tail(data_poland$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_poland),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_poland)
upsert_weight(
