#### USA: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "USA"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/usa.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_usa <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "USA")
source <- data_usa$Source[1]
...
#data_usa$BEV <- data_usa$`Electric/zero-emission`
#data_usa$PHEV <- data_usa$Hybrid
#data_usa$TOTAL <- data_usa$overall
#data_usa$OTHERS <- data_usa$`Other fuel`
#data_usa$TOTAL - data_usa$BEV - data_usa$PHEV <- data_usa$Fossil

data_usa$`Electric/zero-emission` <- data_usa$BEV
data_usa$Hybrid <- data_usa$PHEV
data_usa$overall <- data_usa$TOTAL
data_usa$`Other fuel` <- data_usa$OTHERS
data_usa$Fossil <- data_usa$TOTAL - data_usa$BEV - data_usa$PHEV


data_usa$bev_share   <- data_usa$`Electric/zero-emission`/data_usa$overall
data_usa$ice_share   <- data_usa$Fossil/data_usa$overall
data_usa$hybrid_share<- data_usa$Hybrid/data_usa$overall
data_usa$other_share <- data_usa$`Other fuel`/data_usa$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_usa$pure_ICE    <- (data_usa$Fossil)/data_usa$overall

lastyearly <- ceiling(max(subset(data_usa, data_usa$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_usa_monthly <- subset(data_usa, data_usa$time_interval=="monthly")
data_usa_monthly <- subset(data_usa_monthly, data_usa_monthly$year>=min(data_usa_monthly$year+1))
TTM_shares_usa <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_usa_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_usa_monthly)), "%Y-%m"),
  Other = data_usa_monthly$`Other TTM`,
  `ICE` = data_usa_monthly$`ICE TTM`,#-data_usa_monthly$`HEV TTM`,
  #Petrol = data_usa_monthly$`Petrol TTM`,
  #Diesel = data_usa_monthly$`Diesel TTM`,
  #Ethanol = data_usa_monthly$`Ethanol TTM`,
  #FlexFuel = data_usa_monthly$`FlexFuel TTM`,
  HEV = data_usa_monthly$`HEV TTM`,
  PHEV = data_usa_monthly$`PHEV TTM`,
  BEV = data_usa_monthly$`BEV TTM`
)

colnames(TTM_shares_usa)[colnames(TTM_shares_usa) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_usa)[colnames(TTM_shares_usa) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_usa)[colnames(TTM_shares_usa) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_usa)[colnames(TTM_shares_usa) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_usa)[colnames(TTM_shares_usa) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_usa_long <- TTM_shares_usa %>%
...
TTM_barplot_usa <- ggplot(TTM_shares_usa_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_usa_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_usa_long$month[substr(TTM_shares_usa_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in USA",
...
print(TTM_barplot_usa)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_usa$year)))
extrapol <- 2200
data_usa <- subset(data_usa, data_usa$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_usa$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_usa$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_usa$`Electric/zero-emission`))
for(i in 1:length(data_usa$`Electric/zero-emission`)){
  data_usa_looped <- data_usa[1:i,]
  xg<-data_usa_looped$year; yg<-as.double(data_usa_looped$bev_share)
  ...
  xg<-data_usa_looped$year; yg<-as.double(data_usa_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  usa <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_usa_looped$overall; new_A$time_interval <- data_usa_looped$time_interval
  
  usa$Hybrid       <- pmax(usa$Hybrid, 0)
  usa$Hybrid_upper <- pmax(usa$Hybrid_upper, 0)
  usa$Hybrid_lower <- pmax(usa$Hybrid_lower, 0)
  
  time_80_usa <- max(subset(usa, usa$BEV<=0.8 & usa$BEV >= 0.2)$x)
  time_50_usa <- max(subset(usa, usa$BEV<=0.5 & usa$BEV >= 0.2)$x)
  time_20_usa <- max(subset(usa, usa$BEV<=0.2 & usa$BEV >= 0.1)$x)
  time_20_to_80_usa <- max(subset(usa, usa$BEV<=0.8 & usa$BEV >= 0.2)$x)-min(subset(usa, usa$BEV<=0.8 & usa$BEV >= 0.2)$x)
  time_80_to_20_usa <- max(subset(usa, usa$ICE<=0.8 & usa$ICE >= 0.2)$x)-min(subset(usa, usa$ICE<=0.8 & usa$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_usa; ice_time[i] <- time_80_to_20_usa
}

timer_usa <- data.frame("year"=data_usa[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_usa$BEV_time == -Inf), which(timer_usa$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_usa) > (last_inf_index + 12)) {
  timer_usa_short <- timer_usa[(last_inf_index + 1 + 12):nrow(timer_usa), ]
} else {
  timer_usa_short <- timer_usa
}
...
plot_timer <- ggplot(timer_usa_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_usa_short$BEV_time[length(timer_usa_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for USA transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_usa_short$BEV_time[length(timer_usa_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_usa <- ggplot(usa, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in USA - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_usa), " years ", round(12*(time_20_to_80_usa-floor(time_20_to_80_usa)),0), " months"),
...
plot_usa <- plot_usa + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(usa, usa$x==2024+counter & usa$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_usa <- plot_usa + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(usa, usa$x==2024+counter & usa$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_usa <- plot_usa + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_usa

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_usa$overall)
BEV <- data.frame(BEV, "overall"=data_usa$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_usa$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_usa <- ggplot(usa, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in USA - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_usa), " years ",
      round(12 * (time_80_to_20_usa - floor(time_80_to_20_usa)), 0),
      " months"
    ),
...
plot_ICE_BEV_usa <- plot_ICE_BEV_usa +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(usa, usa$x == 2024 + counter - 1 & usa$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_usa <- plot_ICE_BEV_usa +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(usa, usa$x == 2024 + counter & usa$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_usa)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("usa_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_usa,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_usa",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("usa_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_usa,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_usa",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("usa_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_usa",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("usa_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_usa,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_usa",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_usa$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_usa, "Inf"),
  bev_50 = as_date_from_numeric(time_50_usa, "Inf"),
  bev_80 = as_date_from_numeric(time_80_usa, "Inf"),
  bev_20_80 = time_20_to_80_usa,
  ice_80_20 = time_80_to_20_usa,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_usa$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_usa)
month_label <- last_label_from_data(data_usa, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_usa$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_usa)) nz(tail(data_usa$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_usa)) {
    nz(tail(data_usa$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_usa)) nz(tail(data_usa$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_usa)) {
    nz(tail(data_usa$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_usa),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_usa)
upsert_weight(
