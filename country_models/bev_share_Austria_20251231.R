#### Austria: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Austria"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/austria.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_austria <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Austria")
source <- data_austria$Source[1]
...
#data_austria$BEV <- data_austria$`Electric/zero-emission`
#data_austria$PHEV <- data_austria$Hybrid
#data_austria$TOTAL <- data_austria$overall
#data_austria$OTHERS <- data_austria$`Other fuel`
#data_austria$TOTAL - data_austria$BEV - data_austria$PHEV <- data_austria$Fossil

data_austria$`Electric/zero-emission` <- data_austria$BEV
data_austria$Hybrid <- data_austria$PHEV
data_austria$overall <- data_austria$TOTAL
data_austria$`Other fuel` <- data_austria$OTHERS
data_austria$Fossil <- data_austria$TOTAL - data_austria$BEV - data_austria$PHEV


data_austria$bev_share   <- data_austria$`Electric/zero-emission`/data_austria$overall
data_austria$ice_share   <- data_austria$Fossil/data_austria$overall
data_austria$hybrid_share<- data_austria$Hybrid/data_austria$overall
data_austria$other_share <- data_austria$`Other fuel`/data_austria$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_austria$pure_ICE    <- (data_austria$Fossil)/data_austria$overall

lastyearly <- ceiling(max(subset(data_austria, data_austria$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_austria_monthly <- subset(data_austria, data_austria$time_interval=="monthly")
data_austria_monthly <- subset(data_austria_monthly, data_austria_monthly$year>=min(data_austria_monthly$year+1))
TTM_shares_austria <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_austria_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_austria_monthly)), "%Y-%m"),
  Other = data_austria_monthly$`Other TTM`,
  #`ICE` = data_austria_monthly$`ICE TTM`,#-data_austria_monthly$`HEV TTM`,
  Petrol = data_austria_monthly$`Petrol TTM`,
  Diesel = data_austria_monthly$`Diesel TTM`,
  #Ethanol = data_austria_monthly$`Ethanol TTM`,
  #FlexFuel = data_austria_monthly$`FlexFuel TTM`,
  HEV = data_austria_monthly$`HEV TTM`,
  PHEV = data_austria_monthly$`PHEV TTM`,
  BEV = data_austria_monthly$`BEV TTM`
)

colnames(TTM_shares_austria)[colnames(TTM_shares_austria) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_austria)[colnames(TTM_shares_austria) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_austria)[colnames(TTM_shares_austria) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_austria)[colnames(TTM_shares_austria) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_austria)[colnames(TTM_shares_austria) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_austria_long <- TTM_shares_austria %>%
...
TTM_barplot_austria <- ggplot(TTM_shares_austria_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_austria_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_austria_long$month[substr(TTM_shares_austria_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Austria",
...
print(TTM_barplot_austria)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_austria$year)))
extrapol <- 2200
data_austria <- subset(data_austria, data_austria$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_austria$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_austria$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_austria$`Electric/zero-emission`))
for(i in 1:length(data_austria$`Electric/zero-emission`)){
  data_austria_looped <- data_austria[1:i,]
  xg<-data_austria_looped$year; yg<-as.double(data_austria_looped$bev_share)
  ...
  xg<-data_austria_looped$year; yg<-as.double(data_austria_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  new_A$overall <- data_austria_looped$overall; new_A$time_interval <- data_austria_looped$time_interval
  ...
  time_80_austria <- max(subset(austria, austria$BEV<=0.8 & austria$BEV >= 0.2)$x)
  time_50_austria <- max(subset(austria, austria$BEV<=0.5 & austria$BEV >= 0.2)$x)
  time_20_austria <- max(subset(austria, austria$BEV<=0.2 & austria$BEV >= 0.1)$x)
  time_20_to_80_austria <- max(subset(austria, austria$BEV<=0.8 & austria$BEV >= 0.2)$x)-min(subset(austria, austria$BEV<=0.8 & austria$BEV >= 0.2)$x)
  time_80_to_20_austria <- max(subset(austria, austria$ICE<=0.8 & austria$ICE >= 0.2)$x)-min(subset(austria, austria$ICE<=0.8 & austria$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_austria; ice_time[i] <- time_80_to_20_austria
}

timer_austria <- data.frame("year"=data_austria[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_austria$BEV_time == -Inf), which(timer_austria$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_austria) > (last_inf_index + 12)) {
  timer_austria_short <- timer_austria[(last_inf_index + 1 + 12):nrow(timer_austria), ]
} else {
  timer_austria_short <- timer_austria
}
...
plot_timer <- ggplot(timer_austria_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_austria_short$BEV_time[length(timer_austria_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Austria transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_austria_short$BEV_time[length(timer_austria_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_austria <- ggplot(austria, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Austria - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_austria), " years ", round(12*(time_20_to_80_austria-floor(time_20_to_80_austria)),0), " months"),
...
plot_austria <- plot_austria + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(austria, austria$x==2024+counter & austria$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_austria <- plot_austria + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                              label=paste0("Jan ",2025+counter,": ",round(subset(austria, austria$x==2024+counter & austria$Type=="New Registrations")$BEV*100, 1),"%"),
                                              size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_austria <- plot_austria + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                       width = unit(1*1920/1280
                                                                                    ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_austria

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_austria$overall)
BEV <- data.frame(BEV, "overall"=data_austria$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_austria$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_austria <- ggplot(austria, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Austria - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_austria), " years ",
      round(12 * (time_80_to_20_austria - floor(time_80_to_20_austria)), 0),
      " months"
    ),
...
plot_ICE_BEV_austria <- plot_ICE_BEV_austria +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(austria, austria$x == 2024 + counter - 1 & austria$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_austria <- plot_ICE_BEV_austria +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(austria, austria$x == 2024 + counter & austria$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_austria)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("austria_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_austria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_austria",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("austria_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_austria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_austria",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("austria_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_austria",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("austria_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_austria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_austria",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_austria$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_austria, "Inf"),
  bev_50 = as_date_from_numeric(time_50_austria, "Inf"),
  bev_80 = as_date_from_numeric(time_80_austria, "Inf"),
  bev_20_80 = time_20_to_80_austria,
  ice_80_20 = time_80_to_20_austria,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_austria$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_austria)
month_label <- last_label_from_data(data_austria, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_austria$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_austria)) nz(tail(data_austria$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_austria)) {
    nz(tail(data_austria$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_austria)) nz(tail(data_austria$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_austria)) {
    nz(tail(data_austria$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_austria),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_austria)
upsert_weight(
