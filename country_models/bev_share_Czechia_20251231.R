#### Czechia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Czechia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/czechia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_czechia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Czechia")
source <- data_czechia$Source[1]
...
#data_czechia$BEV <- data_czechia$`Electric/zero-emission`
#data_czechia$PHEV <- data_czechia$Hybrid
#data_czechia$TOTAL <- data_czechia$overall
#data_czechia$OTHERS <- data_czechia$`Other fuel`
#data_czechia$TOTAL - data_czechia$BEV - data_czechia$PHEV <- data_czechia$Fossil

data_czechia$`Electric/zero-emission` <- data_czechia$BEV
data_czechia$Hybrid <- data_czechia$PHEV
data_czechia$overall <- data_czechia$TOTAL
data_czechia$`Other fuel` <- data_czechia$OTHERS
data_czechia$Fossil <- data_czechia$TOTAL - data_czechia$BEV - data_czechia$PHEV


data_czechia$bev_share   <- data_czechia$`Electric/zero-emission`/data_czechia$overall
data_czechia$ice_share   <- data_czechia$Fossil/data_czechia$overall
data_czechia$hybrid_share<- data_czechia$Hybrid/data_czechia$overall
data_czechia$other_share <- data_czechia$`Other fuel`/data_czechia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_czechia$pure_ICE    <- (data_czechia$Fossil)/data_czechia$overall

lastyearly <- ceiling(max(subset(data_czechia, data_czechia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_czechia_monthly <- subset(data_czechia, data_czechia$time_interval=="monthly")
data_czechia_monthly <- subset(data_czechia_monthly, data_czechia_monthly$year>=min(data_czechia_monthly$year+1))
TTM_shares_czechia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_czechia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_czechia_monthly)), "%Y-%m"),
  Other = data_czechia_monthly$`Other TTM`,
  #`ICE` = data_czechia_monthly$`ICE TTM`,#-data_czechia_monthly$`HEV TTM`,
  Petrol = data_czechia_monthly$`Petrol TTM`,
  Diesel = data_czechia_monthly$`Diesel TTM`,
  #Ethanol = data_czechia_monthly$`Ethanol TTM`,
  #FlexFuel = data_czechia_monthly$`FlexFuel TTM`,
  HEV = data_czechia_monthly$`HEV TTM`,
  PHEV = data_czechia_monthly$`PHEV TTM`,
  BEV = data_czechia_monthly$`BEV TTM`
)

colnames(TTM_shares_czechia)[colnames(TTM_shares_czechia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_czechia)[colnames(TTM_shares_czechia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_czechia)[colnames(TTM_shares_czechia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_czechia)[colnames(TTM_shares_czechia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_czechia)[colnames(TTM_shares_czechia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_czechia_long <- TTM_shares_czechia %>%
...
TTM_barplot_czechia <- ggplot(TTM_shares_czechia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_czechia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_czechia_long$month[substr(TTM_shares_czechia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Czechia",
...
print(TTM_barplot_czechia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_czechia$year)))
extrapol <- 2200
data_czechia <- subset(data_czechia, data_czechia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_czechia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_czechia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_czechia$`Electric/zero-emission`))
for(i in 1:length(data_czechia$`Electric/zero-emission`)){
  data_czechia_looped <- data_czechia[1:i,]
  xg<-data_czechia_looped$year; yg<-as.double(data_czechia_looped$bev_share)
  ...
  xg<-data_czechia_looped$year; yg<-as.double(data_czechia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  czechia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_czechia_looped$overall; new_A$time_interval <- data_czechia_looped$time_interval
  
  czechia$Hybrid       <- pmax(czechia$Hybrid, 0)
  czechia$Hybrid_upper <- pmax(czechia$Hybrid_upper, 0)
  czechia$Hybrid_lower <- pmax(czechia$Hybrid_lower, 0)
  
  time_80_czechia <- max(subset(czechia, czechia$BEV<=0.8 & czechia$BEV >= 0.2)$x)
  time_50_czechia <- max(subset(czechia, czechia$BEV<=0.5 & czechia$BEV >= 0.2)$x)
  time_20_czechia <- max(subset(czechia, czechia$BEV<=0.2 & czechia$BEV >= 0.1)$x)
  time_20_to_80_czechia <- max(subset(czechia, czechia$BEV<=0.8 & czechia$BEV >= 0.2)$x)-min(subset(czechia, czechia$BEV<=0.8 & czechia$BEV >= 0.2)$x)
  time_80_to_20_czechia <- max(subset(czechia, czechia$ICE<=0.8 & czechia$ICE >= 0.2)$x)-min(subset(czechia, czechia$ICE<=0.8 & czechia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_czechia; ice_time[i] <- time_80_to_20_czechia
}

timer_czechia <- data.frame("year"=data_czechia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_czechia$BEV_time == -Inf), which(timer_czechia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_czechia) > (last_inf_index + 12)) {
  timer_czechia_short <- timer_czechia[(last_inf_index + 1 + 12):nrow(timer_czechia), ]
} else {
  timer_czechia_short <- timer_czechia
}
...
plot_timer <- ggplot(timer_czechia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_czechia_short$BEV_time[length(timer_czechia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Czechia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_czechia_short$BEV_time[length(timer_czechia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_czechia <- ggplot(czechia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Czechia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_czechia), " years ", round(12*(time_20_to_80_czechia-floor(time_20_to_80_czechia)),0), " months"),
...
plot_czechia <- plot_czechia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(czechia, czechia$x==2024+counter & czechia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_czechia <- plot_czechia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(czechia, czechia$x==2024+counter & czechia$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_czechia <- plot_czechia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_czechia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_czechia$overall)
BEV <- data.frame(BEV, "overall"=data_czechia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_czechia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_czechia <- ggplot(czechia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Czechia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_czechia), " years ",
      round(12 * (time_80_to_20_czechia - floor(time_80_to_20_czechia)), 0),
      " months"
    ),
...
plot_ICE_BEV_czechia <- plot_ICE_BEV_czechia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(czechia, czechia$x == 2024 + counter - 1 & czechia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_czechia <- plot_ICE_BEV_czechia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(czechia, czechia$x == 2024 + counter & czechia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_czechia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("czechia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_czechia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_czechia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("czechia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_czechia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_czechia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("czechia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_czechia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("czechia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_czechia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_czechia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_czechia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_czechia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_czechia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_czechia, "Inf"),
  bev_20_80 = time_20_to_80_czechia,
  ice_80_20 = time_80_to_20_czechia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_czechia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_czechia)
month_label <- last_label_from_data(data_czechia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_czechia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_czechia)) nz(tail(data_czechia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_czechia)) {
    nz(tail(data_czechia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_czechia)) nz(tail(data_czechia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_czechia)) {
    nz(tail(data_czechia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_czechia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_czechia)
upsert_weight(
