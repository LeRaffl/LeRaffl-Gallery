#### Belgium: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Belgium"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/belgium.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_belgium <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Belgium")
source <- data_belgium$Source[1]
...
#data_belgium$BEV <- data_belgium$`Electric/zero-emission`
#data_belgium$PHEV <- data_belgium$Hybrid
#data_belgium$TOTAL <- data_belgium$overall
#data_belgium$OTHERS <- data_belgium$`Other fuel`
#data_belgium$TOTAL - data_belgium$BEV - data_belgium$PHEV <- data_belgium$Fossil
...
data_belgium$Fossil <- data_belgium$TOTAL - data_belgium$BEV - data_belgium$PHEV
...
TTM_barplot_belgium <- ggplot(TTM_shares_belgium_long, aes(x = month, y = value, fill = type)) +
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Belgium",
...
#### regression setup ####
verschiebung <- floor(min(na.omit(data_belgium$year)))
extrapol <- 2200
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_belgium$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_belgium$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_belgium$`Electric/zero-emission`))
for(i in 1:length(data_belgium$`Electric/zero-emission`)){
  data_belgium_looped <- data_belgium[1:i,]
  xg<-data_belgium_looped$year; yg<-as.double(data_belgium_looped$bev_share)
  ...
  xg<-data_belgium_looped$year; yg<-as.double(data_belgium_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  new_A$overall <- data_belgium_looped$overall; new_A$time_interval <- data_belgium_looped$time_interval
  
  belgium$Hybrid       <- pmax(belgium$Hybrid, 0)
  belgium$Hybrid_upper <- pmax(belgium$Hybrid_upper, 0)
  belgium$Hybrid_lower <- pmax(belgium$Hybrid_lower, 0)
  ...
  bev_time[i] <- time_20_to_80_belgium; ice_time[i] <- time_80_to_20_belgium
}

timer_belgium <- data.frame("year"=data_belgium[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_belgium$BEV_time == -Inf), which(timer_belgium$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_belgium) > (last_inf_index + 12)) {
  timer_belgium_short <- timer_belgium[(last_inf_index + 1 + 12):nrow(timer_belgium), ]
} else {
  timer_belgium_short <- timer_belgium
}
...
plot_timer <- ggplot(timer_belgium_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_belgium_short$BEV_time[length(timer_belgium_short$BEV_time)]*2)
  ) +
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_belgium_short$BEV_time[length(timer_belgium_short$BEV_time)]*2,
    ymin = 0
  )
...
plot_belgium <- ggplot(belgium, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Belgium - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_belgium), " years ", round(12*(time_20_to_80_belgium-floor(time_20_to_80_belgium)),0), " months"),
...
plot_belgium <- plot_belgium + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(belgium, belgium$x==2024+counter & belgium$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_belgium <- plot_belgium + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(belgium, belgium$x==2024+counter & belgium$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_belgium <- plot_belgium + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
...
plot_ICE_BEV_belgium <- ggplot(belgium, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Belgium - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_belgium), " years ",
      round(12 * (time_80_to_20_belgium - floor(time_80_to_20_belgium)), 0),
      " months"
    ),
...
plot_ICE_BEV_belgium <- plot_ICE_BEV_belgium +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(belgium, belgium$x == 2024 + counter - 1 & belgium$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_belgium <- plot_ICE_BEV_belgium +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(belgium, belgium$x == 2024 + counter & belgium$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_belgium)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("belgium_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_belgium,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_belgium",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("belgium_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_belgium,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_belgium",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("belgium_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_belgium",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("belgium_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_belgium,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_belgium",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_belgium$`BEV TTM`, 1),
...
last_date <- last_date_from_data(data_belgium)
month_label <- last_label_from_data(data_belgium, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_belgium$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_belgium)) nz(tail(data_belgium$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_belgium)) {
    nz(tail(data_belgium$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_belgium)) nz(tail(data_belgium$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_belgium)) {
    nz(tail(data_belgium$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_belgium),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_belgium)
upsert_weight(
