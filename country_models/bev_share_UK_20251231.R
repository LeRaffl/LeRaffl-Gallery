#### UK: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "UK"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/uk.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_uk <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "UK")
source <- data_uk$Source[1]
...
#data_uk$BEV <- data_uk$`Electric/zero-emission`
#data_uk$PHEV <- data_uk$Hybrid
#data_uk$TOTAL <- data_uk$overall
#data_uk$OTHERS <- data_uk$`Other fuel`
#data_uk$TOTAL - data_uk$BEV - data_uk$PHEV <- data_uk$Fossil

data_uk$`Electric/zero-emission` <- data_uk$BEV
data_uk$Hybrid <- data_uk$PHEV
data_uk$overall <- data_uk$TOTAL
data_uk$`Other fuel` <- data_uk$OTHERS
data_uk$Fossil <- data_uk$TOTAL - data_uk$BEV - data_uk$PHEV


data_uk$bev_share   <- data_uk$`Electric/zero-emission`/data_uk$overall
data_uk$ice_share   <- data_uk$Fossil/data_uk$overall
data_uk$hybrid_share<- data_uk$Hybrid/data_uk$overall
data_uk$other_share <- data_uk$`Other fuel`/data_uk$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_uk$pure_ICE    <- (data_uk$Fossil)/data_uk$overall

lastyearly <- ceiling(max(subset(data_uk, data_uk$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_uk_monthly <- subset(data_uk, data_uk$time_interval=="monthly")
data_uk_monthly <- subset(data_uk_monthly, data_uk_monthly$year>=min(data_uk_monthly$year+1))
TTM_shares_uk <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_uk_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_uk_monthly)), "%Y-%m"),
  Other = data_uk_monthly$`Other TTM`,
  #`ICE` = data_uk_monthly$`ICE TTM`,#-data_uk_monthly$`HEV TTM`,
  Petrol = data_uk_monthly$`Petrol TTM`,
  Diesel = data_uk_monthly$`Diesel TTM`,
  #Ethanol = data_uk_monthly$`Ethanol TTM`,
  #FlexFuel = data_uk_monthly$`FlexFuel TTM`,
  HEV = data_uk_monthly$`HEV TTM`,
  PHEV = data_uk_monthly$`PHEV TTM`,
  BEV = data_uk_monthly$`BEV TTM`
)

colnames(TTM_shares_uk)[colnames(TTM_shares_uk) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_uk)[colnames(TTM_shares_uk) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_uk)[colnames(TTM_shares_uk) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_uk)[colnames(TTM_shares_uk) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_uk)[colnames(TTM_shares_uk) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_uk_long <- TTM_shares_uk %>%
...
TTM_barplot_uk <- ggplot(TTM_shares_uk_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_uk_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_uk_long$month[substr(TTM_shares_uk_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in UK",
...
print(TTM_barplot_uk)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_uk$year)))
extrapol <- 2200
data_uk <- subset(data_uk, data_uk$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_uk$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_uk$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_uk$`Electric/zero-emission`))
for(i in 1:length(data_uk$`Electric/zero-emission`)){
  data_uk_looped <- data_uk[1:i,]
  xg<-data_uk_looped$year; yg<-as.double(data_uk_looped$bev_share)
  ...
  xg<-data_uk_looped$year; yg<-as.double(data_uk_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  uk <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_uk_looped$overall; new_A$time_interval <- data_uk_looped$time_interval
  
  uk$Hybrid       <- pmax(uk$Hybrid, 0)
  uk$Hybrid_upper <- pmax(uk$Hybrid_upper, 0)
  uk$Hybrid_lower <- pmax(uk$Hybrid_lower, 0)
  
  time_80_uk <- max(subset(uk, uk$BEV<=0.8 & uk$BEV >= 0.2)$x)
  time_50_uk <- max(subset(uk, uk$BEV<=0.5 & uk$BEV >= 0.2)$x)
  time_20_uk <- max(subset(uk, uk$BEV<=0.2 & uk$BEV >= 0.1)$x)
  time_20_to_80_uk <- max(subset(uk, uk$BEV<=0.8 & uk$BEV >= 0.2)$x)-min(subset(uk, uk$BEV<=0.8 & uk$BEV >= 0.2)$x)
  time_80_to_20_uk <- max(subset(uk, uk$ICE<=0.8 & uk$ICE >= 0.2)$x)-min(subset(uk, uk$ICE<=0.8 & uk$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_uk; ice_time[i] <- time_80_to_20_uk
}

timer_uk <- data.frame("year"=data_uk[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_uk$BEV_time == -Inf), which(timer_uk$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_uk) > (last_inf_index + 12)) {
  timer_uk_short <- timer_uk[(last_inf_index + 1 + 12):nrow(timer_uk), ]
} else {
  timer_uk_short <- timer_uk
}
...
plot_timer <- ggplot(timer_uk_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_uk_short$BEV_time[length(timer_uk_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for UK transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_uk_short$BEV_time[length(timer_uk_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_uk <- ggplot(uk, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in UK - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_uk), " years ", round(12*(time_20_to_80_uk-floor(time_20_to_80_uk)),0), " months"),
...
plot_uk <- plot_uk + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(uk, uk$x==2024+counter & uk$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_uk <- plot_uk + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                                  label=paste0("Jan ",2025+counter,": ",round(subset(uk, uk$x==2024+counter & uk$Type=="New Registrations")$BEV*100, 1),"%"),
                                                  size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_uk <- plot_uk + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                           width = unit(1*1920/1280
                                                                                        ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_uk

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_uk$overall)
BEV <- data.frame(BEV, "overall"=data_uk$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_uk$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_uk <- ggplot(uk, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in UK - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_uk), " years ",
      round(12 * (time_80_to_20_uk - floor(time_80_to_20_uk)), 0),
      " months"
    ),
...
plot_ICE_BEV_uk <- plot_ICE_BEV_uk +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(uk, uk$x == 2024 + counter - 1 & uk$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_uk <- plot_ICE_BEV_uk +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(uk, uk$x == 2024 + counter & uk$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_uk)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("uk_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_uk,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uk",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("uk_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_uk,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uk",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("uk_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uk",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("uk_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_uk,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uk",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_uk$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_uk, "Inf"),
  bev_50 = as_date_from_numeric(time_50_uk, "Inf"),
  bev_80 = as_date_from_numeric(time_80_uk, "Inf"),
  bev_20_80 = time_20_to_80_uk,
  ice_80_20 = time_80_to_20_uk,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_uk$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_uk)
month_label <- last_label_from_data(data_uk, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_uk$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_uk)) nz(tail(data_uk$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_uk)) {
    nz(tail(data_uk$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_uk)) nz(tail(data_uk$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_uk)) {
    nz(tail(data_uk$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_uk),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_uk)
upsert_weight(
