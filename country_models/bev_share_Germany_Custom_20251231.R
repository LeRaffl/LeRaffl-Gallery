#### Germany (Custom): BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Germany (Custom)"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/germany_custom_no2024.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_germany_custom_no2024 <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Germany (Custom)")
source <- data_germany_custom_no2024$Source[1]
...
#data_germany_custom_no2024$BEV <- data_germany_custom_no2024$`Electric/zero-emission`
#data_germany_custom_no2024$PHEV <- data_germany_custom_no2024$Hybrid
#data_germany_custom_no2024$TOTAL <- data_germany_custom_no2024$overall
#data_germany_custom_no2024$OTHERS <- data_germany_custom_no2024$`Other fuel`
#data_germany_custom_no2024$TOTAL - data_germany_custom_no2024$BEV - data_germany_custom_no2024$PHEV <- data_germany_custom_no2024$Fossil

data_germany_custom_no2024$`Electric/zero-emission` <- data_germany_custom_no2024$BEV
data_germany_custom_no2024$Hybrid <- data_germany_custom_no2024$PHEV
data_germany_custom_no2024$overall <- data_germany_custom_no2024$TOTAL
data_germany_custom_no2024$`Other fuel` <- data_germany_custom_no2024$OTHERS
data_germany_custom_no2024$Fossil <- data_germany_custom_no2024$TOTAL - data_germany_custom_no2024$BEV - data_germany_custom_no2024$PHEV


data_germany_custom_no2024$bev_share   <- data_germany_custom_no2024$`Electric/zero-emission`/data_germany_custom_no2024$overall
data_germany_custom_no2024$ice_share   <- data_germany_custom_no2024$Fossil/data_germany_custom_no2024$overall
data_germany_custom_no2024$hybrid_share<- data_germany_custom_no2024$Hybrid/data_germany_custom_no2024$overall
data_germany_custom_no2024$other_share <- data_germany_custom_no2024$`Other fuel`/data_germany_custom_no2024$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_germany_custom_no2024$pure_ICE    <- (data_germany_custom_no2024$Fossil)/data_germany_custom_no2024$overall

lastyearly <- ceiling(max(subset(data_germany_custom_no2024, data_germany_custom_no2024$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_germany_custom_no2024_monthly <- subset(data_germany_custom_no2024, data_germany_custom_no2024$time_interval=="monthly")
data_germany_custom_no2024_monthly <- subset(data_germany_custom_no2024_monthly, data_germany_custom_no2024_monthly$year>=min(data_germany_custom_no2024_monthly$year+1))
TTM_shares_germany_custom_no2024 <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_germany_custom_no2024_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_germany_custom_no2024_monthly)), "%Y-%m"),
  Other = data_germany_custom_no2024_monthly$`Other TTM`,
  #`ICE` = data_germany_custom_no2024_monthly$`ICE TTM`,#-data_germany_custom_no2024_monthly$`HEV TTM`,
  Petrol = data_germany_custom_no2024_monthly$`Petrol TTM`,
  Diesel = data_germany_custom_no2024_monthly$`Diesel TTM`,
  #Ethanol = data_germany_custom_no2024_monthly$`Ethanol TTM`,
  #FlexFuel = data_germany_custom_no2024_monthly$`FlexFuel TTM`,
  HEV = data_germany_custom_no2024_monthly$`HEV TTM`,
  PHEV = data_germany_custom_no2024_monthly$`PHEV TTM`,
  BEV = data_germany_custom_no2024_monthly$`BEV TTM`
)

colnames(TTM_shares_germany_custom_no2024)[colnames(TTM_shares_germany_custom_no2024) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_germany_custom_no2024)[colnames(TTM_shares_germany_custom_no2024) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_germany_custom_no2024)[colnames(TTM_shares_germany_custom_no2024) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_germany_custom_no2024)[colnames(TTM_shares_germany_custom_no2024) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_germany_custom_no2024)[colnames(TTM_shares_germany_custom_no2024) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_germany_custom_no2024_long <- TTM_shares_germany_custom_no2024 %>%
...
TTM_barplot_germany_custom_no2024 <- ggplot(TTM_shares_germany_custom_no2024_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_germany_custom_no2024_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_germany_custom_no2024_long$month[substr(TTM_shares_germany_custom_no2024_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Germany (Custom) \n-'what if 2024 never happened?' scenario",
...
print(TTM_barplot_germany_custom_no2024)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_germany_custom_no2024$year)))
extrapol <- 2200
data_germany_custom_no2024 <- subset(data_germany_custom_no2024, data_germany_custom_no2024$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_germany_custom_no2024$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_germany_custom_no2024$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_germany_custom_no2024$`Electric/zero-emission`))
for(i in 1:length(data_germany_custom_no2024$`Electric/zero-emission`)){
  data_germany_custom_no2024_looped <- data_germany_custom_no2024[1:i,]
  xg<-data_germany_custom_no2024_looped$year; yg<-as.double(data_germany_custom_no2024_looped$bev_share)
  ...
  xg<-data_germany_custom_no2024_looped$year; yg<-as.double(data_germany_custom_no2024_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  germany_custom_no2024 <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_germany_custom_no2024_looped$overall; new_A$time_interval <- data_germany_custom_no2024_looped$time_interval
  
  germany_custom_no2024$Hybrid       <- pmax(germany_custom_no2024$Hybrid, 0)
  germany_custom_no2024$Hybrid_upper <- pmax(germany_custom_no2024$Hybrid_upper, 0)
  germany_custom_no2024$Hybrid_lower <- pmax(germany_custom_no2024$Hybrid_lower, 0)
  
  time_80_germany_custom_no2024 <- max(subset(germany_custom_no2024, germany_custom_no2024$BEV<=0.8 & germany_custom_no2024$BEV >= 0.2)$x)
  time_50_germany_custom_no2024 <- max(subset(germany_custom_no2024, germany_custom_no2024$BEV<=0.5 & germany_custom_no2024$BEV >= 0.2)$x)
  time_20_germany_custom_no2024 <- max(subset(germany_custom_no2024, germany_custom_no2024$BEV<=0.2 & germany_custom_no2024$BEV >= 0.1)$x)
  time_20_to_80_germany_custom_no2024 <- max(subset(germany_custom_no2024, germany_custom_no2024$BEV<=0.8 & germany_custom_no2024$BEV >= 0.2)$x)-min(subset(germany_custom_no2024, germany_custom_no2024$BEV<=0.8 & germany_custom_no2024$BEV >= 0.2)$x)
  time_80_to_20_germany_custom_no2024 <- max(subset(germany_custom_no2024, germany_custom_no2024$ICE<=0.8 & germany_custom_no2024$ICE >= 0.2)$x)-min(subset(germany_custom_no2024, germany_custom_no2024$ICE<=0.8 & germany_custom_no2024$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_germany_custom_no2024; ice_time[i] <- time_80_to_20_germany_custom_no2024
}

timer_germany_custom_no2024 <- data.frame("year"=data_germany_custom_no2024[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_germany_custom_no2024$BEV_time == -Inf), which(timer_germany_custom_no2024$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_germany_custom_no2024) > (last_inf_index + 12)) {
  timer_germany_custom_no2024_short <- timer_germany_custom_no2024[(last_inf_index + 1 + 12):nrow(timer_germany_custom_no2024), ]
} else {
  timer_germany_custom_no2024_short <- timer_germany_custom_no2024
}
...
plot_timer <- ggplot(timer_germany_custom_no2024_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_germany_custom_no2024_short$BEV_time[length(timer_germany_custom_no2024_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Germany (Custom) transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_germany_custom_no2024_short$BEV_time[length(timer_germany_custom_no2024_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_germany_custom_no2024 <- ggplot(germany_custom_no2024, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Germany (Custom) \n'what if 2024 never happened?' scenario",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_germany_custom_no2024), " years ", round(12*(time_20_to_80_germany_custom_no2024-floor(time_20_to_80_germany_custom_no2024)),0), " months"),
...
plot_germany_custom_no2024 <- plot_germany_custom_no2024 + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(germany_custom_no2024, germany_custom_no2024$x==2024+counter & germany_custom_no2024$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_germany_custom_no2024 <- plot_germany_custom_no2024 + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(germany_custom_no2024, germany_custom_no2024$x==2024+counter & germany_custom_no2024$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_germany_custom_no2024 <- plot_germany_custom_no2024 + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_germany_custom_no2024

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_germany_custom_no2024$overall)
BEV <- data.frame(BEV, "overall"=data_germany_custom_no2024$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_germany_custom_no2024$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_germany_custom_no2024 <- ggplot(germany_custom_no2024, aes(x = x, y = BEV, color = Type)) +
  ...
  geom_point(
    data = data_germany_custom_no2024,
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Germany (Custom) \n'what if 2024 never happened?' scenario",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_germany_custom_no2024), " years ",
      round(12 * (time_80_to_20_germany_custom_no2024 - floor(time_80_to_20_germany_custom_no2024)), 0),
      " months"
    ),
...
plot_ICE_BEV_germany_custom_no2024 <- plot_ICE_BEV_germany_custom_no2024 +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(germany_custom_no2024, germany_custom_no2024$x == 2024 + counter - 1 & germany_custom_no2024$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_germany_custom_no2024 <- plot_ICE_BEV_germany_custom_no2024 +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(germany_custom_no2024, germany_custom_no2024$x == 2024 + counter & germany_custom_no2024$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_germany_custom_no2024)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("germany_custom_no2024_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_germany_custom_no2024,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany_custom_no2024",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("germany_custom_no2024_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_germany_custom_no2024,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany_custom_no2024",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("germany_custom_no2024_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany_custom_no2024",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
#ggsave_git(
#  filename = paste0("germany_custom_no2024_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
#  plot = TTM_barplot_germany_custom_no2024,
#  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany_custom_no2024",
#  repo_dir = repo_dir_local,
#  period = period_folder,         # <=
#  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_germany_custom_no2024$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_germany_custom_no2024, "Inf"),
  bev_50 = as_date_from_numeric(time_50_germany_custom_no2024, "Inf"),
  bev_80 = as_date_from_numeric(time_80_germany_custom_no2024, "Inf"),
  bev_20_80 = time_20_to_80_germany_custom_no2024,
  ice_80_20 = time_80_to_20_germany_custom_no2024,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_germany_custom_no2024$YYYYMMM), n=1)
)
...
last_m <- data_germany_custom_no2024 |>
  dplyr::arrange(year) |>
  dplyr::slice_tail(n = 1)
...
last_date <- last_date_from_data(data_germany_custom_no2024)
month_label <- last_label_from_data(data_germany_custom_no2024, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_germany_custom_no2024$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_germany_custom_no2024)) nz(tail(data_germany_custom_no2024$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_germany_custom_no2024)) {
    nz(tail(data_germany_custom_no2024$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_germany_custom_no2024)) nz(tail(data_germany_custom_no2024$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_germany_custom_no2024)) {
    nz(tail(data_germany_custom_no2024$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_germany_custom_no2024),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_germany_custom_no2024)
upsert_weight(
