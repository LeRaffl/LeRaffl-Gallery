#### Netherlands: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Netherlands"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/netherlands.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_netherlands <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Netherlands")
source <- data_netherlands$Source[1]
...
#data_netherlands$BEV <- data_netherlands$`Electric/zero-emission`
#data_netherlands$PHEV <- data_netherlands$Hybrid
#data_netherlands$TOTAL <- data_netherlands$overall
#data_netherlands$OTHERS <- data_netherlands$`Other fuel`
#data_netherlands$TOTAL - data_netherlands$BEV - data_netherlands$PHEV <- data_netherlands$Fossil

data_netherlands$`Electric/zero-emission` <- data_netherlands$BEV
data_netherlands$Hybrid <- data_netherlands$PHEV
data_netherlands$overall <- data_netherlands$TOTAL
data_netherlands$`Other fuel` <- data_netherlands$OTHERS
data_netherlands$Fossil <- data_netherlands$TOTAL - data_netherlands$BEV - data_netherlands$PHEV


data_netherlands$bev_share   <- data_netherlands$`Electric/zero-emission`/data_netherlands$overall
data_netherlands$ice_share   <- data_netherlands$Fossil/data_netherlands$overall
data_netherlands$hybrid_share<- data_netherlands$Hybrid/data_netherlands$overall
data_netherlands$other_share <- data_netherlands$`Other fuel`/data_netherlands$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_netherlands$pure_ICE    <- (data_netherlands$Fossil)/data_netherlands$overall

lastyearly <- ceiling(max(subset(data_netherlands, data_netherlands$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_netherlands_monthly <- subset(data_netherlands, data_netherlands$time_interval=="monthly")
data_netherlands_monthly <- subset(data_netherlands_monthly, data_netherlands_monthly$year>=min(data_netherlands_monthly$year+1))
TTM_shares_netherlands <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_netherlands_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_netherlands_monthly)), "%Y-%m"),
  Other = data_netherlands_monthly$`Other TTM`,
  #`ICE` = data_netherlands_monthly$`ICE TTM`,#-data_netherlands_monthly$`HEV TTM`,
  Petrol = data_netherlands_monthly$`Petrol TTM`,
  Diesel = data_netherlands_monthly$`Diesel TTM`,
  #Ethanol = data_netherlands_monthly$`Ethanol TTM`,
  #FlexFuel = data_netherlands_monthly$`FlexFuel TTM`,
  HEV = data_netherlands_monthly$`HEV TTM`,
  PHEV = data_netherlands_monthly$`PHEV TTM`,
  BEV = data_netherlands_monthly$`BEV TTM`
)

colnames(TTM_shares_netherlands)[colnames(TTM_shares_netherlands) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_netherlands)[colnames(TTM_shares_netherlands) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_netherlands)[colnames(TTM_shares_netherlands) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_netherlands)[colnames(TTM_shares_netherlands) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_netherlands)[colnames(TTM_shares_netherlands) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_netherlands_long <- TTM_shares_netherlands %>%
...
TTM_barplot_netherlands <- ggplot(TTM_shares_netherlands_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_netherlands_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_netherlands_long$month[substr(TTM_shares_netherlands_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Netherlands",
...
print(TTM_barplot_netherlands)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_netherlands$year)))
extrapol <- 2200
data_netherlands <- subset(data_netherlands, data_netherlands$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_netherlands$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_netherlands$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_netherlands$`Electric/zero-emission`))
for(i in 1:length(data_netherlands$`Electric/zero-emission`)){
  data_netherlands_looped <- data_netherlands[1:i,]
  xg<-data_netherlands_looped$year; yg<-as.double(data_netherlands_looped$bev_share)
  ...
  xg<-data_netherlands_looped$year; yg<-as.double(data_netherlands_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  netherlands <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_netherlands_looped$overall; new_A$time_interval <- data_netherlands_looped$time_interval
  
  netherlands$Hybrid       <- pmax(netherlands$Hybrid, 0)
  netherlands$Hybrid_upper <- pmax(netherlands$Hybrid_upper, 0)
  netherlands$Hybrid_lower <- pmax(netherlands$Hybrid_lower, 0)
  
  time_80_netherlands <- max(subset(netherlands, netherlands$BEV<=0.8 & netherlands$BEV >= 0.2)$x)
  time_50_netherlands <- max(subset(netherlands, netherlands$BEV<=0.5 & netherlands$BEV >= 0.2)$x)
  time_20_netherlands <- max(subset(netherlands, netherlands$BEV<=0.2 & netherlands$BEV >= 0.1)$x)
  time_20_to_80_netherlands <- max(subset(netherlands, netherlands$BEV<=0.8 & netherlands$BEV >= 0.2)$x)-min(subset(netherlands, netherlands$BEV<=0.8 & netherlands$BEV >= 0.2)$x)
  time_80_to_20_netherlands <- max(subset(netherlands, netherlands$ICE<=0.8 & netherlands$ICE >= 0.2)$x)-min(subset(netherlands, netherlands$ICE<=0.8 & netherlands$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_netherlands; ice_time[i] <- time_80_to_20_netherlands
}

timer_netherlands <- data.frame("year"=data_netherlands[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_netherlands$BEV_time == -Inf), which(timer_netherlands$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_netherlands) > (last_inf_index + 12)) {
  timer_netherlands_short <- timer_netherlands[(last_inf_index + 1 + 12):nrow(timer_netherlands), ]
} else {
  timer_netherlands_short <- timer_netherlands
}
...
plot_timer <- ggplot(timer_netherlands_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_netherlands_short$BEV_time[length(timer_netherlands_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Netherlands transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_netherlands_short$BEV_time[length(timer_netherlands_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_netherlands <- ggplot(netherlands, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Netherlands - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_netherlands), " years ", round(12*(time_20_to_80_netherlands-floor(time_20_to_80_netherlands)),0), " months"),
...
plot_netherlands <- plot_netherlands + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(netherlands, netherlands$x==2024+counter & netherlands$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_netherlands <- plot_netherlands + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(netherlands, netherlands$x==2024+counter & netherlands$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_netherlands <- plot_netherlands + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_netherlands

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_netherlands$overall)
BEV <- data.frame(BEV, "overall"=data_netherlands$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_netherlands$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_netherlands <- ggplot(netherlands, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Netherlands - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_netherlands), " years ",
      round(12 * (time_80_to_20_netherlands - floor(time_80_to_20_netherlands)), 0),
      " months"
    ),
...
plot_ICE_BEV_netherlands <- plot_ICE_BEV_netherlands +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(netherlands, netherlands$x == 2024 + counter - 1 & netherlands$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_netherlands <- plot_ICE_BEV_netherlands +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(netherlands, netherlands$x == 2024 + counter & netherlands$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_netherlands)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("netherlands_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_netherlands,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_netherlands",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("netherlands_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_netherlands,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_netherlands",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("netherlands_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_netherlands",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("netherlands_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_netherlands,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_netherlands",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_netherlands$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_netherlands, "Inf"),
  bev_50 = as_date_from_numeric(time_50_netherlands, "Inf"),
  bev_80 = as_date_from_numeric(time_80_netherlands, "Inf"),
  bev_20_80 = time_20_to_80_netherlands,
  ice_80_20 = time_80_to_20_netherlands,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_netherlands$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_netherlands)
month_label <- last_label_from_data(data_netherlands, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_netherlands$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_netherlands)) nz(tail(data_netherlands$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_netherlands)) {
    nz(tail(data_netherlands$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_netherlands)) nz(tail(data_netherlands$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_netherlands)) {
    nz(tail(data_netherlands$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_netherlands),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_netherlands)
upsert_weight(
