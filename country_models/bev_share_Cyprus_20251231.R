#### Cyprus: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Cyprus"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/cyprus.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_cyprus <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Cyprus")
source <- data_cyprus$Source[1]
...
#data_cyprus$BEV <- data_cyprus$`Electric/zero-emission`
#data_cyprus$PHEV <- data_cyprus$Hybrid
#data_cyprus$TOTAL <- data_cyprus$overall
#data_cyprus$OTHERS <- data_cyprus$`Other fuel`
#data_cyprus$TOTAL - data_cyprus$BEV - data_cyprus$PHEV <- data_cyprus$Fossil

data_cyprus$`Electric/zero-emission` <- data_cyprus$BEV
data_cyprus$Hybrid <- data_cyprus$PHEV
data_cyprus$overall <- data_cyprus$TOTAL
data_cyprus$`Other fuel` <- data_cyprus$OTHERS
data_cyprus$Fossil <- data_cyprus$TOTAL - data_cyprus$BEV - data_cyprus$PHEV


data_cyprus$bev_share   <- data_cyprus$`Electric/zero-emission`/data_cyprus$overall
data_cyprus$ice_share   <- data_cyprus$Fossil/data_cyprus$overall
data_cyprus$hybrid_share<- data_cyprus$Hybrid/data_cyprus$overall
data_cyprus$other_share <- data_cyprus$`Other fuel`/data_cyprus$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_cyprus$pure_ICE    <- (data_cyprus$Fossil)/data_cyprus$overall

lastyearly <- ceiling(max(subset(data_cyprus, data_cyprus$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_cyprus_monthly <- subset(data_cyprus, data_cyprus$time_interval=="monthly")
data_cyprus_monthly <- subset(data_cyprus_monthly, data_cyprus_monthly$year>=min(data_cyprus_monthly$year+1))
TTM_shares_cyprus <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_cyprus_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_cyprus_monthly)), "%Y-%m"),
  Other = data_cyprus_monthly$`Other TTM`,
  #`ICE` = data_cyprus_monthly$`ICE TTM`,#-data_cyprus_monthly$`HEV TTM`,
  Petrol = data_cyprus_monthly$`Petrol TTM`,
  Diesel = data_cyprus_monthly$`Diesel TTM`,
  #Ethanol = data_cyprus_monthly$`Ethanol TTM`,
  #FlexFuel = data_cyprus_monthly$`FlexFuel TTM`,
  HEV = data_cyprus_monthly$`HEV TTM`,
  PHEV = data_cyprus_monthly$`PHEV TTM`,
  BEV = data_cyprus_monthly$`BEV TTM`
)

colnames(TTM_shares_cyprus)[colnames(TTM_shares_cyprus) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_cyprus)[colnames(TTM_shares_cyprus) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_cyprus)[colnames(TTM_shares_cyprus) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_cyprus)[colnames(TTM_shares_cyprus) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_cyprus)[colnames(TTM_shares_cyprus) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_cyprus_long <- TTM_shares_cyprus %>%
...
TTM_barplot_cyprus <- ggplot(TTM_shares_cyprus_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_cyprus_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_cyprus_long$month[substr(TTM_shares_cyprus_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Cyprus",
...
print(TTM_barplot_cyprus)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_cyprus$year)))
extrapol <- 2200
data_cyprus <- subset(data_cyprus, data_cyprus$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_cyprus$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_cyprus$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_cyprus$`Electric/zero-emission`))
for(i in 1:length(data_cyprus$`Electric/zero-emission`)){
  data_cyprus_looped <- data_cyprus[1:i,]
  xg<-data_cyprus_looped$year; yg<-as.double(data_cyprus_looped$bev_share)
  ...
  xg<-data_cyprus_looped$year; yg<-as.double(data_cyprus_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  cyprus <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_cyprus_looped$overall; new_A$time_interval <- data_cyprus_looped$time_interval
  
  cyprus$Hybrid       <- pmax(cyprus$Hybrid, 0)
  cyprus$Hybrid_upper <- pmax(cyprus$Hybrid_upper, 0)
  cyprus$Hybrid_lower <- pmax(cyprus$Hybrid_lower, 0)
  
  time_80_cyprus <- max(subset(cyprus, cyprus$BEV<=0.8 & cyprus$BEV >= 0.2)$x)
  time_50_cyprus <- max(subset(cyprus, cyprus$BEV<=0.5 & cyprus$BEV >= 0.2)$x)
  time_20_cyprus <- max(subset(cyprus, cyprus$BEV<=0.2 & cyprus$BEV >= 0.1)$x)
  time_20_to_80_cyprus <- max(subset(cyprus, cyprus$BEV<=0.8 & cyprus$BEV >= 0.2)$x)-min(subset(cyprus, cyprus$BEV<=0.8 & cyprus$BEV >= 0.2)$x)
  time_80_to_20_cyprus <- max(subset(cyprus, cyprus$ICE<=0.8 & cyprus$ICE >= 0.2)$x)-min(subset(cyprus, cyprus$ICE<=0.8 & cyprus$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_cyprus; ice_time[i] <- time_80_to_20_cyprus
}

timer_cyprus <- data.frame("year"=data_cyprus[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_cyprus$BEV_time == -Inf), which(timer_cyprus$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_cyprus) > (last_inf_index + 12)) {
  timer_cyprus_short <- timer_cyprus[(last_inf_index + 1 + 12):nrow(timer_cyprus), ]
} else {
  timer_cyprus_short <- timer_cyprus
}
...
plot_timer <- ggplot(timer_cyprus_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_cyprus_short$BEV_time[length(timer_cyprus_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Cyprus transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_cyprus_short$BEV_time[length(timer_cyprus_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_cyprus <- ggplot(cyprus, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Cyprus - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_cyprus), " years ", round(12*(time_20_to_80_cyprus-floor(time_20_to_80_cyprus)),0), " months"),
...
plot_cyprus <- plot_cyprus + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(cyprus, cyprus$x==2024+counter & cyprus$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_cyprus <- plot_cyprus + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(cyprus, cyprus$x==2024+counter & cyprus$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_cyprus <- plot_cyprus + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_cyprus

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_cyprus$overall)
BEV <- data.frame(BEV, "overall"=data_cyprus$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_cyprus$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_cyprus <- ggplot(cyprus, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Cyprus - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_cyprus), " years ",
      round(12 * (time_80_to_20_cyprus - floor(time_80_to_20_cyprus)), 0),
      " months"
    ),
...
plot_ICE_BEV_cyprus <- plot_ICE_BEV_cyprus +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(cyprus, cyprus$x == 2024 + counter - 1 & cyprus$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_cyprus <- plot_ICE_BEV_cyprus +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(cyprus, cyprus$x == 2024 + counter & cyprus$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_cyprus)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("cyprus_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_cyprus,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_cyprus",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("cyprus_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_cyprus,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_cyprus",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("cyprus_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_cyprus",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("cyprus_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_cyprus,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_cyprus",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_cyprus$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_cyprus, "Inf"),
  bev_50 = as_date_from_numeric(time_50_cyprus, "Inf"),
  bev_80 = as_date_from_numeric(time_80_cyprus, "Inf"),
  bev_20_80 = time_20_to_80_cyprus,
  ice_80_20 = time_80_to_20_cyprus,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_cyprus$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_cyprus)
month_label <- last_label_from_data(data_cyprus, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_cyprus$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_cyprus)) nz(tail(data_cyprus$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_cyprus)) {
    nz(tail(data_cyprus$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_cyprus)) nz(tail(data_cyprus$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_cyprus)) {
    nz(tail(data_cyprus$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_cyprus),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_cyprus)
upsert_weight(
