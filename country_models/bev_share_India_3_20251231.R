#### India (3-Wheelers): BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "India (3-Wheelers)"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/india_3.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_india_3 <- read_sheet("https://docs.google.com/spreadsheets/d/107oxsVahAv0-Nn1TzsP3SK9ETwVj3k-XfjXvkXrf_Aw/edit?gid=0#gid=0", sheet = "India_3")
source <- data_india_3$Source[1]
...
#data_india_3$BEV <- data_india_3$`Electric/zero-emission`
#data_india_3$PHEV <- data_india_3$Hybrid
#data_india_3$TOTAL <- data_india_3$overall
#data_india_3$OTHERS <- data_india_3$`Other fuel`
#data_india_3$TOTAL - data_india_3$BEV - data_india_3$PHEV <- data_india_3$Fossil

data_india_3$`Electric/zero-emission` <- data_india_3$BEV
data_india_3$Hybrid <- data_india_3$PHEV
data_india_3$overall <- data_india_3$TOTAL
data_india_3$`Other fuel` <- data_india_3$OTHERS
data_india_3$Fossil <- data_india_3$TOTAL - data_india_3$BEV - data_india_3$PHEV


data_india_3$bev_share   <- data_india_3$`Electric/zero-emission`/data_india_3$overall
data_india_3$ice_share   <- data_india_3$Fossil/data_india_3$overall
data_india_3$hybrid_share<- data_india_3$Hybrid/data_india_3$overall
data_india_3$other_share <- data_india_3$`Other fuel`/data_india_3$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_india_3$pure_ICE    <- (data_india_3$Fossil)/data_india_3$overall

lastyearly <- ceiling(max(subset(data_india_3, data_india_3$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_india_3_monthly <- subset(data_india_3, data_india_3$time_interval=="monthly")
data_india_3_monthly <- subset(data_india_3_monthly, data_india_3_monthly$year>=min(data_india_3_monthly$year+1))
TTM_shares_india_3 <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_india_3_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_india_3_monthly)), "%Y-%m"),
  Other = data_india_3_monthly$`Other TTM`,
  #`ICE` = data_india_3_monthly$`ICE TTM`,#-data_india_3_monthly$`HEV TTM`,
  Petrol = data_india_3_monthly$`Petrol TTM`,
  Diesel = data_india_3_monthly$`Diesel TTM`,
  #Ethanol = data_india_3_monthly$`Ethanol TTM`,
  #FlexFuel = data_india_3_monthly$`FlexFuel TTM`,
  HEV = data_india_3_monthly$`HEV TTM`,
  PHEV = data_india_3_monthly$`PHEV TTM`,
  BEV = data_india_3_monthly$`BEV TTM`
)

colnames(TTM_shares_india_3)[colnames(TTM_shares_india_3) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_india_3)[colnames(TTM_shares_india_3) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_india_3)[colnames(TTM_shares_india_3) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_india_3)[colnames(TTM_shares_india_3) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_india_3)[colnames(TTM_shares_india_3) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_india_3_long <- TTM_shares_india_3 %>%
...
TTM_barplot_india_3 <- ggplot(TTM_shares_india_3_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_india_3_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_india_3_long$month[substr(TTM_shares_india_3_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in India (3-Wheelers)",
...
print(TTM_barplot_india_3)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_india_3$year)))
extrapol <- 2200
data_india_3 <- subset(data_india_3, data_india_3$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_india_3$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_india_3$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_india_3$`Electric/zero-emission`))
for(i in 1:length(data_india_3$`Electric/zero-emission`)){
  data_india_3_looped <- data_india_3[1:i,]
  xg<-data_india_3_looped$year; yg<-as.double(data_india_3_looped$bev_share)
  ...
  xg<-data_india_3_looped$year; yg<-as.double(data_india_3_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  india_3 <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_india_3_looped$overall; new_A$time_interval <- data_india_3_looped$time_interval
  
  india_3$Hybrid       <- pmax(india_3$Hybrid, 0)
  india_3$Hybrid_upper <- pmax(india_3$Hybrid_upper, 0)
  india_3$Hybrid_lower <- pmax(india_3$Hybrid_lower, 0)
  
  time_80_india_3 <- max(subset(india_3, india_3$BEV<=0.8 & india_3$BEV >= 0.2)$x)
  time_50_india_3 <- max(subset(india_3, india_3$BEV<=0.5 & india_3$BEV >= 0.2)$x)
  time_20_india_3 <- max(subset(india_3, india_3$BEV<=0.2 & india_3$BEV >= 0.1)$x)
  time_20_to_80_india_3 <- max(subset(india_3, india_3$BEV<=0.8 & india_3$BEV >= 0.2)$x)-min(subset(india_3, india_3$BEV<=0.8 & india_3$BEV >= 0.2)$x)
  time_80_to_20_india_3 <- max(subset(india_3, india_3$ICE<=0.8 & india_3$ICE >= 0.2)$x)-min(subset(india_3, india_3$ICE<=0.8 & india_3$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_india_3; ice_time[i] <- time_80_to_20_india_3
}

timer_india_3 <- data.frame("year"=data_india_3[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_india_3$BEV_time == -Inf), which(timer_india_3$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_india_3) > (last_inf_index + 12)) {
  timer_india_3_short <- timer_india_3[(last_inf_index + 1 + 12):nrow(timer_india_3), ]
} else {
  timer_india_3_short <- timer_india_3
}
...
plot_timer <- ggplot(timer_india_3_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_india_3_short$BEV_time[length(timer_india_3_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for India (3-Wheelers) transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_india_3_short$BEV_time[length(timer_india_3_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_india_3 <- ggplot(india_3, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in India (3-Wheelers) - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_india_3), " years ", round(12*(time_20_to_80_india_3-floor(time_20_to_80_india_3)),0), " months"),
...
plot_india_3 <- plot_india_3 + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(india_3, india_3$x==2024+counter & india_3$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_india_3 <- plot_india_3 + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(india_3, india_3$x==2024+counter & india_3$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_india_3 <- plot_india_3 + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_india_3

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_india_3$overall)
BEV <- data.frame(BEV, "overall"=data_india_3$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_india_3$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_india_3 <- ggplot(india_3, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in India (3-Wheelers) - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_india_3), " years ",
      round(12 * (time_80_to_20_india_3 - floor(time_80_to_20_india_3)), 0),
      " months"
    ),
...
plot_ICE_BEV_india_3 <- plot_ICE_BEV_india_3 +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(india_3, india_3$x == 2024 + counter - 1 & india_3$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_india_3 <- plot_ICE_BEV_india_3 +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(india_3, india_3$x == 2024 + counter & india_3$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_india_3)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("india_3-wheelers_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_india_3,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india_3",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("india_3-wheelers_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_india_3,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india_3",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("india_3-wheelers_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india_3",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("india_3-wheelers_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_india_3,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india_3",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_india_3$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_india_3, "Inf"),
  bev_50 = as_date_from_numeric(time_50_india_3, "Inf"),
  bev_80 = as_date_from_numeric(time_80_india_3, "Inf"),
  bev_20_80 = time_20_to_80_india_3,
  ice_80_20 = time_80_to_20_india_3,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_india_3$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_india_3)
month_label <- last_label_from_data(data_india_3, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_india_3$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_india_3)) nz(tail(data_india_3$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_india_3)) {
    nz(tail(data_india_3$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_india_3)) nz(tail(data_india_3$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_india_3)) {
    nz(tail(data_india_3$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_india_3),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_india_3)
upsert_weight(
