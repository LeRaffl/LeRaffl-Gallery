#### Slovakia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Slovakia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/slovakia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_slovakia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Slovakia")
source <- data_slovakia$Source[1]
...
#data_slovakia$BEV <- data_slovakia$`Electric/zero-emission`
#data_slovakia$PHEV <- data_slovakia$Hybrid
#data_slovakia$TOTAL <- data_slovakia$overall
#data_slovakia$OTHERS <- data_slovakia$`Other fuel`
#data_slovakia$TOTAL - data_slovakia$BEV - data_slovakia$PHEV <- data_slovakia$Fossil

data_slovakia$`Electric/zero-emission` <- data_slovakia$BEV
data_slovakia$Hybrid <- data_slovakia$PHEV
data_slovakia$overall <- data_slovakia$TOTAL
data_slovakia$`Other fuel` <- data_slovakia$OTHERS
data_slovakia$Fossil <- data_slovakia$TOTAL - data_slovakia$BEV - data_slovakia$PHEV


data_slovakia$bev_share   <- data_slovakia$`Electric/zero-emission`/data_slovakia$overall
data_slovakia$ice_share   <- data_slovakia$Fossil/data_slovakia$overall
data_slovakia$hybrid_share<- data_slovakia$Hybrid/data_slovakia$overall
data_slovakia$other_share <- data_slovakia$`Other fuel`/data_slovakia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_slovakia$pure_ICE    <- (data_slovakia$Fossil)/data_slovakia$overall

lastyearly <- ceiling(max(subset(data_slovakia, data_slovakia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_slovakia_monthly <- subset(data_slovakia, data_slovakia$time_interval=="monthly")
data_slovakia_monthly <- subset(data_slovakia_monthly, data_slovakia_monthly$year>=min(data_slovakia_monthly$year+1))
TTM_shares_slovakia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_slovakia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_slovakia_monthly)), "%Y-%m"),
  Other = data_slovakia_monthly$`Other TTM`,
  #`ICE` = data_slovakia_monthly$`ICE TTM`,#-data_slovakia_monthly$`HEV TTM`,
  Petrol = data_slovakia_monthly$`Petrol TTM`,
  Diesel = data_slovakia_monthly$`Diesel TTM`,
  #Ethanol = data_slovakia_monthly$`Ethanol TTM`,
  #FlexFuel = data_slovakia_monthly$`FlexFuel TTM`,
  HEV = data_slovakia_monthly$`HEV TTM`,
  PHEV = data_slovakia_monthly$`PHEV TTM`,
  BEV = data_slovakia_monthly$`BEV TTM`
)

colnames(TTM_shares_slovakia)[colnames(TTM_shares_slovakia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_slovakia)[colnames(TTM_shares_slovakia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_slovakia)[colnames(TTM_shares_slovakia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_slovakia)[colnames(TTM_shares_slovakia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_slovakia)[colnames(TTM_shares_slovakia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_slovakia_long <- TTM_shares_slovakia %>%
...
TTM_barplot_slovakia <- ggplot(TTM_shares_slovakia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_slovakia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_slovakia_long$month[substr(TTM_shares_slovakia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Slovakia",
...
print(TTM_barplot_slovakia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_slovakia$year)))
extrapol <- 2200
data_slovakia <- subset(data_slovakia, data_slovakia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_slovakia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_slovakia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_slovakia$`Electric/zero-emission`))
for(i in 1:length(data_slovakia$`Electric/zero-emission`)){
  data_slovakia_looped <- data_slovakia[1:i,]
  xg<-data_slovakia_looped$year; yg<-as.double(data_slovakia_looped$bev_share)
  ...
  xg<-data_slovakia_looped$year; yg<-as.double(data_slovakia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  slovakia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_slovakia_looped$overall; new_A$time_interval <- data_slovakia_looped$time_interval
  
  slovakia$Hybrid       <- pmax(slovakia$Hybrid, 0)
  slovakia$Hybrid_upper <- pmax(slovakia$Hybrid_upper, 0)
  slovakia$Hybrid_lower <- pmax(slovakia$Hybrid_lower, 0)
  
  time_80_slovakia <- max(subset(slovakia, slovakia$BEV<=0.8 & slovakia$BEV >= 0.2)$x)
  time_50_slovakia <- max(subset(slovakia, slovakia$BEV<=0.5 & slovakia$BEV >= 0.2)$x)
  time_20_slovakia <- max(subset(slovakia, slovakia$BEV<=0.2 & slovakia$BEV >= 0.1)$x)
  time_20_to_80_slovakia <- max(subset(slovakia, slovakia$BEV<=0.8 & slovakia$BEV >= 0.2)$x)-min(subset(slovakia, slovakia$BEV<=0.8 & slovakia$BEV >= 0.2)$x)
  time_80_to_20_slovakia <- max(subset(slovakia, slovakia$ICE<=0.8 & slovakia$ICE >= 0.2)$x)-min(subset(slovakia, slovakia$ICE<=0.8 & slovakia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_slovakia; ice_time[i] <- time_80_to_20_slovakia
}

timer_slovakia <- data.frame("year"=data_slovakia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_slovakia$BEV_time == -Inf), which(timer_slovakia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_slovakia) > (last_inf_index + 12)) {
  timer_slovakia_short <- timer_slovakia[(last_inf_index + 1 + 12):nrow(timer_slovakia), ]
} else {
  timer_slovakia_short <- timer_slovakia
}
...
plot_timer <- ggplot(timer_slovakia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_slovakia_short$BEV_time[length(timer_slovakia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Slovakia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_slovakia_short$BEV_time[length(timer_slovakia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_slovakia <- ggplot(slovakia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Slovakia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_slovakia), " years ", round(12*(time_20_to_80_slovakia-floor(time_20_to_80_slovakia)),0), " months"),
...
plot_slovakia <- plot_slovakia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(slovakia, slovakia$x==2024+counter & slovakia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_slovakia <- plot_slovakia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(slovakia, slovakia$x==2024+counter & slovakia$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_slovakia <- plot_slovakia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_slovakia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_slovakia$overall)
BEV <- data.frame(BEV, "overall"=data_slovakia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_slovakia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_slovakia <- ggplot(slovakia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Slovakia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_slovakia), " years ",
      round(12 * (time_80_to_20_slovakia - floor(time_80_to_20_slovakia)), 0),
      " months"
    ),
...
plot_ICE_BEV_slovakia <- plot_ICE_BEV_slovakia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(slovakia, slovakia$x == 2024 + counter - 1 & slovakia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_slovakia <- plot_ICE_BEV_slovakia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(slovakia, slovakia$x == 2024 + counter & slovakia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_slovakia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("slovakia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_slovakia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovakia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("slovakia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_slovakia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovakia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("slovakia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovakia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("slovakia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_slovakia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovakia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_slovakia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_slovakia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_slovakia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_slovakia, "Inf"),
  bev_20_80 = time_20_to_80_slovakia,
  ice_80_20 = time_80_to_20_slovakia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_slovakia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_slovakia)
month_label <- last_label_from_data(data_slovakia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_slovakia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_slovakia)) nz(tail(data_slovakia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_slovakia)) {
    nz(tail(data_slovakia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_slovakia)) nz(tail(data_slovakia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_slovakia)) {
    nz(tail(data_slovakia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_slovakia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_slovakia)
upsert_weight(
