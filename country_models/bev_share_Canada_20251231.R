#### Canada: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Canada"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/canada.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_canada <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Canada")
source <- data_canada$Source[1]
...
#data_canada$BEV <- data_canada$`Electric/zero-emission`
#data_canada$PHEV <- data_canada$Hybrid
#data_canada$TOTAL <- data_canada$overall
#data_canada$OTHERS <- data_canada$`Other fuel`
#data_canada$TOTAL - data_canada$BEV - data_canada$PHEV <- data_canada$Fossil

data_canada$`Electric/zero-emission` <- data_canada$BEV
data_canada$Hybrid <- data_canada$PHEV
data_canada$overall <- data_canada$TOTAL
data_canada$`Other fuel` <- data_canada$OTHERS
data_canada$Fossil <- data_canada$TOTAL - data_canada$BEV - data_canada$PHEV


data_canada$bev_share   <- data_canada$`Electric/zero-emission`/data_canada$overall
data_canada$ice_share   <- data_canada$Fossil/data_canada$overall
data_canada$hybrid_share<- data_canada$Hybrid/data_canada$overall
data_canada$other_share <- data_canada$`Other fuel`/data_canada$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_canada$pure_ICE    <- (data_canada$Fossil)/data_canada$overall

lastyearly <- ceiling(max(subset(data_canada, data_canada$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_canada_monthly <- subset(data_canada, data_canada$time_interval=="quarterly")
data_canada_monthly <- subset(data_canada_monthly, data_canada_monthly$year>=min(data_canada_monthly$year+1))
TTM_shares_canada <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_canada_monthly$year)+1),"-01-01")), by = "quarter", length.out = nrow(data_canada_monthly)), "%Y-%m"),
  Other = data_canada_monthly$`Other TTM`,
  #`ICE` = data_canada_monthly$`ICE TTM`,#-data_canada_monthly$`HEV TTM`,
  Petrol = data_canada_monthly$`Petrol TTM`,
  Diesel = data_canada_monthly$`Diesel TTM`,
  #Ethanol = data_canada_monthly$`Ethanol TTM`,
  #FlexFuel = data_canada_monthly$`FlexFuel TTM`,
  HEV = data_canada_monthly$`HEV TTM`,
  PHEV = data_canada_monthly$`PHEV TTM`,
  BEV = data_canada_monthly$`BEV TTM`
)

colnames(TTM_shares_canada)[colnames(TTM_shares_canada) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_canada)[colnames(TTM_shares_canada) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_canada)[colnames(TTM_shares_canada) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_canada)[colnames(TTM_shares_canada) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_canada)[colnames(TTM_shares_canada) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_canada_long <- TTM_shares_canada %>%
...
TTM_barplot_canada <- ggplot(TTM_shares_canada_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_canada_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_canada_long$month[substr(TTM_shares_canada_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Canada",
...
print(TTM_barplot_canada)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_canada$year)))
extrapol <- 2200
data_canada <- subset(data_canada, data_canada$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_canada$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_canada$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_canada$`Electric/zero-emission`))
for(i in 1:length(data_canada$`Electric/zero-emission`)){
  data_canada_looped <- data_canada[1:i,]
  xg<-data_canada_looped$year; yg<-as.double(data_canada_looped$bev_share)
  ...
  xg<-data_canada_looped$year; yg<-as.double(data_canada_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  canada <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_canada_looped$overall; new_A$time_interval <- data_canada_looped$time_interval
  
  canada$Hybrid       <- pmax(canada$Hybrid, 0)
  canada$Hybrid_upper <- pmax(canada$Hybrid_upper, 0)
  canada$Hybrid_lower <- pmax(canada$Hybrid_lower, 0)
  
  time_80_canada <- max(subset(canada, canada$BEV<=0.8 & canada$BEV >= 0.2)$x)
  time_50_canada <- max(subset(canada, canada$BEV<=0.5 & canada$BEV >= 0.2)$x)
  time_20_canada <- max(subset(canada, canada$BEV<=0.2 & canada$BEV >= 0.1)$x)
  time_20_to_80_canada <- max(subset(canada, canada$BEV<=0.8 & canada$BEV >= 0.2)$x)-min(subset(canada, canada$BEV<=0.8 & canada$BEV >= 0.2)$x)
  time_80_to_20_canada <- max(subset(canada, canada$ICE<=0.8 & canada$ICE >= 0.2)$x)-min(subset(canada, canada$ICE<=0.8 & canada$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_canada; ice_time[i] <- time_80_to_20_canada
}

timer_canada <- data.frame("year"=data_canada[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_canada$BEV_time == -Inf), which(timer_canada$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_canada) > (last_inf_index + 12)) {
  timer_canada_short <- timer_canada[(last_inf_index + 1 + 12):nrow(timer_canada), ]
} else {
  timer_canada_short <- timer_canada
}
...
plot_timer <- ggplot(timer_canada_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_canada_short$BEV_time[length(timer_canada_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Canada transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_canada_short$BEV_time[length(timer_canada_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_canada <- ggplot(canada, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Canada - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_canada), " years ", round(12*(time_20_to_80_canada-floor(time_20_to_80_canada)),0), " months"),
...
plot_canada <- plot_canada + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(canada, canada$x==2024+counter & canada$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_canada <- plot_canada + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(canada, canada$x==2024+counter & canada$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_canada <- plot_canada + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_canada

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_canada$overall)
BEV <- data.frame(BEV, "overall"=data_canada$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_canada$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_canada <- ggplot(canada, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Canada - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_canada), " years ",
      round(12 * (time_80_to_20_canada - floor(time_80_to_20_canada)), 0),
      " months"
    ),
...
plot_ICE_BEV_canada <- plot_ICE_BEV_canada +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(canada, canada$x == 2024 + counter - 1 & canada$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_canada <- plot_ICE_BEV_canada +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(canada, canada$x == 2024 + counter & canada$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_canada)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("canada_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_canada,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_canada",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("canada_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_canada,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_canada",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("canada_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_canada",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("canada_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_canada,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_canada",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_canada$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_canada, "Inf"),
  bev_50 = as_date_from_numeric(time_50_canada, "Inf"),
  bev_80 = as_date_from_numeric(time_80_canada, "Inf"),
  bev_20_80 = time_20_to_80_canada,
  ice_80_20 = time_80_to_20_canada,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_canada$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_canada)
month_label <- last_label_from_data(data_canada, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_canada$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_canada)) nz(tail(data_canada$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_canada)) {
    nz(tail(data_canada$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_canada)) nz(tail(data_canada$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_canada)) {
    nz(tail(data_canada$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_canada),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_canada)
upsert_weight(
