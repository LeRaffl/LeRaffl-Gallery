#### Estonia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Estonia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/estonia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_estonia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Estonia")
source <- data_estonia$Source[1]
...
#data_estonia$BEV <- data_estonia$`Electric/zero-emission`
#data_estonia$PHEV <- data_estonia$Hybrid
#data_estonia$TOTAL <- data_estonia$overall
#data_estonia$OTHERS <- data_estonia$`Other fuel`
#data_estonia$TOTAL - data_estonia$BEV - data_estonia$PHEV <- data_estonia$Fossil

data_estonia$`Electric/zero-emission` <- data_estonia$BEV
data_estonia$Hybrid <- data_estonia$PHEV
data_estonia$overall <- data_estonia$TOTAL
data_estonia$`Other fuel` <- data_estonia$OTHERS
data_estonia$Fossil <- data_estonia$TOTAL - data_estonia$BEV - data_estonia$PHEV


data_estonia$bev_share   <- data_estonia$`Electric/zero-emission`/data_estonia$overall
data_estonia$ice_share   <- data_estonia$Fossil/data_estonia$overall
data_estonia$hybrid_share<- data_estonia$Hybrid/data_estonia$overall
data_estonia$other_share <- data_estonia$`Other fuel`/data_estonia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_estonia$pure_ICE    <- (data_estonia$Fossil)/data_estonia$overall

lastyearly <- ceiling(max(subset(data_estonia, data_estonia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_estonia_monthly <- subset(data_estonia, data_estonia$time_interval=="monthly")
data_estonia_monthly <- subset(data_estonia_monthly, data_estonia_monthly$year>=min(data_estonia_monthly$year+1))
TTM_shares_estonia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_estonia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_estonia_monthly)), "%Y-%m"),
  Other = data_estonia_monthly$`Other TTM`,
  #`ICE` = data_estonia_monthly$`ICE TTM`,#-data_estonia_monthly$`HEV TTM`,
  Petrol = data_estonia_monthly$`Petrol TTM`,
  Diesel = data_estonia_monthly$`Diesel TTM`,
  #Ethanol = data_estonia_monthly$`Ethanol TTM`,
  #FlexFuel = data_estonia_monthly$`FlexFuel TTM`,
  HEV = data_estonia_monthly$`HEV TTM`,
  PHEV = data_estonia_monthly$`PHEV TTM`,
  BEV = data_estonia_monthly$`BEV TTM`
)

colnames(TTM_shares_estonia)[colnames(TTM_shares_estonia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_estonia)[colnames(TTM_shares_estonia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_estonia)[colnames(TTM_shares_estonia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_estonia)[colnames(TTM_shares_estonia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_estonia)[colnames(TTM_shares_estonia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_estonia_long <- TTM_shares_estonia %>%
...
TTM_barplot_estonia <- ggplot(TTM_shares_estonia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_estonia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_estonia_long$month[substr(TTM_shares_estonia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Estonia",
...
print(TTM_barplot_estonia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_estonia$year)))
extrapol <- 2200
data_estonia <- subset(data_estonia, data_estonia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_estonia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_estonia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_estonia$`Electric/zero-emission`))
for(i in 1:length(data_estonia$`Electric/zero-emission`)){
  data_estonia_looped <- data_estonia[1:i,]
  xg<-data_estonia_looped$year; yg<-as.double(data_estonia_looped$bev_share)
  ...
  xg<-data_estonia_looped$year; yg<-as.double(data_estonia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  estonia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_estonia_looped$overall; new_A$time_interval <- data_estonia_looped$time_interval
  
  estonia$Hybrid       <- pmax(estonia$Hybrid, 0)
  estonia$Hybrid_upper <- pmax(estonia$Hybrid_upper, 0)
  estonia$Hybrid_lower <- pmax(estonia$Hybrid_lower, 0)
  
  time_80_estonia <- max(subset(estonia, estonia$BEV<=0.8 & estonia$BEV >= 0.2)$x)
  time_50_estonia <- max(subset(estonia, estonia$BEV<=0.5 & estonia$BEV >= 0.2)$x)
  time_20_estonia <- max(subset(estonia, estonia$BEV<=0.2 & estonia$BEV >= 0.1)$x)
  time_20_to_80_estonia <- max(subset(estonia, estonia$BEV<=0.8 & estonia$BEV >= 0.2)$x)-min(subset(estonia, estonia$BEV<=0.8 & estonia$BEV >= 0.2)$x)
  time_80_to_20_estonia <- max(subset(estonia, estonia$ICE<=0.8 & estonia$ICE >= 0.2)$x)-min(subset(estonia, estonia$ICE<=0.8 & estonia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_estonia; ice_time[i] <- time_80_to_20_estonia
}

timer_estonia <- data.frame("year"=data_estonia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_estonia$BEV_time == -Inf), which(timer_estonia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_estonia) > (last_inf_index + 12)) {
  timer_estonia_short <- timer_estonia[(last_inf_index + 1 + 12):nrow(timer_estonia), ]
} else {
  timer_estonia_short <- timer_estonia
}
...
plot_timer <- ggplot(timer_estonia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_estonia_short$BEV_time[length(timer_estonia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Estonia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_estonia_short$BEV_time[length(timer_estonia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_estonia <- ggplot(estonia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Estonia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_estonia), " years ", round(12*(time_20_to_80_estonia-floor(time_20_to_80_estonia)),0), " months"),
...
plot_estonia <- plot_estonia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(estonia, estonia$x==2024+counter & estonia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_estonia <- plot_estonia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(estonia, estonia$x==2024+counter & estonia$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_estonia <- plot_estonia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_estonia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_estonia$overall)
BEV <- data.frame(BEV, "overall"=data_estonia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_estonia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_estonia <- ggplot(estonia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Estonia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_estonia), " years ",
      round(12 * (time_80_to_20_estonia - floor(time_80_to_20_estonia)), 0),
      " months"
    ),
...
plot_ICE_BEV_estonia <- plot_ICE_BEV_estonia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(estonia, estonia$x == 2024 + counter - 1 & estonia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_estonia <- plot_ICE_BEV_estonia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(estonia, estonia$x == 2024 + counter & estonia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_estonia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("estonia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_estonia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_estonia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("estonia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_estonia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_estonia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("estonia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_estonia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("estonia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_estonia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_estonia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_estonia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_estonia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_estonia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_estonia, "Inf"),
  bev_20_80 = time_20_to_80_estonia,
  ice_80_20 = time_80_to_20_estonia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_estonia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_estonia)
month_label <- last_label_from_data(data_estonia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_estonia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_estonia)) nz(tail(data_estonia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_estonia)) {
    nz(tail(data_estonia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_estonia)) nz(tail(data_estonia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_estonia)) {
    nz(tail(data_estonia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_estonia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_estonia)
upsert_weight(
