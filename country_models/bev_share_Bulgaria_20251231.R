#### Bulgaria: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Bulgaria"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/bulgaria.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_bulgaria <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Bulgaria")
source <- data_bulgaria$Source[1]
...
#data_bulgaria$BEV <- data_bulgaria$`Electric/zero-emission`
#data_bulgaria$PHEV <- data_bulgaria$Hybrid
#data_bulgaria$TOTAL <- data_bulgaria$overall
#data_bulgaria$OTHERS <- data_bulgaria$`Other fuel`
#data_bulgaria$TOTAL - data_bulgaria$BEV - data_bulgaria$PHEV <- data_bulgaria$Fossil

data_bulgaria$`Electric/zero-emission` <- data_bulgaria$BEV
data_bulgaria$Hybrid <- data_bulgaria$PHEV
data_bulgaria$overall <- data_bulgaria$TOTAL
data_bulgaria$`Other fuel` <- data_bulgaria$OTHERS
data_bulgaria$Fossil <- data_bulgaria$TOTAL - data_bulgaria$BEV - data_bulgaria$PHEV


data_bulgaria$bev_share   <- data_bulgaria$`Electric/zero-emission`/data_bulgaria$overall
data_bulgaria$ice_share   <- data_bulgaria$Fossil/data_bulgaria$overall
data_bulgaria$hybrid_share<- data_bulgaria$Hybrid/data_bulgaria$overall
data_bulgaria$other_share <- data_bulgaria$`Other fuel`/data_bulgaria$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_bulgaria$pure_ICE    <- (data_bulgaria$Fossil)/data_bulgaria$overall

lastyearly <- ceiling(max(subset(data_bulgaria, data_bulgaria$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_bulgaria_monthly <- subset(data_bulgaria, data_bulgaria$time_interval=="monthly")
data_bulgaria_monthly <- subset(data_bulgaria_monthly, data_bulgaria_monthly$year>=min(data_bulgaria_monthly$year+1))
TTM_shares_bulgaria <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_bulgaria_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_bulgaria_monthly)), "%Y-%m"),
  Other = data_bulgaria_monthly$`Other TTM`,
  #`ICE` = data_bulgaria_monthly$`ICE TTM`,#-data_bulgaria_monthly$`HEV TTM`,
  Petrol = data_bulgaria_monthly$`Petrol TTM`,
  Diesel = data_bulgaria_monthly$`Diesel TTM`,
  #Ethanol = data_bulgaria_monthly$`Ethanol TTM`,
  #FlexFuel = data_bulgaria_monthly$`FlexFuel TTM`,
  HEV = data_bulgaria_monthly$`HEV TTM`,
  PHEV = data_bulgaria_monthly$`PHEV TTM`,
  BEV = data_bulgaria_monthly$`BEV TTM`
)

colnames(TTM_shares_bulgaria)[colnames(TTM_shares_bulgaria) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_bulgaria)[colnames(TTM_shares_bulgaria) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_bulgaria)[colnames(TTM_shares_bulgaria) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_bulgaria)[colnames(TTM_shares_bulgaria) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_bulgaria)[colnames(TTM_shares_bulgaria) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_bulgaria_long <- TTM_shares_bulgaria %>%
...
TTM_barplot_bulgaria <- ggplot(TTM_shares_bulgaria_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_bulgaria_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_bulgaria_long$month[substr(TTM_shares_bulgaria_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Bulgaria",
...
print(TTM_barplot_bulgaria)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_bulgaria$year)))
extrapol <- 2200
data_bulgaria <- subset(data_bulgaria, data_bulgaria$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_bulgaria$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_bulgaria$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_bulgaria$`Electric/zero-emission`))
for(i in 1:length(data_bulgaria$`Electric/zero-emission`)){
  data_bulgaria_looped <- data_bulgaria[1:i,]
  xg<-data_bulgaria_looped$year; yg<-as.double(data_bulgaria_looped$bev_share)
  ...
  xg<-data_bulgaria_looped$year; yg<-as.double(data_bulgaria_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  bulgaria <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_bulgaria_looped$overall; new_A$time_interval <- data_bulgaria_looped$time_interval
  
  bulgaria$Hybrid       <- pmax(bulgaria$Hybrid, 0)
  bulgaria$Hybrid_upper <- pmax(bulgaria$Hybrid_upper, 0)
  bulgaria$Hybrid_lower <- pmax(bulgaria$Hybrid_lower, 0)
  
  time_80_bulgaria <- max(subset(bulgaria, bulgaria$BEV<=0.8 & bulgaria$BEV >= 0.2)$x)
  time_50_bulgaria <- max(subset(bulgaria, bulgaria$BEV<=0.5 & bulgaria$BEV >= 0.2)$x)
  time_20_bulgaria <- max(subset(bulgaria, bulgaria$BEV<=0.2 & bulgaria$BEV >= 0.1)$x)
  time_20_to_80_bulgaria <- max(subset(bulgaria, bulgaria$BEV<=0.8 & bulgaria$BEV >= 0.2)$x)-min(subset(bulgaria, bulgaria$BEV<=0.8 & bulgaria$BEV >= 0.2)$x)
  time_80_to_20_bulgaria <- max(subset(bulgaria, bulgaria$ICE<=0.8 & bulgaria$ICE >= 0.2)$x)-min(subset(bulgaria, bulgaria$ICE<=0.8 & bulgaria$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_bulgaria; ice_time[i] <- time_80_to_20_bulgaria
}

timer_bulgaria <- data.frame("year"=data_bulgaria[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_bulgaria$BEV_time == -Inf), which(timer_bulgaria$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_bulgaria) > (last_inf_index + 12)) {
  timer_bulgaria_short <- timer_bulgaria[(last_inf_index + 1 + 12):nrow(timer_bulgaria), ]
} else {
  timer_bulgaria_short <- timer_bulgaria
}
...
plot_timer <- ggplot(timer_bulgaria_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_bulgaria_short$BEV_time[length(timer_bulgaria_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Bulgaria transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_bulgaria_short$BEV_time[length(timer_bulgaria_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_bulgaria <- ggplot(bulgaria, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Bulgaria - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_bulgaria), " years ", round(12*(time_20_to_80_bulgaria-floor(time_20_to_80_bulgaria)),0), " months"),
...
plot_bulgaria <- plot_bulgaria + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(bulgaria, bulgaria$x==2024+counter & bulgaria$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_bulgaria <- plot_bulgaria + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(bulgaria, bulgaria$x==2024+counter & bulgaria$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_bulgaria <- plot_bulgaria + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_bulgaria

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_bulgaria$overall)
BEV <- data.frame(BEV, "overall"=data_bulgaria$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_bulgaria$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_bulgaria <- ggplot(bulgaria, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Bulgaria - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_bulgaria), " years ",
      round(12 * (time_80_to_20_bulgaria - floor(time_80_to_20_bulgaria)), 0),
      " months"
    ),
...
plot_ICE_BEV_bulgaria <- plot_ICE_BEV_bulgaria +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(bulgaria, bulgaria$x == 2024 + counter - 1 & bulgaria$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_bulgaria <- plot_ICE_BEV_bulgaria +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(bulgaria, bulgaria$x == 2024 + counter & bulgaria$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_bulgaria)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("bulgaria_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_bulgaria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_bulgaria",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("bulgaria_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_bulgaria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_bulgaria",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("bulgaria_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_bulgaria",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("bulgaria_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_bulgaria,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_bulgaria",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_bulgaria$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_bulgaria, "Inf"),
  bev_50 = as_date_from_numeric(time_50_bulgaria, "Inf"),
  bev_80 = as_date_from_numeric(time_80_bulgaria, "Inf"),
  bev_20_80 = time_20_to_80_bulgaria,
  ice_80_20 = time_80_to_20_bulgaria,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_bulgaria$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_bulgaria)
month_label <- last_label_from_data(data_bulgaria, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_bulgaria$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_bulgaria)) nz(tail(data_bulgaria$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_bulgaria)) {
    nz(tail(data_bulgaria$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_bulgaria)) nz(tail(data_bulgaria$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_bulgaria)) {
    nz(tail(data_bulgaria$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_bulgaria),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_bulgaria)
upsert_weight(
