#### Albania: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Albania"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/albania.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_albania <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Albania")
source <- data_albania$Source[1]
...
#data_albania$BEV <- data_albania$`Electric/zero-emission`
#data_albania$PHEV <- data_albania$Hybrid
#data_albania$TOTAL <- data_albania$overall
#data_albania$OTHERS <- data_albania$`Other fuel`
#data_albania$TOTAL - data_albania$BEV - data_albania$PHEV <- data_albania$Fossil

data_albania$`Electric/zero-emission` <- data_albania$BEV
data_albania$Hybrid <- data_albania$PHEV
data_albania$overall <- data_albania$TOTAL
data_albania$`Other fuel` <- data_albania$OTHERS
data_albania$Fossil <- data_albania$TOTAL - data_albania$BEV - data_albania$PHEV


data_albania$bev_share   <- data_albania$`Electric/zero-emission`/data_albania$overall
data_albania$ice_share   <- data_albania$Fossil/data_albania$overall
data_albania$hybrid_share<- data_albania$Hybrid/data_albania$overall
data_albania$other_share <- data_albania$`Other fuel`/data_albania$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_albania$pure_ICE    <- (data_albania$Fossil)/data_albania$overall

lastyearly <- ceiling(max(subset(data_albania, data_albania$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_albania_monthly <- subset(data_albania, data_albania$time_interval=="monthly")
data_albania_monthly <- subset(data_albania_monthly, data_albania_monthly$year>=min(data_albania_monthly$year+1))
TTM_shares_albania <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_albania_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_albania_monthly)), "%Y-%m"),
  Other = data_albania_monthly$`Other TTM`,
  #`ICE` = data_albania_monthly$`ICE TTM`,#-data_albania_monthly$`HEV TTM`,
  Petrol = data_albania_monthly$`Petrol TTM`,
  Diesel = data_albania_monthly$`Diesel TTM`,
  #Ethanol = data_albania_monthly$`Ethanol TTM`,
  #FlexFuel = data_albania_monthly$`FlexFuel TTM`,
  HEV = data_albania_monthly$`HEV TTM`,
  PHEV = data_albania_monthly$`PHEV TTM`,
  BEV = data_albania_monthly$`BEV TTM`
)

colnames(TTM_shares_albania)[colnames(TTM_shares_albania) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_albania)[colnames(TTM_shares_albania) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_albania)[colnames(TTM_shares_albania) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_albania)[colnames(TTM_shares_albania) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_albania)[colnames(TTM_shares_albania) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_albania_long <- TTM_shares_albania %>%
...
TTM_barplot_albania <- ggplot(TTM_shares_albania_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_albania_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_albania_long$month[substr(TTM_shares_albania_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Albania",
...
print(TTM_barplot_albania)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_albania$year)))
extrapol <- 2200
data_albania <- subset(data_albania, data_albania$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_albania$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_albania$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_albania$`Electric/zero-emission`))
for(i in 1:length(data_albania$`Electric/zero-emission`)){
  data_albania_looped <- data_albania[1:i,]
  xg<-data_albania_looped$year; yg<-as.double(data_albania_looped$bev_share)
  ...
  xg<-data_albania_looped$year; yg<-as.double(data_albania_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  albania <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_albania_looped$overall; new_A$time_interval <- data_albania_looped$time_interval
  
  albania$Hybrid       <- pmax(albania$Hybrid, 0)
  albania$Hybrid_upper <- pmax(albania$Hybrid_upper, 0)
  albania$Hybrid_lower <- pmax(albania$Hybrid_lower, 0)
  
  time_80_albania <- max(subset(albania, albania$BEV<=0.8 & albania$BEV >= 0.2)$x)
  time_50_albania <- max(subset(albania, albania$BEV<=0.5 & albania$BEV >= 0.2)$x)
  time_20_albania <- max(subset(albania, albania$BEV<=0.2 & albania$BEV >= 0.1)$x)
  time_20_to_80_albania <- max(subset(albania, albania$BEV<=0.8 & albania$BEV >= 0.2)$x)-min(subset(albania, albania$BEV<=0.8 & albania$BEV >= 0.2)$x)
  time_80_to_20_albania <- max(subset(albania, albania$ICE<=0.8 & albania$ICE >= 0.2)$x)-min(subset(albania, albania$ICE<=0.8 & albania$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_albania; ice_time[i] <- time_80_to_20_albania
}

timer_albania <- data.frame("year"=data_albania[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_albania$BEV_time == -Inf), which(timer_albania$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_albania) > (last_inf_index + 12)) {
  timer_albania_short <- timer_albania[(last_inf_index + 1 + 12):nrow(timer_albania), ]
} else {
  timer_albania_short <- timer_albania
}
...
plot_timer <- ggplot(timer_albania_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_albania_short$BEV_time[length(timer_albania_short$BEV_time)]*4)
  ) +
  
  labs(
    title = "Time expectation for Albania transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_albania_short$BEV_time[length(timer_albania_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_albania <- ggplot(albania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Albania - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_albania), " years ", round(12*(time_20_to_80_albania-floor(time_20_to_80_albania)),0), " months"),
...
plot_albania <- plot_albania + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(albania, albania$x==2024+counter & albania$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_albania <- plot_albania + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(albania, albania$x==2024+counter & albania$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_albania <- plot_albania + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_albania

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_albania$overall)
BEV <- data.frame(BEV, "overall"=data_albania$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_albania$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_albania <- ggplot(albania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Albania - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_albania), " years ",
      round(12 * (time_80_to_20_albania - floor(time_80_to_20_albania)), 0),
      " months"
    ),
...
plot_ICE_BEV_albania <- plot_ICE_BEV_albania +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(albania, albania$x == 2024 + counter - 1 & albania$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_albania <- plot_ICE_BEV_albania +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(albania, albania$x == 2024 + counter & albania$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_albania)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("albania_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_albania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_albania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("albania_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_albania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_albania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("albania_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_albania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("albania_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_albania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_albania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_albania$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_albania, "Inf"),
  bev_50 = as_date_from_numeric(time_50_albania, "Inf"),
  bev_80 = as_date_from_numeric(time_80_albania, "Inf"),
  bev_20_80 = time_20_to_80_albania,
  ice_80_20 = time_80_to_20_albania,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_albania$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_albania)
month_label <- last_label_from_data(data_albania, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_albania$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_albania)) nz(tail(data_albania$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_albania)) {
    nz(tail(data_albania$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_albania)) nz(tail(data_albania$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_albania)) {
    nz(tail(data_albania$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_albania),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_albania)
upsert_weight(
