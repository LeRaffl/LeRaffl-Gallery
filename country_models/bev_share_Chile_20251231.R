#### Chile: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Chile"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/chile.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_chile <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Chile")
source <- data_chile$Source[1]
...
#data_chile$BEV <- data_chile$`Electric/zero-emission`
#data_chile$PHEV <- data_chile$Hybrid
#data_chile$TOTAL <- data_chile$overall
#data_chile$OTHERS <- data_chile$`Other fuel`
#data_chile$TOTAL - data_chile$BEV - data_chile$PHEV <- data_chile$Fossil

data_chile$`Electric/zero-emission` <- data_chile$BEV
data_chile$Hybrid <- data_chile$PHEV
data_chile$overall <- data_chile$TOTAL
data_chile$`Other fuel` <- data_chile$OTHERS
data_chile$Fossil <- data_chile$TOTAL - data_chile$BEV - data_chile$PHEV


data_chile$bev_share   <- data_chile$`Electric/zero-emission`/data_chile$overall
data_chile$ice_share   <- data_chile$Fossil/data_chile$overall
data_chile$hybrid_share<- data_chile$Hybrid/data_chile$overall
data_chile$other_share <- data_chile$`Other fuel`/data_chile$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_chile$pure_ICE    <- (data_chile$Fossil)/data_chile$overall

lastyearly <- ceiling(max(subset(data_chile, data_chile$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_chile_monthly <- subset(data_chile, data_chile$time_interval=="monthly")
data_chile_monthly <- subset(data_chile_monthly, data_chile_monthly$year>=min(data_chile_monthly$year+1))
TTM_shares_chile <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_chile_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_chile_monthly)), "%Y-%m"),
  Other = data_chile_monthly$`Other TTM`,
  `ICE` = data_chile_monthly$`ICE TTM`,#-data_chile_monthly$`HEV TTM`,
  Petrol = data_chile_monthly$`Petrol TTM`,
  Diesel = data_chile_monthly$`Diesel TTM`,
  #Ethanol = data_chile_monthly$`Ethanol TTM`,
  #FlexFuel = data_chile_monthly$`FlexFuel TTM`,
  HEV = data_chile_monthly$`HEV TTM`,
  PHEV = data_chile_monthly$`PHEV TTM`,
  BEV = data_chile_monthly$`BEV TTM`
)

colnames(TTM_shares_chile)[colnames(TTM_shares_chile) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_chile)[colnames(TTM_shares_chile) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_chile)[colnames(TTM_shares_chile) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_chile)[colnames(TTM_shares_chile) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_chile)[colnames(TTM_shares_chile) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_chile_long <- TTM_shares_chile %>%
...
TTM_barplot_chile <- ggplot(TTM_shares_chile_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_chile_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_chile_long$month[substr(TTM_shares_chile_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Chile",
...
print(TTM_barplot_chile)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_chile$year)))
extrapol <- 2200
data_chile <- subset(data_chile, data_chile$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_chile$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_chile$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_chile$`Electric/zero-emission`))
for(i in 1:length(data_chile$`Electric/zero-emission`)){
  data_chile_looped <- data_chile[1:i,]
  xg<-data_chile_looped$year; yg<-as.double(data_chile_looped$bev_share)
  ...
  xg<-data_chile_looped$year; yg<-as.double(data_chile_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  chile <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_chile_looped$overall; new_A$time_interval <- data_chile_looped$time_interval
  
  chile$Hybrid       <- pmax(chile$Hybrid, 0)
  chile$Hybrid_upper <- pmax(chile$Hybrid_upper, 0)
  chile$Hybrid_lower <- pmax(chile$Hybrid_lower, 0)
  
  time_80_chile <- max(subset(chile, chile$BEV<=0.8 & chile$BEV >= 0.2)$x)
  time_50_chile <- max(subset(chile, chile$BEV<=0.5 & chile$BEV >= 0.2)$x)
  time_20_chile <- max(subset(chile, chile$BEV<=0.2 & chile$BEV >= 0.1)$x)
  time_20_to_80_chile <- max(subset(chile, chile$BEV<=0.8 & chile$BEV >= 0.2)$x)-min(subset(chile, chile$BEV<=0.8 & chile$BEV >= 0.2)$x)
  time_80_to_20_chile <- max(subset(chile, chile$ICE<=0.8 & chile$ICE >= 0.2)$x)-min(subset(chile, chile$ICE<=0.8 & chile$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_chile; ice_time[i] <- time_80_to_20_chile
}

timer_chile <- data.frame("year"=data_chile[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_chile$BEV_time == -Inf), which(timer_chile$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_chile) > (last_inf_index + 12)) {
  timer_chile_short <- timer_chile[(last_inf_index + 1 + 12):nrow(timer_chile), ]
} else {
  timer_chile_short <- timer_chile
}
...
plot_timer <- ggplot(timer_chile_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_chile_short$BEV_time[length(timer_chile_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Chile transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_chile_short$BEV_time[length(timer_chile_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_chile <- ggplot(chile, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Chile - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_chile), " years ", round(12*(time_20_to_80_chile-floor(time_20_to_80_chile)),0), " months"),
...
plot_chile <- plot_chile + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(chile, chile$x==2024+counter & chile$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_chile <- plot_chile + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(chile, chile$x==2024+counter & chile$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_chile <- plot_chile + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_chile

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_chile$overall)
BEV <- data.frame(BEV, "overall"=data_chile$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_chile$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_chile <- ggplot(chile, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Chile - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_chile), " years ",
      round(12 * (time_80_to_20_chile - floor(time_80_to_20_chile)), 0),
      " months"
    ),
...
plot_ICE_BEV_chile <- plot_ICE_BEV_chile +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(chile, chile$x == 2024 + counter - 1 & chile$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_chile <- plot_ICE_BEV_chile +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(chile, chile$x == 2024 + counter & chile$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_chile)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("chile_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_chile,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_chile",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("chile_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_chile,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_chile",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("chile_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_chile",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("chile_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_chile,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_chile",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_chile$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_chile, "Inf"),
  bev_50 = as_date_from_numeric(time_50_chile, "Inf"),
  bev_80 = as_date_from_numeric(time_80_chile, "Inf"),
  bev_20_80 = time_20_to_80_chile,
  ice_80_20 = time_80_to_20_chile,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_chile$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_chile)
month_label <- last_label_from_data(data_chile, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_chile$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_chile)) nz(tail(data_chile$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_chile)) {
    nz(tail(data_chile$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_chile)) nz(tail(data_chile$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_chile)) {
    nz(tail(data_chile$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_chile),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_chile)
upsert_weight(
