#### South Korea: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "South Korea"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/southkorea.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_southkorea <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "South Korea")
source <- data_southkorea$Source[1]
...
#data_southkorea$BEV <- data_southkorea$`Electric/zero-emission`
#data_southkorea$PHEV <- data_southkorea$Hybrid
#data_southkorea$TOTAL <- data_southkorea$overall
#data_southkorea$OTHERS <- data_southkorea$`Other fuel`
#data_southkorea$TOTAL - data_southkorea$BEV - data_southkorea$PHEV <- data_southkorea$Fossil

data_southkorea$`Electric/zero-emission` <- data_southkorea$BEV
data_southkorea$Hybrid <- data_southkorea$PHEV
data_southkorea$overall <- data_southkorea$TOTAL
data_southkorea$`Other fuel` <- data_southkorea$OTHERS
data_southkorea$Fossil <- data_southkorea$TOTAL - data_southkorea$BEV - data_southkorea$PHEV


data_southkorea$bev_share   <- data_southkorea$`Electric/zero-emission`/data_southkorea$overall
data_southkorea$ice_share   <- data_southkorea$Fossil/data_southkorea$overall
data_southkorea$hybrid_share<- data_southkorea$Hybrid/data_southkorea$overall
data_southkorea$other_share <- data_southkorea$`Other fuel`/data_southkorea$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_southkorea$pure_ICE    <- (data_southkorea$Fossil)/data_southkorea$overall

lastyearly <- ceiling(max(subset(data_southkorea, data_southkorea$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_southkorea_monthly <- subset(data_southkorea, data_southkorea$time_interval=="monthly")
data_southkorea_monthly <- subset(data_southkorea_monthly, data_southkorea_monthly$year>=min(data_southkorea_monthly$year+1))
TTM_shares_southkorea <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_southkorea_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_southkorea_monthly)), "%Y-%m"),
  Other = data_southkorea_monthly$`Other TTM`,
  `ICE` = data_southkorea_monthly$`ICE TTM`,#-data_southkorea_monthly$`HEV TTM`,
  #Petrol = data_southkorea_monthly$`Petrol TTM`,
  #Diesel = data_southkorea_monthly$`Diesel TTM`,
  #Ethanol = data_southkorea_monthly$`Ethanol TTM`,
  #FlexFuel = data_southkorea_monthly$`FlexFuel TTM`,
  HEV = data_southkorea_monthly$`HEV TTM`,
  PHEV = data_southkorea_monthly$`PHEV TTM`,
  BEV = data_southkorea_monthly$`BEV TTM`
)

colnames(TTM_shares_southkorea)[colnames(TTM_shares_southkorea) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_southkorea)[colnames(TTM_shares_southkorea) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_southkorea)[colnames(TTM_shares_southkorea) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_southkorea)[colnames(TTM_shares_southkorea) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_southkorea)[colnames(TTM_shares_southkorea) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_southkorea_long <- TTM_shares_southkorea %>%
...
TTM_barplot_southkorea <- ggplot(TTM_shares_southkorea_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_southkorea_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_southkorea_long$month[substr(TTM_shares_southkorea_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in South Korea",
...
print(TTM_barplot_southkorea)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_southkorea$year)))
extrapol <- 2200
data_southkorea <- subset(data_southkorea, data_southkorea$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_southkorea$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_southkorea$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_southkorea$`Electric/zero-emission`))
for(i in 1:length(data_southkorea$`Electric/zero-emission`)){
  data_southkorea_looped <- data_southkorea[1:i,]
  xg<-data_southkorea_looped$year; yg<-as.double(data_southkorea_looped$bev_share)
  ...
  xg<-data_southkorea_looped$year; yg<-as.double(data_southkorea_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  southkorea <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_southkorea_looped$overall; new_A$time_interval <- data_southkorea_looped$time_interval
  
  southkorea$Hybrid       <- pmax(southkorea$Hybrid, 0)
  southkorea$Hybrid_upper <- pmax(southkorea$Hybrid_upper, 0)
  southkorea$Hybrid_lower <- pmax(southkorea$Hybrid_lower, 0)
  
  time_80_southkorea <- max(subset(southkorea, southkorea$BEV<=0.8 & southkorea$BEV >= 0.2)$x)
  time_50_southkorea <- max(subset(southkorea, southkorea$BEV<=0.5 & southkorea$BEV >= 0.2)$x)
  time_20_southkorea <- max(subset(southkorea, southkorea$BEV<=0.2 & southkorea$BEV >= 0.1)$x)
  time_20_to_80_southkorea <- max(subset(southkorea, southkorea$BEV<=0.8 & southkorea$BEV >= 0.2)$x)-min(subset(southkorea, southkorea$BEV<=0.8 & southkorea$BEV >= 0.2)$x)
  time_80_to_20_southkorea <- max(subset(southkorea, southkorea$ICE<=0.8 & southkorea$ICE >= 0.2)$x)-min(subset(southkorea, southkorea$ICE<=0.8 & southkorea$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_southkorea; ice_time[i] <- time_80_to_20_southkorea
}

timer_southkorea <- data.frame("year"=data_southkorea[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_southkorea$BEV_time == -Inf), which(timer_southkorea$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_southkorea) > (last_inf_index + 12)) {
  timer_southkorea_short <- timer_southkorea[(last_inf_index + 1 + 12):nrow(timer_southkorea), ]
} else {
  timer_southkorea_short <- timer_southkorea
}
...
plot_timer <- ggplot(timer_southkorea_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_southkorea_short$BEV_time[length(timer_southkorea_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for South Korea transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_southkorea_short$BEV_time[length(timer_southkorea_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_southkorea <- ggplot(southkorea, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in South Korea - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_southkorea), " years ", round(12*(time_20_to_80_southkorea-floor(time_20_to_80_southkorea)),0), " months"),
...
plot_southkorea <- plot_southkorea + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(southkorea, southkorea$x==2024+counter & southkorea$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_southkorea <- plot_southkorea + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(southkorea, southkorea$x==2024+counter & southkorea$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_southkorea <- plot_southkorea + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_southkorea

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_southkorea$overall)
BEV <- data.frame(BEV, "overall"=data_southkorea$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_southkorea$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_southkorea <- ggplot(southkorea, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in South Korea - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_southkorea), " years ",
      round(12 * (time_80_to_20_southkorea - floor(time_80_to_20_southkorea)), 0),
      " months"
    ),
...
plot_ICE_BEV_southkorea <- plot_ICE_BEV_southkorea +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(southkorea, southkorea$x == 2024 + counter - 1 & southkorea$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_southkorea <- plot_ICE_BEV_southkorea +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(southkorea, southkorea$x == 2024 + counter & southkorea$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_southkorea)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("southkorea_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_southkorea,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_southkorea",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("southkorea_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_southkorea,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_southkorea",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("southkorea_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_southkorea",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("southkorea_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_southkorea,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_southkorea",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_southkorea$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_southkorea, "Inf"),
  bev_50 = as_date_from_numeric(time_50_southkorea, "Inf"),
  bev_80 = as_date_from_numeric(time_80_southkorea, "Inf"),
  bev_20_80 = time_20_to_80_southkorea,
  ice_80_20 = time_80_to_20_southkorea,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_southkorea$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_southkorea)
month_label <- last_label_from_data(data_southkorea, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_southkorea$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_southkorea)) nz(tail(data_southkorea$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_southkorea)) {
    nz(tail(data_southkorea$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_southkorea)) nz(tail(data_southkorea$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_southkorea)) {
    nz(tail(data_southkorea$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_southkorea),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_southkorea)
upsert_weight(
