#### Georgia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Georgia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/georgia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_georgia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Georgia")
source <- data_georgia$Source[1]
...
#data_georgia$BEV <- data_georgia$`Electric/zero-emission`
#data_georgia$PHEV <- data_georgia$Hybrid
#data_georgia$TOTAL <- data_georgia$overall
#data_georgia$OTHERS <- data_georgia$`Other fuel`
#data_georgia$TOTAL - data_georgia$BEV - data_georgia$PHEV <- data_georgia$Fossil

data_georgia$`Electric/zero-emission` <- data_georgia$BEV
#data_georgia$PHEV <- data_georgia$Hybrid
data_georgia$overall <- data_georgia$TOTAL
data_georgia$`Other fuel` <- data_georgia$OTHERS
data_georgia$Fossil <- data_georgia$TOTAL - data_georgia$BEV - data_georgia$Hybrid


data_georgia$bev_share   <- data_georgia$`Electric/zero-emission`/data_georgia$overall
data_georgia$ice_share   <- data_georgia$Fossil/data_georgia$overall
data_georgia$hybrid_share<- data_georgia$Hybrid/data_georgia$overall
data_georgia$other_share <- data_georgia$`Other fuel`/data_georgia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_georgia$pure_ICE    <- (data_georgia$Fossil)/data_georgia$overall

lastyearly <- ceiling(max(subset(data_georgia, data_georgia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_georgia_monthly <- subset(data_georgia, data_georgia$time_interval=="quarterly")
data_georgia_monthly <- subset(data_georgia_monthly, data_georgia_monthly$year>=min(data_georgia_monthly$year+1))
TTM_shares_georgia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_georgia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_georgia_monthly)), "%Y-%m"),
  Other = data_georgia_monthly$`Other TTM`,
  #`ICE` = data_georgia_monthly$`ICE TTM`,#-data_georgia_monthly$`HEV TTM`,
  Petrol = data_georgia_monthly$`Petrol TTM`,
  Diesel = data_georgia_monthly$`Diesel TTM`,
  #Ethanol = data_georgia_monthly$`Ethanol TTM`,
  #FlexFuel = data_georgia_monthly$`FlexFuel TTM`,
  #HEV = data_georgia_monthly$`HEV TTM`,
  #PHEV = data_georgia_monthly$`PHEV TTM`,
  Hybrids = data_georgia_monthly$`Hybrid TTM`,
  BEV = data_georgia_monthly$`BEV TTM`
)

colnames(TTM_shares_georgia)[colnames(TTM_shares_georgia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_georgia)[colnames(TTM_shares_georgia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_georgia)[colnames(TTM_shares_georgia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_georgia)[colnames(TTM_shares_georgia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_georgia)[colnames(TTM_shares_georgia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_georgia_long <- TTM_shares_georgia %>%
...
TTM_barplot_georgia <- ggplot(TTM_shares_georgia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_georgia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_georgia_long$month[substr(TTM_shares_georgia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Georgia",
...
print(TTM_barplot_georgia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_georgia$year)))
extrapol <- 2200
data_georgia <- subset(data_georgia, data_georgia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_georgia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_georgia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_georgia$`Electric/zero-emission`))
for(i in 1:length(data_georgia$`Electric/zero-emission`)){
  data_georgia_looped <- data_georgia[1:i,]
  xg<-data_georgia_looped$year; yg<-as.double(data_georgia_looped$bev_share)
  ...
  xg<-data_georgia_looped$year; yg<-as.double(data_georgia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  georgia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_georgia_looped$overall; new_A$time_interval <- data_georgia_looped$time_interval
  
  georgia$Hybrid       <- pmax(georgia$Hybrid, 0)
  georgia$Hybrid_upper <- pmax(georgia$Hybrid_upper, 0)
  georgia$Hybrid_lower <- pmax(georgia$Hybrid_lower, 0)
  
  time_80_georgia <- max(subset(georgia, georgia$BEV<=0.8 & georgia$BEV >= 0.2)$x)
  time_50_georgia <- max(subset(georgia, georgia$BEV<=0.5 & georgia$BEV >= 0.2)$x)
  time_20_georgia <- max(subset(georgia, georgia$BEV<=0.2 & georgia$BEV >= 0.1)$x)
  time_20_to_80_georgia <- max(subset(georgia, georgia$BEV<=0.8 & georgia$BEV >= 0.2)$x)-min(subset(georgia, georgia$BEV<=0.8 & georgia$BEV >= 0.2)$x)
  time_80_to_20_georgia <- max(subset(georgia, georgia$ICE<=0.8 & georgia$ICE >= 0.2)$x)-min(subset(georgia, georgia$ICE<=0.8 & georgia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_georgia; ice_time[i] <- time_80_to_20_georgia
}

timer_georgia <- data.frame("year"=data_georgia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_georgia$BEV_time == -Inf), which(timer_georgia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_georgia) > (last_inf_index + 12)) {
  timer_georgia_short <- timer_georgia[(last_inf_index + 1 + 12):nrow(timer_georgia), ]
} else {
  timer_georgia_short <- timer_georgia
}
...
plot_timer <- ggplot(timer_georgia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_georgia_short$BEV_time[length(timer_georgia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Georgia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_georgia_short$BEV_time[length(timer_georgia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_georgia <- ggplot(georgia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Georgia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_georgia), " years ", round(12*(time_20_to_80_georgia-floor(time_20_to_80_georgia)),0), " months"),
...
plot_georgia <- plot_georgia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(georgia, georgia$x==2024+counter & georgia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_georgia <- plot_georgia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(georgia, georgia$x==2024+counter & georgia$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_georgia <- plot_georgia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_georgia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_georgia$overall)
BEV <- data.frame(BEV, "overall"=data_georgia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_georgia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_georgia <- ggplot(georgia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Georgia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_georgia), " years ",
      round(12 * (time_80_to_20_georgia - floor(time_80_to_20_georgia)), 0),
      " months"
    ),
...
plot_ICE_BEV_georgia <- plot_ICE_BEV_georgia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(georgia, georgia$x == 2024 + counter - 1 & georgia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_georgia <- plot_ICE_BEV_georgia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(georgia, georgia$x == 2024 + counter & georgia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_georgia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("georgia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_georgia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_georgia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("georgia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_georgia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_georgia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("georgia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_georgia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("georgia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_georgia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_georgia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_georgia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_georgia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_georgia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_georgia, "Inf"),
  bev_20_80 = time_20_to_80_georgia,
  ice_80_20 = time_80_to_20_georgia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_georgia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_georgia)
month_label <- last_label_from_data(data_georgia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_georgia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_georgia)) nz(tail(data_georgia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_georgia)) {
    nz(tail(data_georgia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_georgia)) nz(tail(data_georgia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_georgia)) {
    nz(tail(data_georgia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_georgia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_georgia)
upsert_weight(
