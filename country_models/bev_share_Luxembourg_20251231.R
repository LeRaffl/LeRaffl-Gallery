#### Luxembourg: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Luxembourg"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/luxembourg.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_luxembourg <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Luxembourg")
source <- data_luxembourg$Source[1]
...
#data_luxembourg$BEV <- data_luxembourg$`Electric/zero-emission`
#data_luxembourg$PHEV <- data_luxembourg$Hybrid
#data_luxembourg$TOTAL <- data_luxembourg$overall
#data_luxembourg$OTHERS <- data_luxembourg$`Other fuel`
#data_luxembourg$TOTAL - data_luxembourg$BEV - data_luxembourg$PHEV <- data_luxembourg$Fossil

data_luxembourg$`Electric/zero-emission` <- data_luxembourg$BEV
data_luxembourg$Hybrid <- data_luxembourg$PHEV
data_luxembourg$overall <- data_luxembourg$TOTAL
data_luxembourg$`Other fuel` <- data_luxembourg$OTHERS
data_luxembourg$Fossil <- data_luxembourg$TOTAL - data_luxembourg$BEV - data_luxembourg$PHEV


data_luxembourg$bev_share   <- data_luxembourg$`Electric/zero-emission`/data_luxembourg$overall
data_luxembourg$ice_share   <- data_luxembourg$Fossil/data_luxembourg$overall
data_luxembourg$hybrid_share<- data_luxembourg$Hybrid/data_luxembourg$overall
data_luxembourg$other_share <- data_luxembourg$`Other fuel`/data_luxembourg$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_luxembourg$pure_ICE    <- (data_luxembourg$Fossil)/data_luxembourg$overall

lastyearly <- ceiling(max(subset(data_luxembourg, data_luxembourg$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_luxembourg_monthly <- subset(data_luxembourg, data_luxembourg$time_interval=="monthly")
data_luxembourg_monthly <- subset(data_luxembourg_monthly, data_luxembourg_monthly$year>=min(data_luxembourg_monthly$year+1))
TTM_shares_luxembourg <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_luxembourg_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_luxembourg_monthly)), "%Y-%m"),
  Other = data_luxembourg_monthly$`Other TTM`,
  #`ICE` = data_luxembourg_monthly$`ICE TTM`,#-data_luxembourg_monthly$`HEV TTM`,
  Petrol = data_luxembourg_monthly$`Petrol TTM`,
  Diesel = data_luxembourg_monthly$`Diesel TTM`,
  #Ethanol = data_luxembourg_monthly$`Ethanol TTM`,
  #FlexFuel = data_luxembourg_monthly$`FlexFuel TTM`,
  HEV = data_luxembourg_monthly$`HEV TTM`,
  PHEV = data_luxembourg_monthly$`PHEV TTM`,
  BEV = data_luxembourg_monthly$`BEV TTM`
)

colnames(TTM_shares_luxembourg)[colnames(TTM_shares_luxembourg) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_luxembourg)[colnames(TTM_shares_luxembourg) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_luxembourg)[colnames(TTM_shares_luxembourg) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_luxembourg)[colnames(TTM_shares_luxembourg) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_luxembourg)[colnames(TTM_shares_luxembourg) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_luxembourg_long <- TTM_shares_luxembourg %>%
...
TTM_barplot_luxembourg <- ggplot(TTM_shares_luxembourg_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_luxembourg_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_luxembourg_long$month[substr(TTM_shares_luxembourg_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Luxembourg",
...
print(TTM_barplot_luxembourg)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_luxembourg$year)))
extrapol <- 2200
data_luxembourg <- subset(data_luxembourg, data_luxembourg$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_luxembourg$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_luxembourg$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_luxembourg$`Electric/zero-emission`))
for(i in 1:length(data_luxembourg$`Electric/zero-emission`)){
  data_luxembourg_looped <- data_luxembourg[1:i,]
  xg<-data_luxembourg_looped$year; yg<-as.double(data_luxembourg_looped$bev_share)
  ...
  xg<-data_luxembourg_looped$year; yg<-as.double(data_luxembourg_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  luxembourg <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_luxembourg_looped$overall; new_A$time_interval <- data_luxembourg_looped$time_interval
  
  luxembourg$Hybrid       <- pmax(luxembourg$Hybrid, 0)
  luxembourg$Hybrid_upper <- pmax(luxembourg$Hybrid_upper, 0)
  luxembourg$Hybrid_lower <- pmax(luxembourg$Hybrid_lower, 0)
  
  time_80_luxembourg <- max(subset(luxembourg, luxembourg$BEV<=0.8 & luxembourg$BEV >= 0.2)$x)
  time_50_luxembourg <- max(subset(luxembourg, luxembourg$BEV<=0.5 & luxembourg$BEV >= 0.2)$x)
  time_20_luxembourg <- max(subset(luxembourg, luxembourg$BEV<=0.2 & luxembourg$BEV >= 0.1)$x)
  time_20_to_80_luxembourg <- max(subset(luxembourg, luxembourg$BEV<=0.8 & luxembourg$BEV >= 0.2)$x)-min(subset(luxembourg, luxembourg$BEV<=0.8 & luxembourg$BEV >= 0.2)$x)
  time_80_to_20_luxembourg <- max(subset(luxembourg, luxembourg$ICE<=0.8 & luxembourg$ICE >= 0.2)$x)-min(subset(luxembourg, luxembourg$ICE<=0.8 & luxembourg$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_luxembourg; ice_time[i] <- time_80_to_20_luxembourg
}

timer_luxembourg <- data.frame("year"=data_luxembourg[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_luxembourg$BEV_time == -Inf), which(timer_luxembourg$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_luxembourg) > (last_inf_index + 12)) {
  timer_luxembourg_short <- timer_luxembourg[(last_inf_index + 1 + 12):nrow(timer_luxembourg), ]
} else {
  timer_luxembourg_short <- timer_luxembourg
}
...
plot_timer <- ggplot(timer_luxembourg_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_luxembourg_short$BEV_time[length(timer_luxembourg_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Luxembourg transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_luxembourg_short$BEV_time[length(timer_luxembourg_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_luxembourg <- ggplot(luxembourg, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Luxembourg - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_luxembourg), " years ", round(12*(time_20_to_80_luxembourg-floor(time_20_to_80_luxembourg)),0), " months"),
...
plot_luxembourg <- plot_luxembourg + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(luxembourg, luxembourg$x==2024+counter & luxembourg$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_luxembourg <- plot_luxembourg + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(luxembourg, luxembourg$x==2024+counter & luxembourg$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_luxembourg <- plot_luxembourg + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_luxembourg

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_luxembourg$overall)
BEV <- data.frame(BEV, "overall"=data_luxembourg$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_luxembourg$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_luxembourg <- ggplot(luxembourg, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Luxembourg - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_luxembourg), " years ",
      round(12 * (time_80_to_20_luxembourg - floor(time_80_to_20_luxembourg)), 0),
      " months"
    ),
...
plot_ICE_BEV_luxembourg <- plot_ICE_BEV_luxembourg +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(luxembourg, luxembourg$x == 2024 + counter - 1 & luxembourg$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_luxembourg <- plot_ICE_BEV_luxembourg +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(luxembourg, luxembourg$x == 2024 + counter & luxembourg$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_luxembourg)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("luxembourg_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_luxembourg,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_luxembourg",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("luxembourg_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_luxembourg,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_luxembourg",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("luxembourg_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_luxembourg",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("luxembourg_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_luxembourg,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_luxembourg",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_luxembourg$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_luxembourg, "Inf"),
  bev_50 = as_date_from_numeric(time_50_luxembourg, "Inf"),
  bev_80 = as_date_from_numeric(time_80_luxembourg, "Inf"),
  bev_20_80 = time_20_to_80_luxembourg,
  ice_80_20 = time_80_to_20_luxembourg,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_luxembourg$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_luxembourg)
month_label <- last_label_from_data(data_luxembourg, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_luxembourg$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_luxembourg)) nz(tail(data_luxembourg$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_luxembourg)) {
    nz(tail(data_luxembourg$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_luxembourg)) nz(tail(data_luxembourg$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_luxembourg)) {
    nz(tail(data_luxembourg$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_luxembourg),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_luxembourg)
upsert_weight(
