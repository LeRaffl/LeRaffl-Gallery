#### Denmark (Industry): BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Denmark (Industry)"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/denmark_industry.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_denmark_industry <- read_sheet("https://docs.google.com/spreadsheets/d/1n6QacQ7BIWMa9-vQpbDuuwkquSzYk7XIRbXzjcIsnyg/edit?gid=0#gid=0", sheet = "registrations_industry")
source <- data_denmark_industry$Source[1]
...
#data_denmark_industry$BEV <- data_denmark_industry$`Electric/zero-emission`
#data_denmark_industry$PHEV <- data_denmark_industry$Hybrid
#data_denmark_industry$TOTAL <- data_denmark_industry$overall
#data_denmark_industry$OTHERS <- data_denmark_industry$`Other fuel`
#data_denmark_industry$TOTAL - data_denmark_industry$BEV - data_denmark_industry$PHEV <- data_denmark_industry$Fossil

data_denmark_industry$`Electric/zero-emission` <- data_denmark_industry$BEV
data_denmark_industry$Hybrid <- data_denmark_industry$PHEV
data_denmark_industry$overall <- data_denmark_industry$TOTAL
data_denmark_industry$`Other fuel` <- data_denmark_industry$OTHERS
data_denmark_industry$Fossil <- data_denmark_industry$TOTAL - data_denmark_industry$BEV - data_denmark_industry$PHEV


data_denmark_industry$bev_share   <- data_denmark_industry$`Electric/zero-emission`/data_denmark_industry$overall
data_denmark_industry$ice_share   <- data_denmark_industry$Fossil/data_denmark_industry$overall
data_denmark_industry$hybrid_share<- data_denmark_industry$Hybrid/data_denmark_industry$overall
data_denmark_industry$other_share <- data_denmark_industry$`Other fuel`/data_denmark_industry$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_denmark_industry$pure_ICE    <- (data_denmark_industry$Fossil)/data_denmark_industry$overall

lastyearly <- ceiling(max(subset(data_denmark_industry, data_denmark_industry$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_denmark_industry_monthly <- subset(data_denmark_industry, data_denmark_industry$time_interval=="monthly")
data_denmark_industry_monthly <- subset(data_denmark_industry_monthly, data_denmark_industry_monthly$year>=min(data_denmark_industry_monthly$year+1))
TTM_shares_denmark_industry <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_denmark_industry_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_denmark_industry_monthly)), "%Y-%m"),
  Other = data_denmark_industry_monthly$`Other TTM`,
  #`ICE` = data_denmark_industry_monthly$`ICE TTM`,#-data_denmark_industry_monthly$`HEV TTM`,
  Petrol = data_denmark_industry_monthly$`Petrol TTM`,
  Diesel = data_denmark_industry_monthly$`Diesel TTM`,
  #Ethanol = data_denmark_industry_monthly$`Ethanol TTM`,
  #FlexFuel = data_denmark_industry_monthly$`FlexFuel TTM`,
  #HEV = data_denmark_industry_monthly$`HEV TTM`,
  PHEV = data_denmark_industry_monthly$`PHEV TTM`,
  BEV = data_denmark_industry_monthly$`BEV TTM`
)

colnames(TTM_shares_denmark_industry)[colnames(TTM_shares_denmark_industry) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_denmark_industry)[colnames(TTM_shares_denmark_industry) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_denmark_industry)[colnames(TTM_shares_denmark_industry) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_denmark_industry)[colnames(TTM_shares_denmark_industry) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_denmark_industry)[colnames(TTM_shares_denmark_industry) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_denmark_industry_long <- TTM_shares_denmark_industry %>%
...
TTM_barplot_denmark_industry <- ggplot(TTM_shares_denmark_industry_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_denmark_industry_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_denmark_industry_long$month[substr(TTM_shares_denmark_industry_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Denmark (Industry)",
...
print(TTM_barplot_denmark_industry)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_denmark_industry$year)))
extrapol <- 2200
data_denmark_industry <- subset(data_denmark_industry, data_denmark_industry$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_denmark_industry$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_denmark_industry$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_denmark_industry$`Electric/zero-emission`))
for(i in 1:length(data_denmark_industry$`Electric/zero-emission`)){
  data_denmark_industry_looped <- data_denmark_industry[1:i,]
  xg<-data_denmark_industry_looped$year; yg<-as.double(data_denmark_industry_looped$bev_share)
  ...
  xg<-data_denmark_industry_looped$year; yg<-as.double(data_denmark_industry_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  denmark_industry <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_denmark_industry_looped$overall; new_A$time_interval <- data_denmark_industry_looped$time_interval
  
  denmark_industry$Hybrid       <- pmax(denmark_industry$Hybrid, 0)
  denmark_industry$Hybrid_upper <- pmax(denmark_industry$Hybrid_upper, 0)
  denmark_industry$Hybrid_lower <- pmax(denmark_industry$Hybrid_lower, 0)
  
  time_80_denmark_industry <- max(subset(denmark_industry, denmark_industry$BEV<=0.8 & denmark_industry$BEV >= 0.2)$x)
  time_50_denmark_industry <- max(subset(denmark_industry, denmark_industry$BEV<=0.5 & denmark_industry$BEV >= 0.2)$x)
  time_20_denmark_industry <- max(subset(denmark_industry, denmark_industry$BEV<=0.2 & denmark_industry$BEV >= 0.1)$x)
  time_20_to_80_denmark_industry <- max(subset(denmark_industry, denmark_industry$BEV<=0.8 & denmark_industry$BEV >= 0.2)$x)-min(subset(denmark_industry, denmark_industry$BEV<=0.8 & denmark_industry$BEV >= 0.2)$x)
  time_80_to_20_denmark_industry <- max(subset(denmark_industry, denmark_industry$ICE<=0.8 & denmark_industry$ICE >= 0.2)$x)-min(subset(denmark_industry, denmark_industry$ICE<=0.8 & denmark_industry$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_denmark_industry; ice_time[i] <- time_80_to_20_denmark_industry
}

timer_denmark_industry <- data.frame("year"=data_denmark_industry[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_denmark_industry$BEV_time == -Inf), which(timer_denmark_industry$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_denmark_industry) > (last_inf_index + 12)) {
  timer_denmark_industry_short <- timer_denmark_industry[(last_inf_index + 1 + 12):nrow(timer_denmark_industry), ]
} else {
  timer_denmark_industry_short <- timer_denmark_industry
}
...
plot_timer <- ggplot(timer_denmark_industry_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_denmark_industry_short$BEV_time[length(timer_denmark_industry_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Denmark (Industry) transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_denmark_industry_short$BEV_time[length(timer_denmark_industry_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_denmark_industry <- ggplot(denmark_industry, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Denmark (Industry) - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_denmark_industry), " years ", round(12*(time_20_to_80_denmark_industry-floor(time_20_to_80_denmark_industry)),0), " months"),
...
plot_denmark_industry <- plot_denmark_industry + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(denmark_industry, denmark_industry$x==2024+counter & denmark_industry$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_denmark_industry <- plot_denmark_industry + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(denmark_industry, denmark_industry$x==2024+counter & denmark_industry$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_denmark_industry <- plot_denmark_industry + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_denmark_industry

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_denmark_industry$overall)
BEV <- data.frame(BEV, "overall"=data_denmark_industry$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_denmark_industry$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_denmark_industry <- ggplot(denmark_industry, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Denmark (Industry) - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_denmark_industry), " years ",
      round(12 * (time_80_to_20_denmark_industry - floor(time_80_to_20_denmark_industry)), 0),
      " months"
    ),
...
plot_ICE_BEV_denmark_industry <- plot_ICE_BEV_denmark_industry +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(denmark_industry, denmark_industry$x == 2024 + counter - 1 & denmark_industry$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_denmark_industry <- plot_ICE_BEV_denmark_industry +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(denmark_industry, denmark_industry$x == 2024 + counter & denmark_industry$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_denmark_industry)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("denmark_industry_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_denmark_industry,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark_industry",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("denmark_industry_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_denmark_industry,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark_industry",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("denmark_industry_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark_industry",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("denmark_industry_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_denmark_industry,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark_industry",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_denmark_industry$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_denmark_industry, "Inf"),
  bev_50 = as_date_from_numeric(time_50_denmark_industry, "Inf"),
  bev_80 = as_date_from_numeric(time_80_denmark_industry, "Inf"),
  bev_20_80 = time_20_to_80_denmark_industry,
  ice_80_20 = time_80_to_20_denmark_industry,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_denmark_industry$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_denmark_industry)
month_label <- last_label_from_data(data_denmark_industry, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_denmark_industry$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_denmark_industry)) nz(tail(data_denmark_industry$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_denmark_industry)) {
    nz(tail(data_denmark_industry$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_denmark_industry)) nz(tail(data_denmark_industry$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_denmark_industry)) {
    nz(tail(data_denmark_industry$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_denmark_industry),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_denmark_industry)
upsert_weight(
