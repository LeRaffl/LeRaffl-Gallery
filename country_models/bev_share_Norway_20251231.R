#### Norway: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Norway"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/norway.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_norway <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Norway")
source <- data_norway$Source[1]
...
#data_norway$BEV <- data_norway$`Electric/zero-emission`
#data_norway$PHEV <- data_norway$Hybrid
#data_norway$TOTAL <- data_norway$overall
#data_norway$OTHERS <- data_norway$`Other fuel`
#data_norway$TOTAL - data_norway$BEV - data_norway$PHEV <- data_norway$Fossil

data_norway$`Electric/zero-emission` <- data_norway$BEV
data_norway$Hybrid <- data_norway$PHEV
data_norway$overall <- data_norway$TOTAL
data_norway$`Other fuel` <- data_norway$OTHERS
data_norway$Fossil <- data_norway$TOTAL - data_norway$BEV - data_norway$PHEV


data_norway$bev_share   <- data_norway$`Electric/zero-emission`/data_norway$overall
data_norway$ice_share   <- data_norway$Fossil/data_norway$overall
data_norway$hybrid_share<- data_norway$Hybrid/data_norway$overall
data_norway$other_share <- data_norway$`Other fuel`/data_norway$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_norway$pure_ICE    <- (data_norway$Fossil)/data_norway$overall

lastyearly <- ceiling(max(subset(data_norway, data_norway$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_norway_monthly <- subset(data_norway, data_norway$time_interval=="monthly")
data_norway_monthly <- subset(data_norway_monthly, data_norway_monthly$year>=min(data_norway_monthly$year+1))
TTM_shares_norway <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_norway_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_norway_monthly)), "%Y-%m"),
  Other = data_norway_monthly$`Other TTM`,
  #`ICE` = data_norway_monthly$`ICE TTM`,#-data_norway_monthly$`HEV TTM`,
  Petrol = data_norway_monthly$`Petrol TTM`,
  Diesel = data_norway_monthly$`Diesel TTM`,
  #Ethanol = data_norway_monthly$`Ethanol TTM`,
  #FlexFuel = data_norway_monthly$`FlexFuel TTM`,
  HEV = data_norway_monthly$`HEV TTM`,
  PHEV = data_norway_monthly$`PHEV TTM`,
  BEV = data_norway_monthly$`BEV TTM`
)

colnames(TTM_shares_norway)[colnames(TTM_shares_norway) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_norway)[colnames(TTM_shares_norway) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_norway)[colnames(TTM_shares_norway) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_norway)[colnames(TTM_shares_norway) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_norway)[colnames(TTM_shares_norway) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_norway_long <- TTM_shares_norway %>%
...
TTM_barplot_norway <- ggplot(TTM_shares_norway_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_norway_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_norway_long$month[substr(TTM_shares_norway_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Norway",
...
print(TTM_barplot_norway)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_norway$year)))
extrapol <- 2200
data_norway <- subset(data_norway, data_norway$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_norway$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_norway$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_norway$`Electric/zero-emission`))
for(i in 1:length(data_norway$`Electric/zero-emission`)){
  data_norway_looped <- data_norway[1:i,]
  xg<-data_norway_looped$year; yg<-as.double(data_norway_looped$bev_share)
  ...
  xg<-data_norway_looped$year; yg<-as.double(data_norway_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  norway <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_norway_looped$overall; new_A$time_interval <- data_norway_looped$time_interval
  
  norway$Hybrid       <- pmax(norway$Hybrid, 0)
  norway$Hybrid_upper <- pmax(norway$Hybrid_upper, 0)
  norway$Hybrid_lower <- pmax(norway$Hybrid_lower, 0)
  
  time_80_norway <- max(subset(norway, norway$BEV<=0.8 & norway$BEV >= 0.2)$x)
  time_50_norway <- max(subset(norway, norway$BEV<=0.5 & norway$BEV >= 0.2)$x)
  time_20_norway <- max(subset(norway, norway$BEV<=0.2 & norway$BEV >= 0.1)$x)
  time_20_to_80_norway <- max(subset(norway, norway$BEV<=0.8 & norway$BEV >= 0.2)$x)-min(subset(norway, norway$BEV<=0.8 & norway$BEV >= 0.2)$x)
  time_80_to_20_norway <- max(subset(norway, norway$ICE<=0.8 & norway$ICE >= 0.2)$x)-min(subset(norway, norway$ICE<=0.8 & norway$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_norway; ice_time[i] <- time_80_to_20_norway
}

timer_norway <- data.frame("year"=data_norway[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_norway$BEV_time == -Inf), which(timer_norway$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_norway) > (last_inf_index + 12)) {
  timer_norway_short <- timer_norway[(last_inf_index + 1 + 12):nrow(timer_norway), ]
} else {
  timer_norway_short <- timer_norway
}
...
plot_timer <- ggplot(timer_norway_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_norway_short$BEV_time[length(timer_norway_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Norway transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_norway_short$BEV_time[length(timer_norway_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_norway <- ggplot(norway, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Norway - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_norway), " years ", round(12*(time_20_to_80_norway-floor(time_20_to_80_norway)),0), " months"),
...
plot_norway <- plot_norway + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(norway, norway$x==2024+counter & norway$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_norway <- plot_norway + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                      label=paste0("Jan ",2025+counter,": ",round(subset(norway, norway$x==2024+counter & norway$Type=="New Registrations")$BEV*100, 1),"%"),
                                      size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_norway <- plot_norway + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                               width = unit(1*1920/1280
                                                                            ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_norway

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_norway$overall)
BEV <- data.frame(BEV, "overall"=data_norway$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_norway$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_norway <- ggplot(norway, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Norway - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_norway), " years ",
      round(12 * (time_80_to_20_norway - floor(time_80_to_20_norway)), 0),
      " months"
    ),
...
plot_ICE_BEV_norway <- plot_ICE_BEV_norway +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(norway, norway$x == 2024 + counter - 1 & norway$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_norway <- plot_ICE_BEV_norway +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(norway, norway$x == 2024 + counter & norway$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_norway)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("norway_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_norway,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_norway",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("norway_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_norway,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_norway",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("norway_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_norway",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("norway_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_norway,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_norway",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_norway$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_norway, "Inf"),
  bev_50 = as_date_from_numeric(time_50_norway, "Inf"),
  bev_80 = as_date_from_numeric(time_80_norway, "Inf"),
  bev_20_80 = time_20_to_80_norway,
  ice_80_20 = time_80_to_20_norway,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_norway$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_norway)
month_label <- last_label_from_data(data_norway, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_norway$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_norway)) nz(tail(data_norway$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_norway)) {
    nz(tail(data_norway$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_norway)) nz(tail(data_norway$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_norway)) {
    nz(tail(data_norway$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_norway),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_norway)
upsert_weight(
