#### Spain: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Spain"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/spain.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_spain <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Spain")
source <- data_spain$Source[1]
...
#data_spain$BEV <- data_spain$`Electric/zero-emission`
#data_spain$PHEV <- data_spain$Hybrid
#data_spain$TOTAL <- data_spain$overall
#data_spain$OTHERS <- data_spain$`Other fuel`
#data_spain$TOTAL - data_spain$BEV - data_spain$PHEV <- data_spain$Fossil

data_spain$`Electric/zero-emission` <- data_spain$BEV
data_spain$Hybrid <- data_spain$PHEV
data_spain$overall <- data_spain$TOTAL
data_spain$`Other fuel` <- data_spain$OTHERS
data_spain$Fossil <- data_spain$TOTAL - data_spain$BEV - data_spain$PHEV


data_spain$bev_share   <- data_spain$`Electric/zero-emission`/data_spain$overall
data_spain$ice_share   <- data_spain$Fossil/data_spain$overall
data_spain$hybrid_share<- data_spain$Hybrid/data_spain$overall
data_spain$other_share <- data_spain$`Other fuel`/data_spain$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_spain$pure_ICE    <- (data_spain$Fossil)/data_spain$overall

lastyearly <- ceiling(max(subset(data_spain, data_spain$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_spain_monthly <- subset(data_spain, data_spain$time_interval=="monthly")
data_spain_monthly <- subset(data_spain_monthly, data_spain_monthly$year>=min(data_spain_monthly$year+1))
TTM_shares_spain <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_spain_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_spain_monthly)), "%Y-%m"),
  Other = data_spain_monthly$`Other TTM`,
  #`ICE` = data_spain_monthly$`ICE TTM`,#-data_spain_monthly$`HEV TTM`,
  Petrol = data_spain_monthly$`Petrol TTM`,
  Diesel = data_spain_monthly$`Diesel TTM`,
  #Ethanol = data_spain_monthly$`Ethanol TTM`,
  #FlexFuel = data_spain_monthly$`FlexFuel TTM`,
  HEV = data_spain_monthly$`HEV TTM`,
  PHEV = data_spain_monthly$`PHEV TTM`,
  BEV = data_spain_monthly$`BEV TTM`
)

colnames(TTM_shares_spain)[colnames(TTM_shares_spain) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_spain)[colnames(TTM_shares_spain) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_spain)[colnames(TTM_shares_spain) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_spain)[colnames(TTM_shares_spain) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_spain)[colnames(TTM_shares_spain) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_spain_long <- TTM_shares_spain %>%
...
TTM_barplot_spain <- ggplot(TTM_shares_spain_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_spain_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_spain_long$month[substr(TTM_shares_spain_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Spain",
...
print(TTM_barplot_spain)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_spain$year)))
extrapol <- 2200
data_spain <- subset(data_spain, data_spain$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_spain$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_spain$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_spain$`Electric/zero-emission`))
for(i in 1:length(data_spain$`Electric/zero-emission`)){
  data_spain_looped <- data_spain[1:i,]
  xg<-data_spain_looped$year; yg<-as.double(data_spain_looped$bev_share)
  ...
  xg<-data_spain_looped$year; yg<-as.double(data_spain_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  spain <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_spain_looped$overall; new_A$time_interval <- data_spain_looped$time_interval
  
  spain$Hybrid       <- pmax(spain$Hybrid, 0)
  spain$Hybrid_upper <- pmax(spain$Hybrid_upper, 0)
  spain$Hybrid_lower <- pmax(spain$Hybrid_lower, 0)
  
  time_80_spain <- max(subset(spain, spain$BEV<=0.8 & spain$BEV >= 0.2)$x)
  time_50_spain <- max(subset(spain, spain$BEV<=0.5 & spain$BEV >= 0.2)$x)
  time_20_spain <- max(subset(spain, spain$BEV<=0.2 & spain$BEV >= 0.1)$x)
  time_20_to_80_spain <- max(subset(spain, spain$BEV<=0.8 & spain$BEV >= 0.2)$x)-min(subset(spain, spain$BEV<=0.8 & spain$BEV >= 0.2)$x)
  time_80_to_20_spain <- max(subset(spain, spain$ICE<=0.8 & spain$ICE >= 0.2)$x)-min(subset(spain, spain$ICE<=0.8 & spain$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_spain; ice_time[i] <- time_80_to_20_spain
}

timer_spain <- data.frame("year"=data_spain[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_spain$BEV_time == -Inf), which(timer_spain$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_spain) > (last_inf_index + 12)) {
  timer_spain_short <- timer_spain[(last_inf_index + 1 + 12):nrow(timer_spain), ]
} else {
  timer_spain_short <- timer_spain
}
...
plot_timer <- ggplot(timer_spain_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_spain_short$BEV_time[length(timer_spain_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Spain transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_spain_short$BEV_time[length(timer_spain_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_spain <- ggplot(spain, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Spain - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_spain), " years ", round(12*(time_20_to_80_spain-floor(time_20_to_80_spain)),0), " months"),
...
plot_spain <- plot_spain + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(spain, spain$x==2024+counter & spain$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_spain <- plot_spain + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(spain, spain$x==2024+counter & spain$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_spain <- plot_spain + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_spain

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_spain$overall)
BEV <- data.frame(BEV, "overall"=data_spain$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_spain$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_spain <- ggplot(spain, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Spain - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_spain), " years ",
      round(12 * (time_80_to_20_spain - floor(time_80_to_20_spain)), 0),
      " months"
    ),
...
plot_ICE_BEV_spain <- plot_ICE_BEV_spain +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(spain, spain$x == 2024 + counter - 1 & spain$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_spain <- plot_ICE_BEV_spain +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(spain, spain$x == 2024 + counter & spain$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_spain)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("spain_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_spain,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_spain",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("spain_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_spain,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_spain",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("spain_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_spain",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("spain_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_spain,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_spain",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_spain$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_spain, "Inf"),
  bev_50 = as_date_from_numeric(time_50_spain, "Inf"),
  bev_80 = as_date_from_numeric(time_80_spain, "Inf"),
  bev_20_80 = time_20_to_80_spain,
  ice_80_20 = time_80_to_20_spain,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_spain$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_spain)
month_label <- last_label_from_data(data_spain, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_spain$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_spain)) nz(tail(data_spain$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_spain)) {
    nz(tail(data_spain$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_spain)) nz(tail(data_spain$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_spain)) {
    nz(tail(data_spain$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_spain),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_spain)
upsert_weight(
