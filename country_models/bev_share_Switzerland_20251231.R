#### Switzerland: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Switzerland"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/switzerland.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_switzerland <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Switzerland")
source <- data_switzerland$Source[1]
...
#data_switzerland$BEV <- data_switzerland$`Electric/zero-emission`
#data_switzerland$PHEV <- data_switzerland$Hybrid
#data_switzerland$TOTAL <- data_switzerland$overall
#data_switzerland$OTHERS <- data_switzerland$`Other fuel`
#data_switzerland$TOTAL - data_switzerland$BEV - data_switzerland$PHEV <- data_switzerland$Fossil

data_switzerland$`Electric/zero-emission` <- data_switzerland$BEV
data_switzerland$Hybrid <- data_switzerland$PHEV
data_switzerland$overall <- data_switzerland$TOTAL
data_switzerland$`Other fuel` <- data_switzerland$OTHERS
data_switzerland$Fossil <- data_switzerland$TOTAL - data_switzerland$BEV - data_switzerland$PHEV


data_switzerland$bev_share   <- data_switzerland$`Electric/zero-emission`/data_switzerland$overall
data_switzerland$ice_share   <- data_switzerland$Fossil/data_switzerland$overall
data_switzerland$hybrid_share<- data_switzerland$Hybrid/data_switzerland$overall
data_switzerland$other_share <- data_switzerland$`Other fuel`/data_switzerland$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_switzerland$pure_ICE    <- (data_switzerland$Fossil)/data_switzerland$overall

lastyearly <- ceiling(max(subset(data_switzerland, data_switzerland$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_switzerland_monthly <- subset(data_switzerland, data_switzerland$time_interval=="monthly")
data_switzerland_monthly <- subset(data_switzerland_monthly, data_switzerland_monthly$year>=min(data_switzerland_monthly$year+1))
TTM_shares_switzerland <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_switzerland_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_switzerland_monthly)), "%Y-%m"),
  Other = data_switzerland_monthly$`Other TTM`,
  #`ICE` = data_switzerland_monthly$`ICE TTM`,#-data_switzerland_monthly$`HEV TTM`,
  Petrol = data_switzerland_monthly$`Petrol TTM`,
  Diesel = data_switzerland_monthly$`Diesel TTM`,
  #Ethanol = data_switzerland_monthly$`Ethanol TTM`,
  #FlexFuel = data_switzerland_monthly$`FlexFuel TTM`,
  HEV = data_switzerland_monthly$`HEV TTM`,
  PHEV = data_switzerland_monthly$`PHEV TTM`,
  BEV = data_switzerland_monthly$`BEV TTM`
)

colnames(TTM_shares_switzerland)[colnames(TTM_shares_switzerland) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_switzerland)[colnames(TTM_shares_switzerland) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_switzerland)[colnames(TTM_shares_switzerland) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_switzerland)[colnames(TTM_shares_switzerland) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_switzerland)[colnames(TTM_shares_switzerland) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_switzerland_long <- TTM_shares_switzerland %>%
...
TTM_barplot_switzerland <- ggplot(TTM_shares_switzerland_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_switzerland_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_switzerland_long$month[substr(TTM_shares_switzerland_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Switzerland",
...
print(TTM_barplot_switzerland)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_switzerland$year)))
extrapol <- 2200
data_switzerland <- subset(data_switzerland, data_switzerland$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_switzerland$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_switzerland$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_switzerland$`Electric/zero-emission`))
for(i in 1:length(data_switzerland$`Electric/zero-emission`)){
  data_switzerland_looped <- data_switzerland[1:i,]
  xg<-data_switzerland_looped$year; yg<-as.double(data_switzerland_looped$bev_share)
  ...
  xg<-data_switzerland_looped$year; yg<-as.double(data_switzerland_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  switzerland <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_switzerland_looped$overall; new_A$time_interval <- data_switzerland_looped$time_interval
  
  switzerland$Hybrid       <- pmax(switzerland$Hybrid, 0)
  switzerland$Hybrid_upper <- pmax(switzerland$Hybrid_upper, 0)
  switzerland$Hybrid_lower <- pmax(switzerland$Hybrid_lower, 0)
  
  time_80_switzerland <- max(subset(switzerland, switzerland$BEV<=0.8 & switzerland$BEV >= 0.2)$x)
  time_50_switzerland <- max(subset(switzerland, switzerland$BEV<=0.5 & switzerland$BEV >= 0.2)$x)
  time_20_switzerland <- max(subset(switzerland, switzerland$BEV<=0.2 & switzerland$BEV >= 0.1)$x)
  time_20_to_80_switzerland <- max(subset(switzerland, switzerland$BEV<=0.8 & switzerland$BEV >= 0.2)$x)-min(subset(switzerland, switzerland$BEV<=0.8 & switzerland$BEV >= 0.2)$x)
  time_80_to_20_switzerland <- max(subset(switzerland, switzerland$ICE<=0.8 & switzerland$ICE >= 0.2)$x)-min(subset(switzerland, switzerland$ICE<=0.8 & switzerland$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_switzerland; ice_time[i] <- time_80_to_20_switzerland
}

timer_switzerland <- data.frame("year"=data_switzerland[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_switzerland$BEV_time == -Inf), which(timer_switzerland$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_switzerland) > (last_inf_index + 12)) {
  timer_switzerland_short <- timer_switzerland[(last_inf_index + 1 + 12):nrow(timer_switzerland), ]
} else {
  timer_switzerland_short <- timer_switzerland
}
...
plot_timer <- ggplot(timer_switzerland_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_switzerland_short$BEV_time[length(timer_switzerland_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Switzerland transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_switzerland_short$BEV_time[length(timer_switzerland_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_switzerland <- ggplot(switzerland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Switzerland - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_switzerland), " years ", round(12*(time_20_to_80_switzerland-floor(time_20_to_80_switzerland)),0), " months"),
...
plot_switzerland <- plot_switzerland + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(switzerland, switzerland$x==2024+counter & switzerland$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_switzerland <- plot_switzerland + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                      label=paste0("Jan ",2025+counter,": ",round(subset(switzerland, switzerland$x==2024+counter & switzerland$Type=="New Registrations")$BEV*100, 1),"%"),
                                      size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_switzerland <- plot_switzerland + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                               width = unit(1#*1920/1280
                                                                            ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_switzerland

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_switzerland$overall)
BEV <- data.frame(BEV, "overall"=data_switzerland$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_switzerland$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_switzerland <- ggplot(switzerland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Switzerland - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_switzerland), " years ",
      round(12 * (time_80_to_20_switzerland - floor(time_80_to_20_switzerland)), 0),
      " months"
    ),
...
plot_ICE_BEV_switzerland <- plot_ICE_BEV_switzerland +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(switzerland, switzerland$x == 2024 + counter - 1 & switzerland$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_switzerland <- plot_ICE_BEV_switzerland +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(switzerland, switzerland$x == 2024 + counter & switzerland$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_switzerland)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("switzerland_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_switzerland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_switzerland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("switzerland_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_switzerland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_switzerland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("switzerland_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_switzerland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("switzerland_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_switzerland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_switzerland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_switzerland$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_switzerland, "Inf"),
  bev_50 = as_date_from_numeric(time_50_switzerland, "Inf"),
  bev_80 = as_date_from_numeric(time_80_switzerland, "Inf"),
  bev_20_80 = time_20_to_80_switzerland,
  ice_80_20 = time_80_to_20_switzerland,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_switzerland$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_switzerland)
month_label <- last_label_from_data(data_switzerland, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_switzerland$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_switzerland)) nz(tail(data_switzerland$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_switzerland)) {
    nz(tail(data_switzerland$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_switzerland)) nz(tail(data_switzerland$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_switzerland)) {
    nz(tail(data_switzerland$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_switzerland),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_switzerland)
upsert_weight(
