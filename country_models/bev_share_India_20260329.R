#### India: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "India"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/india.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_india <- read_sheet("https://docs.google.com/spreadsheets/d/107oxsVahAv0-Nn1TzsP3SK9ETwVj3k-XfjXvkXrf_Aw/edit?gid=1880107920#gid=1880107920", sheet = "india_4")
source <- data_india$Source[1]
...
#data_india$BEV <- data_india$`Electric/zero-emission`
#data_india$PHEV <- data_india$Hybrid
#data_india$TOTAL <- data_india$overall
#data_india$OTHERS <- data_india$`Other fuel`
#data_india$TOTAL - data_india$BEV - data_india$PHEV <- data_india$Fossil

data_india$`Electric/zero-emission` <- data_india$BEV
data_india$Hybrid <- data_india$PHEV
data_india$overall <- data_india$TOTAL
data_india$`Other fuel` <- data_india$OTHERS
data_india$Fossil <- data_india$TOTAL - data_india$BEV - data_india$PHEV


data_india$bev_share   <- data_india$`Electric/zero-emission`/data_india$overall
data_india$ice_share   <- data_india$Fossil/data_india$overall
data_india$hybrid_share<- data_india$Hybrid/data_india$overall
data_india$other_share <- data_india$`Other fuel`/data_india$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_india$pure_ICE    <- (data_india$Fossil)/data_india$overall

lastyearly <- ceiling(max(subset(data_india, data_india$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_india_monthly <- subset(data_india, data_india$time_interval=="monthly")
data_india_monthly <- subset(data_india_monthly, data_india_monthly$year>=min(data_india_monthly$year+1))
TTM_shares_india <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_india_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_india_monthly)), "%Y-%m"),
  Other = data_india_monthly$`Other TTM`,
  `ICE` = data_india_monthly$`ICE TTM`,#-data_india_monthly$`HEV TTM`,
  Petrol = data_india_monthly$`Petrol TTM`,
  Diesel = data_india_monthly$`Diesel TTM`,
  #Ethanol = data_india_monthly$`Ethanol TTM`,
  #FlexFuel = data_india_monthly$`FlexFuel TTM`,
  HEV = data_india_monthly$`HEV TTM`,
  PHEV = data_india_monthly$`PHEV TTM`,
  BEV = data_india_monthly$`BEV TTM`
)

colnames(TTM_shares_india)[colnames(TTM_shares_india) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_india)[colnames(TTM_shares_india) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_india)[colnames(TTM_shares_india) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_india)[colnames(TTM_shares_india) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_india)[colnames(TTM_shares_india) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_india_long <- TTM_shares_india %>%
...
TTM_barplot_india <- ggplot(TTM_shares_india_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_india_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_india_long$month[substr(TTM_shares_india_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in India",
...
print(TTM_barplot_india)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_india$year)))
extrapol <- 2200
data_india <- subset(data_india, data_india$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_india$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_india$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_india$`Electric/zero-emission`))
for(i in 1:length(data_india$`Electric/zero-emission`)){
  data_india_looped <- data_india[1:i,]
  xg<-data_india_looped$year; yg<-as.double(data_india_looped$bev_share)
  ...
  xg<-data_india_looped$year; yg<-as.double(data_india_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  india <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_india_looped$overall; new_A$time_interval <- data_india_looped$time_interval
  
  india$Hybrid       <- pmax(india$Hybrid, 0)
  india$Hybrid_upper <- pmax(india$Hybrid_upper, 0)
  india$Hybrid_lower <- pmax(india$Hybrid_lower, 0)
  
  time_80_india <- max(subset(india, india$BEV<=0.8 & india$BEV >= 0.2)$x)
  time_50_india <- max(subset(india, india$BEV<=0.5 & india$BEV >= 0.2)$x)
  time_20_india <- max(subset(india, india$BEV<=0.2 & india$BEV >= 0.1)$x)
  time_20_to_80_india <- max(subset(india, india$BEV<=0.8 & india$BEV >= 0.2)$x)-min(subset(india, india$BEV<=0.8 & india$BEV >= 0.2)$x)
  time_80_to_20_india <- max(subset(india, india$ICE<=0.8 & india$ICE >= 0.2)$x)-min(subset(india, india$ICE<=0.8 & india$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_india; ice_time[i] <- time_80_to_20_india
}

timer_india <- data.frame("year"=data_india[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_india$BEV_time == -Inf), which(timer_india$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_india) > (last_inf_index + 12)) {
  timer_india_short <- timer_india[(last_inf_index + 1 + 12):nrow(timer_india), ]
} else {
  timer_india_short <- timer_india
}
...
plot_timer <- ggplot(timer_india_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_india_short$BEV_time[length(timer_india_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for India transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_india_short$BEV_time[length(timer_india_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_india <- ggplot(india, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in India - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_india), " years ", round(12*(time_20_to_80_india-floor(time_20_to_80_india)),0), " months"),
...
plot_india <- plot_india + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(india, india$x==2024+counter & india$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_india <- plot_india + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(india, india$x==2024+counter & india$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_india <- plot_india + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_india

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_india$overall)
BEV <- data.frame(BEV, "overall"=data_india$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_india$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_india <- ggplot(india, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in India - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_india), " years ",
      round(12 * (time_80_to_20_india - floor(time_80_to_20_india)), 0),
      " months"
    ),
...
plot_ICE_BEV_india <- plot_ICE_BEV_india +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(india, india$x == 2024 + counter - 1 & india$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_india <- plot_ICE_BEV_india +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(india, india$x == 2024 + counter & india$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_india)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("india_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_india,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("india_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_india,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("india_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("india_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_india,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_india",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_india$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_india, "Inf"),
  bev_50 = as_date_from_numeric(time_50_india, "Inf"),
  bev_80 = as_date_from_numeric(time_80_india, "Inf"),
  bev_20_80 = time_20_to_80_india,
  ice_80_20 = time_80_to_20_india,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_india$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_india)
month_label <- last_label_from_data(data_india, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_india$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_india)) nz(tail(data_india$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_india)) {
    nz(tail(data_india$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_india)) nz(tail(data_india$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_india)) {
    nz(tail(data_india$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_india),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_india)
upsert_weight(
