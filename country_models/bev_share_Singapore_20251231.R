#### Singapore: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Singapore"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/singapore.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_singapore <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Singapore")
source <- data_singapore$Source[1]
...
#data_singapore$BEV <- data_singapore$`Electric/zero-emission`
#data_singapore$PHEV <- data_singapore$Hybrid
#data_singapore$TOTAL <- data_singapore$overall
#data_singapore$OTHERS <- data_singapore$`Other fuel`
#data_singapore$TOTAL - data_singapore$BEV - data_singapore$PHEV <- data_singapore$Fossil

data_singapore$`Electric/zero-emission` <- data_singapore$BEV
data_singapore$Hybrid <- data_singapore$PHEV
data_singapore$overall <- data_singapore$TOTAL
data_singapore$`Other fuel` <- data_singapore$OTHERS
data_singapore$Fossil <- data_singapore$TOTAL - data_singapore$BEV - data_singapore$PHEV


data_singapore$bev_share   <- data_singapore$`Electric/zero-emission`/data_singapore$overall
data_singapore$ice_share   <- data_singapore$Fossil/data_singapore$overall
data_singapore$hybrid_share<- data_singapore$Hybrid/data_singapore$overall
data_singapore$other_share <- data_singapore$`Other fuel`/data_singapore$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_singapore$pure_ICE    <- (data_singapore$Fossil)/data_singapore$overall

lastyearly <- ceiling(max(subset(data_singapore, data_singapore$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_singapore_monthly <- subset(data_singapore, data_singapore$time_interval=="monthly")
data_singapore_monthly <- subset(data_singapore_monthly, data_singapore_monthly$year>=min(data_singapore_monthly$year+1))
TTM_shares_singapore <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_singapore_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_singapore_monthly)), "%Y-%m"),
  Other = data_singapore_monthly$`Other TTM`,
  #`ICE` = data_singapore_monthly$`ICE TTM`,#-data_singapore_monthly$`HEV TTM`,
  Petrol = data_singapore_monthly$`Petrol TTM`,
  Diesel = data_singapore_monthly$`Diesel TTM`,
  #Ethanol = data_singapore_monthly$`Ethanol TTM`,
  #FlexFuel = data_singapore_monthly$`FlexFuel TTM`,
  HEV = data_singapore_monthly$`HEV TTM`,
  PHEV = data_singapore_monthly$`PHEV TTM`,
  BEV = data_singapore_monthly$`BEV TTM`
)

colnames(TTM_shares_singapore)[colnames(TTM_shares_singapore) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_singapore)[colnames(TTM_shares_singapore) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_singapore)[colnames(TTM_shares_singapore) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_singapore)[colnames(TTM_shares_singapore) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_singapore)[colnames(TTM_shares_singapore) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_singapore_long <- TTM_shares_singapore %>%
...
TTM_barplot_singapore <- ggplot(TTM_shares_singapore_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_singapore_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_singapore_long$month[substr(TTM_shares_singapore_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Singapore",
...
print(TTM_barplot_singapore)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_singapore$year)))
extrapol <- 2200
data_singapore <- subset(data_singapore, data_singapore$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_singapore$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_singapore$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_singapore$`Electric/zero-emission`))
for(i in 1:length(data_singapore$`Electric/zero-emission`)){
  data_singapore_looped <- data_singapore[1:i,]
  xg<-data_singapore_looped$year; yg<-as.double(data_singapore_looped$bev_share)
  ...
  xg<-data_singapore_looped$year; yg<-as.double(data_singapore_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  singapore <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_singapore_looped$overall; new_A$time_interval <- data_singapore_looped$time_interval
  
  singapore$Hybrid       <- pmax(singapore$Hybrid, 0)
  singapore$Hybrid_upper <- pmax(singapore$Hybrid_upper, 0)
  singapore$Hybrid_lower <- pmax(singapore$Hybrid_lower, 0)
  
  time_80_singapore <- max(subset(singapore, singapore$BEV<=0.8 & singapore$BEV >= 0.2)$x)
  time_50_singapore <- max(subset(singapore, singapore$BEV<=0.5 & singapore$BEV >= 0.2)$x)
  time_20_singapore <- max(subset(singapore, singapore$BEV<=0.2 & singapore$BEV >= 0.1)$x)
  time_20_to_80_singapore <- max(subset(singapore, singapore$BEV<=0.8 & singapore$BEV >= 0.2)$x)-min(subset(singapore, singapore$BEV<=0.8 & singapore$BEV >= 0.2)$x)
  time_80_to_20_singapore <- max(subset(singapore, singapore$ICE<=0.8 & singapore$ICE >= 0.2)$x)-min(subset(singapore, singapore$ICE<=0.8 & singapore$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_singapore; ice_time[i] <- time_80_to_20_singapore
}

timer_singapore <- data.frame("year"=data_singapore[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_singapore$BEV_time == -Inf), which(timer_singapore$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_singapore) > (last_inf_index + 12)) {
  timer_singapore_short <- timer_singapore[(last_inf_index + 1 + 12):nrow(timer_singapore), ]
} else {
  timer_singapore_short <- timer_singapore
}
...
plot_timer <- ggplot(timer_singapore_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_singapore_short$BEV_time[length(timer_singapore_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Singapore transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_singapore_short$BEV_time[length(timer_singapore_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_singapore <- ggplot(singapore, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Singapore - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_singapore), " years ", round(12*(time_20_to_80_singapore-floor(time_20_to_80_singapore)),0), " months"),
...
plot_singapore <- plot_singapore + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(singapore, singapore$x==2024+counter & singapore$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_singapore <- plot_singapore + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(singapore, singapore$x==2024+counter & singapore$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_singapore <- plot_singapore + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_singapore

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_singapore$overall)
BEV <- data.frame(BEV, "overall"=data_singapore$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_singapore$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_singapore <- ggplot(singapore, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Singapore - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_singapore), " years ",
      round(12 * (time_80_to_20_singapore - floor(time_80_to_20_singapore)), 0),
      " months"
    ),
...
plot_ICE_BEV_singapore <- plot_ICE_BEV_singapore +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(singapore, singapore$x == 2024 + counter - 1 & singapore$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_singapore <- plot_ICE_BEV_singapore +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(singapore, singapore$x == 2024 + counter & singapore$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_singapore)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("singapore_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_singapore,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_singapore",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("singapore_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_singapore,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_singapore",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("singapore_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_singapore",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("singapore_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_singapore,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_singapore",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_singapore$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_singapore, "Inf"),
  bev_50 = as_date_from_numeric(time_50_singapore, "Inf"),
  bev_80 = as_date_from_numeric(time_80_singapore, "Inf"),
  bev_20_80 = time_20_to_80_singapore,
  ice_80_20 = time_80_to_20_singapore,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_singapore$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_singapore)
month_label <- last_label_from_data(data_singapore, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_singapore$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_singapore)) nz(tail(data_singapore$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_singapore)) {
    nz(tail(data_singapore$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_singapore)) nz(tail(data_singapore$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_singapore)) {
    nz(tail(data_singapore$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_singapore),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_singapore)
upsert_weight(
