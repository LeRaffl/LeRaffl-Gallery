#### Thailand (HDV): BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Thailand (HDV)"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/thailand_hdv.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_thailand_hdv <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Thailand (HDV)")
source <- data_thailand_hdv$Source[1]
...
#data_thailand_hdv$BEV <- data_thailand_hdv$`Electric/zero-emission`
#data_thailand_hdv$PHEV <- data_thailand_hdv$Hybrid
#data_thailand_hdv$TOTAL <- data_thailand_hdv$overall
#data_thailand_hdv$OTHERS <- data_thailand_hdv$`Other fuel`
#data_thailand_hdv$TOTAL - data_thailand_hdv$BEV - data_thailand_hdv$PHEV <- data_thailand_hdv$Fossil

data_thailand_hdv$`Electric/zero-emission` <- data_thailand_hdv$BEV
data_thailand_hdv$Hybrid <- data_thailand_hdv$PHEV
data_thailand_hdv$overall <- data_thailand_hdv$TOTAL
data_thailand_hdv$`Other fuel` <- data_thailand_hdv$OTHERS
data_thailand_hdv$Fossil <- data_thailand_hdv$TOTAL - data_thailand_hdv$BEV - data_thailand_hdv$PHEV


data_thailand_hdv$bev_share   <- data_thailand_hdv$`Electric/zero-emission`/data_thailand_hdv$overall
data_thailand_hdv$ice_share   <- data_thailand_hdv$Fossil/data_thailand_hdv$overall
data_thailand_hdv$hybrid_share<- data_thailand_hdv$Hybrid/data_thailand_hdv$overall
data_thailand_hdv$other_share <- data_thailand_hdv$`Other fuel`/data_thailand_hdv$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_thailand_hdv$pure_ICE    <- (data_thailand_hdv$Fossil)/data_thailand_hdv$overall

lastyearly <- ceiling(max(subset(data_thailand_hdv, data_thailand_hdv$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_thailand_hdv_monthly <- subset(data_thailand_hdv, data_thailand_hdv$time_interval=="monthly")
data_thailand_hdv_monthly <- subset(data_thailand_hdv_monthly, data_thailand_hdv_monthly$year>=min(data_thailand_hdv_monthly$year+1))
TTM_shares_thailand_hdv <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_thailand_hdv_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_thailand_hdv_monthly)), "%Y-%m"),
  Other = data_thailand_hdv_monthly$`Other TTM`,
  `ICE` = data_thailand_hdv_monthly$`ICE TTM`,#-data_thailand_hdv_monthly$`HEV TTM`,
  Petrol = data_thailand_hdv_monthly$`Petrol TTM`,
  Diesel = data_thailand_hdv_monthly$`Diesel TTM`,
  #Ethanol = data_thailand_hdv_monthly$`Ethanol TTM`,
  #FlexFuel = data_thailand_hdv_monthly$`FlexFuel TTM`,
  HEV = data_thailand_hdv_monthly$`HEV TTM`,
  PHEV = data_thailand_hdv_monthly$`PHEV TTM`,
  BEV = data_thailand_hdv_monthly$`BEV TTM`
)

colnames(TTM_shares_thailand_hdv)[colnames(TTM_shares_thailand_hdv) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_thailand_hdv)[colnames(TTM_shares_thailand_hdv) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_thailand_hdv)[colnames(TTM_shares_thailand_hdv) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_thailand_hdv)[colnames(TTM_shares_thailand_hdv) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_thailand_hdv)[colnames(TTM_shares_thailand_hdv) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_thailand_hdv_long <- TTM_shares_thailand_hdv %>%
...
TTM_barplot_thailand_hdv <- ggplot(TTM_shares_thailand_hdv_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_thailand_hdv_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_thailand_hdv_long$month[substr(TTM_shares_thailand_hdv_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Thailand (HDV)",
...
print(TTM_barplot_thailand_hdv)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_thailand_hdv$year)))
extrapol <- 2200
data_thailand_hdv <- subset(data_thailand_hdv, data_thailand_hdv$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_thailand_hdv$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_thailand_hdv$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_thailand_hdv$`Electric/zero-emission`))
for(i in 1:length(data_thailand_hdv$`Electric/zero-emission`)){
  data_thailand_hdv_looped <- data_thailand_hdv[1:i,]
  xg<-data_thailand_hdv_looped$year; yg<-as.double(data_thailand_hdv_looped$bev_share)
  ...
  xg<-data_thailand_hdv_looped$year; yg<-as.double(data_thailand_hdv_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  thailand_hdv <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_thailand_hdv_looped$overall; new_A$time_interval <- data_thailand_hdv_looped$time_interval
  
  thailand_hdv$Hybrid       <- pmax(thailand_hdv$Hybrid, 0)
  thailand_hdv$Hybrid_upper <- pmax(thailand_hdv$Hybrid_upper, 0)
  thailand_hdv$Hybrid_lower <- pmax(thailand_hdv$Hybrid_lower, 0)
  
  time_80_thailand_hdv <- max(subset(thailand_hdv, thailand_hdv$BEV<=0.8 & thailand_hdv$BEV >= 0.2)$x)
  time_50_thailand_hdv <- max(subset(thailand_hdv, thailand_hdv$BEV<=0.5 & thailand_hdv$BEV >= 0.2)$x)
  time_20_thailand_hdv <- max(subset(thailand_hdv, thailand_hdv$BEV<=0.2 & thailand_hdv$BEV >= 0.1)$x)
  time_20_to_80_thailand_hdv <- max(subset(thailand_hdv, thailand_hdv$BEV<=0.8 & thailand_hdv$BEV >= 0.2)$x)-min(subset(thailand_hdv, thailand_hdv$BEV<=0.8 & thailand_hdv$BEV >= 0.2)$x)
  time_80_to_20_thailand_hdv <- max(subset(thailand_hdv, thailand_hdv$ICE<=0.8 & thailand_hdv$ICE >= 0.2)$x)-min(subset(thailand_hdv, thailand_hdv$ICE<=0.8 & thailand_hdv$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_thailand_hdv; ice_time[i] <- time_80_to_20_thailand_hdv
}

timer_thailand_hdv <- data.frame("year"=data_thailand_hdv[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_thailand_hdv$BEV_time == -Inf), which(timer_thailand_hdv$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_thailand_hdv) > (last_inf_index + 12)) {
  timer_thailand_hdv_short <- timer_thailand_hdv[(last_inf_index + 1 + 12):nrow(timer_thailand_hdv), ]
} else {
  timer_thailand_hdv_short <- timer_thailand_hdv
}
...
plot_timer <- ggplot(timer_thailand_hdv_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_thailand_hdv_short$BEV_time[length(timer_thailand_hdv_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Thailand (HDV) transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_thailand_hdv_short$BEV_time[length(timer_thailand_hdv_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_thailand_hdv <- ggplot(thailand_hdv, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Thailand (HDV) - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_thailand_hdv), " years ", round(12*(time_20_to_80_thailand_hdv-floor(time_20_to_80_thailand_hdv)),0), " months"),
...
plot_thailand_hdv <- plot_thailand_hdv + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(thailand_hdv, thailand_hdv$x==2024+counter & thailand_hdv$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_thailand_hdv <- plot_thailand_hdv + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(thailand_hdv, thailand_hdv$x==2024+counter & thailand_hdv$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_thailand_hdv <- plot_thailand_hdv + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_thailand_hdv

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_thailand_hdv$overall)
BEV <- data.frame(BEV, "overall"=data_thailand_hdv$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_thailand_hdv$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_thailand_hdv <- ggplot(thailand_hdv, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Thailand (HDV) - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_thailand_hdv), " years ",
      round(12 * (time_80_to_20_thailand_hdv - floor(time_80_to_20_thailand_hdv)), 0),
      " months"
    ),
...
plot_ICE_BEV_thailand_hdv <- plot_ICE_BEV_thailand_hdv +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(thailand_hdv, thailand_hdv$x == 2024 + counter - 1 & thailand_hdv$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_thailand_hdv <- plot_ICE_BEV_thailand_hdv +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(thailand_hdv, thailand_hdv$x == 2024 + counter & thailand_hdv$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_thailand_hdv)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("thailand_hdv_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_thailand_hdv,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_thailand_hdv",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("thailand_hdv_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_thailand_hdv,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_thailand_hdv",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("thailand_hdv_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_thailand_hdv",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("thailand_hdv_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_thailand_hdv,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_thailand_hdv",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_thailand_hdv$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_thailand_hdv, "Inf"),
  bev_50 = as_date_from_numeric(time_50_thailand_hdv, "Inf"),
  bev_80 = as_date_from_numeric(time_80_thailand_hdv, "Inf"),
  bev_20_80 = time_20_to_80_thailand_hdv,
  ice_80_20 = time_80_to_20_thailand_hdv,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_thailand_hdv$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_thailand_hdv)
month_label <- last_label_from_data(data_thailand_hdv, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_thailand_hdv$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_thailand_hdv)) nz(tail(data_thailand_hdv$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_thailand_hdv)) {
    nz(tail(data_thailand_hdv$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_thailand_hdv)) nz(tail(data_thailand_hdv$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_thailand_hdv)) {
    nz(tail(data_thailand_hdv$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_thailand_hdv),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_thailand_hdv)
upsert_weight(
