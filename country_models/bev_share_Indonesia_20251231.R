#### Indonesia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Indonesia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/indonesia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_indonesia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Indonesia")
source <- data_indonesia$Source[1]
...
#data_indonesia$BEV <- data_indonesia$`Electric/zero-emission`
#data_indonesia$PHEV <- data_indonesia$Hybrid
#data_indonesia$TOTAL <- data_indonesia$overall
#data_indonesia$OTHERS <- data_indonesia$`Other fuel`
#data_indonesia$TOTAL - data_indonesia$BEV - data_indonesia$PHEV <- data_indonesia$Fossil

data_indonesia$`Electric/zero-emission` <- data_indonesia$BEV
data_indonesia$Hybrid <- data_indonesia$PHEV
data_indonesia$overall <- data_indonesia$TOTAL
data_indonesia$`Other fuel` <- data_indonesia$OTHERS
data_indonesia$Fossil <- data_indonesia$TOTAL - data_indonesia$BEV - data_indonesia$PHEV


data_indonesia$bev_share   <- data_indonesia$`Electric/zero-emission`/data_indonesia$overall
data_indonesia$ice_share   <- data_indonesia$Fossil/data_indonesia$overall
data_indonesia$hybrid_share<- data_indonesia$Hybrid/data_indonesia$overall
data_indonesia$other_share <- data_indonesia$`Other fuel`/data_indonesia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_indonesia$pure_ICE    <- (data_indonesia$Fossil)/data_indonesia$overall

lastyearly <- ceiling(max(subset(data_indonesia, data_indonesia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_indonesia_monthly <- subset(data_indonesia, data_indonesia$time_interval=="monthly")
data_indonesia_monthly <- subset(data_indonesia_monthly, data_indonesia_monthly$year>=min(data_indonesia_monthly$year+1))
TTM_shares_indonesia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_indonesia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_indonesia_monthly)), "%Y-%m"),
  Other = data_indonesia_monthly$`Other TTM`,
  #`ICE` = data_indonesia_monthly$`ICE TTM`,#-data_indonesia_monthly$`HEV TTM`,
  Petrol = data_indonesia_monthly$`Petrol TTM`,
  Diesel = data_indonesia_monthly$`Diesel TTM`,
  #Ethanol = data_indonesia_monthly$`Ethanol TTM`,
  #FlexFuel = data_indonesia_monthly$`FlexFuel TTM`,
  HEV = data_indonesia_monthly$`HEV TTM`,
  PHEV = data_indonesia_monthly$`PHEV TTM`,
  BEV = data_indonesia_monthly$`BEV TTM`
)

colnames(TTM_shares_indonesia)[colnames(TTM_shares_indonesia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_indonesia)[colnames(TTM_shares_indonesia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_indonesia)[colnames(TTM_shares_indonesia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_indonesia)[colnames(TTM_shares_indonesia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_indonesia)[colnames(TTM_shares_indonesia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_indonesia_long <- TTM_shares_indonesia %>%
...
TTM_barplot_indonesia <- ggplot(TTM_shares_indonesia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_indonesia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_indonesia_long$month[substr(TTM_shares_indonesia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Indonesia",
...
print(TTM_barplot_indonesia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_indonesia$year)))
extrapol <- 2200
data_indonesia <- subset(data_indonesia, data_indonesia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_indonesia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_indonesia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_indonesia$`Electric/zero-emission`))
for(i in 1:length(data_indonesia$`Electric/zero-emission`)){
  data_indonesia_looped <- data_indonesia[1:i,]
  xg<-data_indonesia_looped$year; yg<-as.double(data_indonesia_looped$bev_share)
  ...
  xg<-data_indonesia_looped$year; yg<-as.double(data_indonesia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  indonesia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_indonesia_looped$overall; new_A$time_interval <- data_indonesia_looped$time_interval
  
  indonesia$Hybrid       <- pmax(indonesia$Hybrid, 0)
  indonesia$Hybrid_upper <- pmax(indonesia$Hybrid_upper, 0)
  indonesia$Hybrid_lower <- pmax(indonesia$Hybrid_lower, 0)
  
  time_80_indonesia <- max(subset(indonesia, indonesia$BEV<=0.8 & indonesia$BEV >= 0.2)$x)
  time_50_indonesia <- max(subset(indonesia, indonesia$BEV<=0.5 & indonesia$BEV >= 0.2)$x)
  time_20_indonesia <- max(subset(indonesia, indonesia$BEV<=0.2 & indonesia$BEV >= 0.1)$x)
  time_20_to_80_indonesia <- max(subset(indonesia, indonesia$BEV<=0.8 & indonesia$BEV >= 0.2)$x)-min(subset(indonesia, indonesia$BEV<=0.8 & indonesia$BEV >= 0.2)$x)
  time_80_to_20_indonesia <- max(subset(indonesia, indonesia$ICE<=0.8 & indonesia$ICE >= 0.2)$x)-min(subset(indonesia, indonesia$ICE<=0.8 & indonesia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_indonesia; ice_time[i] <- time_80_to_20_indonesia
}

timer_indonesia <- data.frame("year"=data_indonesia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_indonesia$BEV_time == -Inf), which(timer_indonesia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_indonesia) > (last_inf_index + 12)) {
  timer_indonesia_short <- timer_indonesia[(last_inf_index + 1 + 12):nrow(timer_indonesia), ]
} else {
  timer_indonesia_short <- timer_indonesia
}
...
plot_timer <- ggplot(timer_indonesia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_indonesia_short$BEV_time[length(timer_indonesia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Indonesia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_indonesia_short$BEV_time[length(timer_indonesia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_indonesia <- ggplot(indonesia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Indonesia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_indonesia), " years ", round(12*(time_20_to_80_indonesia-floor(time_20_to_80_indonesia)),0), " months"),
...
plot_indonesia <- plot_indonesia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(indonesia, indonesia$x==2024+counter & indonesia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_indonesia <- plot_indonesia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(indonesia, indonesia$x==2024+counter & indonesia$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_indonesia <- plot_indonesia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_indonesia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_indonesia$overall)
BEV <- data.frame(BEV, "overall"=data_indonesia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_indonesia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_indonesia <- ggplot(indonesia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Indonesia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_indonesia), " years ",
      round(12 * (time_80_to_20_indonesia - floor(time_80_to_20_indonesia)), 0),
      " months"
    ),
...
plot_ICE_BEV_indonesia <- plot_ICE_BEV_indonesia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(indonesia, indonesia$x == 2024 + counter - 1 & indonesia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_indonesia <- plot_ICE_BEV_indonesia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(indonesia, indonesia$x == 2024 + counter & indonesia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_indonesia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("indonesia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_indonesia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_indonesia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("indonesia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_indonesia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_indonesia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("indonesia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_indonesia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("indonesia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_indonesia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_indonesia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_indonesia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_indonesia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_indonesia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_indonesia, "Inf"),
  bev_20_80 = time_20_to_80_indonesia,
  ice_80_20 = time_80_to_20_indonesia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_indonesia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_indonesia)
month_label <- last_label_from_data(data_indonesia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_indonesia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_indonesia)) nz(tail(data_indonesia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_indonesia)) {
    nz(tail(data_indonesia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_indonesia)) nz(tail(data_indonesia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_indonesia)) {
    nz(tail(data_indonesia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_indonesia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_indonesia)
upsert_weight(
