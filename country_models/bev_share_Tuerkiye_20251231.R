#### Türkiye: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Türkiye"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/tuerkiye.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_tuerkiye <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Türkiye")
source <- data_tuerkiye$Source[1]
...
#data_tuerkiye$BEV <- data_tuerkiye$`Electric/zero-emission`
#data_tuerkiye$PHEV <- data_tuerkiye$Hybrid
#data_tuerkiye$TOTAL <- data_tuerkiye$overall
#data_tuerkiye$OTHERS <- data_tuerkiye$`Other fuel`
#data_tuerkiye$TOTAL - data_tuerkiye$BEV - data_tuerkiye$PHEV <- data_tuerkiye$Fossil

data_tuerkiye$`Electric/zero-emission` <- data_tuerkiye$BEV
data_tuerkiye$Hybrid <- data_tuerkiye$HYBRIDS
data_tuerkiye$overall <- data_tuerkiye$TOTAL
data_tuerkiye$`Other fuel` <- data_tuerkiye$OTHERS
data_tuerkiye$Fossil <- data_tuerkiye$TOTAL - data_tuerkiye$BEV - data_tuerkiye$HYBRIDS


data_tuerkiye$bev_share   <- data_tuerkiye$`Electric/zero-emission`/data_tuerkiye$overall
data_tuerkiye$ice_share   <- data_tuerkiye$Fossil/data_tuerkiye$overall
data_tuerkiye$hybrid_share<- data_tuerkiye$Hybrid/data_tuerkiye$overall
data_tuerkiye$other_share <- data_tuerkiye$`Other fuel`/data_tuerkiye$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_tuerkiye$pure_ICE    <- (data_tuerkiye$Fossil)/data_tuerkiye$overall

lastyearly <- ceiling(max(subset(data_tuerkiye, data_tuerkiye$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_tuerkiye_monthly <- subset(data_tuerkiye, data_tuerkiye$time_interval=="monthly")
data_tuerkiye_monthly <- subset(data_tuerkiye_monthly, data_tuerkiye_monthly$year>=min(data_tuerkiye_monthly$year+1))
TTM_shares_tuerkiye <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_tuerkiye_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_tuerkiye_monthly)), "%Y-%m"),
  Other = data_tuerkiye_monthly$`Other TTM`,
  #`ICE` = data_tuerkiye_monthly$`ICE TTM`,#-data_tuerkiye_monthly$`HEV TTM`,
  Petrol = data_tuerkiye_monthly$`Petrol TTM`,
  Diesel = data_tuerkiye_monthly$`Diesel TTM`,
  #Ethanol = data_tuerkiye_monthly$`Ethanol TTM`,
  #FlexFuel = data_tuerkiye_monthly$`FlexFuel TTM`,
  #HEV = data_tuerkiye_monthly$`HEV TTM`,
  #PHEV = data_tuerkiye_monthly$`PHEV TTM`,
  Hybrid = data_tuerkiye_monthly$`Hybrid TTM`,
  BEV = data_tuerkiye_monthly$`BEV TTM`
)

colnames(TTM_shares_tuerkiye)[colnames(TTM_shares_tuerkiye) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_tuerkiye)[colnames(TTM_shares_tuerkiye) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_tuerkiye)[colnames(TTM_shares_tuerkiye) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_tuerkiye)[colnames(TTM_shares_tuerkiye) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_tuerkiye)[colnames(TTM_shares_tuerkiye) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_tuerkiye_long <- TTM_shares_tuerkiye %>%
...
TTM_barplot_tuerkiye <- ggplot(TTM_shares_tuerkiye_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_tuerkiye_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_tuerkiye_long$month[substr(TTM_shares_tuerkiye_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Türkiye",
...
print(TTM_barplot_tuerkiye)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_tuerkiye$year)))
extrapol <- 2200
data_tuerkiye <- subset(data_tuerkiye, data_tuerkiye$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_tuerkiye$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_tuerkiye$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_tuerkiye$`Electric/zero-emission`))
for(i in 1:length(data_tuerkiye$`Electric/zero-emission`)){
  data_tuerkiye_looped <- data_tuerkiye[1:i,]
  xg<-data_tuerkiye_looped$year; yg<-as.double(data_tuerkiye_looped$bev_share)
  ...
  xg<-data_tuerkiye_looped$year; yg<-as.double(data_tuerkiye_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  tuerkiye <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_tuerkiye_looped$overall; new_A$time_interval <- data_tuerkiye_looped$time_interval
  
  tuerkiye$Hybrid       <- pmax(tuerkiye$Hybrid, 0)
  tuerkiye$Hybrid_upper <- pmax(tuerkiye$Hybrid_upper, 0)
  tuerkiye$Hybrid_lower <- pmax(tuerkiye$Hybrid_lower, 0)
  
  time_80_tuerkiye <- max(subset(tuerkiye, tuerkiye$BEV<=0.8 & tuerkiye$BEV >= 0.2)$x)
  time_50_tuerkiye <- max(subset(tuerkiye, tuerkiye$BEV<=0.5 & tuerkiye$BEV >= 0.2)$x)
  time_20_tuerkiye <- max(subset(tuerkiye, tuerkiye$BEV<=0.2 & tuerkiye$BEV >= 0.1)$x)
  time_20_to_80_tuerkiye <- max(subset(tuerkiye, tuerkiye$BEV<=0.8 & tuerkiye$BEV >= 0.2)$x)-min(subset(tuerkiye, tuerkiye$BEV<=0.8 & tuerkiye$BEV >= 0.2)$x)
  time_80_to_20_tuerkiye <- max(subset(tuerkiye, tuerkiye$ICE<=0.8 & tuerkiye$ICE >= 0.2)$x)-min(subset(tuerkiye, tuerkiye$ICE<=0.8 & tuerkiye$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_tuerkiye; ice_time[i] <- time_80_to_20_tuerkiye
}

timer_tuerkiye <- data.frame("year"=data_tuerkiye[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_tuerkiye$BEV_time == -Inf), which(timer_tuerkiye$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_tuerkiye) > (last_inf_index + 12)) {
  timer_tuerkiye_short <- timer_tuerkiye[(last_inf_index + 1 + 12):nrow(timer_tuerkiye), ]
} else {
  timer_tuerkiye_short <- timer_tuerkiye
}
...
plot_timer <- ggplot(timer_tuerkiye_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_tuerkiye_short$BEV_time[length(timer_tuerkiye_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Türkiye transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_tuerkiye_short$BEV_time[length(timer_tuerkiye_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_tuerkiye <- ggplot(tuerkiye, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Türkiye - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_tuerkiye), " years ", round(12*(time_20_to_80_tuerkiye-floor(time_20_to_80_tuerkiye)),0), " months"),
...
plot_tuerkiye <- plot_tuerkiye + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(tuerkiye, tuerkiye$x==2024+counter & tuerkiye$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_tuerkiye <- plot_tuerkiye + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(tuerkiye, tuerkiye$x==2024+counter & tuerkiye$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_tuerkiye <- plot_tuerkiye + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_tuerkiye

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_tuerkiye$overall)
BEV <- data.frame(BEV, "overall"=data_tuerkiye$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_tuerkiye$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_tuerkiye <- ggplot(tuerkiye, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Türkiye - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_tuerkiye), " years ",
      round(12 * (time_80_to_20_tuerkiye - floor(time_80_to_20_tuerkiye)), 0),
      " months"
    ),
...
plot_ICE_BEV_tuerkiye <- plot_ICE_BEV_tuerkiye +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(tuerkiye, tuerkiye$x == 2024 + counter - 1 & tuerkiye$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_tuerkiye <- plot_ICE_BEV_tuerkiye +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(tuerkiye, tuerkiye$x == 2024 + counter & tuerkiye$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_tuerkiye)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("tuerkiye_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_tuerkiye,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_tuerkiye",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("tuerkiye_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_tuerkiye,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_tuerkiye",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("tuerkiye_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_tuerkiye",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("tuerkiye_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_tuerkiye,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_tuerkiye",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_tuerkiye$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_tuerkiye, "Inf"),
  bev_50 = as_date_from_numeric(time_50_tuerkiye, "Inf"),
  bev_80 = as_date_from_numeric(time_80_tuerkiye, "Inf"),
  bev_20_80 = time_20_to_80_tuerkiye,
  ice_80_20 = time_80_to_20_tuerkiye,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_tuerkiye$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_tuerkiye)
month_label <- last_label_from_data(data_tuerkiye, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_tuerkiye$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_tuerkiye)) nz(tail(data_tuerkiye$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_tuerkiye)) {
    nz(tail(data_tuerkiye$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_tuerkiye)) nz(tail(data_tuerkiye$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_tuerkiye)) {
    nz(tail(data_tuerkiye$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_tuerkiye),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_tuerkiye)
upsert_weight(
