#### Malaysia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Malaysia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/malaysia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)
source <- "storage.data.gov.my"
entire_caption <- paste0(social_caption, " | \t ", Sys.Date(), "  | \t    Source: ", source)

#### read data ####
#### M.a.l.a.y.s.i.a  specific data reading block ####

# ARROW PACKAGE BUILD
# run these 3 lines if arrow package yields problems, it's probably the brotli package
# this builds libarrow + brotli from C++ source
#Sys.setenv(LIBARROW_MINIMAL = "false", ARROW_WITH_BROTLI = "ON")
...
years <- 2018:2026  # Beispieljahre

# Funktion zum Laden einer einzelnen Datei
load_parquet <- function(year) {
  url <- paste0("https://storage.data.gov.my/transportation/cars_", year, ".parquet")
  read_parquet(url)
}

# Laden der Dateien für jedes Jahr
data_malaysia <- map_df(years, load_parquet)

# Überprüfen, ob die Jahre korrekt sind
library(tidyr)
library(zoo)  # Für rollende Berechnungen

data_malaysia <- data_malaysia %>%
  mutate(
    YYYYMMM = format(date_reg, "%YM%m"),
    fuel = case_when(
      fuel == "greendiesel" ~ "greendiesel",
      fuel == "diesel" ~ "Diesel",
      fuel == "petrol" ~ "Petrol",
      fuel == "electric" ~ "Electric/zero-emission",
      fuel == "hybrid_petrol" ~ "Hybrid (Petrol)",
...
data_malaysia <- data_malaysia %>%
  group_by(YYYYMMM, fuel) %>%
  summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = fuel, values_from = count, values_fill = 0)

# Hinzufügen der "overall" Spalte und Formatierung
data_malaysia <- data_malaysia %>%
  mutate(
    overall = rowSums(.[, c("Electric/zero-emission", "Hybrid (Petrol)", "Hybrid (Diesel)", "Other fuel", "Petrol", "Diesel")], na.rm = TRUE),
    year = as.numeric(substr(YYYYMMM, 1, 4))-1 + (as.numeric(substr(YYYYMMM, 6, 7)) - 1) / 12
  ) %>%
  select(YYYYMMM, year, everything())

# Berechnung der Shares
data_malaysia$overall <- data_malaysia$`Electric/zero-emission` + data_malaysia$Diesel + data_malaysia$Petrol + data_malaysia$`Hybrid (Petrol)` + data_malaysia$`Hybrid (Diesel)` + data_malaysia$`Other fuel` + data_malaysia$greendiesel
data_malaysia$bev_share <- data_malaysia$`Electric/zero-emission` / data_malaysia$overall
data_malaysia$ice_share <- (data_malaysia$Diesel + data_malaysia$Petrol + data_malaysia$greendiesel + data_malaysia$`Other fuel`) / data_malaysia$overall
data_malaysia$hybrid_share <- (data_malaysia$`Hybrid (Petrol)` + data_malaysia$`Hybrid (Diesel)`) / data_malaysia$overall
data_malaysia$hybrid_p_share <- data_malaysia$`Hybrid (Petrol)` / data_malaysia$overall
data_malaysia$hybrid_d_share <- data_malaysia$`Hybrid (Diesel)` / data_malaysia$overall
data_malaysia$other_share <- data_malaysia$`Other fuel` / data_malaysia$overall
data_malaysia$petrol_share <- data_malaysia$Petrol / data_malaysia$overall
data_malaysia$diesel_share <- data_malaysia$Diesel / data_malaysia$overall
data_malaysia$greendiesel_share <- data_malaysia$greendiesel / data_malaysia$overall


# TTM (Trailing 12 Months) Berechnungen
# Zuerst sicherstellen, dass die Daten chronologisch sortiert sind
data_malaysia <- data_malaysia %>%
  arrange(YYYYMMM)

# Funktion zur Berechnung des rollenden Mittelwerts für die letzten 12 Monate
calculate_ttm <- function(x) {
  rollsum(x, 12, fill = NA, align = "right")
}

# Anwendung der TTM-Berechnung auf die interessierenden Spalten
data_malaysia <- data_malaysia %>%
  mutate(
    across(c(`Electric/zero-emission`, `Hybrid (Diesel)`, `Hybrid (Petrol)`, `Other fuel`, overall, Petrol, Diesel, greendiesel), 
           ~calculate_ttm(.), 
           .names = "ttm_{.col}"),
    `BEV TTM` = `ttm_Electric/zero-emission` / ttm_overall,
    `Green Diesel TTM` = `ttm_greendiesel` / ttm_overall,
    `Hybrid (Diesel) TTM` = `ttm_Hybrid (Diesel)` / ttm_overall,
    `Hybrid (Petrol) TTM` = `ttm_Hybrid (Petrol)` / ttm_overall,
    `Petrol TTM` = ttm_Petrol / ttm_overall,
    `Diesel TTM` = ttm_Diesel / ttm_overall,
    `Other TTM` = `ttm_Other fuel` / ttm_overall
  )

# Ausgabe
tail(data_malaysia, n = 5)

data_malaysia$time_interval <- "monthly"
lastyearly <- ceiling(max(subset(data_malaysia, data_malaysia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_malaysia_monthly <- subset(data_malaysia, data_malaysia$time_interval=="monthly")
data_malaysia_monthly <- subset(data_malaysia_monthly, data_malaysia_monthly$year>=min(data_malaysia_monthly$year+1))
TTM_shares_malaysia <- data.frame(
  "month" = format(seq.Date(from = as.Date(paste0(floor(min(data_malaysia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_malaysia_monthly)), "%Y-%m"),
  "Petrol" = data_malaysia_monthly$`Petrol TTM`,# - data_malaysia_monthly$`HEV TTM`,
  "Diesel" = data_malaysia_monthly$`Diesel TTM`,# - data_malaysia_monthly$`HEV TTM`,
  "Green Diesel" = data_malaysia_monthly$`Green Diesel TTM`,# - data_malaysia_monthly$`HEV TTM`,
  "Hybrid (Petrol)" = data_malaysia_monthly$`Hybrid (Petrol) TTM`, 
  "Hybrid (Diesel)" = data_malaysia_monthly$`Hybrid (Diesel) TTM`, 
  "Other" = data_malaysia_monthly$`Other TTM`, 
  "BEV" = data_malaysia_monthly$`BEV TTM`
)

colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "Green.Diesel"]   <- "Green Diesel"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "Hybrid..Petrol."]   <- "Hybrid (Petrol)"
colnames(TTM_shares_malaysia)[colnames(TTM_shares_malaysia) == "Hybrid..Diesel."]   <- "Hybrid (Diesel)"

TTM_shares_malaysia_long <- TTM_shares_malaysia %>%
  pivot_longer(cols = c("Other","Petrol","Diesel", "Green Diesel", "Hybrid (Petrol)", "Hybrid (Diesel)", "BEV"), # das Spiel mit den Pünktchen muss sein weil die Spalte so benannt wird von R
               names_to = "type", 
               values_to = "value") %>%
  mutate(type = factor(type, levels = c("Other","Petrol", "Diesel", "Green Diesel", "Hybrid (Petrol)", "Hybrid (Diesel)", "BEV")))

TTM_shares_malaysia_long <- TTM_shares_malaysia_long %>%
  filter(!is.na(value))
...
TTM_barplot_malaysia <- ggplot(TTM_shares_malaysia_long, aes(x = month, y = value, fill = type)) +
  # Balkendiagramm
  geom_bar(stat = "identity", position = "stack", width = 1) +
  
  # Vertikale Linien für Januar
  geom_vline(
    data = TTM_shares_malaysia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_malaysia_long$month[substr(TTM_shares_malaysia_long$month, 6, 7) == "01"],
  ...
  # Labels und Titel
  labs(
    title = "12-Month Trailing Market Shares by Fuel Type in Malaysia",
...
print(TTM_barplot_malaysia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_malaysia$year)))
extrapol <- 2200
data_malaysia <- subset(data_malaysia, data_malaysia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_malaysia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_malaysia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_malaysia$`Electric/zero-emission`))
for(i in 1:length(data_malaysia$`Electric/zero-emission`)){
  data_malaysia_looped <- data_malaysia[1:i,]
  xg<-data_malaysia_looped$year; yg<-as.double(data_malaysia_looped$bev_share)
  ...
  xg<-data_malaysia_looped$year; yg<-as.double(data_malaysia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  malaysia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_malaysia_looped$overall; new_A$time_interval <- data_malaysia_looped$time_interval
  
  malaysia$Hybrid       <- pmax(malaysia$Hybrid, 0)
  malaysia$Hybrid_upper <- pmax(malaysia$Hybrid_upper, 0)
  malaysia$Hybrid_lower <- pmax(malaysia$Hybrid_lower, 0)
  
  time_80_malaysia <- max(subset(malaysia, malaysia$BEV<=0.8 & malaysia$BEV >= 0.2)$x)
  time_50_malaysia <- max(subset(malaysia, malaysia$BEV<=0.5 & malaysia$BEV >= 0.2)$x)
  time_20_malaysia <- max(subset(malaysia, malaysia$BEV<=0.2 & malaysia$BEV >= 0.1)$x)
  time_20_to_80_malaysia <- max(subset(malaysia, malaysia$BEV<=0.8 & malaysia$BEV >= 0.2)$x)-min(subset(malaysia, malaysia$BEV<=0.8 & malaysia$BEV >= 0.2)$x)
  time_80_to_20_malaysia <- max(subset(malaysia, malaysia$ICE<=0.8 & malaysia$ICE >= 0.2)$x)-min(subset(malaysia, malaysia$ICE<=0.8 & malaysia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_malaysia; ice_time[i] <- time_80_to_20_malaysia
}

timer_malaysia <- data.frame("year"=data_malaysia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_malaysia$BEV_time == -Inf), which(timer_malaysia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_malaysia) > (last_inf_index + 12)) {
  timer_malaysia_short <- timer_malaysia[(last_inf_index + 1 + 12):nrow(timer_malaysia), ]
} else {
  timer_malaysia_short <- timer_malaysia
}
...
plot_timer <- ggplot(timer_malaysia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_malaysia_short$BEV_time[length(timer_malaysia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Malaysia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_malaysia_short$BEV_time[length(timer_malaysia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_malaysia <- ggplot(malaysia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Malaysia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_malaysia), " years ", round(12*(time_20_to_80_malaysia-floor(time_20_to_80_malaysia)),0), " months"),
...
plot_malaysia <- plot_malaysia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(malaysia, malaysia$x==2024+counter & malaysia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_malaysia <- plot_malaysia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(malaysia, malaysia$x==2024+counter & malaysia$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_malaysia <- plot_malaysia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_malaysia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_malaysia$overall)
BEV <- data.frame(BEV, "overall"=data_malaysia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_malaysia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_malaysia <- ggplot(malaysia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Malaysia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_malaysia), " years ",
      round(12 * (time_80_to_20_malaysia - floor(time_80_to_20_malaysia)), 0),
      " months"
    ),
    caption = entire_caption,
    x = " ",
...
plot_ICE_BEV_malaysia <- plot_ICE_BEV_malaysia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(malaysia, malaysia$x == 2024 + counter - 1 & malaysia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_malaysia <- plot_ICE_BEV_malaysia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(malaysia, malaysia$x == 2024 + counter & malaysia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_malaysia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("malaysia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_malaysia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malaysia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("malaysia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_malaysia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malaysia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("malaysia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malaysia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("malaysia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_malaysia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_malaysia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_malaysia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_malaysia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_malaysia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_malaysia, "Inf"),
  bev_20_80 = time_20_to_80_malaysia,
  ice_80_20 = time_80_to_20_malaysia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_malaysia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_malaysia)
month_label <- last_label_from_data(data_malaysia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_malaysia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_malaysia)) nz(tail(data_malaysia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_malaysia)) {
    nz(tail(data_malaysia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_malaysia)) nz(tail(data_malaysia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_malaysia)) {
    nz(tail(data_malaysia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_malaysia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_malaysia)
upsert_weight(
