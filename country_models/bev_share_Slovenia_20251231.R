#### Slovenia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Slovenia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/slovenia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_slovenia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Slovenia")
source <- data_slovenia$Source[1]
...
#data_slovenia$BEV <- data_slovenia$`Electric/zero-emission`
#data_slovenia$PHEV <- data_slovenia$Hybrid
#data_slovenia$TOTAL <- data_slovenia$overall
#data_slovenia$OTHERS <- data_slovenia$`Other fuel`
#data_slovenia$TOTAL - data_slovenia$BEV - data_slovenia$PHEV <- data_slovenia$Fossil

data_slovenia$`Electric/zero-emission` <- data_slovenia$BEV
data_slovenia$Hybrid <- data_slovenia$PHEV
data_slovenia$overall <- data_slovenia$TOTAL
data_slovenia$`Other fuel` <- data_slovenia$OTHERS
data_slovenia$Fossil <- data_slovenia$TOTAL - data_slovenia$BEV - data_slovenia$PHEV


data_slovenia$bev_share   <- data_slovenia$`Electric/zero-emission`/data_slovenia$overall
data_slovenia$ice_share   <- data_slovenia$Fossil/data_slovenia$overall
data_slovenia$hybrid_share<- data_slovenia$Hybrid/data_slovenia$overall
data_slovenia$other_share <- data_slovenia$`Other fuel`/data_slovenia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_slovenia$pure_ICE    <- (data_slovenia$Fossil)/data_slovenia$overall

lastyearly <- ceiling(max(subset(data_slovenia, data_slovenia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_slovenia_monthly <- subset(data_slovenia, data_slovenia$time_interval=="monthly")
data_slovenia_monthly <- subset(data_slovenia_monthly, data_slovenia_monthly$year>=min(data_slovenia_monthly$year+1))
TTM_shares_slovenia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_slovenia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_slovenia_monthly)), "%Y-%m"),
  Other = data_slovenia_monthly$`Other TTM`,
  `ICE` = data_slovenia_monthly$`ICE TTM`,#-data_slovenia_monthly$`HEV TTM`,
  Petrol = data_slovenia_monthly$`Petrol TTM`,
  Diesel = data_slovenia_monthly$`Diesel TTM`,
  #Ethanol = data_slovenia_monthly$`Ethanol TTM`,
  #FlexFuel = data_slovenia_monthly$`FlexFuel TTM`,
  HEV = data_slovenia_monthly$`HEV TTM`,
  PHEV = data_slovenia_monthly$`PHEV TTM`,
  BEV = data_slovenia_monthly$`BEV TTM`
)

colnames(TTM_shares_slovenia)[colnames(TTM_shares_slovenia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_slovenia)[colnames(TTM_shares_slovenia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_slovenia)[colnames(TTM_shares_slovenia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_slovenia)[colnames(TTM_shares_slovenia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_slovenia)[colnames(TTM_shares_slovenia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_slovenia_long <- TTM_shares_slovenia %>%
...
TTM_barplot_slovenia <- ggplot(TTM_shares_slovenia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_slovenia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_slovenia_long$month[substr(TTM_shares_slovenia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Slovenia",
...
print(TTM_barplot_slovenia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_slovenia$year)))
extrapol <- 2200
data_slovenia <- subset(data_slovenia, data_slovenia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_slovenia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_slovenia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_slovenia$`Electric/zero-emission`))
for(i in 1:length(data_slovenia$`Electric/zero-emission`)){
  data_slovenia_looped <- data_slovenia[1:i,]
  xg<-data_slovenia_looped$year; yg<-as.double(data_slovenia_looped$bev_share)
  ...
  xg<-data_slovenia_looped$year; yg<-as.double(data_slovenia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  slovenia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_slovenia_looped$overall; new_A$time_interval <- data_slovenia_looped$time_interval
  
  slovenia$Hybrid       <- pmax(slovenia$Hybrid, 0)
  slovenia$Hybrid_upper <- pmax(slovenia$Hybrid_upper, 0)
  slovenia$Hybrid_lower <- pmax(slovenia$Hybrid_lower, 0)
  
  time_80_slovenia <- max(subset(slovenia, slovenia$BEV<=0.8 & slovenia$BEV >= 0.2)$x)
  time_50_slovenia <- max(subset(slovenia, slovenia$BEV<=0.5 & slovenia$BEV >= 0.2)$x)
  time_20_slovenia <- max(subset(slovenia, slovenia$BEV<=0.2 & slovenia$BEV >= 0.1)$x)
  time_20_to_80_slovenia <- max(subset(slovenia, slovenia$BEV<=0.8 & slovenia$BEV >= 0.2)$x)-min(subset(slovenia, slovenia$BEV<=0.8 & slovenia$BEV >= 0.2)$x)
  time_80_to_20_slovenia <- max(subset(slovenia, slovenia$ICE<=0.8 & slovenia$ICE >= 0.2)$x)-min(subset(slovenia, slovenia$ICE<=0.8 & slovenia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_slovenia; ice_time[i] <- time_80_to_20_slovenia
}

timer_slovenia <- data.frame("year"=data_slovenia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_slovenia$BEV_time == -Inf), which(timer_slovenia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_slovenia) > (last_inf_index + 12)) {
  timer_slovenia_short <- timer_slovenia[(last_inf_index + 1 + 12):nrow(timer_slovenia), ]
} else {
  timer_slovenia_short <- timer_slovenia
}
...
plot_timer <- ggplot(timer_slovenia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_slovenia_short$BEV_time[length(timer_slovenia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Slovenia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_slovenia_short$BEV_time[length(timer_slovenia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_slovenia <- ggplot(slovenia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Slovenia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_slovenia), " years ", round(12*(time_20_to_80_slovenia-floor(time_20_to_80_slovenia)),0), " months"),
...
plot_slovenia <- plot_slovenia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(slovenia, slovenia$x==2024+counter & slovenia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_slovenia <- plot_slovenia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(slovenia, slovenia$x==2024+counter & slovenia$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_slovenia <- plot_slovenia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_slovenia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_slovenia$overall)
BEV <- data.frame(BEV, "overall"=data_slovenia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_slovenia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_slovenia <- ggplot(slovenia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Slovenia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_slovenia), " years ",
      round(12 * (time_80_to_20_slovenia - floor(time_80_to_20_slovenia)), 0),
      " months"
    ),
...
plot_ICE_BEV_slovenia <- plot_ICE_BEV_slovenia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(slovenia, slovenia$x == 2024 + counter - 1 & slovenia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_slovenia <- plot_ICE_BEV_slovenia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(slovenia, slovenia$x == 2024 + counter & slovenia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_slovenia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("slovenia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_slovenia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovenia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("slovenia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_slovenia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovenia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("slovenia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovenia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("slovenia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_slovenia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_slovenia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_slovenia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_slovenia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_slovenia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_slovenia, "Inf"),
  bev_20_80 = time_20_to_80_slovenia,
  ice_80_20 = time_80_to_20_slovenia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_slovenia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_slovenia)
month_label <- last_label_from_data(data_slovenia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_slovenia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_slovenia)) nz(tail(data_slovenia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_slovenia)) {
    nz(tail(data_slovenia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_slovenia)) nz(tail(data_slovenia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_slovenia)) {
    nz(tail(data_slovenia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_slovenia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_slovenia)
upsert_weight(
