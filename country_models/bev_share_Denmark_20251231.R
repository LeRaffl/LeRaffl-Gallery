#### Denmark: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Denmark"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/denmark.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_denmark <- read_sheet("https://docs.google.com/spreadsheets/d/1n6QacQ7BIWMa9-vQpbDuuwkquSzYk7XIRbXzjcIsnyg/edit?gid=0#gid=0", sheet = "registrations")
source <- data_denmark$Source[1]
...
#data_denmark$BEV <- data_denmark$`Electric/zero-emission`
#data_denmark$PHEV <- data_denmark$Hybrid
#data_denmark$TOTAL <- data_denmark$overall
#data_denmark$OTHERS <- data_denmark$`Other fuel`
#data_denmark$TOTAL - data_denmark$BEV - data_denmark$PHEV <- data_denmark$Fossil

data_denmark$`Electric/zero-emission` <- data_denmark$BEV
data_denmark$Hybrid <- data_denmark$PHEV
data_denmark$overall <- data_denmark$TOTAL
data_denmark$`Other fuel` <- data_denmark$OTHERS
data_denmark$Fossil <- data_denmark$TOTAL - data_denmark$BEV - data_denmark$PHEV


data_denmark$bev_share   <- data_denmark$`Electric/zero-emission`/data_denmark$overall
data_denmark$ice_share   <- data_denmark$Fossil/data_denmark$overall
data_denmark$hybrid_share<- data_denmark$Hybrid/data_denmark$overall
data_denmark$other_share <- data_denmark$`Other fuel`/data_denmark$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_denmark$pure_ICE    <- (data_denmark$Fossil)/data_denmark$overall

lastyearly <- ceiling(max(subset(data_denmark, data_denmark$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_denmark_monthly <- subset(data_denmark, data_denmark$time_interval=="monthly")
data_denmark_monthly <- subset(data_denmark_monthly, data_denmark_monthly$year>=min(data_denmark_monthly$year+1))
TTM_shares_denmark <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_denmark_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_denmark_monthly)), "%Y-%m"),
  Other = data_denmark_monthly$`Other TTM`,
  #`ICE` = data_denmark_monthly$`ICE TTM`,#-data_denmark_monthly$`HEV TTM`,
  Petrol = data_denmark_monthly$`Petrol TTM`,
  Diesel = data_denmark_monthly$`Diesel TTM`,
  #Ethanol = data_denmark_monthly$`Ethanol TTM`,
  #FlexFuel = data_denmark_monthly$`FlexFuel TTM`,
  #HEV = data_denmark_monthly$`HEV TTM`,
  PHEV = data_denmark_monthly$`PHEV TTM`,
  BEV = data_denmark_monthly$`BEV TTM`
)

colnames(TTM_shares_denmark)[colnames(TTM_shares_denmark) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_denmark)[colnames(TTM_shares_denmark) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_denmark)[colnames(TTM_shares_denmark) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_denmark)[colnames(TTM_shares_denmark) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_denmark)[colnames(TTM_shares_denmark) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_denmark_long <- TTM_shares_denmark %>%
...
TTM_barplot_denmark <- ggplot(TTM_shares_denmark_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_denmark_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_denmark_long$month[substr(TTM_shares_denmark_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Denmark",
...
print(TTM_barplot_denmark)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_denmark$year)))
extrapol <- 2200
data_denmark <- subset(data_denmark, data_denmark$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_denmark$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_denmark$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_denmark$`Electric/zero-emission`))
for(i in 1:length(data_denmark$`Electric/zero-emission`)){
  data_denmark_looped <- data_denmark[1:i,]
  xg<-data_denmark_looped$year; yg<-as.double(data_denmark_looped$bev_share)
  ...
  xg<-data_denmark_looped$year; yg<-as.double(data_denmark_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  denmark <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_denmark_looped$overall; new_A$time_interval <- data_denmark_looped$time_interval
  
  denmark$Hybrid       <- pmax(denmark$Hybrid, 0)
  denmark$Hybrid_upper <- pmax(denmark$Hybrid_upper, 0)
  denmark$Hybrid_lower <- pmax(denmark$Hybrid_lower, 0)
  
  time_80_denmark <- max(subset(denmark, denmark$BEV<=0.8 & denmark$BEV >= 0.2)$x)
  time_50_denmark <- max(subset(denmark, denmark$BEV<=0.5 & denmark$BEV >= 0.2)$x)
  time_20_denmark <- max(subset(denmark, denmark$BEV<=0.2 & denmark$BEV >= 0.1)$x)
  time_20_to_80_denmark <- max(subset(denmark, denmark$BEV<=0.8 & denmark$BEV >= 0.2)$x)-min(subset(denmark, denmark$BEV<=0.8 & denmark$BEV >= 0.2)$x)
  time_80_to_20_denmark <- max(subset(denmark, denmark$ICE<=0.8 & denmark$ICE >= 0.2)$x)-min(subset(denmark, denmark$ICE<=0.8 & denmark$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_denmark; ice_time[i] <- time_80_to_20_denmark
}

timer_denmark <- data.frame("year"=data_denmark[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_denmark$BEV_time == -Inf), which(timer_denmark$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_denmark) > (last_inf_index + 12)) {
  timer_denmark_short <- timer_denmark[(last_inf_index + 1 + 12):nrow(timer_denmark), ]
} else {
  timer_denmark_short <- timer_denmark
}
...
plot_timer <- ggplot(timer_denmark_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_denmark_short$BEV_time[length(timer_denmark_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Denmark transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_denmark_short$BEV_time[length(timer_denmark_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_denmark <- ggplot(denmark, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Denmark - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_denmark), " years ", round(12*(time_20_to_80_denmark-floor(time_20_to_80_denmark)),0), " months"),
...
plot_denmark <- plot_denmark + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(denmark, denmark$x==2024+counter & denmark$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_denmark <- plot_denmark + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(denmark, denmark$x==2024+counter & denmark$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_denmark <- plot_denmark + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_denmark

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_denmark$overall)
BEV <- data.frame(BEV, "overall"=data_denmark$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_denmark$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_denmark <- ggplot(denmark, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Denmark - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_denmark), " years ",
      round(12 * (time_80_to_20_denmark - floor(time_80_to_20_denmark)), 0),
      " months"
    ),
...
plot_ICE_BEV_denmark <- plot_ICE_BEV_denmark +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(denmark, denmark$x == 2024 + counter - 1 & denmark$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_denmark <- plot_ICE_BEV_denmark +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(denmark, denmark$x == 2024 + counter & denmark$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_denmark)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("denmark_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_denmark,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("denmark_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_denmark,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("denmark_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("denmark_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_denmark,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_denmark",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_denmark$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_denmark, "Inf"),
  bev_50 = as_date_from_numeric(time_50_denmark, "Inf"),
  bev_80 = as_date_from_numeric(time_80_denmark, "Inf"),
  bev_20_80 = time_20_to_80_denmark,
  ice_80_20 = time_80_to_20_denmark,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_denmark$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_denmark)
month_label <- last_label_from_data(data_denmark, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_denmark$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_denmark)) nz(tail(data_denmark$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_denmark)) {
    nz(tail(data_denmark$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_denmark)) nz(tail(data_denmark$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_denmark)) {
    nz(tail(data_denmark$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_denmark),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_denmark)
upsert_weight(
