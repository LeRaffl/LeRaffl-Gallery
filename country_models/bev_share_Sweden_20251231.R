#### Sweden: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Sweden"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/sweden.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_sweden <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Sweden")
source <- data_sweden$Source[1]
...
#data_sweden$BEV <- data_sweden$`Electric/zero-emission`
#data_sweden$PHEV <- data_sweden$Hybrid
#data_sweden$TOTAL <- data_sweden$overall
#data_sweden$OTHERS <- data_sweden$`Other fuel`
#data_sweden$TOTAL - data_sweden$BEV - data_sweden$PHEV <- data_sweden$Fossil

data_sweden$`Electric/zero-emission` <- data_sweden$BEV
data_sweden$Hybrid <- data_sweden$PHEV
data_sweden$overall <- data_sweden$TOTAL
data_sweden$`Other fuel` <- data_sweden$OTHERS
data_sweden$Fossil <- data_sweden$TOTAL - data_sweden$BEV - data_sweden$PHEV


data_sweden$bev_share   <- data_sweden$`Electric/zero-emission`/data_sweden$overall
data_sweden$ice_share   <- data_sweden$Fossil/data_sweden$overall
data_sweden$hybrid_share<- data_sweden$Hybrid/data_sweden$overall
data_sweden$other_share <- data_sweden$`Other fuel`/data_sweden$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_sweden$pure_ICE    <- (data_sweden$Fossil)/data_sweden$overall

lastyearly <- ceiling(max(subset(data_sweden, data_sweden$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_sweden_monthly <- subset(data_sweden, data_sweden$time_interval=="monthly")
data_sweden_monthly <- subset(data_sweden_monthly, data_sweden_monthly$year>=min(data_sweden_monthly$year+1))
TTM_shares_sweden <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_sweden_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_sweden_monthly)), "%Y-%m"),
  Other = data_sweden_monthly$`Other TTM`,
  #`ICE` = data_sweden_monthly$`ICE TTM`,#-data_sweden_monthly$`HEV TTM`,
  Petrol = data_sweden_monthly$`Petrol TTM`,
  Diesel = data_sweden_monthly$`Diesel TTM`,
  #Ethanol = data_sweden_monthly$`Ethanol TTM`,
  FlexFuel = data_sweden_monthly$`FlexFuel TTM`,
  HEV = data_sweden_monthly$`HEV TTM`,
  PHEV = data_sweden_monthly$`PHEV TTM`,
  BEV = data_sweden_monthly$`BEV TTM`
)

colnames(TTM_shares_sweden)[colnames(TTM_shares_sweden) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_sweden)[colnames(TTM_shares_sweden) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_sweden)[colnames(TTM_shares_sweden) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_sweden)[colnames(TTM_shares_sweden) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_sweden)[colnames(TTM_shares_sweden) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_sweden_long <- TTM_shares_sweden %>%
...
TTM_barplot_sweden <- ggplot(TTM_shares_sweden_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_sweden_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_sweden_long$month[substr(TTM_shares_sweden_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Sweden",
...
print(TTM_barplot_sweden)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_sweden$year)))
extrapol <- 2200
data_sweden <- subset(data_sweden, data_sweden$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_sweden$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_sweden$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_sweden$`Electric/zero-emission`))
for(i in 1:length(data_sweden$`Electric/zero-emission`)){
  data_sweden_looped <- data_sweden[1:i,]
  xg<-data_sweden_looped$year; yg<-as.double(data_sweden_looped$bev_share)
  ...
  xg<-data_sweden_looped$year; yg<-as.double(data_sweden_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  sweden <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_sweden_looped$overall; new_A$time_interval <- data_sweden_looped$time_interval
  
  sweden$Hybrid       <- pmax(sweden$Hybrid, 0)
  sweden$Hybrid_upper <- pmax(sweden$Hybrid_upper, 0)
  sweden$Hybrid_lower <- pmax(sweden$Hybrid_lower, 0)
  
  time_80_sweden <- max(subset(sweden, sweden$BEV<=0.8 & sweden$BEV >= 0.2)$x)
  time_50_sweden <- max(subset(sweden, sweden$BEV<=0.5 & sweden$BEV >= 0.2)$x)
  time_20_sweden <- max(subset(sweden, sweden$BEV<=0.2 & sweden$BEV >= 0.1)$x)
  time_20_to_80_sweden <- max(subset(sweden, sweden$BEV<=0.8 & sweden$BEV >= 0.2)$x)-min(subset(sweden, sweden$BEV<=0.8 & sweden$BEV >= 0.2)$x)
  time_80_to_20_sweden <- max(subset(sweden, sweden$ICE<=0.8 & sweden$ICE >= 0.2)$x)-min(subset(sweden, sweden$ICE<=0.8 & sweden$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_sweden; ice_time[i] <- time_80_to_20_sweden
}

timer_sweden <- data.frame("year"=data_sweden[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_sweden$BEV_time == -Inf), which(timer_sweden$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_sweden) > (last_inf_index + 12)) {
  timer_sweden_short <- timer_sweden[(last_inf_index + 1 + 12):nrow(timer_sweden), ]
} else {
  timer_sweden_short <- timer_sweden
}
...
plot_timer <- ggplot(timer_sweden_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_sweden_short$BEV_time[length(timer_sweden_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Sweden transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_sweden_short$BEV_time[length(timer_sweden_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_sweden <- ggplot(sweden, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Sweden - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_sweden), " years ", round(12*(time_20_to_80_sweden-floor(time_20_to_80_sweden)),0), " months"),
...
plot_sweden <- plot_sweden + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(sweden, sweden$x==2024+counter & sweden$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_sweden <- plot_sweden + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(sweden, sweden$x==2024+counter & sweden$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_sweden <- plot_sweden + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_sweden

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_sweden$overall)
BEV <- data.frame(BEV, "overall"=data_sweden$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_sweden$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_sweden <- ggplot(sweden, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Sweden - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_sweden), " years ",
      round(12 * (time_80_to_20_sweden - floor(time_80_to_20_sweden)), 0),
      " months"
    ),
...
plot_ICE_BEV_sweden <- plot_ICE_BEV_sweden +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(sweden, sweden$x == 2024 + counter - 1 & sweden$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_sweden <- plot_ICE_BEV_sweden +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(sweden, sweden$x == 2024 + counter & sweden$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_sweden)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("sweden_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_sweden,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_sweden",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("sweden_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_sweden,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_sweden",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("sweden_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_sweden",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("sweden_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_sweden,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_sweden",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_sweden$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_sweden, "Inf"),
  bev_50 = as_date_from_numeric(time_50_sweden, "Inf"),
  bev_80 = as_date_from_numeric(time_80_sweden, "Inf"),
  bev_20_80 = time_20_to_80_sweden,
  ice_80_20 = time_80_to_20_sweden,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_sweden$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_sweden)
month_label <- last_label_from_data(data_sweden, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_sweden$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_sweden)) nz(tail(data_sweden$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_sweden)) {
    nz(tail(data_sweden$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_sweden)) nz(tail(data_sweden$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_sweden)) {
    nz(tail(data_sweden$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_sweden),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_sweden)
upsert_weight(
