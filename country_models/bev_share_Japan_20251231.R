#### Japan: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Japan"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/japan.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_japan <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Japan")
source <- data_japan$Source[1]
...
#data_japan$BEV <- data_japan$`Electric/zero-emission`
#data_japan$PHEV <- data_japan$Hybrid
#data_japan$TOTAL <- data_japan$overall
#data_japan$OTHERS <- data_japan$`Other fuel`
#data_japan$TOTAL - data_japan$BEV - data_japan$PHEV <- data_japan$Fossil

data_japan$`Electric/zero-emission` <- data_japan$BEV
data_japan$Hybrid <- data_japan$PHEV
data_japan$overall <- data_japan$TOTAL
data_japan$`Other fuel` <- data_japan$OTHERS
data_japan$Fossil <- data_japan$TOTAL - data_japan$BEV - data_japan$PHEV


data_japan$bev_share   <- data_japan$`Electric/zero-emission`/data_japan$overall
data_japan$ice_share   <- data_japan$Fossil/data_japan$overall
data_japan$hybrid_share<- data_japan$Hybrid/data_japan$overall
data_japan$other_share <- data_japan$`Other fuel`/data_japan$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_japan$pure_ICE    <- (data_japan$Fossil)/data_japan$overall

lastyearly <- ceiling(max(subset(data_japan, data_japan$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_japan_monthly <- subset(data_japan, data_japan$time_interval=="monthly")
data_japan_monthly <- subset(data_japan_monthly, data_japan_monthly$year>=min(data_japan_monthly$year+1))
TTM_shares_japan <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_japan_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_japan_monthly)), "%Y-%m"),
  Other = data_japan_monthly$`Other TTM`,
  #`ICE` = data_japan_monthly$`ICE TTM`,#-data_japan_monthly$`HEV TTM`,
  Petrol = data_japan_monthly$`Petrol TTM`,
  Diesel = data_japan_monthly$`Diesel TTM`,
  #Ethanol = data_japan_monthly$`Ethanol TTM`,
  #FlexFuel = data_japan_monthly$`FlexFuel TTM`,
  HEV = data_japan_monthly$`HEV TTM`,
  PHEV = data_japan_monthly$`PHEV TTM`,
  BEV = data_japan_monthly$`BEV TTM`
)

colnames(TTM_shares_japan)[colnames(TTM_shares_japan) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_japan)[colnames(TTM_shares_japan) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_japan)[colnames(TTM_shares_japan) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_japan)[colnames(TTM_shares_japan) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_japan)[colnames(TTM_shares_japan) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_japan_long <- TTM_shares_japan %>%
...
TTM_barplot_japan <- ggplot(TTM_shares_japan_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_japan_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_japan_long$month[substr(TTM_shares_japan_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Japan",
...
print(TTM_barplot_japan)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_japan$year)))
extrapol <- 2200
data_japan <- subset(data_japan, data_japan$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_japan$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_japan$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_japan$`Electric/zero-emission`))
for(i in 1:length(data_japan$`Electric/zero-emission`)){
  data_japan_looped <- data_japan[1:i,]
  xg<-data_japan_looped$year; yg<-as.double(data_japan_looped$bev_share)
  ...
  xg<-data_japan_looped$year; yg<-as.double(data_japan_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  japan <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_japan_looped$overall; new_A$time_interval <- data_japan_looped$time_interval
  
  japan$Hybrid       <- pmax(japan$Hybrid, 0)
  japan$Hybrid_upper <- pmax(japan$Hybrid_upper, 0)
  japan$Hybrid_lower <- pmax(japan$Hybrid_lower, 0)
  
  time_80_japan <- max(subset(japan, japan$BEV<=0.8 & japan$BEV >= 0.2)$x)
  time_50_japan <- max(subset(japan, japan$BEV<=0.5 & japan$BEV >= 0.2)$x)
  time_20_japan <- max(subset(japan, japan$BEV<=0.2 & japan$BEV >= 0.1)$x)
  time_20_to_80_japan <- max(subset(japan, japan$BEV<=0.8 & japan$BEV >= 0.2)$x)-min(subset(japan, japan$BEV<=0.8 & japan$BEV >= 0.2)$x)
  time_80_to_20_japan <- max(subset(japan, japan$ICE<=0.8 & japan$ICE >= 0.2)$x)-min(subset(japan, japan$ICE<=0.8 & japan$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_japan; ice_time[i] <- time_80_to_20_japan
}

timer_japan <- data.frame("year"=data_japan[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_japan$BEV_time == -Inf), which(timer_japan$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_japan) > (last_inf_index + 12)) {
  timer_japan_short <- timer_japan[(last_inf_index + 1 + 12):nrow(timer_japan), ]
} else {
  timer_japan_short <- timer_japan
}
...
# 
# plot_timer <- ggplot(timer_japan_short, aes(x = year)) +
...
#     limits = c(0, timer_japan_short$BEV_time[length(timer_japan_short$BEV_time)]*2)
#   ) +
#   
#   labs(
#     title = "Time expectation for Japan transition time using historical data",
...
#     ymax = 0.3*timer_japan_short$BEV_time[length(timer_japan_short$BEV_time)]*2,
#     ymin = 0
#   )
# 
# plot_timer
...
theme_set(theme_minimal(base_size = 14))
plot_japan <- ggplot(japan, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Japan - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_japan), " years ", round(12*(time_20_to_80_japan-floor(time_20_to_80_japan)),0), " months"),
...
plot_japan <- plot_japan + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(japan, japan$x==2024+counter & japan$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_japan <- plot_japan + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(japan, japan$x==2024+counter & japan$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_japan <- plot_japan + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_japan

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_japan$overall)
BEV <- data.frame(BEV, "overall"=data_japan$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_japan$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_japan <- ggplot(japan, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Japan - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_japan), " years ",
      round(12 * (time_80_to_20_japan - floor(time_80_to_20_japan)), 0),
      " months"
    ),
...
plot_ICE_BEV_japan <- plot_ICE_BEV_japan +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(japan, japan$x == 2024 + counter - 1 & japan$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_japan <- plot_ICE_BEV_japan +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(japan, japan$x == 2024 + counter & japan$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_japan)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("japan_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_japan,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_japan",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("japan_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_japan,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_japan",
  repo_dir = repo_dir_local,
...
#ggsave_git(
#  filename = paste0("japan_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
#  plot = plot_timer,
#  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_japan",
#  repo_dir = repo_dir_local,
#  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("japan_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_japan,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_japan",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_japan$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_japan, "Inf"),
  bev_50 = as_date_from_numeric(time_50_japan, "Inf"),
  bev_80 = as_date_from_numeric(time_80_japan, "Inf"),
  bev_20_80 = time_20_to_80_japan,
  ice_80_20 = time_80_to_20_japan,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_japan$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_japan)
month_label <- last_label_from_data(data_japan, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_japan$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_japan)) nz(tail(data_japan$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_japan)) {
    nz(tail(data_japan$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_japan)) nz(tail(data_japan$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_japan)) {
    nz(tail(data_japan$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_japan),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_japan)
upsert_weight(
