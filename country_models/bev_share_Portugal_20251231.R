#### Portugal: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Portugal"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/portugal.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_portugal <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Portugal")
source <- data_portugal$Source[1]
...
#data_portugal$BEV <- data_portugal$`Electric/zero-emission`
#data_portugal$PHEV <- data_portugal$Hybrid
#data_portugal$TOTAL <- data_portugal$overall
#data_portugal$OTHERS <- data_portugal$`Other fuel`
#data_portugal$TOTAL - data_portugal$BEV - data_portugal$PHEV <- data_portugal$Fossil

data_portugal$`Electric/zero-emission` <- data_portugal$BEV
data_portugal$Hybrid <- data_portugal$PHEV
data_portugal$overall <- data_portugal$TOTAL
data_portugal$`Other fuel` <- data_portugal$OTHERS
data_portugal$Fossil <- data_portugal$TOTAL - data_portugal$BEV - data_portugal$PHEV


data_portugal$bev_share   <- data_portugal$`Electric/zero-emission`/data_portugal$overall
data_portugal$ice_share   <- data_portugal$Fossil/data_portugal$overall
data_portugal$hybrid_share<- data_portugal$Hybrid/data_portugal$overall
data_portugal$other_share <- data_portugal$`Other fuel`/data_portugal$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_portugal$pure_ICE    <- (data_portugal$Fossil)/data_portugal$overall

lastyearly <- ceiling(max(subset(data_portugal, data_portugal$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_portugal_monthly <- subset(data_portugal, data_portugal$time_interval=="monthly")
data_portugal_monthly <- subset(data_portugal_monthly, data_portugal_monthly$year>=min(data_portugal_monthly$year+1))
TTM_shares_portugal <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_portugal_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_portugal_monthly)), "%Y-%m"),
  Other = data_portugal_monthly$`Other TTM`,
  #`ICE` = data_portugal_monthly$`ICE TTM`,#-data_portugal_monthly$`HEV TTM`,
  Petrol = data_portugal_monthly$`Petrol TTM`,
  Diesel = data_portugal_monthly$`Diesel TTM`,
  #Ethanol = data_portugal_monthly$`Ethanol TTM`,
  #FlexFuel = data_portugal_monthly$`FlexFuel TTM`,
  HEV = data_portugal_monthly$`HEV TTM`,
  PHEV = data_portugal_monthly$`PHEV TTM`,
  BEV = data_portugal_monthly$`BEV TTM`
)

colnames(TTM_shares_portugal)[colnames(TTM_shares_portugal) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_portugal)[colnames(TTM_shares_portugal) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_portugal)[colnames(TTM_shares_portugal) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_portugal)[colnames(TTM_shares_portugal) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_portugal)[colnames(TTM_shares_portugal) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_portugal_long <- TTM_shares_portugal %>%
...
TTM_barplot_portugal <- ggplot(TTM_shares_portugal_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_portugal_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_portugal_long$month[substr(TTM_shares_portugal_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Portugal",
...
print(TTM_barplot_portugal)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_portugal$year)))
extrapol <- 2200
data_portugal <- subset(data_portugal, data_portugal$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_portugal$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_portugal$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_portugal$`Electric/zero-emission`))
for(i in 1:length(data_portugal$`Electric/zero-emission`)){
  data_portugal_looped <- data_portugal[1:i,]
  xg<-data_portugal_looped$year; yg<-as.double(data_portugal_looped$bev_share)
  ...
  xg<-data_portugal_looped$year; yg<-as.double(data_portugal_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  portugal <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_portugal_looped$overall; new_A$time_interval <- data_portugal_looped$time_interval
  
  portugal$Hybrid       <- pmax(portugal$Hybrid, 0)
  portugal$Hybrid_upper <- pmax(portugal$Hybrid_upper, 0)
  portugal$Hybrid_lower <- pmax(portugal$Hybrid_lower, 0)
  
  time_80_portugal <- max(subset(portugal, portugal$BEV<=0.8 & portugal$BEV >= 0.2)$x)
  time_50_portugal <- max(subset(portugal, portugal$BEV<=0.5 & portugal$BEV >= 0.2)$x)
  time_20_portugal <- max(subset(portugal, portugal$BEV<=0.2 & portugal$BEV >= 0.1)$x)
  time_20_to_80_portugal <- max(subset(portugal, portugal$BEV<=0.8 & portugal$BEV >= 0.2)$x)-min(subset(portugal, portugal$BEV<=0.8 & portugal$BEV >= 0.2)$x)
  time_80_to_20_portugal <- max(subset(portugal, portugal$ICE<=0.8 & portugal$ICE >= 0.2)$x)-min(subset(portugal, portugal$ICE<=0.8 & portugal$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_portugal; ice_time[i] <- time_80_to_20_portugal
}

timer_portugal <- data.frame("year"=data_portugal[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_portugal$BEV_time == -Inf), which(timer_portugal$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_portugal) > (last_inf_index + 12)) {
  timer_portugal_short <- timer_portugal[(last_inf_index + 1 + 12):nrow(timer_portugal), ]
} else {
  timer_portugal_short <- timer_portugal
}
...
plot_timer <- ggplot(timer_portugal_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_portugal_short$BEV_time[length(timer_portugal_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Portugal transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_portugal_short$BEV_time[length(timer_portugal_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_portugal <- ggplot(portugal, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Portugal - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_portugal), " years ", round(12*(time_20_to_80_portugal-floor(time_20_to_80_portugal)),0), " months"),
...
plot_portugal <- plot_portugal + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(portugal, portugal$x==2024+counter & portugal$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_portugal <- plot_portugal + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(portugal, portugal$x==2024+counter & portugal$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_portugal <- plot_portugal + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_portugal

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_portugal$overall)
BEV <- data.frame(BEV, "overall"=data_portugal$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_portugal$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_portugal <- ggplot(portugal, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Portugal - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_portugal), " years ",
      round(12 * (time_80_to_20_portugal - floor(time_80_to_20_portugal)), 0),
      " months"
    ),
...
plot_ICE_BEV_portugal <- plot_ICE_BEV_portugal +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(portugal, portugal$x == 2024 + counter - 1 & portugal$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_portugal <- plot_ICE_BEV_portugal +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(portugal, portugal$x == 2024 + counter & portugal$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_portugal)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("portugal_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_portugal,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_portugal",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("portugal_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_portugal,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_portugal",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("portugal_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_portugal",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("portugal_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_portugal,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_portugal",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_portugal$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_portugal, "Inf"),
  bev_50 = as_date_from_numeric(time_50_portugal, "Inf"),
  bev_80 = as_date_from_numeric(time_80_portugal, "Inf"),
  bev_20_80 = time_20_to_80_portugal,
  ice_80_20 = time_80_to_20_portugal,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_portugal$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_portugal)
month_label <- last_label_from_data(data_portugal, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_portugal$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_portugal)) nz(tail(data_portugal$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_portugal)) {
    nz(tail(data_portugal$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_portugal)) nz(tail(data_portugal$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_portugal)) {
    nz(tail(data_portugal$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_portugal),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_portugal)
upsert_weight(
