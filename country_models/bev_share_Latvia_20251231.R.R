#### Croatia: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Croatia"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/croatia.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_croatia <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Croatia")
source <- data_croatia$Source[1]
...
#data_croatia$BEV <- data_croatia$`Electric/zero-emission`
#data_croatia$PHEV <- data_croatia$Hybrid
#data_croatia$TOTAL <- data_croatia$overall
#data_croatia$OTHERS <- data_croatia$`Other fuel`
#data_croatia$TOTAL - data_croatia$BEV - data_croatia$PHEV <- data_croatia$Fossil

data_croatia$`Electric/zero-emission` <- data_croatia$BEV
data_croatia$Hybrid <- data_croatia$PHEV
data_croatia$overall <- data_croatia$TOTAL
data_croatia$`Other fuel` <- data_croatia$OTHERS
data_croatia$Fossil <- data_croatia$TOTAL - data_croatia$BEV - data_croatia$PHEV


data_croatia$bev_share   <- data_croatia$`Electric/zero-emission`/data_croatia$overall
data_croatia$ice_share   <- data_croatia$Fossil/data_croatia$overall
data_croatia$hybrid_share<- data_croatia$Hybrid/data_croatia$overall
data_croatia$other_share <- data_croatia$`Other fuel`/data_croatia$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_croatia$pure_ICE    <- (data_croatia$Fossil)/data_croatia$overall

lastyearly <- ceiling(max(subset(data_croatia, data_croatia$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_croatia_monthly <- subset(data_croatia, data_croatia$time_interval=="monthly")
data_croatia_monthly <- subset(data_croatia_monthly, data_croatia_monthly$year>=min(data_croatia_monthly$year+1))
TTM_shares_croatia <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_croatia_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_croatia_monthly)), "%Y-%m"),
  Other = data_croatia_monthly$`Other TTM`,
  #`ICE` = data_croatia_monthly$`ICE TTM`,#-data_croatia_monthly$`HEV TTM`,
  Petrol = data_croatia_monthly$`Petrol TTM`,
  Diesel = data_croatia_monthly$`Diesel TTM`,
  #Ethanol = data_croatia_monthly$`Ethanol TTM`,
  #FlexFuel = data_croatia_monthly$`FlexFuel TTM`,
  HEV = data_croatia_monthly$`HEV TTM`,
  PHEV = data_croatia_monthly$`PHEV TTM`,
  BEV = data_croatia_monthly$`BEV TTM`
)

colnames(TTM_shares_croatia)[colnames(TTM_shares_croatia) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_croatia)[colnames(TTM_shares_croatia) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_croatia)[colnames(TTM_shares_croatia) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_croatia)[colnames(TTM_shares_croatia) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_croatia)[colnames(TTM_shares_croatia) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_croatia_long <- TTM_shares_croatia %>%
...
TTM_barplot_croatia <- ggplot(TTM_shares_croatia_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_croatia_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_croatia_long$month[substr(TTM_shares_croatia_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Croatia",
...
print(TTM_barplot_croatia)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_croatia$year)))
extrapol <- 2200
data_croatia <- subset(data_croatia, data_croatia$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_croatia$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_croatia$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_croatia$`Electric/zero-emission`))
for(i in 1:length(data_croatia$`Electric/zero-emission`)){
  data_croatia_looped <- data_croatia[1:i,]
  xg<-data_croatia_looped$year; yg<-as.double(data_croatia_looped$bev_share)
  ...
  xg<-data_croatia_looped$year; yg<-as.double(data_croatia_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  croatia <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_croatia_looped$overall; new_A$time_interval <- data_croatia_looped$time_interval
  
  croatia$Hybrid       <- pmax(croatia$Hybrid, 0)
  croatia$Hybrid_upper <- pmax(croatia$Hybrid_upper, 0)
  croatia$Hybrid_lower <- pmax(croatia$Hybrid_lower, 0)
  
  time_80_croatia <- max(subset(croatia, croatia$BEV<=0.8 & croatia$BEV >= 0.2)$x)
  time_50_croatia <- max(subset(croatia, croatia$BEV<=0.5 & croatia$BEV >= 0.2)$x)
  time_20_croatia <- max(subset(croatia, croatia$BEV<=0.2 & croatia$BEV >= 0.1)$x)
  time_20_to_80_croatia <- max(subset(croatia, croatia$BEV<=0.8 & croatia$BEV >= 0.2)$x)-min(subset(croatia, croatia$BEV<=0.8 & croatia$BEV >= 0.2)$x)
  time_80_to_20_croatia <- max(subset(croatia, croatia$ICE<=0.8 & croatia$ICE >= 0.2)$x)-min(subset(croatia, croatia$ICE<=0.8 & croatia$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_croatia; ice_time[i] <- time_80_to_20_croatia
}

timer_croatia <- data.frame("year"=data_croatia[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_croatia$BEV_time == -Inf), which(timer_croatia$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_croatia) > (last_inf_index + 12)) {
  timer_croatia_short <- timer_croatia[(last_inf_index + 1 + 12):nrow(timer_croatia), ]
} else {
  timer_croatia_short <- timer_croatia
}
...
plot_timer <- ggplot(timer_croatia_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_croatia_short$BEV_time[length(timer_croatia_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Croatia transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_croatia_short$BEV_time[length(timer_croatia_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_croatia <- ggplot(croatia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Croatia - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_croatia), " years ", round(12*(time_20_to_80_croatia-floor(time_20_to_80_croatia)),0), " months"),
...
plot_croatia <- plot_croatia + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(croatia, croatia$x==2024+counter & croatia$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_croatia <- plot_croatia + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                            label=paste0("Jan ",2025+counter,": ",round(subset(croatia, croatia$x==2024+counter & croatia$Type=="New Registrations")$BEV*100, 1),"%"),
                                            size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_croatia <- plot_croatia + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                     width = unit(1*1920/1280
                                                                                  ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_croatia

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_croatia$overall)
BEV <- data.frame(BEV, "overall"=data_croatia$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_croatia$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_croatia <- ggplot(croatia, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Croatia - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_croatia), " years ",
      round(12 * (time_80_to_20_croatia - floor(time_80_to_20_croatia)), 0),
      " months"
    ),
...
plot_ICE_BEV_croatia <- plot_ICE_BEV_croatia +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(croatia, croatia$x == 2024 + counter - 1 & croatia$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_croatia <- plot_ICE_BEV_croatia +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(croatia, croatia$x == 2024 + counter & croatia$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_croatia)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("croatia_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_croatia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_croatia",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("croatia_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_croatia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_croatia",
  repo_dir = repo_dir_local,
...
#ggsave_git(
#  filename = paste0("croatia_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
#  plot = plot_timer,
#  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_croatia",
#  repo_dir = repo_dir_local,
#  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("croatia_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_croatia,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_croatia",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_croatia$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_croatia, "Inf"),
  bev_50 = as_date_from_numeric(time_50_croatia, "Inf"),
  bev_80 = as_date_from_numeric(time_80_croatia, "Inf"),
  bev_20_80 = time_20_to_80_croatia,
  ice_80_20 = time_80_to_20_croatia,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_croatia$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_croatia)
month_label <- last_label_from_data(data_croatia, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_croatia$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_croatia)) nz(tail(data_croatia$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_croatia)) {
    nz(tail(data_croatia$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_croatia)) nz(tail(data_croatia$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_croatia)) {
    nz(tail(data_croatia$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_croatia),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_croatia)
upsert_weight(
