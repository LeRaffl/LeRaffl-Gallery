#### Italy: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Italy"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/italy.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_italy <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Italy")
source <- data_italy$Source[1]
...
#data_italy$BEV <- data_italy$`Electric/zero-emission`
#data_italy$PHEV <- data_italy$Hybrid
#data_italy$TOTAL <- data_italy$overall
#data_italy$OTHERS <- data_italy$`Other fuel`
#data_italy$TOTAL - data_italy$BEV - data_italy$PHEV <- data_italy$Fossil

data_italy$`Electric/zero-emission` <- data_italy$BEV
data_italy$Hybrid <- data_italy$PHEV
data_italy$overall <- data_italy$TOTAL
data_italy$`Other fuel` <- data_italy$OTHERS
data_italy$Fossil <- data_italy$TOTAL - data_italy$BEV - data_italy$PHEV


data_italy$bev_share   <- data_italy$`Electric/zero-emission`/data_italy$overall
data_italy$ice_share   <- data_italy$Fossil/data_italy$overall
data_italy$hybrid_share<- data_italy$Hybrid/data_italy$overall
data_italy$other_share <- data_italy$`Other fuel`/data_italy$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_italy$pure_ICE    <- (data_italy$Fossil)/data_italy$overall

lastyearly <- ceiling(max(subset(data_italy, data_italy$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_italy_monthly <- subset(data_italy, data_italy$time_interval=="monthly")
data_italy_monthly <- subset(data_italy_monthly, data_italy_monthly$year>=min(data_italy_monthly$year+1))
TTM_shares_italy <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_italy_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_italy_monthly)), "%Y-%m"),
  Other = data_italy_monthly$`Other TTM`,
  #`ICE` = data_italy_monthly$`ICE TTM`,#-data_italy_monthly$`HEV TTM`,
  Petrol = data_italy_monthly$`Petrol TTM`,
  Diesel = data_italy_monthly$`Diesel TTM`,
  #Ethanol = data_italy_monthly$`Ethanol TTM`,
  #FlexFuel = data_italy_monthly$`FlexFuel TTM`,
  HEV = data_italy_monthly$`HEV TTM`,
  PHEV = data_italy_monthly$`PHEV TTM`,
  BEV = data_italy_monthly$`BEV TTM`
)

colnames(TTM_shares_italy)[colnames(TTM_shares_italy) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_italy)[colnames(TTM_shares_italy) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_italy)[colnames(TTM_shares_italy) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_italy)[colnames(TTM_shares_italy) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_italy)[colnames(TTM_shares_italy) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_italy_long <- TTM_shares_italy %>%
...
TTM_barplot_italy <- ggplot(TTM_shares_italy_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_italy_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_italy_long$month[substr(TTM_shares_italy_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Italy",
...
print(TTM_barplot_italy)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_italy$year)))
extrapol <- 2200
data_italy <- subset(data_italy, data_italy$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_italy$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_italy$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_italy$`Electric/zero-emission`))
for(i in 1:length(data_italy$`Electric/zero-emission`)){
  data_italy_looped <- data_italy[1:i,]
  xg<-data_italy_looped$year; yg<-as.double(data_italy_looped$bev_share)
  ...
  xg<-data_italy_looped$year; yg<-as.double(data_italy_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  italy <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_italy_looped$overall; new_A$time_interval <- data_italy_looped$time_interval
  
  italy$Hybrid       <- pmax(italy$Hybrid, 0)
  italy$Hybrid_upper <- pmax(italy$Hybrid_upper, 0)
  italy$Hybrid_lower <- pmax(italy$Hybrid_lower, 0)
  
  time_80_italy <- max(subset(italy, italy$BEV<=0.8 & italy$BEV >= 0.2)$x)
  time_50_italy <- max(subset(italy, italy$BEV<=0.5 & italy$BEV >= 0.2)$x)
  time_20_italy <- max(subset(italy, italy$BEV<=0.2 & italy$BEV >= 0.1)$x)
  time_20_to_80_italy <- max(subset(italy, italy$BEV<=0.8 & italy$BEV >= 0.2)$x)-min(subset(italy, italy$BEV<=0.8 & italy$BEV >= 0.2)$x)
  time_80_to_20_italy <- max(subset(italy, italy$ICE<=0.8 & italy$ICE >= 0.2)$x)-min(subset(italy, italy$ICE<=0.8 & italy$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_italy; ice_time[i] <- time_80_to_20_italy
}

timer_italy <- data.frame("year"=data_italy[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_italy$BEV_time == -Inf), which(timer_italy$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_italy) > (last_inf_index + 12)) {
  timer_italy_short <- timer_italy[(last_inf_index + 1 + 12):nrow(timer_italy), ]
} else {
  timer_italy_short <- timer_italy
}
...
plot_timer <- ggplot(timer_italy_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_italy_short$BEV_time[length(timer_italy_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Italy transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_italy_short$BEV_time[length(timer_italy_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_italy <- ggplot(italy, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Italy - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_italy), " years ", round(12*(time_20_to_80_italy-floor(time_20_to_80_italy)),0), " months"),
...
plot_italy <- plot_italy + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(italy, italy$x==2024+counter & italy$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_italy <- plot_italy + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                                  label=paste0("Jan ",2025+counter,": ",round(subset(italy, italy$x==2024+counter & italy$Type=="New Registrations")$BEV*100, 1),"%"),
                                                  size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_italy <- plot_italy + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                           width = unit(1*1920/1280
                                                                                        ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_italy

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_italy$overall)
BEV <- data.frame(BEV, "overall"=data_italy$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_italy$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_italy <- ggplot(italy, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Italy - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_italy), " years ",
      round(12 * (time_80_to_20_italy - floor(time_80_to_20_italy)), 0),
      " months"
    ),
...
plot_ICE_BEV_italy <- plot_ICE_BEV_italy +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(italy, italy$x == 2024 + counter - 1 & italy$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_italy <- plot_ICE_BEV_italy +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(italy, italy$x == 2024 + counter & italy$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_italy)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("italy_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_italy,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_italy",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("italy_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_italy,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_italy",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("italy_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_italy",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("italy_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_italy,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_italy",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_italy$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_italy, "Inf"),
  bev_50 = as_date_from_numeric(time_50_italy, "Inf"),
  bev_80 = as_date_from_numeric(time_80_italy, "Inf"),
  bev_20_80 = time_20_to_80_italy,
  ice_80_20 = time_80_to_20_italy,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_italy$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_italy)
month_label <- last_label_from_data(data_italy, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_italy$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_italy)) nz(tail(data_italy$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_italy)) {
    nz(tail(data_italy$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_italy)) nz(tail(data_italy$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_italy)) {
    nz(tail(data_italy$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_italy),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_italy)
upsert_weight(
