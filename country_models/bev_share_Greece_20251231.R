#### Greece: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Greece"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/greece.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_greece <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Greece")
source <- data_greece$Source[1]
...
#data_greece$BEV <- data_greece$`Electric/zero-emission`
#data_greece$PHEV <- data_greece$Hybrid
#data_greece$TOTAL <- data_greece$overall
#data_greece$OTHERS <- data_greece$`Other fuel`
#data_greece$TOTAL - data_greece$BEV - data_greece$PHEV <- data_greece$Fossil

data_greece$`Electric/zero-emission` <- data_greece$BEV
data_greece$Hybrid <- data_greece$PHEV
data_greece$overall <- data_greece$TOTAL
data_greece$`Other fuel` <- data_greece$OTHERS
data_greece$Fossil <- data_greece$TOTAL - data_greece$BEV - data_greece$PHEV


data_greece$bev_share   <- data_greece$`Electric/zero-emission`/data_greece$overall
data_greece$ice_share   <- data_greece$Fossil/data_greece$overall
data_greece$hybrid_share<- data_greece$Hybrid/data_greece$overall
data_greece$other_share <- data_greece$`Other fuel`/data_greece$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_greece$pure_ICE    <- (data_greece$Fossil)/data_greece$overall

lastyearly <- ceiling(max(subset(data_greece, data_greece$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_greece_monthly <- subset(data_greece, data_greece$time_interval=="monthly")
data_greece_monthly <- subset(data_greece_monthly, data_greece_monthly$year>=min(data_greece_monthly$year+1))
TTM_shares_greece <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_greece_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_greece_monthly)), "%Y-%m"),
  Other = data_greece_monthly$`Other TTM`,
  #`ICE` = data_greece_monthly$`ICE TTM`,#-data_greece_monthly$`HEV TTM`,
  Petrol = data_greece_monthly$`Petrol TTM`,
  Diesel = data_greece_monthly$`Diesel TTM`,
  #Ethanol = data_greece_monthly$`Ethanol TTM`,
  #FlexFuel = data_greece_monthly$`FlexFuel TTM`,
  HEV = data_greece_monthly$`HEV TTM`,
  PHEV = data_greece_monthly$`PHEV TTM`,
  BEV = data_greece_monthly$`BEV TTM`
)

colnames(TTM_shares_greece)[colnames(TTM_shares_greece) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_greece)[colnames(TTM_shares_greece) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_greece)[colnames(TTM_shares_greece) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_greece)[colnames(TTM_shares_greece) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_greece)[colnames(TTM_shares_greece) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_greece_long <- TTM_shares_greece %>%
...
TTM_barplot_greece <- ggplot(TTM_shares_greece_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_greece_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_greece_long$month[substr(TTM_shares_greece_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Greece",
...
print(TTM_barplot_greece)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_greece$year)))
extrapol <- 2200
data_greece <- subset(data_greece, data_greece$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_greece$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_greece$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_greece$`Electric/zero-emission`))
for(i in 1:length(data_greece$`Electric/zero-emission`)){
  data_greece_looped <- data_greece[1:i,]
  xg<-data_greece_looped$year; yg<-as.double(data_greece_looped$bev_share)
  ...
  xg<-data_greece_looped$year; yg<-as.double(data_greece_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  greece <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_greece_looped$overall; new_A$time_interval <- data_greece_looped$time_interval
  
  greece$Hybrid       <- pmax(greece$Hybrid, 0)
  greece$Hybrid_upper <- pmax(greece$Hybrid_upper, 0)
  greece$Hybrid_lower <- pmax(greece$Hybrid_lower, 0)
  
  time_80_greece <- max(subset(greece, greece$BEV<=0.8 & greece$BEV >= 0.2)$x)
  time_50_greece <- max(subset(greece, greece$BEV<=0.5 & greece$BEV >= 0.2)$x)
  time_20_greece <- max(subset(greece, greece$BEV<=0.2 & greece$BEV >= 0.1)$x)
  time_20_to_80_greece <- max(subset(greece, greece$BEV<=0.8 & greece$BEV >= 0.2)$x)-min(subset(greece, greece$BEV<=0.8 & greece$BEV >= 0.2)$x)
  time_80_to_20_greece <- max(subset(greece, greece$ICE<=0.8 & greece$ICE >= 0.2)$x)-min(subset(greece, greece$ICE<=0.8 & greece$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_greece; ice_time[i] <- time_80_to_20_greece
}

timer_greece <- data.frame("year"=data_greece[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_greece$BEV_time == -Inf), which(timer_greece$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_greece) > (last_inf_index + 12)) {
  timer_greece_short <- timer_greece[(last_inf_index + 1 + 12):nrow(timer_greece), ]
} else {
  timer_greece_short <- timer_greece
}
...
plot_timer <- ggplot(timer_greece_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_greece_short$BEV_time[length(timer_greece_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Greece transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_greece_short$BEV_time[length(timer_greece_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_greece <- ggplot(greece, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Greece - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_greece), " years ", round(12*(time_20_to_80_greece-floor(time_20_to_80_greece)),0), " months"),
...
plot_greece <- plot_greece + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(greece, greece$x==2024+counter & greece$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_greece <- plot_greece + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(greece, greece$x==2024+counter & greece$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_greece <- plot_greece + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_greece

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_greece$overall)
BEV <- data.frame(BEV, "overall"=data_greece$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_greece$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_greece <- ggplot(greece, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Greece - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_greece), " years ",
      round(12 * (time_80_to_20_greece - floor(time_80_to_20_greece)), 0),
      " months"
    ),
...
plot_ICE_BEV_greece <- plot_ICE_BEV_greece +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(greece, greece$x == 2024 + counter - 1 & greece$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_greece <- plot_ICE_BEV_greece +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(greece, greece$x == 2024 + counter & greece$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_greece)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("greece_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_greece,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_greece",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("greece_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_greece,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_greece",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("greece_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_greece",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("greece_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_greece,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_greece",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_greece$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_greece, "Inf"),
  bev_50 = as_date_from_numeric(time_50_greece, "Inf"),
  bev_80 = as_date_from_numeric(time_80_greece, "Inf"),
  bev_20_80 = time_20_to_80_greece,
  ice_80_20 = time_80_to_20_greece,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_greece$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_greece)
month_label <- last_label_from_data(data_greece, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_greece$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_greece)) nz(tail(data_greece$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_greece)) {
    nz(tail(data_greece$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_greece)) nz(tail(data_greece$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_greece)) {
    nz(tail(data_greece$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_greece),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_greece)
upsert_weight(
