#### France: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "France"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/france.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_france <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "France")
source <- data_france$Source[1]
...
#data_france$BEV <- data_france$`Electric/zero-emission`
#data_france$PHEV <- data_france$Hybrid
#data_france$TOTAL <- data_france$overall
#data_france$OTHERS <- data_france$`Other fuel`
#data_france$TOTAL - data_france$BEV - data_france$PHEV <- data_france$Fossil

data_france$`Electric/zero-emission` <- data_france$BEV
data_france$Hybrid <- data_france$PHEV
data_france$overall <- data_france$TOTAL
data_france$`Other fuel` <- data_france$OTHERS
data_france$Fossil <- data_france$TOTAL - data_france$BEV - data_france$PHEV


data_france$bev_share   <- data_france$`Electric/zero-emission`/data_france$overall
data_france$ice_share   <- data_france$Fossil/data_france$overall
data_france$hybrid_share<- data_france$Hybrid/data_france$overall
data_france$other_share <- data_france$`Other fuel`/data_france$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_france$pure_ICE    <- (data_france$Fossil)/data_france$overall

lastyearly <- ceiling(max(subset(data_france, data_france$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_france_monthly <- subset(data_france, data_france$time_interval=="monthly")
data_france_monthly <- subset(data_france_monthly, data_france_monthly$year>=min(data_france_monthly$year+1))
TTM_shares_france <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_france_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_france_monthly)), "%Y-%m"),
  Other = data_france_monthly$`Other TTM`,
  #`ICE` = data_france_monthly$`ICE TTM`,#-data_france_monthly$`HEV TTM`,
  Petrol = data_france_monthly$`Petrol TTM`,
  Diesel = data_france_monthly$`Diesel TTM`,
  #Ethanol = data_france_monthly$`Ethanol TTM`,
  #FlexFuel = data_france_monthly$`FlexFuel TTM`,
  HEV = data_france_monthly$`HEV TTM`,
  PHEV = data_france_monthly$`PHEV TTM`,
  BEV = data_france_monthly$`BEV TTM`
)

colnames(TTM_shares_france)[colnames(TTM_shares_france) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_france)[colnames(TTM_shares_france) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_france)[colnames(TTM_shares_france) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_france)[colnames(TTM_shares_france) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_france)[colnames(TTM_shares_france) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_france_long <- TTM_shares_france %>%
...
TTM_barplot_france <- ggplot(TTM_shares_france_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_france_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_france_long$month[substr(TTM_shares_france_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in France",
...
print(TTM_barplot_france)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_france$year)))
extrapol <- 2200
data_france <- subset(data_france, data_france$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_france$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_france$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_france$`Electric/zero-emission`))
for(i in 1:length(data_france$`Electric/zero-emission`)){
  data_france_looped <- data_france[1:i,]
  xg<-data_france_looped$year; yg<-as.double(data_france_looped$bev_share)
  ...
  xg<-data_france_looped$year; yg<-as.double(data_france_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  france <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_france_looped$overall; new_A$time_interval <- data_france_looped$time_interval
  
  france$Hybrid       <- pmax(france$Hybrid, 0)
  france$Hybrid_upper <- pmax(france$Hybrid_upper, 0)
  france$Hybrid_lower <- pmax(france$Hybrid_lower, 0)
  
  time_80_france <- max(subset(france, france$BEV<=0.8 & france$BEV >= 0.2)$x)
  time_50_france <- max(subset(france, france$BEV<=0.5 & france$BEV >= 0.2)$x)
  time_20_france <- max(subset(france, france$BEV<=0.2 & france$BEV >= 0.1)$x)
  time_20_to_80_france <- max(subset(france, france$BEV<=0.8 & france$BEV >= 0.2)$x)-min(subset(france, france$BEV<=0.8 & france$BEV >= 0.2)$x)
  time_80_to_20_france <- max(subset(france, france$ICE<=0.8 & france$ICE >= 0.2)$x)-min(subset(france, france$ICE<=0.8 & france$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_france; ice_time[i] <- time_80_to_20_france
}

timer_france <- data.frame("year"=data_france[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_france$BEV_time == -Inf), which(timer_france$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_france) > (last_inf_index + 12)) {
  timer_france_short <- timer_france[(last_inf_index + 1 + 12):nrow(timer_france), ]
} else {
  timer_france_short <- timer_france
}
...
plot_timer <- ggplot(timer_france_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_france_short$BEV_time[length(timer_france_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for France transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_france_short$BEV_time[length(timer_france_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_france <- ggplot(france, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in France - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_france), " years ", round(12*(time_20_to_80_france-floor(time_20_to_80_france)),0), " months"),
...
plot_france <- plot_france + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(france, france$x==2024+counter & france$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_france <- plot_france + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                          label=paste0("Jan ",2025+counter,": ",round(subset(france, france$x==2024+counter & france$Type=="New Registrations")$BEV*100, 1),"%"),
                                          size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_france <- plot_france + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                   width = unit(1*1920/1280
                                                                                ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_france

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_france$overall)
BEV <- data.frame(BEV, "overall"=data_france$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_france$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_france <- ggplot(france, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in France - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_france), " years ",
      round(12 * (time_80_to_20_france - floor(time_80_to_20_france)), 0),
      " months"
    ),
...
plot_ICE_BEV_france <- plot_ICE_BEV_france +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(france, france$x == 2024 + counter - 1 & france$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_france <- plot_ICE_BEV_france +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(france, france$x == 2024 + counter & france$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_france)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("france_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_france,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_france",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("france_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_france,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_france",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("france_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_france",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("france_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_france,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_france",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_france$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_france, "Inf"),
  bev_50 = as_date_from_numeric(time_50_france, "Inf"),
  bev_80 = as_date_from_numeric(time_80_france, "Inf"),
  bev_20_80 = time_20_to_80_france,
  ice_80_20 = time_80_to_20_france,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_france$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_france)
month_label <- last_label_from_data(data_france, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_france$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_france)) nz(tail(data_france$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_france)) {
    nz(tail(data_france$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_france)) nz(tail(data_france$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_france)) {
    nz(tail(data_france$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_france),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_france)
upsert_weight(
