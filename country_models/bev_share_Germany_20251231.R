#### Germany: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Germany"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/germany.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_germany <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Germany")
source <- data_germany$Source[1]
...
#data_germany$BEV <- data_germany$`Electric/zero-emission`
#data_germany$PHEV <- data_germany$Hybrid
#data_germany$TOTAL <- data_germany$overall
#data_germany$OTHERS <- data_germany$`Other fuel`
#data_germany$TOTAL - data_germany$BEV - data_germany$PHEV <- data_germany$Fossil

data_germany$`Electric/zero-emission` <- data_germany$BEV
data_germany$Hybrid <- data_germany$PHEV
data_germany$overall <- data_germany$TOTAL
data_germany$`Other fuel` <- data_germany$OTHERS
data_germany$Fossil <- data_germany$TOTAL - data_germany$BEV - data_germany$PHEV


data_germany$bev_share   <- data_germany$`Electric/zero-emission`/data_germany$overall
data_germany$ice_share   <- data_germany$Fossil/data_germany$overall
data_germany$hybrid_share<- data_germany$Hybrid/data_germany$overall
data_germany$other_share <- data_germany$`Other fuel`/data_germany$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_germany$pure_ICE    <- (data_germany$Fossil)/data_germany$overall

lastyearly <- ceiling(max(subset(data_germany, data_germany$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_germany_monthly <- subset(data_germany, data_germany$time_interval=="monthly")
data_germany_monthly <- subset(data_germany_monthly, data_germany_monthly$year>=min(data_germany_monthly$year+1))
TTM_shares_germany <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_germany_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_germany_monthly)), "%Y-%m"),
  Other = data_germany_monthly$`Other TTM`,
  #`ICE` = data_germany_monthly$`ICE TTM`,#-data_germany_monthly$`HEV TTM`,
  Petrol = data_germany_monthly$`Petrol TTM`,
  Diesel = data_germany_monthly$`Diesel TTM`,
  #Ethanol = data_germany_monthly$`Ethanol TTM`,
  #FlexFuel = data_germany_monthly$`FlexFuel TTM`,
  HEV = data_germany_monthly$`HEV TTM`,
  PHEV = data_germany_monthly$`PHEV TTM`,
  BEV = data_germany_monthly$`BEV TTM`
)

colnames(TTM_shares_germany)[colnames(TTM_shares_germany) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_germany)[colnames(TTM_shares_germany) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_germany)[colnames(TTM_shares_germany) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_germany)[colnames(TTM_shares_germany) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_germany)[colnames(TTM_shares_germany) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_germany_long <- TTM_shares_germany %>%
...
TTM_barplot_germany <- ggplot(TTM_shares_germany_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_germany_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_germany_long$month[substr(TTM_shares_germany_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Germany",
...
print(TTM_barplot_germany)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_germany$year)))
extrapol <- 2200
data_germany <- subset(data_germany, data_germany$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_germany$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_germany$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_germany$`Electric/zero-emission`))
for(i in 1:length(data_germany$`Electric/zero-emission`)){
  data_germany_looped <- data_germany[1:i,]
  xg<-data_germany_looped$year; yg<-as.double(data_germany_looped$bev_share)
  ...
  xg<-data_germany_looped$year; yg<-as.double(data_germany_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  germany <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_germany_looped$overall; new_A$time_interval <- data_germany_looped$time_interval
  
  germany$Hybrid       <- pmax(germany$Hybrid, 0)
  germany$Hybrid_upper <- pmax(germany$Hybrid_upper, 0)
  germany$Hybrid_lower <- pmax(germany$Hybrid_lower, 0)
  
  time_80_germany <- max(subset(germany, germany$BEV<=0.8 & germany$BEV >= 0.2)$x)
  time_50_germany <- max(subset(germany, germany$BEV<=0.5 & germany$BEV >= 0.2)$x)
  time_20_germany <- max(subset(germany, germany$BEV<=0.2 & germany$BEV >= 0.1)$x)
  time_20_to_80_germany <- max(subset(germany, germany$BEV<=0.8 & germany$BEV >= 0.2)$x)-min(subset(germany, germany$BEV<=0.8 & germany$BEV >= 0.2)$x)
  time_80_to_20_germany <- max(subset(germany, germany$ICE<=0.8 & germany$ICE >= 0.2)$x)-min(subset(germany, germany$ICE<=0.8 & germany$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_germany; ice_time[i] <- time_80_to_20_germany
}

timer_germany <- data.frame("year"=data_germany[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_germany$BEV_time == -Inf), which(timer_germany$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_germany) > (last_inf_index + 12)) {
  timer_germany_short <- timer_germany[(last_inf_index + 1 + 12):nrow(timer_germany), ]
} else {
  timer_germany_short <- timer_germany
}
...
plot_timer <- ggplot(timer_germany_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_germany_short$BEV_time[length(timer_germany_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Germany transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_germany_short$BEV_time[length(timer_germany_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_germany <- ggplot(germany, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Germany - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_germany), " years ", round(12*(time_20_to_80_germany-floor(time_20_to_80_germany)),0), " months"),
...
plot_germany <- plot_germany + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(germany, germany$x==2024+counter & germany$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_germany <- plot_germany + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(germany, germany$x==2024+counter & germany$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_germany <- plot_germany + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_germany

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_germany$overall)
BEV <- data.frame(BEV, "overall"=data_germany$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_germany$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_germany <- ggplot(germany, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Germany - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_germany), " years ",
      round(12 * (time_80_to_20_germany - floor(time_80_to_20_germany)), 0),
      " months"
    ),
...
plot_ICE_BEV_germany <- plot_ICE_BEV_germany +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(germany, germany$x == 2024 + counter - 1 & germany$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_germany <- plot_ICE_BEV_germany +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(germany, germany$x == 2024 + counter & germany$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_germany)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("germany_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_germany,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("germany_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_germany,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("germany_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("germany_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_germany,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_germany",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_germany$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_germany, "Inf"),
  bev_50 = as_date_from_numeric(time_50_germany, "Inf"),
  bev_80 = as_date_from_numeric(time_80_germany, "Inf"),
  bev_20_80 = time_20_to_80_germany,
  ice_80_20 = time_80_to_20_germany,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_germany$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_germany)
month_label <- last_label_from_data(data_germany, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_germany$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_germany)) nz(tail(data_germany$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_germany)) {
    nz(tail(data_germany$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_germany)) nz(tail(data_germany$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_germany)) {
    nz(tail(data_germany$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_germany),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_germany)
upsert_weight(
