#### Lithuania: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Lithuania"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/lithuania.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_lithuania <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Lithuania")
source <- data_lithuania$Source[1]
...
#data_lithuania$BEV <- data_lithuania$`Electric/zero-emission`
#data_lithuania$PHEV <- data_lithuania$Hybrid
#data_lithuania$TOTAL <- data_lithuania$overall
#data_lithuania$OTHERS <- data_lithuania$`Other fuel`
#data_lithuania$TOTAL - data_lithuania$BEV - data_lithuania$PHEV <- data_lithuania$Fossil

data_lithuania$`Electric/zero-emission` <- data_lithuania$BEV
data_lithuania$Hybrid <- data_lithuania$PHEV
data_lithuania$overall <- data_lithuania$TOTAL
data_lithuania$`Other fuel` <- data_lithuania$OTHERS
data_lithuania$Fossil <- data_lithuania$TOTAL - data_lithuania$BEV - data_lithuania$PHEV


data_lithuania$bev_share   <- data_lithuania$`Electric/zero-emission`/data_lithuania$overall
data_lithuania$ice_share   <- data_lithuania$Fossil/data_lithuania$overall
data_lithuania$hybrid_share<- data_lithuania$Hybrid/data_lithuania$overall
data_lithuania$other_share <- data_lithuania$`Other fuel`/data_lithuania$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_lithuania$pure_ICE    <- (data_lithuania$Fossil)/data_lithuania$overall

lastyearly <- ceiling(max(subset(data_lithuania, data_lithuania$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_lithuania_monthly <- subset(data_lithuania, data_lithuania$time_interval=="monthly")
data_lithuania_monthly <- subset(data_lithuania_monthly, data_lithuania_monthly$year>=min(data_lithuania_monthly$year+1))
TTM_shares_lithuania <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_lithuania_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_lithuania_monthly)), "%Y-%m"),
  Other = data_lithuania_monthly$`Other TTM`,
  #`ICE` = data_lithuania_monthly$`ICE TTM`,#-data_lithuania_monthly$`HEV TTM`,
  Petrol = data_lithuania_monthly$`Petrol TTM`,
  Diesel = data_lithuania_monthly$`Diesel TTM`,
  #Ethanol = data_lithuania_monthly$`Ethanol TTM`,
  #FlexFuel = data_lithuania_monthly$`FlexFuel TTM`,
  HEV = data_lithuania_monthly$`HEV TTM`,
  PHEV = data_lithuania_monthly$`PHEV TTM`,
  BEV = data_lithuania_monthly$`BEV TTM`
)

colnames(TTM_shares_lithuania)[colnames(TTM_shares_lithuania) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_lithuania)[colnames(TTM_shares_lithuania) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_lithuania)[colnames(TTM_shares_lithuania) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_lithuania)[colnames(TTM_shares_lithuania) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_lithuania)[colnames(TTM_shares_lithuania) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_lithuania_long <- TTM_shares_lithuania %>%
...
TTM_barplot_lithuania <- ggplot(TTM_shares_lithuania_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_lithuania_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_lithuania_long$month[substr(TTM_shares_lithuania_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Lithuania",
...
print(TTM_barplot_lithuania)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_lithuania$year)))
extrapol <- 2200
data_lithuania <- subset(data_lithuania, data_lithuania$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_lithuania$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_lithuania$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_lithuania$`Electric/zero-emission`))
for(i in 1:length(data_lithuania$`Electric/zero-emission`)){
  data_lithuania_looped <- data_lithuania[1:i,]
  xg<-data_lithuania_looped$year; yg<-as.double(data_lithuania_looped$bev_share)
  ...
  xg<-data_lithuania_looped$year; yg<-as.double(data_lithuania_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  lithuania <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_lithuania_looped$overall; new_A$time_interval <- data_lithuania_looped$time_interval
  
  lithuania$Hybrid       <- pmax(lithuania$Hybrid, 0)
  lithuania$Hybrid_upper <- pmax(lithuania$Hybrid_upper, 0)
  lithuania$Hybrid_lower <- pmax(lithuania$Hybrid_lower, 0)
  
  time_80_lithuania <- max(subset(lithuania, lithuania$BEV<=0.8 & lithuania$BEV >= 0.2)$x)
  time_50_lithuania <- max(subset(lithuania, lithuania$BEV<=0.5 & lithuania$BEV >= 0.2)$x)
  time_20_lithuania <- max(subset(lithuania, lithuania$BEV<=0.2 & lithuania$BEV >= 0.1)$x)
  time_20_to_80_lithuania <- max(subset(lithuania, lithuania$BEV<=0.8 & lithuania$BEV >= 0.2)$x)-min(subset(lithuania, lithuania$BEV<=0.8 & lithuania$BEV >= 0.2)$x)
  time_80_to_20_lithuania <- max(subset(lithuania, lithuania$ICE<=0.8 & lithuania$ICE >= 0.2)$x)-min(subset(lithuania, lithuania$ICE<=0.8 & lithuania$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_lithuania; ice_time[i] <- time_80_to_20_lithuania
}

timer_lithuania <- data.frame("year"=data_lithuania[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_lithuania$BEV_time == -Inf), which(timer_lithuania$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_lithuania) > (last_inf_index + 12)) {
  timer_lithuania_short <- timer_lithuania[(last_inf_index + 1 + 12):nrow(timer_lithuania), ]
} else {
  timer_lithuania_short <- timer_lithuania
}
...
plot_timer <- ggplot(timer_lithuania_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_lithuania_short$BEV_time[length(timer_lithuania_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Lithuania transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_lithuania_short$BEV_time[length(timer_lithuania_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_lithuania <- ggplot(lithuania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Lithuania - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_lithuania), " years ", round(12*(time_20_to_80_lithuania-floor(time_20_to_80_lithuania)),0), " months"),
...
plot_lithuania <- plot_lithuania + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(lithuania, lithuania$x==2024+counter & lithuania$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_lithuania <- plot_lithuania + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                        label=paste0("Jan ",2025+counter,": ",round(subset(lithuania, lithuania$x==2024+counter & lithuania$Type=="New Registrations")$BEV*100, 1),"%"),
                                        size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_lithuania <- plot_lithuania + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                 width = unit(1*1920/1280
                                                                              ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_lithuania

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_lithuania$overall)
BEV <- data.frame(BEV, "overall"=data_lithuania$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_lithuania$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_lithuania <- ggplot(lithuania, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Lithuania - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_lithuania), " years ",
      round(12 * (time_80_to_20_lithuania - floor(time_80_to_20_lithuania)), 0),
      " months"
    ),
...
plot_ICE_BEV_lithuania <- plot_ICE_BEV_lithuania +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(lithuania, lithuania$x == 2024 + counter - 1 & lithuania$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_lithuania <- plot_ICE_BEV_lithuania +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(lithuania, lithuania$x == 2024 + counter & lithuania$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_lithuania)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("lithuania_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_lithuania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_lithuania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("lithuania_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_lithuania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_lithuania",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("lithuania_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_lithuania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("lithuania_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_lithuania,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_lithuania",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_lithuania$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_lithuania, "Inf"),
  bev_50 = as_date_from_numeric(time_50_lithuania, "Inf"),
  bev_80 = as_date_from_numeric(time_80_lithuania, "Inf"),
  bev_20_80 = time_20_to_80_lithuania,
  ice_80_20 = time_80_to_20_lithuania,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_lithuania$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_lithuania)
month_label <- last_label_from_data(data_lithuania, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_lithuania$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_lithuania)) nz(tail(data_lithuania$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_lithuania)) {
    nz(tail(data_lithuania$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_lithuania)) nz(tail(data_lithuania$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_lithuania)) {
    nz(tail(data_lithuania$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_lithuania),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_lithuania)
upsert_weight(
