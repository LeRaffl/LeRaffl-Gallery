#### Ireland: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Ireland"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/ireland.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_ireland <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Ireland")
source <- data_ireland$Source[1]
...
#data_ireland$BEV <- data_ireland$`Electric/zero-emission`
#data_ireland$PHEV <- data_ireland$Hybrid
#data_ireland$TOTAL <- data_ireland$overall
#data_ireland$OTHERS <- data_ireland$`Other fuel`
#data_ireland$TOTAL - data_ireland$BEV - data_ireland$PHEV <- data_ireland$Fossil

data_ireland$`Electric/zero-emission` <- data_ireland$BEV
data_ireland$Hybrid <- data_ireland$PHEV
data_ireland$overall <- data_ireland$TOTAL
data_ireland$`Other fuel` <- data_ireland$OTHERS
data_ireland$Fossil <- data_ireland$TOTAL - data_ireland$BEV - data_ireland$PHEV


data_ireland$bev_share   <- data_ireland$`Electric/zero-emission`/data_ireland$overall
data_ireland$ice_share   <- data_ireland$Fossil/data_ireland$overall
data_ireland$hybrid_share<- data_ireland$Hybrid/data_ireland$overall
data_ireland$other_share <- data_ireland$`Other fuel`/data_ireland$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_ireland$pure_ICE    <- (data_ireland$Fossil)/data_ireland$overall

lastyearly <- ceiling(max(subset(data_ireland, data_ireland$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_ireland_monthly <- subset(data_ireland, data_ireland$time_interval=="monthly")
data_ireland_monthly <- subset(data_ireland_monthly, data_ireland_monthly$year>=min(data_ireland_monthly$year+1))
TTM_shares_ireland <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_ireland_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_ireland_monthly)), "%Y-%m"),
  Other = data_ireland_monthly$`Other TTM`,
  #`ICE` = data_ireland_monthly$`ICE TTM`,#-data_ireland_monthly$`HEV TTM`,
  Petrol = data_ireland_monthly$`Petrol TTM`,
  Diesel = data_ireland_monthly$`Diesel TTM`,
  #Ethanol = data_ireland_monthly$`Ethanol TTM`,
  #FlexFuel = data_ireland_monthly$`FlexFuel TTM`,
  HEV = data_ireland_monthly$`HEV TTM`,
  PHEV = data_ireland_monthly$`PHEV TTM`,
  BEV = data_ireland_monthly$`BEV TTM`
)

colnames(TTM_shares_ireland)[colnames(TTM_shares_ireland) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_ireland)[colnames(TTM_shares_ireland) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_ireland)[colnames(TTM_shares_ireland) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_ireland)[colnames(TTM_shares_ireland) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_ireland)[colnames(TTM_shares_ireland) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_ireland_long <- TTM_shares_ireland %>%
...
TTM_barplot_ireland <- ggplot(TTM_shares_ireland_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_ireland_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_ireland_long$month[substr(TTM_shares_ireland_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Ireland",
...
print(TTM_barplot_ireland)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_ireland$year)))
extrapol <- 2200
data_ireland <- subset(data_ireland, data_ireland$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_ireland$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_ireland$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_ireland$`Electric/zero-emission`))
for(i in 1:length(data_ireland$`Electric/zero-emission`)){
  data_ireland_looped <- data_ireland[1:i,]
  xg<-data_ireland_looped$year; yg<-as.double(data_ireland_looped$bev_share)
  ...
  xg<-data_ireland_looped$year; yg<-as.double(data_ireland_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  ireland <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_ireland_looped$overall; new_A$time_interval <- data_ireland_looped$time_interval
  
  ireland$Hybrid       <- pmax(ireland$Hybrid, 0)
  ireland$Hybrid_upper <- pmax(ireland$Hybrid_upper, 0)
  ireland$Hybrid_lower <- pmax(ireland$Hybrid_lower, 0)
  
  time_80_ireland <- max(subset(ireland, ireland$BEV<=0.8 & ireland$BEV >= 0.2)$x)
  time_50_ireland <- max(subset(ireland, ireland$BEV<=0.5 & ireland$BEV >= 0.2)$x)
  time_20_ireland <- max(subset(ireland, ireland$BEV<=0.2 & ireland$BEV >= 0.1)$x)
  time_20_to_80_ireland <- max(subset(ireland, ireland$BEV<=0.8 & ireland$BEV >= 0.2)$x)-min(subset(ireland, ireland$BEV<=0.8 & ireland$BEV >= 0.2)$x)
  time_80_to_20_ireland <- max(subset(ireland, ireland$ICE<=0.8 & ireland$ICE >= 0.2)$x)-min(subset(ireland, ireland$ICE<=0.8 & ireland$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_ireland; ice_time[i] <- time_80_to_20_ireland
}

timer_ireland <- data.frame("year"=data_ireland[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_ireland$BEV_time == -Inf), which(timer_ireland$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_ireland) > (last_inf_index + 12)) {
  timer_ireland_short <- timer_ireland[(last_inf_index + 1 + 12):nrow(timer_ireland), ]
} else {
  timer_ireland_short <- timer_ireland
}
...
plot_timer <- ggplot(timer_ireland_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_ireland_short$BEV_time[length(timer_ireland_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Ireland transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_ireland_short$BEV_time[length(timer_ireland_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_ireland <- ggplot(ireland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Ireland - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_ireland), " years ", round(12*(time_20_to_80_ireland-floor(time_20_to_80_ireland)),0), " months"),
...
plot_ireland <- plot_ireland + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(ireland, ireland$x==2024+counter & ireland$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_ireland <- plot_ireland + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                      label=paste0("Jan ",2025+counter,": ",round(subset(ireland, ireland$x==2024+counter & ireland$Type=="New Registrations")$BEV*100, 1),"%"),
                                      size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_ireland <- plot_ireland + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                               width = unit(1*1920/1280
                                                                            ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_ireland

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_ireland$overall)
BEV <- data.frame(BEV, "overall"=data_ireland$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_ireland$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_ireland <- ggplot(ireland, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Ireland - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_ireland), " years ",
      round(12 * (time_80_to_20_ireland - floor(time_80_to_20_ireland)), 0),
      " months"
    ),
...
plot_ICE_BEV_ireland <- plot_ICE_BEV_ireland +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(ireland, ireland$x == 2024 + counter - 1 & ireland$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_ireland <- plot_ICE_BEV_ireland +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(ireland, ireland$x == 2024 + counter & ireland$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_ireland)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("ireland_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ireland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_ireland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("ireland_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_ireland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_ireland",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("ireland_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_ireland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("ireland_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_ireland,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_ireland",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_ireland$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_ireland, "Inf"),
  bev_50 = as_date_from_numeric(time_50_ireland, "Inf"),
  bev_80 = as_date_from_numeric(time_80_ireland, "Inf"),
  bev_20_80 = time_20_to_80_ireland,
  ice_80_20 = time_80_to_20_ireland,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_ireland$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_ireland)
month_label <- last_label_from_data(data_ireland, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_ireland$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_ireland)) nz(tail(data_ireland$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_ireland)) {
    nz(tail(data_ireland$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_ireland)) nz(tail(data_ireland$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_ireland)) {
    nz(tail(data_ireland$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_ireland),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_ireland)
upsert_weight(
