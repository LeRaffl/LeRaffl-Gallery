#### Uruguay: BEV share, trajectories, TTM etc. + auto‑publish to GitHub ####
country <- "Uruguay"
...
flag_path <- "/Users/leraffl/Projects/bev_assets/flags/uruguay.png"
flag_img  <- readPNG(flag_path)
flag_base64 <- base64enc::dataURI(file = flag_path)

#### read data ####
# googlesheets4::gs4_deauth(); googlesheets4::gs4_auth()
data_uruguay <- read_sheet("https://docs.google.com/spreadsheets/d/1tT_Ja3de_S528_JeSBkj74q-lfEIekE5-GRm9_pWgUo/", sheet = "Uruguay")
source <- data_uruguay$Source[1]
...
#data_uruguay$BEV <- data_uruguay$`Electric/zero-emission`
#data_uruguay$PHEV <- data_uruguay$Hybrid
#data_uruguay$TOTAL <- data_uruguay$overall
#data_uruguay$OTHERS <- data_uruguay$`Other fuel`
#data_uruguay$TOTAL - data_uruguay$BEV - data_uruguay$PHEV <- data_uruguay$Fossil

data_uruguay$`Electric/zero-emission` <- data_uruguay$BEV
data_uruguay$Hybrid <- data_uruguay$PHEV
data_uruguay$overall <- data_uruguay$TOTAL
data_uruguay$`Other fuel` <- data_uruguay$OTHERS
data_uruguay$Fossil <- data_uruguay$TOTAL - data_uruguay$BEV - data_uruguay$PHEV


data_uruguay$bev_share   <- data_uruguay$`Electric/zero-emission`/data_uruguay$overall
data_uruguay$ice_share   <- data_uruguay$Fossil/data_uruguay$overall
data_uruguay$hybrid_share<- data_uruguay$Hybrid/data_uruguay$overall
data_uruguay$other_share <- data_uruguay$`Other fuel`/data_uruguay$overall
# pure ICE ohne HEV-Abzug (deine Wahl)
data_uruguay$pure_ICE    <- (data_uruguay$Fossil)/data_uruguay$overall

lastyearly <- ceiling(max(subset(data_uruguay, data_uruguay$time_interval=="yearly")$year))

#### trailing 12 months data and plot ####
data_uruguay_monthly <- subset(data_uruguay, data_uruguay$time_interval=="monthly")
data_uruguay_monthly <- subset(data_uruguay_monthly, data_uruguay_monthly$year>=min(data_uruguay_monthly$year+1))
TTM_shares_uruguay <- data.frame(
  month = format(seq.Date(from = as.Date(paste0(floor(min(data_uruguay_monthly$year)+1),"-01-01")), by = "month", length.out = nrow(data_uruguay_monthly)), "%Y-%m"),
  Other = data_uruguay_monthly$`Other TTM`,
  #`ICE` = data_uruguay_monthly$`ICE TTM`,#-data_uruguay_monthly$`HEV TTM`,
  Petrol = data_uruguay_monthly$`Petrol TTM`,
  Diesel = data_uruguay_monthly$`Diesel TTM`,
  #Ethanol = data_uruguay_monthly$`Ethanol TTM`,
  #FlexFuel = data_uruguay_monthly$`FlexFuel TTM`,
  HEV = data_uruguay_monthly$`HEV TTM`,
  PHEV = data_uruguay_monthly$`PHEV TTM`,
  BEV = data_uruguay_monthly$`BEV TTM`
)

colnames(TTM_shares_uruguay)[colnames(TTM_shares_uruguay) == "HEV.Petrol"]   <- "HEV Petrol"
colnames(TTM_shares_uruguay)[colnames(TTM_shares_uruguay) == "HEV.Diesel"]   <- "HEV Diesel"
colnames(TTM_shares_uruguay)[colnames(TTM_shares_uruguay) == "PHEV.Petrol"]  <- "PHEV Petrol"
colnames(TTM_shares_uruguay)[colnames(TTM_shares_uruguay) == "PHEV.Diesel"]  <- "PHEV Diesel"
colnames(TTM_shares_uruguay)[colnames(TTM_shares_uruguay) == "ICE..excl..HEV."] <- "ICE (excl. HEV)"

TTM_shares_uruguay_long <- TTM_shares_uruguay %>%
...
TTM_barplot_uruguay <- ggplot(TTM_shares_uruguay_long, aes(x = month, y = value, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 1) +
  geom_vline(
    data = TTM_shares_uruguay_long %>% filter(substr(month, 6, 7) == "01") %>% distinct(numeric_month),
  ...
  scale_x_discrete(
    breaks = TTM_shares_uruguay_long$month[substr(TTM_shares_uruguay_long$month, 6, 7) == "01"],
  ...
  labs(title = "12-Month Trailing Market Shares by Fuel Type in Uruguay",
...
print(TTM_barplot_uruguay)

#### regression setup ####
verschiebung <- floor(min(na.omit(data_uruguay$year)))
extrapol <- 2200
data_uruguay <- subset(data_uruguay, data_uruguay$year>=verschiebung)
...
RSS <- function(v, type="BEV"){ forecast <- reg(v, BEV$x, type); residuals <- BEV$y-forecast; sum((residuals*data_uruguay$overall)^2) }
...
RSS_ice <- function(v, type="ICE"){ forecast <- reg_ice(v, ICE$x, type); residuals <- ICE$y-forecast; sum((residuals*data_uruguay$overall)^2) }

bev_time <- ice_time <- seq(0,0,length=length(data_uruguay$`Electric/zero-emission`))
for(i in 1:length(data_uruguay$`Electric/zero-emission`)){
  data_uruguay_looped <- data_uruguay[1:i,]
  xg<-data_uruguay_looped$year; yg<-as.double(data_uruguay_looped$bev_share)
  ...
  xg<-data_uruguay_looped$year; yg<-as.double(data_uruguay_looped$ice_share); ICE <- data.frame(x=xg,y=yg)
  ...
  uruguay <- data.frame(B, "Type"="New Registrations")
  ...
  new_A$overall <- data_uruguay_looped$overall; new_A$time_interval <- data_uruguay_looped$time_interval
  
  uruguay$Hybrid       <- pmax(uruguay$Hybrid, 0)
  uruguay$Hybrid_upper <- pmax(uruguay$Hybrid_upper, 0)
  uruguay$Hybrid_lower <- pmax(uruguay$Hybrid_lower, 0)
  
  time_80_uruguay <- max(subset(uruguay, uruguay$BEV<=0.8 & uruguay$BEV >= 0.2)$x)
  time_50_uruguay <- max(subset(uruguay, uruguay$BEV<=0.5 & uruguay$BEV >= 0.2)$x)
  time_20_uruguay <- max(subset(uruguay, uruguay$BEV<=0.2 & uruguay$BEV >= 0.1)$x)
  time_20_to_80_uruguay <- max(subset(uruguay, uruguay$BEV<=0.8 & uruguay$BEV >= 0.2)$x)-min(subset(uruguay, uruguay$BEV<=0.8 & uruguay$BEV >= 0.2)$x)
  time_80_to_20_uruguay <- max(subset(uruguay, uruguay$ICE<=0.8 & uruguay$ICE >= 0.2)$x)-min(subset(uruguay, uruguay$ICE<=0.8 & uruguay$ICE >= 0.2)$x)
  bev_time[i] <- time_20_to_80_uruguay; ice_time[i] <- time_80_to_20_uruguay
}

timer_uruguay <- data.frame("year"=data_uruguay[1:length(bev_time),]$year, "BEV_time"=bev_time, "ICE_time"=ice_time)
last_inf_index <- max(c(which(timer_uruguay$BEV_time == -Inf), which(timer_uruguay$ICE_time == -Inf)), na.rm = TRUE)
if (!is.finite(last_inf_index)) last_inf_index <- 0
# DataFrame ab dem letzten -Inf-Wert kürzen, und 12 Monate später
if (nrow(timer_uruguay) > (last_inf_index + 12)) {
  timer_uruguay_short <- timer_uruguay[(last_inf_index + 1 + 12):nrow(timer_uruguay), ]
} else {
  timer_uruguay_short <- timer_uruguay
}
...
plot_timer <- ggplot(timer_uruguay_short, aes(x = year)) +
  ...
  scale_y_continuous(
    ...
    limits = c(0, timer_uruguay_short$BEV_time[length(timer_uruguay_short$BEV_time)]*2)
  ) +
  
  labs(
    title = "Time expectation for Uruguay transition time using historical data",
...
plot_timer <- plot_timer +
  annotation_custom(
    ...
    ymax = 0.3*timer_uruguay_short$BEV_time[length(timer_uruguay_short$BEV_time)]*2,
    ymin = 0
  )
...
theme_set(theme_minimal(base_size = 14))
plot_uruguay <- ggplot(uruguay, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(title = "BEV share in new registrations in Uruguay - an Extrapolation",
       subtitle = paste0("expected time for BEV to rise from 20% to 80%: ",
                         floor(time_20_to_80_uruguay), " years ", round(12*(time_20_to_80_uruguay-floor(time_20_to_80_uruguay)),0), " months"),
...
plot_uruguay <- plot_uruguay + annotate("text", x=2010, y=1, label="New Registration estimates in", size=rel(6),hjust=0, vjust=1, col="red")
counter <- 0
while(round(subset(uruguay, uruguay$x==2024+counter & uruguay$Type=="New Registrations")$BEV*100, 1) < 100 & 1-0.05*(counter+1)>0.1){
  plot_uruguay <- plot_uruguay + annotate("text", x=2010+0.5, y=1-0.05*(counter+1),
                                                  label=paste0("Jan ",2025+counter,": ",round(subset(uruguay, uruguay$x==2024+counter & uruguay$Type=="New Registrations")$BEV*100, 1),"%"),
                                                  size=rel(5),hjust=0, vjust=1, col="red")
  counter <- counter+1
}
plot_uruguay <- plot_uruguay + annotation_custom(grob = rasterGrob(as.raster(flag_img), interpolate = TRUE,
                                                                           width = unit(1*1920/1280
                                                                                        ,"in"), height = unit(1, "in")), xmin = min(extrapol-4, 2045-4), ymin=-0.9)
plot_uruguay

#### Chart: ICE/BEV/PHEV ####
ICE <- data.frame(ICE, "overall"=data_uruguay$overall)
BEV <- data.frame(BEV, "overall"=data_uruguay$overall)
Hybrid <- data.frame(Hybrid, "overall"=data_uruguay$overall)

theme_set(theme_minimal(base_size = 14))

plot_ICE_BEV_uruguay <- ggplot(uruguay, aes(x = x, y = BEV, color = Type)) +
  ...
  labs(
    title = "BEV / ICE / PHEV share of new registrations in Uruguay - an Extrapolation",
    subtitle = paste0(
      "expected time for ICE to drop from 80% to 20%: ",
      floor(time_80_to_20_uruguay), " years ",
      round(12 * (time_80_to_20_uruguay - floor(time_80_to_20_uruguay)), 0),
      " months"
    ),
...
plot_ICE_BEV_uruguay <- plot_ICE_BEV_uruguay +
  annotate(
    "text",
    x = 2010,
    y = 0.9,
...
counter <- 0
while (
  5 < round(subset(uruguay, uruguay$x == 2024 + counter - 1 & uruguay$Type == "New Registrations")$ICE * 100, 1) &
  ...
  plot_ICE_BEV_uruguay <- plot_ICE_BEV_uruguay +
    annotate(
      ...
      label = paste0(
        "Jan ", 2024 + counter + 1, ": ",
        round(subset(uruguay, uruguay$x == 2024 + counter & uruguay$Type == "New Registrations")$ICE * 100, 1), "%"
      ),
      size = rel(5),
      hjust = 0,
      vjust = 1,
...
period_folder <- data_per_from_data(data_uruguay)
updated_to    <- period_folder

# 1) BEV trajectory
ggsave_git(
  filename = paste0("uruguay_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_uruguay,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uruguay",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("uruguay_ICE_BEV_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_ICE_BEV_uruguay,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uruguay",
  repo_dir = repo_dir_local,
...
ggsave_git(
  filename = paste0("uruguay_time_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = plot_timer,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uruguay",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
...
# 4) TTM market split
ggsave_git(
  filename = paste0("uruguay_ttm_shares_", format(Sys.Date(), "%Y%m%d"), ".png"),
  plot = TTM_barplot_uruguay,
  path_icloud = "~/Library/Mobile Documents/com~apple~CloudDocs/R/bev_share_uruguay",
  repo_dir = repo_dir_local,
  period = period_folder,         # <=
  width = 12.80, height = 7.20, units = "in", dpi = 300, bg = "white"
...
upsert_country_result(
  sheet_id = sheet_id,
  country = country,
  bev_ttm = tail(data_uruguay$`BEV TTM`, 1),
  bev_20 = as_date_from_numeric(time_20_uruguay, "Inf"),
  bev_50 = as_date_from_numeric(time_50_uruguay, "Inf"),
  bev_80 = as_date_from_numeric(time_80_uruguay, "Inf"),
  bev_20_80 = time_20_to_80_uruguay,
  ice_80_20 = time_80_to_20_uruguay,
  scale = res$par[1],
  shape = res$par[2],
  movement = verschiebung,
  updated_to = tail(sub("M", "-", data_uruguay$YYYYMMM), n=1)
)
...
last_date <- last_date_from_data(data_uruguay)
month_label <- last_label_from_data(data_uruguay, last_date)
...
# ---------- TTM (letzte Zeile) ----------
bev_ttm <- nz(tail(data_uruguay$`BEV TTM`, 1))

hybrid_ttm <- {
  if (has("Hybrid TTM", data_uruguay)) nz(tail(data_uruguay$`Hybrid TTM`, 1)) else NA_real_
}

phev_ttm <- {
  if (has("PHEV TTM", data_uruguay)) {
    nz(tail(data_uruguay$`PHEV TTM`, 1))
  } else if (is.finite(hybrid_ttm)) {
    NA_real_
  } else 0
}

erev_ttm <- if (has("EREV TTM", data_uruguay)) nz(tail(data_uruguay$`EREV TTM`, 1)) else NA_real_

hev_ttm <- {
  if (has("HEV TTM", data_uruguay)) {
    nz(tail(data_uruguay$`HEV TTM`, 1))
...
row <- data.frame(
  ...
  data_per = data_per_from_data(data_uruguay),
  model_date = format(Sys.Date(), "%Y-%m-%d"),
...
# ------------------ weights.csv updaten ------
weight_total <- compute_weight_from_data(data_uruguay)
upsert_weight(
