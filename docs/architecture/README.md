...
| 35 | [35-proposal-live-trend-charts.md](35-proposal-live-trend-charts.md) | Concept (decisions locked) for a live, configurable **observation** chart — a "Monthly" tab with the trailing window `N` as a slider (1 = raw month … 12 = TTM), backed by a new build-time `series/` artifact. Covers the native-monthly-only cadence policy (contiguous runs drawn as segments, never bridged; Canada + Georgia excluded as quarterly), the rolling-window contract (ratio of sums, never a mean of shares), the Python generator and its `ttm_bev_share` cross-language contract test, CI wiring, the `#tab?query` deep-link change, the "DIY Curves" → "DIY Charts" group rename, and why the static PNGs coexist permanently as the tamper-evident record. |
