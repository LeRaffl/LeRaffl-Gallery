...
| 35 | [35-proposal-live-trend-charts.md](35-proposal-live-trend-charts.md) | Concept for a live, configurable **observation** chart ("Monthly" tab): trailing-window `N` as a slider (1 = raw month … 12 = TTM), backed by a new build-time `series/` artifact that normalises the 10 CSV header shapes, derives the 3-curve rollup once, and marks which periods are genuinely monthly. Covers the rolling-window contract (ratio of sums, never a mean of shares), the Python-vs-R generator call and its `ttm_bev_share` contract test, CI wiring, the `#tab?query` deep-link change, and phasing. |
