...
| 14 | [14-data-source-gaps.md](14-data-source-gaps.md) | Countries investigated but **not** added, and why — Latin America (Mexico, plus the since-solved Argentina and Colombia, and the zemo-la aggregator) and Africa (Ethiopia's 100 %-EV import mandate over a used-import market, + the South Africa/Morocco candidates). The "why isn't X on the map?" reference. |
...
| 36 | [36-source-france.md](36-source-france.md) | Per-country source playbook for France (**SDES** motorisations série VP — registry-derived, replaces the ACEA aggregate). Full énergie split incl. a real HEV column back to **2011**; `Whole` = EU M1 (SUVs in; Vans/HDV/Buses are fast-follow variants). Also the cross-source **reconciliation** — one **SIV** registry behind ACEA/SDES/AAA, the **raw-vs-CVS-CJO** trap (June 2026: 188 482 raw vs 142 100 adjusted), the **AAA = PFA = ACEA** identity, the one-time per-fuel definitional step (MHEV→petrol, the ND bucket) with the **BEV share essentially unchanged vs ACEA** (§3g: a few tenths of a pp, no consistent direction) — the "**what's in `Whole`**" table, the média-id rotation, and the pipeline/rollout (§9). Old series parked as `data/France_legacy.csv`. |
| 37 | [37-fleet-data-harmonization.md](37-fleet-data-harmonization.md) | **Proposal / working spec.** Pulls the hand-curated **Fleet (stock/parc) dataset** (`fleet/fleet_initial.csv`, doc 03 §3.8) onto a harmonized format so its yearly refresh becomes mechanical. Diagnoses the four defects (the `HEV`-vs-`HYBRID` double column, no `source`, no `TOTAL`, ISO codes with the `GE`/`DE` collision); proposes a schema mirroring `data/<Country>.csv` (full names, `variant`, `source`, `TOTAL`) that **adopts the registration category contract** (doc 35 §2 — combined-hybrid→`HEV`+`hev_note`, aggregate-ICE, empty≠0, collapse-to-common-denominator); names the two roll-up **danger flags** (`hev_in_ice`, `ev_contaminated`); and gives the **per-country source catalogue** for all 12 countries (publisher, split capability, extent, quality, scope flags) with the open `⚠ verify` endpoint points. China's `HYBRID` is really PHEV; NL/DK/FI fold HEV into ICE; Georgia's source is unconfirmed. |
| 39 | [39-source-argentina.md](39-source-argentina.md) | Per-country source playbook for Argentina (**DNRPA** *Inscripciones iniciales* open microdata — one record per registration, free CKAN portal). Whole/Private/Industry/Pickups from one download; **no fuel field**, so the powertrain is classified from the model designation — the rule table, its traps ("Hybrid" mild hybrids, codes like T8/330E/DM-i) and the validation against ACARA's electrified totals live here. |
| 40 | [40-source-ukraine.md](40-source-ukraine.md) | Per-country source playbook for Ukraine (**MIA** open vehicle register on data.gov.ua — one record per registration operation, **with a fuel field**). Whole/Private/Industry/Used/Vans from one download; new vs used from the register's own first-registration codes (why the series starts 2018-09), the combined Hybrid bucket, the governance checks every run performs, and the validation against Ukrautoprom (BEV within 1 %). |
| 44 | [44-uncertainty-bands.md](44-uncertainty-bands.md) | **Uncertainty bands (CI / PI / TI).** What the 95 % confidence, prediction and tolerance bands in `bands/<slug>.json` mean (the rewind-and-rerun definition), the maths (inverse-Hessian sandwich with AR(2)-prewhitened scores; empirical month scatter; block bootstrap), the simulation that validates them, how to read and quote them (for people and AIs), the file schema and the open points. Frontend data only — the PNGs are unchanged. |
| 43 | [43-source-south-korea.md](43-source-south-korea.md) | Per-country source playbook for South Korea (**MOTIR** «자동차산업 동향» monthly press release, PDF). Domestic sales of all vehicle types by powertrain — BEV / PHEV / HEV / hydrogen, ICE as the remainder; the three-column (year-earlier / previous / current) table and the newest-publication-wins rule; scope caveats (sales not registrations, commercial vehicles included, imported hybrids incl. MHEV); the validation that found two transcription errors in the hand-entered history; why the KOTSA registration API was not used; debugging runbook. |
| 42 | [42-adding-a-country.md](42-adding-a-country.md) | **Playbook: adding a country.** The complete, ordered checklist from the first CI probe to the first render — scope decisions, fetcher/test/workflow conventions, every file of gallery wiring, Builder groups in both mirrors, the doc set to update, backfilling the backtest/Time-lapse history on the PR branch, the checks to run. Enforced by `scripts/check_country_integration.py` (CI on every PR). |
| 41 | [41-source-hong-kong.md](41-source-hong-kong.md) | Per-country source playbook for Hong Kong (**Transport Department** record-level *Particulars of first registered vehicles* on DATA.GOV.HK — one row per vehicle, with fuel, make, model and TD's own first-registration status). Whole/Used/Vans from one set of monthly files; new vs used-import from TD's status codes (A/B/C1 vs C2); plug-in hybrids classified from the model designation (the fuel field has no hybrid value) with the rule table; the every-run cross-check against TD's aggregate table 4.1(e) (exact over 80 months); the EV-tax deadlines of April 2024 and April 2026; a debugging runbook. |
