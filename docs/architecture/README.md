...
| 29 | [29-expansion-candidates.md](29-expansion-candidates.md) | July-2026 expansion investigation: viable new-country candidates (Israel, India, Morocco, South Africa) with sources and open verification points; shelved regions (Gulf, Jordan, Egypt, Sub-Saharan Africa minus SA); variant leads for existing countries (KBA/SMMT/SDES/OFV/Vahan 2W-3W). |
| 30 | [30-source-israel.md](30-source-israel.md) | Per-country source playbook for Israel (data.gov.il MoT registry, CKAN datastore). Stock-snapshot-derived monthly counts (unpadded month keys, P-scope), and THE trap: regular HEVs are coded as plain petrol — recovered via the WLTP model-catalogue join (trim-level key, majority-vote fallback). Validated against I-VIA. |
