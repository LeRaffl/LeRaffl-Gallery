...
| 35 | [35-proposal-raw-data-tab.md](35-proposal-raw-data-tab.md) | Concept (decisions locked) for a **"Raw Data" tab**: the 51 `Whole` country CSVs drawn as stacked bars, absolute or relative, with a window control summing N periods per bar. No model, no `params.csv`. The substance is the **category contract** — per-row split-ICE vs aggregate-ICE resolution (Chile/Colombia carry vestigial petrol/diesel columns), the combined-hybrid bucket via the existing `hev_note` front-matter, collapse-to-common-denominator before any comparison or aggregation, and an explicit `Unreported` band because **1 070 of 6 849 rows do not close to TOTAL**. Also: native cadence only with contiguous runs as separate blocks, negative residuals failing the build (France, 11 rows), and the `#tab?query` deep-link change. |
