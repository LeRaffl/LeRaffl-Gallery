...
| 36 | [36-source-france.md](36-source-france.md) | **Findings / reconciliation** for France (still on ACEA; not yet page-driving): the ready answer to "why does your France number differ from ACEA / SDES / AAA?" — one **SIV** registry behind them all, differing only by scope, adjustment and energy taxonomy. The **raw-vs-CVS-CJO** trap (June 2026: 188 787 raw vs 142 100 seasonally/working-day-adjusted), the **AAA = PFA = ACEA** identity (exact unit matches Aug-2025 / Jun-2026), the provisional/DOM/rounding residuals, the "**what's in `Whole`** — SUVs yes; pickups/vans/HDV/buses no" reference table, and the **SDES migration plan** (the detailed "Motorisations" xlsx keeps the HEV column; the clean coarse API drops it). |
