...
| 28 | [28-source-thailand.md](28-source-thailand.md) | Per-country source playbook for Thailand (TAI/AIU member portal, `taiapi.thaiauto.or.th:3000` JSON API — cookie-session login, `raw_rows`). Whole/HDV/Buses/3-Wheelers from one call. Documents the pickups-bundled-into-Whole caveat, the Whole rebase + `Thailand_legacy.csv` archive, the 3-Wheelers high-BEV/tiny-volume (not no-transition) quirk, and the port-3000 egress note. |
