...
| [`fetch-acea-cv.yml`](../../.github/workflows/fetch-acea-cv.yml) | 10:15 UTC, 18th → 31st Jan | ACEA **annual** Commercial Vehicle PDF press release ([38-source-acea-cv.md](38-source-acea-cv.md)) | `Vans` + `HDV` + `Buses` for the 19-country ACEA roster; conditional per (country, variant, period) row — skips any file whose existing source isn't `ACEA` | `max(ACEA-sourced year)` across all target files ≥ target |
