...
| [`fetch-ireland.yml`](../../.github/workflows/fetch-ireland.yml) | `0 4 1-5 * *` + `0 13 1-5 * *` | Twice daily 04:00 & 13:00 UTC, 1st → 5th | SIMI motorstats (`stats.simi.ie`, Inertia SPA) | Ireland — single variant (Whole) | `latest_period(Ireland.csv) ≥ target` (trailing-window re-fetch for revisions) |
...
- **Ireland sits at 04:00 & 13:00 UTC** (twice daily, since SIMI publishes very early on the 1st), **Finland at 04:40 UTC**, **Denmark at 05:15 UTC**, **Sweden at 05:50 UTC** and **Netherlands at 06:30 UTC** to clear the 08:00 crowd; **China at 11:00 UTC** to clear it from the other side; **USA at 10:00 UTC** to avoid piling onto the 10th's Brazil window.
- **Day-1 starters** (Japan, Uruguay, China, Netherlands, Denmark, Finland, Sweden; Ireland from the 1st too) rely entirely on the self-throttle to keep the empty days free — they fire 1-15 or 1-31 (Ireland 1-5) times per month but only do real HTTP on the days the source publishes.
- **Date-window starters** (USA 10+, Chile 14+, Türkiye 15+, Netherlands ≤15, Denmark ≤15, Finland ≤15, Sweden ≤15, Ireland ≤5, ACEA 16+) reflect the earliest plausible publication day for the previous month from that source; cutting off the empty days saves a handful of self-throttle checks but doesn't change correctness.

Country coverage of automated fetchers: **see also** [02-components.md](02-components.md#27-fetch-actions-overview) for which CSVs are auto-fed vs hand-maintained. Countries without an entry in the table above (Australia, Austria, Canada, Georgia, Germany, Indonesia, Italy, New Zealand, Portugal, Singapore, South Korea, Thailand, UK — and all conditional-list ACEA countries until their source flips to pure `ACEA`) are maintained manually via the legacy local R pipeline or via public-submit PRs.
