...
## 3.13 Freshness + Arrival Data (`sources/schedule.json`)

### Where

`sources/schedule.json`, generated by [`scripts/build_schedule.py`](../../scripts/build_schedule.py) from the `fetch-*.yml` crons, the data CSVs and `manifest.json`. Read at runtime by the **Data freshness** tab. **Generated — do not hand-edit.**
...
| | answers | source |
|---|---|---|
| `countries[].status` / `last_render` | *"Is this country up to date, and when is the next point due?"* | CSV periods + cron windows + newest manifest date |
| `runs` | *"What landed, and when?"* — one row per (country, render date) | every `manifest.json` entry, deduped on that pair |
