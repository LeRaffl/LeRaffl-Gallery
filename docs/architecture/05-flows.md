...
> Nepal, Albania, New Zealand, Argentina, Ukraine, Hong Kong and South Korea — never got one. They are not undocumented:
...
sequenceDiagram
    ...
    Browser->>Pages: GET params.csv (landing hero chart)
    Pages-->>Browser: CSV
    Browser->>Pages: GET series/index.json (once)
    Pages-->>Browser: JSON
    Browser->>Pages: GET series/<slug>.json (home market, from browser timezone)
    Pages-->>Browser: JSON
    Browser->>Browser: draw inline-SVG hero chart
    ...
    par (on tab switch)
        ...
        Pages-->>Browser: CSV
        Browser->>Pages: GET series/<slug>.json (Raw Data, per country on demand)
        Pages-->>Browser: JSON
        Browser->>Pages: GET data/<Country>.csv (Compare observations)
        Pages-->>Browser: CSV
    end
```
...
**`series/` is a read-path dependency (2026-09 redesign).** `series/index.json` is fetched once and `series/<slug>.json` per country on demand, by Raw Data **and the landing hero chart** — so it is on the first-paint path now, not just behind a tab switch. Those files are generated by [`scripts/build_series.py`](../../scripts/build_series.py) via `build-series.yml`; if they are stale or missing, those surfaces degrade rather than merely age. Still static files off Pages, so the "no backend on the read path" rule holds. Compare is the exception: it reads `data/<Country>[_<Variant>].csv` directly and does not go through `series/` at all.

The hero chart fails soft by design: if `params.csv` does not load, the figure hides itself and the rest of the page is unaffected. The country it shows comes from `Intl.DateTimeFormat().resolvedOptions().timeZone`, so there is **no IP lookup and no geolocation prompt** on page load.
...
**Where parsing lives:** [scripts/fetch_chile.py](../../scripts/fetch_chile.py). `parse_emisiones` layers three strategies and merges their results (because ANAC keeps reshaping the report): (1) the legacy summary table (`EMISIONES_LABELS` + `_EMISIONES_ROW`, pre-2026); (2) the Power-BI tables of the Julio-2026 rebuild — monthly BEV / PHEV(+P-REEV) from the code-keyed "Tipo Tecnología" rows (`_parse_tech_table`) and monthly HEV from the monthly convencional ranking's grand total (`_parse_convencional_monthly`); (3) the narrative-prose YTD fallback (`_NARRATIVE_RE`, YTD-cumulative minus prior CSV months). Strategies 1–2 are monthly-direct, so a missing prior month can't corrupt the result the way the YTD subtraction can. The module docstring documents the MHEV-into-ICE convention and the no-partial-writes rule; a total parse failure dumps the **full** extracted PDF text (not sampled windows) so the next format change is diagnosable from one workflow log.
...
2. **Mid-2026 Cero y Bajas Emisiones layout change froze the CSV at 2026-04.** From the Mayo-2026 report ANAC restyled the alternative-drivetrain summary: each table row gained a parenthetical drivetrain tag (`Eléctricos (BEV) 4.168 96,8 % 870 97,3%`, `Híbrido Enchufable (PHEV) …`) and the plug-in extended-range line (`Eléct. de Rango Ext. Enchufable (P-REEV)`) was split out. The primary table parser keys off a label *immediately* followed by digits, so the inserted `(BEV)`/`(PHEV)` broke every row match and parsing fell through to the narrative-prose fallback (`_NARRATIVE_RE` — reads the YTD-cumulative figures from the intro paragraphs and subtracts the prior months already in the CSV). That fallback then *also* missed PHEV and HEV, because `pypdf` sprinkles line breaks and double spaces mid-sentence (`"híbridos \nconvencionales acumularon"`, `"sumaron 2.955 \nunidades"`) and the single-space patterns didn't match — so the run raised `RuntimeError` and the daily cron went red from the 2026-06-30 attempt onward. Fix: collapse all whitespace (`re.sub(r"\s+", " ", text)`) before applying the narrative patterns. Verified against the Mayo-2026 report: BEV=870, PHEV=709 (512 PHEV + 197 P-REEV, folded into one column per the EREV→PHEV convention), HEV=1337 — the derived monthlies cross-check exactly against the report's own restyled summary table. **Backfill caveat:** the schedule only ever targets the *previous* calendar month, so a month that fails is never retried automatically once the cron moves on — 2026-05 had to be backfilled with a manual `workflow_dispatch` (`year=2026, month=5`) after the fix landed.

3. **Julio-2026 Power-BI rebuild broke both parsers again — even with URLs supplied.** ANAC rebuilt the whole Cero y Bajas Emisiones report around Power BI exports. The old label table (issue 2's primary path) was already dead, and now the narrative fallback died too: every verb changed (`sumaron`→`alcanzaron`/`acumularon`, `contabilizaron`→`acumularon`, and HEV is now phrased `"híbridos convencionales y de rango extendido no enchufables (HEV y REEV) acumularon 8.846"`), so no `_NARRATIVE_RE` pattern matched. The run raised `RuntimeError: Could not extract ['BEV','HEV','PHEV']` and the workflow went red — the Mercado PDF still parsed fine (TOTAL=24722), and supplying both PDF URLs by hand via `workflow_dispatch` made no difference because the failure is in parsing, not discovery. Fix: read the monthly figures **directly** from the new Power-BI tables instead of deriving them. BEV and PHEV(+P-REEV) come from the code-keyed "Tipo Tecnología" rows — e.g. `Eléctricos (BEV) 6.206 105,4 % 781 61,0%`, where the 3rd column is the month (`_parse_tech_table`); keying on the parenthetical technology code is stable across ANAC's relabelling, and requiring a digit right after the code skips prose mentions like `"vehículos 100% eléctricos (BEV) sumaron 6.206 unidades"`. HEV no longer has a monthly summary row, so it is taken from the grand total of the *monthly* `RANKING DE VENTAS POR MARCA - HÍBRIDOS CONVENCIONALES - <Month> <Year>` table (`Total 18 100,0% 1.004 100,0% 41 100,0% 1.063 100,0%` → 1.063; its `ACUMULADO` twin is skipped) via `_parse_convencional_monthly`. Verified against the Julio-2026 report: BEV=781, PHEV=654 (618 PHEV + 36 P-REEV), HEV=1063, TOTAL=24722, ICE=22224 — cross-checks: 781+618+36 = 1.435 (the report's own enchufables monthly total) and HEV +5,6% over Julio-2025's 1.007 ≈ the report's stated 5,7%. Because the values are now monthly-direct, the fix does **not** depend on the prior month being present. **Backfill caveat:** the CSV currently jumps 2026-05 → 2026-07 — 2026-06 was never ingested and needs a manual `workflow_dispatch` (`year=2026, month=6`) with the June PDFs to close the gap.

4. **Discovery silently found no Julio-2026 PDF while Junio worked.** After issue 3's parser fix, an auto-discover run for 2026-07 (no URLs supplied) logged `PDFs not yet published for 2026-07` and no-op'd — a *green* run, since a missing PDF is a legitimate "retry tomorrow", so nothing looked broken. Yet the Julio files were live, and a 2026-06 run minutes earlier discovered and ingested Junio fine. Root cause is in `discover_pdfs`, not parsing: it only matches direct `…-<Month>-<Year>.pdf` hrefs on the `estudio-de-mercado/` category listing, and ANAC's newest post linked to its article page rather than exposing the PDF href directly (Junio's older post still carried its direct link, so it was found). Fix: a canonical-URL fallback — ANAC stores every report at a deterministic path (`/wp-content/uploads/<pub-year>/<pub-month>/<MM>-ANAC-<report>-<Month>-YEAR.pdf`, published the month after the data month), so when the scrape misses either PDF we probe that URL (`canonical_urls` + `_url_resolves`, which checks HTTP 200 + the `%PDF` magic) before giving up. Verified the construction against the known May/June/July URLs (incl. the Dec→Jan year rollover). This is a fallback: the scrape still wins when it finds a link (so version-suffixed filenames like `…-Abril-202634.pdf` keep working), and a genuinely-unpublished month still no-ops. **Immediate workaround** for a month the scrape misses before this ships: dispatch with explicit `mercado_url` / `emisiones_url` inputs.

5. **`0.0` PETROL/DIESEL suppressed the TTM chart from 2025-09 on.** Chile's TTM (trailing-12-month) shares chart silently stopped rendering: `render_country` produced every other Chile chart but no `chile_ttm_shares_*.png`. Root cause is a data-shape inconsistency, not the renderer. ANAC publishes no petrol/diesel split — all combustion sits in `ICE` — so `PETROL`/`DIESEL` had always been left **empty**; but from the 2025-09 row `build_row` started writing `0.0` there. `compute_ttm_long` (R/data.R) skips a fuel column only when it is *entirely* NA; the new `0.0`s made PETROL/DIESEL no longer all-NA, so they were kept, and the **strict** rolling window (a bar needs all 12 trailing months non-NA to hit 100%) never found a complete window — PETROL/DIESEL only had values for the trailing ~11 months — so *every* row was dropped and the function returned `NULL`. The chart worked before 2025-09 precisely because those columns were fully blank and thus skipped. Fix (two parts): `build_row` now writes `PETROL`/`DIESEL` as `""` (matching the pre-2025-09 history and the "no split" semantics), and the 11 stray `0.0` rows (2025-09…2026-07) in `data/Chile.csv` were blanked. Simulating the strict-window logic confirms the flip: 0 kept rows → 128 kept rows (2015-12…2026-07). A `render-country` dispatch for Chile regenerates the chart. **General gotcha:** for a source with no split for a fuel column, leave the cell empty — never `0.0` — or the TTM chart drops the whole series.
...
6. **The same race, on a fetcher: a data commit built and then thrown away.** Immediately after a PR merge, `build-source-pages.yml` and `build-manifest.yml` both pushed to `master` inside the ~70 s between `fetch-colombia.yml` checking out and pushing. The fetch itself succeeded and the commit was *created* (`summary: { changes: 1, insertions: 1 }`), but the push was rejected non-fast-forward and the commit discarded — `committed: true, pushed: false`, the step red, the new month absent. Issue 5 had already found this fix for `render-country.yml`, and `fetch-israel.yml`, `fetch-nepal.yml`, `build-manifest.yml` and `snapshot-builder.yml` had each acquired it after being bitten in turn; the remaining 33 workflows were one unlucky window away from the same thing. Fix: `pull: '--rebase --autostash'` on **every** `EndBug/add-and-commit@v9` step in the repo, so this is now an invariant rather than a patch applied per incident. Any new workflow that commits back to the branch must carry it.

   Note what this class of bug is *not*: the fetch and the parse were correct, and the run went red, so nothing silently rotted. The cost is a wasted run and a month that doesn't appear until someone re-dispatches.
