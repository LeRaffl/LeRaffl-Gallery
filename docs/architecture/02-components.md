...
| [`fetch-austria.yml`](../../.github/workflows/fetch-austria.yml) | Statistik Austria DE2/DE3 `.ods` (+ `market/austria_top.json` from DE2 Tabelle 7/14) (via the Cloudflare Worker relay — the source blocks datacenter IPs) | `Whole` + `HDV` + `Vans` | 09:25 UTC, 8th → 22nd |
...
| [`fetch-finland.yml`](../../.github/workflows/fetch-finland.yml) | StatFin 121d (`pxdata.stat.fi` PxWeb); Traficom's make / model-series tables (`trafi2.stat.fi`) also write `market/finland_top.json` | `Whole` + `Private` + `Industry` + `HDV` + `Vans` + `Buses` | 04:40 UTC, 1st → 15th |
...
| [`fetch-ireland.yml`](../../.github/workflows/fetch-ireland.yml) | SIMI motorstats (`stats.simi.ie`, Inertia SPA) (+ `market/ireland_top.json`, make/model partials per engine type) | `Whole` + `Vans` + `HDV` + `Buses` | 04:00 & 13:00 UTC, 1st → 5th |
...
| [`fetch-japan.yml`](../../.github/workflows/fetch-japan.yml) | JADA monthly registrations file (XLSX preferred, PDF fallback); its maker rows also write `market/japan_top.json` (+ month store `japan_months.json`) | `Whole` — 登録車 only, kei cars excluded | 08:00 UTC, 1st → EOM |
...
| [`fetch-singapore.yml`](../../.github/workflows/fetch-singapore.yml) | LTA Monthly Vehicle Statistics, file M03 (PDF); its make rows also write `market/singapore_top.json` (+ `singapore_months.json`) | `Whole` | 08:00 UTC, 15th → EOM |
...
| [`fetch-uruguay.yml`](../../.github/workflows/fetch-uruguay.yml) | ACAU «Compilado YYYY» xlsx; its per-model rows also write `market/uruguay_top.json` (+ `uruguay_months.json`) | `Whole` (AUTOS + SUV) + `Vans` + `HDV` + `Buses` | 08:10 UTC, 1st → EOM |
