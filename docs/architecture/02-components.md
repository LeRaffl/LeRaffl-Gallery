...
| [`fetch-acea-cv.yml`](../../.github/workflows/fetch-acea-cv.yml) | ACEA **annual** Commercial Vehicle PDF press release (`scripts/fetch_acea_cv.py`) | `Vans` + `HDV` + `Buses` for the 19-country ACEA roster (fetch-acea.yml's always+conditional lists), skipping any file whose existing source isn't `ACEA` — so Austria/Luxembourg/Poland's national HDV/Vans/Buses CSVs are left untouched. Only the full-year (January) release is ever ingested — see [38-source-acea-cv.md](38-source-acea-cv.md) for why | 10:15 UTC, 18th → 31st Jan |
