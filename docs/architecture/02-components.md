...
| [`fetch-acea-cv.yml`](../../.github/workflows/fetch-acea-cv.yml) | ACEA's quarterly-cumulative Commercial Vehicle PDF press releases (`scripts/fetch_acea_cv.py`) | `Vans` + `HDV` + `Buses` for the 19-country ACEA roster (fetch-acea.yml's always+conditional lists), skipping any file whose existing source isn't `ACEA` — so Austria/Luxembourg/Poland's national HDV/Vans/Buses CSVs are left untouched. Reconstructs genuine Q1/Q2/Q3/Q4 rows from ACEA's cumulative Q1/H1/Q1-Q3/FY checkpoints, falling back to a yearly row only with no quarterly baseline yet — see [38-source-acea-cv.md](38-source-acea-cv.md) | 10:15 UTC, four windows (Jan/Apr/Jul-Aug/Oct) — see 08-deploy-ops.md § 8.11 |
