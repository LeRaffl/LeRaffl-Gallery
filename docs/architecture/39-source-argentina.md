...
**Other countries** use the same section: the schema and builder now live in
`scripts/market_top.py` (Argentina's `build_top` calls it too), other
countries write `market/<slug>_top.json`, and Spain and Malaysia were the
first to adopt it — see [03-data-objects §3.16](03-data-objects.md#316-top-brands--models-market).
The `classification:` block stays Argentina-only: it is for countries whose
powertrain is derived rather than reported.
