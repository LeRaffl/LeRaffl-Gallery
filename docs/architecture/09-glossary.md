...
| South Korea | MOTIR «자동차산업 동향» (data: KAMA + KAIDA) | **`Whole` = domestic sales of new vehicles of all types** — passenger cars and commercial vehicles, domestic makers plus KAIDA-reported imports. Sales, not registrations. Hybrids incl. imported mild hybrids; hydrogen FCEV in OTHERS. | Nothing is split off — trucks, vans and buses are inside the total (**not** EU M1). No petrol/diesel split (aggregate ICE). | Ministry of Trade, Industry and Resources press release; see [43-source-south-korea.md](43-source-south-korea.md). |
