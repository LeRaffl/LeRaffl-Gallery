...
| South Africa | naamsa | **`Whole` = all-vehicle domestic new sales** (passenger + LCV + MCV/HCV + buses). The NEV drivetrain table is not split by body type, so the denominator is the same all-vehicle TOTAL. Quarterly from 2024-Q1; yearly 2020–2023 at mid-year. ICE = TOTAL − BEV − PHEV − HEV (no petrol/diesel split). | Not EU M1. Manufacturer-reported (Geely/Dongfeng and some importers absent). 2026 NEV includes new reporters joining naamsa/Lightstone. | naamsa Quarterly Review of Business Conditions + Industry Vehicle Sales `TOTAL AGGREGATE MARKET`. See [38-source-south-africa.md](38-source-south-africa.md). |
