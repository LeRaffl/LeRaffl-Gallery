...
| Luxembourg | New car registrations (VEHICLE_TYPE = CAR) | — | — | New van registrations (VEHICLE_TYPE = VAN) | Trucks + buses + road tractors combined | New bus & coach registrations (M2+M3), via ACEA's Commercial Vehicle report — Luxembourg's own STATEC source doesn't carry a separate Buses variant |
...
> `Buses`, Malta, Norway, Romania, Slovakia, Slovenia, Switzerland) are
