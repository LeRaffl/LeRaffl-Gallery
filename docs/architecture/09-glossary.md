...
| Japan | JADA | 登録車 (registered passenger cars): the 乗用車計 row of JADA's "燃料別メーカー別登録台数（乗用車）" file. Covers domestic + 輸入車 (imported) passenger cars across all reported makers. | 軽自動車 (kei cars, ≤ 660 cc): explicitly excluded by JADA's footer "２．軽自動車は含みません。" Trucks and buses are not on this page either (JADA publishes them separately under pages/343/). | 道路運送車両法 (Road Transport Vehicle Act). The 登録車 vs 軽自動車 split follows engine displacement and length/width thresholds; JADA covers the former, 全国軽自動車協会連合会 covers the latter. |
