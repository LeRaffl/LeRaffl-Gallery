...
| **CI (confidence interval / band)** | Where the **true curve** is. 95 % CI: in 95 of 100 hypothetical re-runs of the same transition, the band from that re-run contains the true curve at that date. Pointwise. Computed from the inverse Hessian with prewhitened scores and published in `bands/<slug>.json` together with CI ranges for the 10/20/50/80/90 % crossing years. The only band that gives meaningful **years**. Not the ribbon on the PNGs (see `confidence_level`). Full explanation: [44-uncertainty-bands.md](44-uncertainty-bands.md). |
...
| **persistence** | In `bands/<slug>.json`: `a₁ + a₂` of the AR(2) prewhitening, i.e. how long monthly deviations from the curve last (0 = independent months, near 1 = years). High persistence widens the CI; it means the data keep deviating from the curve in one direction. |
