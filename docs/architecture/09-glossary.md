...
| South Korea | MOTIR «자동차산업 동향» (data: KAMA + KAIDA) | **`Whole` = domestic sales of new vehicles of all types** — passenger cars and commercial vehicles, domestic makers plus KAIDA-reported imports. Sales, not registrations. Hybrids incl. imported mild hybrids; hydrogen FCEV in OTHERS. Tesla not counted before 2024 (KAIDA coverage). | Nothing is split off — trucks, vans and buses are inside the total (**not** EU M1). No petrol/diesel split (aggregate ICE). | Ministry of Trade, Industry and Resources press release; see [43-source-south-korea.md](43-source-south-korea.md). |
...
| **confidence_level** | `0.999` — used to derive the grey/coloured ribbon around each fitted curve. **This is not a true statistical confidence interval.** It is a visual band derived from the fit's standard error scaled by the z-quantile at this level. Useful as a "the curve could plausibly sit anywhere in here" hint, not for rigorous inference. A real 95 % CI (plus PI and TI) now exists as frontend data in `bands/<slug>.json` — see **CI** below; the PNG ribbon is unchanged. |
| **CI (confidence interval / band)** | Where the **true curve** is. 95 % CI: in 95 of 100 hypothetical re-runs of the same transition, the band from that re-run contains the true curve at that date. Pointwise. Computed by profile likelihood calibrated to the inverse Hessian with prewhitened scores, and published in `bands/<slug>.json` together with CI ranges for the 10/20/50/80/90 % crossing years. The only band that gives meaningful **years**. Not the ribbon on the PNGs (see `confidence_level`). Full explanation: [44-uncertainty-bands.md](44-uncertainty-bands.md). |
| **PI (prediction interval)** | Where **one more month** lands: the CI's uncertainty for the line plus the real month-to-month scatter. 95 %, for one row of the series' cadence (one month for monthly data, not a TTM total). Describes data points, so it has no meaningful crossing years. |
| **TI (tolerance interval, 95 / 95)** | Where **95 % of all possible months** land, with 95 % confidence. Block bootstrap. Always contains the PI, which contains the CI. |
| **persistence** | In `bands/<slug>.json`: `a₁ + a₂` of the AR(2) prewhitening, i.e. how long monthly deviations from the curve last (0 = independent months, near 1 = years). High persistence widens the CI; from 0.9 the S-curve counts as a **poor fit** and the band is marked `quality.usable = false` (as is any series with fewer than 48 rows). |
