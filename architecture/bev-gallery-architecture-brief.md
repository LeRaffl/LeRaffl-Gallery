...
- **Module:**
  ...
  - `R/render_country.R` — Entry-Point: `Rscript R/render_country.R <Country> [<Variant>]`. Lädt Daten, fittet, rendert vier PNGs nach `images/<period>/`, upsertet `params.csv` + `weights.csv` und schreibt `bands/<slug>.json` (Konfidenz-, Prognose- und Toleranzband fürs Frontend, `R/bands.R`; siehe `docs/architecture/44-uncertainty-bands.md`).
...
- **`Backfill uncertainty bands` (`.github/workflows/backfill-bands.yml`):** nur `workflow_dispatch` (optional `countries`, `;`-getrennt). Berechnet `bands/<slug>.json` für alle Serien bzw. die angegebenen Länder mit demselben Codepfad wie ein Render (`scripts/backfill_bands.R`) und committet nur `bands/` — keine PNGs, kein `params.csv`. Für die Erstbefüllung und nach Änderungen an `R/bands.R`.
- **`Render preview` (`.github/workflows/preview-render.yml`):** bei jedem PR, der `R/**` berührt, plus `workflow_dispatch`. Rendert eine feste Serienliste (24 Serien, von stabil bis kollabiert) zweimal aus denselben Daten — mit dem `R/` des PRs und mit dem des Basis-Branches — und lädt Seite-an-Seite-PNGs, `index.html` und die `params.csv`-Zeile vorher/nachher als Artifact `render-preview` hoch; ein PR-Kommentar verlinkt es und wird bei jedem Push aktualisiert. Nur lesend: kein Commit, kein Dispatch.
- **`Check country integration` (`.github/workflows/check-country-integration.yml`):** bei jedem PR, der Daten, `index.html`, R, Flaggen, Docs, `backtest/params` oder einen Fetch-Workflow berührt: `scripts/check_country_integration.py` prüft, dass jedes Land überall verdrahtet ist (Checkliste: `docs/architecture/42-adding-a-country.md`).
...
- **Country Fetch Actions (Familie `.github/workflows/fetch-*.yml`):** Ein Workflow pro Datenquelle. Cron-getrieben, self-throttling über `latest_period(<CSV>) ≥ target`. Nach Schreib-Diff dispatcht ein Single-Country-Fetcher `render-country.yml` **einmal** mit der pipe-separierten Liste seiner betroffenen Varianten (ACEA als Multi-Country-Ausnahme fächert weiter per `workflow_call`-Matrix über die Länder auf). Aktueller Stand (Schedule in UTC):
  ...
  - `fetch-uruguay.yml` — täglich 08:00, 1.→EOM. Uruguay (ACAU; die Modellzeilen schreiben auch `market/uruguay_top.json` + `uruguay_months.json`).
...
5. Runner: committet `images/` + `params.csv` + `weights.csv` + `posts/` + `bands/`.
