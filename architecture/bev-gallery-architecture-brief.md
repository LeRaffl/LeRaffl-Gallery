...
- **`Snapshot Builder curves` (`.github/workflows/snapshot-builder.yml`):** Cron 25. jedes Monats 09:00 UTC plus `workflow_dispatch`. Zwei getrennte Aufgaben in einem Lauf: (1) dumpt die aggregierten Builder-Kurven nach `builder_history/<date>.csv` — das Protokoll dessen, was die Seite an dem Tag tatsaechlich zeigte, spaeter nicht rekonstruierbar; (2) verlaengert per `R/build_backtest.R` den Backtest um den neuen Monat und baut `backtest/series/` samt GIFs neu, das die Time-lapse-Ansicht im Builder-Tab liest. Der Backtest beantwortet eine andere Frage ("was haette dieses Modell zum Zeitpunkt X gesagt") und reicht bis 2015 zurueck, wo `builder_history/` nicht vor 2025-09 beginnen kann; beide duerfen nie in einer Serie gemischt werden.
- **Country Fetch Actions (Familie `.github/workflows/fetch-*.yml`):** Ein Workflow pro Datenquelle. Cron-getrieben, self-throttling über `latest_period(<CSV>) ≥ target`. Nach Schreib-Diff dispatcht ein Single-Country-Fetcher `render-country.yml` **einmal** mit der pipe-separierten Liste seiner betroffenen Varianten (ACEA als Multi-Country-Ausnahme fächert weiter per `workflow_call`-Matrix über die Länder auf). Aktueller Stand (Schedule in UTC):
  ...
  - `fetch-argentina.yml` — täglich 09:15 & 21:15, 8.→25. Argentina (DNRPA-Open-Microdata, Whole / Private / Industry / Pickups; Antriebsart aus der Modellbezeichnung klassifiziert, gegen ACARA validiert — siehe [39-source-argentina.md](../docs/architecture/39-source-argentina.md)).
